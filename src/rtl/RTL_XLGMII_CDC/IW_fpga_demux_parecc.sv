...
//
// File Name:$RCSfile: IW_fpga_demux_parecc.sv.rca $
// File Revision:$
// Created by:    Balaji G
// Updated by:    $Author: balaji $ $Date: Mon Jun  1 06:38:28 2015 $
//------------------------------------------------------------------------------
// ECC checking and recv over Muxed bus
// Module to be instanced with highesh mux clock that is used in the design with a dedicated
// Pin for receiving the ECC over a parallel line
// The input is connected to signals which requires ECC checking
...
module IW_fpga_demux_parecc #(
  ...
  ,parameter INSTANCE_NAME        = "u_demux_parecc"  //Can hold upto 16 ASCII characters
) (

    input                          clk_demux    // DeMux clock sync to clk
  , input                          rst_demux_n  
...
localparam cbts = 10;
    ...
    wire   [(MULTIPLEX_RATIO*1)-1 : 0 ]        ecc_gen_w;
...
//============================================================
 ...
 // pipeline to break DW_ecc timing towards system
 always @(posedge clk ) begin
   ecc_err   <= (|ecc_err_multpl) | (|ecc_err_detect);
 end
 always @(*)
   ecc_gen   = ecc_gen_w[(cbts*NUMBER_OF_256PAIRS)-1 : 0 ];
...
// Change History:
//
// $Log: IW_fpga_demux_parecc.sv.rca $
// 
//  Revision: 1.1 Mon Jun  1 06:38:28 2015 balaji
