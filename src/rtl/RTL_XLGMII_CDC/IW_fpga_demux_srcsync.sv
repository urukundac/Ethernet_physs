...
//------------------------------------------------------------------------------
// Parallel demux with source sync clock version
// Should run at very hight rate compared to Synchronous clock due to source sync clock
// The source sync clock has to be driven out separately for each FPGA through IW_fpga_source_clk.sv
// The source sync clock input should be connected to CLKPAD and then to the IW_fpga_rx_syncclk for deskew.
//------------------------------------------------------------------------------


`timescale 10fs/10fs

module IW_fpga_demux_srcsync #(
    parameter NUMBER_OF_INPUTS     = 5
  , parameter CLOCK_RATIO          = 56 // RATIO between fast clock and system clock
  , parameter INPUT_BUDGET         = 3  // Number of fast clock required for slow to fast clock path. set Mutlicycle in sdc accordingly. Min 2 required
  , parameter SYNC_DELAY           = 2  // Skew between 2 FPGA system clk. The SYNC ckt works on much lower mux clock ratio and its possible we have a skew between system clock.Min 2 required
  , parameter SRC_SYNC_DELAY       = 4  // Max number of fast clock  required for DATA to make it on the receiver. This does not come into data latching timing, but overall guaranteed single clock margin. This includes pad + routing + ddr conversion latency on either side
  , parameter OUTPUT_BUDGET        = 3  // Number of fast clock required for fast to slow path on receiver. Set MC in sdc.
  ...
  , parameter INSTANCE_NAME        = "u_demux_srcsync"  //Can hold upto 16 ASCII characters
  ...
  , output [INFRA_AVST_DATA_W-1:0]        avst_egr_data
    ...
    reg [(NUMBER_OF_INPUTS*MULTIPLEX_RATIO)-1:0] outbus;
    reg   [7 : 0]                  rcv_cnt;
    reg                            slow_edge_3d;
    reg                            slow_edge_2d;
    reg                            slow_edge_async;
    wire                           slow_edge;
    wire                           rst_demux_sync_n;
...
assign local_clk_demux_int = pos_divby2 ^ neg_divby2;

always @(posedge clk or negedge rst_to_logic_n ) begin
  if ( !rst_to_logic_n ) begin
    pos_divby2   <= 1'b1;
  end
  else begin
    pos_divby2   <= ~pos_divby2;
  end
end
always @(negedge clk or negedge rst_to_logic_n ) begin
  if ( !rst_to_logic_n ) begin
    neg_divby2   <= 1'b0;
  end
  else begin
    neg_divby2   <= ~neg_divby2;
  end
end

IW_sync_reset u_IW_sync_reset_demux (
  .clk             (clk_demux),
  .scan_mode       (1'b0),
  .rst_n           (rst_demux_n),
  .rst_n_sync      (rst_demux_sync_n)
);


// Sequence count starting from rising edge of system clk
always @(posedge clk_demux or negedge rst_demux_sync_n ) begin
  if ( !rst_demux_sync_n ) begin
    slow_edge_async  <= 1'b1;
    slow_edge_2d <= 1'b1;
    slow_edge_3d <= 1'b1;
  end
  else begin
    slow_edge_async  <= local_clk_demux_int;
...
IW_fpga_avst2csr #(
  ...
  assign params0_reg = {NUMBER_OF_INPUTS[7:0],SRC_SYNC_DELAY[3:0],SYNC_DELAY[3:0],OUTPUT_BUDGET[7:0],INPUT_BUDGET[7:0]};
