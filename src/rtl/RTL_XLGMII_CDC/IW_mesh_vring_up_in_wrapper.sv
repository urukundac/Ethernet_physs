module IW_mesh_vring_up_in_wrapper (
...
//};  
    ...
    //input           				vring_up_in_pyld_ddrt_throttle;
    //input           				vring_up_in_pyld_dpt_throttle;
    //output          				vring_up_out_pyld_ddrt_throttle;
    //output          				vring_up_out_pyld_dpt_throttle;
    // Ports for Interface vring_up_in_data
    input           				vring_up_in_data_bl_data_beparity;
    input   [MESH_BL_DATA_BE_SIZE-1:0]  	vring_up_in_data_bl_data_byte_en;
    input   [MESH_BL_DATA_SIZE-1:0] 		vring_up_in_data_bl_data_data;
    input   [MESH_BL_DATA_ECC_SIZE-1:0]  	vring_up_in_data_bl_data_ecc;
    input           				vring_up_in_data_bl_data_ecc_valid;
    input   [MESH_BL_DATA_PAR_SIZE-1:0]   	vring_up_in_data_bl_data_parity;
    input          	 			vring_up_in_data_bl_data_poison;
    // Ports for Interface vring_up_in_pyld
    input   [MESH_AD_ADR_SIZE-1:0]  		vring_up_in_pyld_ad_addr;
    input           				vring_up_in_pyld_ad_dest_dst_bounce;
    input   [MESH_STOP_ID_SIZE-1:0]   		vring_up_in_pyld_ad_dest_dst_id;
    input           				vring_up_in_pyld_ad_dest_dst_type;
    input           				vring_up_in_pyld_ad_dest_for_me;
    input           				vring_up_in_pyld_ad_dest_remote_dir;
    input           				vring_up_in_pyld_ad_dest_remote_pol;
    input           				vring_up_in_pyld_ad_dest_reserve;
    input   [MESH_STOP_ID_SIZE-1:0]   		vring_up_in_pyld_ad_dest_src_id;
    input           				vring_up_in_pyld_ad_dest_src_type;
    input           				vring_up_in_pyld_ad_dest_tgr_bounce;
    input   [MESH_TGR_ID_SIZE-1:0]   		vring_up_in_pyld_ad_dest_tgr_id;
    input           				vring_up_in_pyld_ad_dest_valid;
    input   [MESH_AD_HDR_SIZE-1:0]  		vring_up_in_pyld_ad_hdr;
    input           				vring_up_in_pyld_ads_anti_dead_s;
    input           				vring_up_in_pyld_ak_dest_dst_bounce;
    input   [MESH_STOP_ID_SIZE-1:0]   		vring_up_in_pyld_ak_dest_dst_id;
    input           				vring_up_in_pyld_ak_dest_dst_type;
    input           				vring_up_in_pyld_ak_dest_for_me;
    input           				vring_up_in_pyld_ak_dest_remote_dir;
    input           				vring_up_in_pyld_ak_dest_remote_pol;
    input           				vring_up_in_pyld_ak_dest_reserve;
    input   [MESH_STOP_ID_SIZE-1:0]   		vring_up_in_pyld_ak_dest_src_id;
    input           				vring_up_in_pyld_ak_dest_src_type;
    input   [MESH_TGR_ID_SIZE-1:0]   		vring_up_in_pyld_ak_dest_tgr_id;
    input           				vring_up_in_pyld_ak_dest_valid;
    input   [MESH_AK_HDR_SIZE-1:0] 		vring_up_in_pyld_ak_hdr;
    input   [MESH_STOP_ID_SIZE-1:0]   		vring_up_in_pyld_akc_dest_dst_id;
    input           				vring_up_in_pyld_akc_dest_dst_type;
    input           				vring_up_in_pyld_akc_dest_for_me;
    input           				vring_up_in_pyld_akc_dest_remote_dir;
    input           				vring_up_in_pyld_akc_dest_remote_pol;
    input           				vring_up_in_pyld_akc_dest_reserve;
    input   [MESH_STOP_ID_SIZE-1:0]   		vring_up_in_pyld_akc_dest_src_id;
    input           				vring_up_in_pyld_akc_dest_src_type;
    input   [MESH_TGR_ID_SIZE-1:0]   		vring_up_in_pyld_akc_dest_tgr_id;
    input           				vring_up_in_pyld_akc_dest_valid;
    input   [MESH_AKC_HDR_SIZE-1:0]   		vring_up_in_pyld_akc_hdr;
    input   [MESH_BL_ADR_SIZE-1:0]  		vring_up_in_pyld_bl_addr;
    input   [MESH_STOP_ID_SIZE-1:0]   		vring_up_in_pyld_bl_dest_dst_id;
    input           				vring_up_in_pyld_bl_dest_dst_type;
    input           				vring_up_in_pyld_bl_dest_for_me;
    input           				vring_up_in_pyld_bl_dest_half;
    input           				vring_up_in_pyld_bl_dest_remote_dir;
    input           				vring_up_in_pyld_bl_dest_remote_pol;
    input           				vring_up_in_pyld_bl_dest_reserve;
    input   [MESH_STOP_ID_SIZE-1:0]  	 	vring_up_in_pyld_bl_dest_src_id;
    input           				vring_up_in_pyld_bl_dest_src_type;
    input          				vring_up_in_pyld_bl_dest_tgr_bounce;
    input   [MESH_TGR_ID_SIZE-1:0]   		vring_up_in_pyld_bl_dest_tgr_id;
    input           				vring_up_in_pyld_bl_dest_valid;
    input   [MESH_BL_HDR_SIZE-1:0]  		vring_up_in_pyld_bl_hdr;
    input           				vring_up_in_pyld_global_fatal;
    input           				vring_up_in_pyld_global_victim_pending;
    input           				vring_up_in_pyld_global_viral;
    input   [MESH_IV_ADR_SIZE-1:0]  		vring_up_in_pyld_iv_addr;
    input   [MESH_IV_DST_SIZE-1:0]  		vring_up_in_pyld_iv_dest;
    input           				vring_up_in_pyld_sc_dest_for_me;
    input           				vring_up_in_pyld_sc_dest_reserve;
    input   [MESH_TGR_ID_SIZE-1:0]   		vring_up_in_pyld_sc_dest_src_tgr_id;
    input   [MESH_TGR_ID_SIZE-1:0]   		vring_up_in_pyld_sc_dest_tgr_id;
    input           				vring_up_in_pyld_sc_dest_valid;
    input   [MESH_SC_HDR_SIZE-1:0]   		vring_up_in_pyld_sc_hdr;
    input           				vring_up_in_pyld_shared_vn0;
    input           				vring_up_in_pyld_thr_distress;
    // Ports for Interface vring_up_out_data
    output          				vring_up_out_data_bl_data_beparity;
    output  [MESH_BL_DATA_BE_SIZE-1:0]  	vring_up_out_data_bl_data_byte_en;
    output  [MESH_BL_DATA_SIZE-1:0] 		vring_up_out_data_bl_data_data;
    output  [MESH_BL_DATA_ECC_SIZE-1:0]   	vring_up_out_data_bl_data_ecc;
    output          				vring_up_out_data_bl_data_ecc_valid;
    output  [MESH_BL_DATA_PAR_SIZE-1:0]   	vring_up_out_data_bl_data_parity;
    output          				vring_up_out_data_bl_data_poison;
    // Ports for Interface vring_up_out_pyld
    output  [MESH_AD_ADR_SIZE-1:0]  		vring_up_out_pyld_ad_addr;
    output          				vring_up_out_pyld_ad_dest_dst_bounce;
    output  [MESH_STOP_ID_SIZE-1:0]   		vring_up_out_pyld_ad_dest_dst_id;
    output          				vring_up_out_pyld_ad_dest_dst_type;
    output          				vring_up_out_pyld_ad_dest_for_me;
    output          				vring_up_out_pyld_ad_dest_remote_dir;
    output          				vring_up_out_pyld_ad_dest_remote_pol;
    output          				vring_up_out_pyld_ad_dest_reserve;
    output  [MESH_STOP_ID_SIZE-1:0]   		vring_up_out_pyld_ad_dest_src_id;
    output          				vring_up_out_pyld_ad_dest_src_type;
    output          				vring_up_out_pyld_ad_dest_tgr_bounce;
    output  [MESH_TGR_ID_SIZE-1:0]   		vring_up_out_pyld_ad_dest_tgr_id;
    output          				vring_up_out_pyld_ad_dest_valid;
    output  [MESH_AD_HDR_SIZE-1:0]  		vring_up_out_pyld_ad_hdr;
    output          				vring_up_out_pyld_ads_anti_dead_s;
    output          				vring_up_out_pyld_ak_dest_dst_bounce;
    output  [MESH_STOP_ID_SIZE-1:0]   		vring_up_out_pyld_ak_dest_dst_id;
    output          				vring_up_out_pyld_ak_dest_dst_type;
    output          				vring_up_out_pyld_ak_dest_for_me;
    output          				vring_up_out_pyld_ak_dest_remote_dir;
    output          				vring_up_out_pyld_ak_dest_remote_pol;
    output          				vring_up_out_pyld_ak_dest_reserve;
    output  [MESH_STOP_ID_SIZE-1:0]   		vring_up_out_pyld_ak_dest_src_id;
    output          				vring_up_out_pyld_ak_dest_src_type;
    output  [MESH_TGR_ID_SIZE-1:0]   		vring_up_out_pyld_ak_dest_tgr_id;
    output          				vring_up_out_pyld_ak_dest_valid;
    output  [MESH_AK_HDR_SIZE-1:0]  		vring_up_out_pyld_ak_hdr;
    output  [MESH_STOP_ID_SIZE-1:0]   		vring_up_out_pyld_akc_dest_dst_id;
    output          				vring_up_out_pyld_akc_dest_dst_type;
    output          				vring_up_out_pyld_akc_dest_for_me;
    output          				vring_up_out_pyld_akc_dest_remote_dir;
    output          				vring_up_out_pyld_akc_dest_remote_pol;
    output          				vring_up_out_pyld_akc_dest_reserve;
    output  [MESH_STOP_ID_SIZE-1:0]   		vring_up_out_pyld_akc_dest_src_id;
    output          				vring_up_out_pyld_akc_dest_src_type;
    output  [MESH_TGR_ID_SIZE-1:0]   		vring_up_out_pyld_akc_dest_tgr_id;	
    output          				vring_up_out_pyld_akc_dest_valid;
    output  [MESH_AKC_HDR_SIZE-1:0]  		vring_up_out_pyld_akc_hdr;
    output  [MESH_BL_ADR_SIZE-1:0]  		vring_up_out_pyld_bl_addr;
    output  [MESH_STOP_ID_SIZE-1:0]  		vring_up_out_pyld_bl_dest_dst_id;
    output          				vring_up_out_pyld_bl_dest_dst_type;
    output          				vring_up_out_pyld_bl_dest_for_me;
    output          				vring_up_out_pyld_bl_dest_half;
    output          				vring_up_out_pyld_bl_dest_remote_dir;
    output          				vring_up_out_pyld_bl_dest_remote_pol;
    output          				vring_up_out_pyld_bl_dest_reserve;
    output  [MESH_STOP_ID_SIZE-1:0]   		vring_up_out_pyld_bl_dest_src_id;
    output          				vring_up_out_pyld_bl_dest_src_type;
    output          				vring_up_out_pyld_bl_dest_tgr_bounce;
    output  [MESH_TGR_ID_SIZE-1:0]   		vring_up_out_pyld_bl_dest_tgr_id;
    output          				vring_up_out_pyld_bl_dest_valid;
    output  [MESH_BL_HDR_SIZE-1:0]  		vring_up_out_pyld_bl_hdr;
    output          				vring_up_out_pyld_global_fatal;
    output          				vring_up_out_pyld_global_victim_pending;
    output          				vring_up_out_pyld_global_viral;
    output  [MESH_IV_ADR_SIZE-1:0]  		vring_up_out_pyld_iv_addr;
    output  [MESH_IV_DST_SIZE-1:0]  		vring_up_out_pyld_iv_dest;
    output           				vring_up_out_pyld_sc_dest_for_me;
    output           				vring_up_out_pyld_sc_dest_reserve;
    output  [MESH_TGR_ID_SIZE-1:0]   		vring_up_out_pyld_sc_dest_src_tgr_id;
    output  [MESH_TGR_ID_SIZE-1:0]   		vring_up_out_pyld_sc_dest_tgr_id;
    output           				vring_up_out_pyld_sc_dest_valid;
    output  [MESH_SC_HDR_SIZE-1:0]   		vring_up_out_pyld_sc_hdr;
    output          				vring_up_out_pyld_shared_vn0;
    output          				vring_up_out_pyld_thr_distress;
   /****************************************************************************************
   * Bundelled signals for mux/demux
   ****************************************************************************************/
    input   [DEMUX_DATA_WIDTH-1:0]        	vring_up_out_demux;
    output  [MUX_DATA_WIDTH-1:0]        	vring_up_in_mux;
   
   /****************************************************************************************/
    logic  [DEMUX_DATA_WIDTH-1:0]       	vring_up_out_demux;
    logic  [MUX_DATA_WIDTH-1:0]        		vring_up_in_mux;

assign vring_up_in_mux = {
    //vring_up_in_pyld_ddrt_throttle,
    //vring_up_in_pyld_dpt_throttle,
    vring_up_in_data_bl_data_beparity,
