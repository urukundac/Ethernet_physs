...
module fpga_vram2dq (vram_clock, vram_reset_b, write_data, write_enable, 
                        write_byte_enable, write_address, read_address1, 
                        read_address2, dt_latchopen, dt_latchclosed_b, 
                        dt_ramrst_b, read_enable1, read_data1, read_data2, read_enable2,
...
//--------------------------- Signal Declarations ----------------------------//
   ...
   wire[width*depth-1:0]    data_memory1_dqual;
   wire[width*depth-1:0]    data_memory2_dqual;
...
reg [width*depth-1:0] data_memory;
   ...
   reg [width*depth-1:0] read_select1;
   reg [width*depth-1:0] read_select2;
...
endgenerate
   ...
   generate
      if(((use_ports > 0))) begin: dq1
         cxp_ctech_dqual_sync
           i_dq1[width*depth-1:0]
            (
             .d(data_memory),
             .en(read_select1),
             .q(data_memory1_dqual) 
             );
      end else begin : no_dq1
         assign data_memory1_dqual = '0;
      end
   endgenerate

   generate
      if(((use_ports > 2))) begin: dq2
         cxp_ctech_dqual_sync
           i_dq2[width*depth-1:0]
          (
           .d(data_memory),
           .en(read_select2),
           .q(data_memory2_dqual) 
           );
      end else begin : no_dq2
         assign data_memory2_dqual = '0;
      end
   endgenerate
...
// Needed to support one port mode.
   assign nc_read_address1[address_size-1:0] = ((depth == 1)) ? 
                                               {address_size{1'b0}} : ((use_ports == 1)) ? 
                                               write_address[address_size-1:0] : read_address1[address_size-1:0];


   // Force address to zero if depth is equal to one. (rev4.3a1).
   assign nc_read_address2[address_size-1:0] = ((depth == 1)) ? 
                                               {address_size{1'b0}} : read_address2[address_size-1:0];
   ...
   always @(
     ...
     begin : read_address1_decoder
        for (int i = depth-1; i >= 0; i = i - 1)
          begin
             if ((nc_read_address1 == i[address_size-1:0]) && (read_enable1 == 1'b1))
               begin
                  read_select1[i*width+:width] = {width{1'b1}};
               end
             else
               begin
                  read_select1[i*width+:width] = {width{1'b0}};
               end
          end
     end // begin : read_address1_decoder

   always @(
            nc_read_address2 or
            read_enable2
            )
     begin : read_address2_decoder
        for (int i = depth-1; i >= 0; i = i - 1)
          begin
             if ((nc_read_address2 == i[address_size-1:0]) && (read_enable2 == 1'b1))
               begin
                  read_select2[i*width+:width] = {width{1'b1}};
               end
             else
               begin
                  read_select2[i*width+:width] = {width{1'b0}};
               end
          end
     end // begin : read_address2_decoder
...
//Free Form Table Code
//--------------------------------------------------------//
   always @( data_memory1_dqual )
     begin : mux_read_data1_loop
        mux_read_data1 = {width{1'b0}};
        for (int i = depth-1; i >= 0; i = i - 1)
          begin
             mux_read_data1 = mux_read_data1 |
                  data_memory1_dqual[i*width+:width];
          end
     end // begin : mux_read_data1_loop

   always @( data_memory2_dqual )
     begin : mux_read_data2_loop
        mux_read_data2 = {width{1'b0}};
        for (int i = depth-1; i >= 0; i = i - 1)
          begin
             mux_read_data2 = mux_read_data2 |
                  data_memory2_dqual[i*width+:width];
          end
     end // begin : mux_read_data2_loop
...
//------------------------------------//
   ...
   // Do not bypass read mux even when depth is only one. (rev4.3a1).
   assign read_data1[width-1:0] = ((use_ports < 1)) ? 
                                  {width{1'b0}} : mux_read_data1[width-1:0];
   ...
   assign read_data2[width-1:0] = ((use_ports < 3)) ? 
                                  {width{1'b0}} : mux_read_data2[width-1:0];
