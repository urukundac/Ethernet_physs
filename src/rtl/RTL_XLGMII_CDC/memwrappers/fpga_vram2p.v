...
module fpga_vram2p (vram_clock, vram_reset_b, vram_sync_reset_b, write_data, write_enable, 
    ...
    parameter  row_reset_value = 0;
    input           vram_clock;
    input         vram_reset_b;
    input                          vram_sync_reset_b;   // synchronous assert/deassert
...
//--------------------------- Signal Declarations ----------------------------//
   ...
   wire [width-1:0] write_data_eff;
   assign write_data_eff = vram_sync_reset_b ? write_data : row_reset_value;
...
begin: vram21_internal_reset_flops
    if (vram_reset_b == 1'b0)
      ...
      begin
        write_data_flopped1[width-1:0] <= #100 write_data_eff[width-1:0];
       end
...
begin: vram21_internal_noreset_flops
      ...
      write_data_flopped2[width-1:0] <= #100 write_data_eff[width-1:0];
...
assign i_write_data[width-1:0] = ((use_preflops == 1)) ? 
                        write_data_flopped3[width-1:0] : write_data_eff[width-1:0];
...
begin : write_line_select_table
        ...
        for ( i = depth*write_enable_count -1; i >= 0; i = i - 1)
        begin
            write_line_select[i] <= ~vram_sync_reset_b | 
...
begin : vram_mem_cell0_internal_noreset_latches
        ...
        begin
            ...
            begin
                ...
                begin
                    data_memory[((i*width)+(j*bwidth))+:bwidth] <=
                        write_data_eff[(j*bwidth)+:bwidth];
                end
            end
        end
end // vram_mem_cell0_internal_noreset_latches
