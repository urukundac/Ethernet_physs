//This is a generic pattern generator
module  IW_fpga_pttrn_gen #(
  ...
  , parameter INSTANCE_NAME        = "u_pttrn_gen"  //Can hold upto 16 ASCII characters
  ...
  , output  wire  [PTTRN_DATA_W-1:0]      pttrn_data
  ...
  reg                       pttrn_gen_en;
  reg                       linear_pttrn_gen_en;
  reg   [PTTRN_CNTR_W-1:0]  pttrn_cntr;
  wire [7:0]                pttrn_cntr_w;
  wire [15:0]               pttrn_data_w;
...
IW_fpga_avst2csr #(
  ...
  wire [7:0]  pttrn_data_width;

  assign pttrn_cntr_width = PTTRN_CNTR_W;
  assign pttrn_data_width = PTTRN_DATA_W; 
  assign params_reg       = {8'h00,pttrn_cntr_width,LFSR_POLY,pttrn_data_width};
    ...
    begin
      ...
      begin
        case(csr_addr)
          16 : //Control register
          begin
            pttrn_gen_en        <=  csr_wr_data[0];
            linear_pttrn_gen_en <=  csr_wr_data[1];
          end
        endcase
      end
    end
  ...
  begin
    ...
    begin
      ...
      case(csr_addr)
        ...
        16 : /*  Control Reg */
        begin
          csr_rd_data         <=  {{((4*INFRA_AVST_DATA_W)-2){1'b0}}, linear_pttrn_gen_en, pttrn_gen_en};
        end
        20 : /*  Params Reg */
        begin
  ...
  /*
    * Counter Logic
    * This counter value is replicated across pttrn_data
  */
  always@(posedge  clk,  negedge rst_n)
  begin
    if(~rst_n)
      ...
      if(~pttrn_gen_en) begin
        pttrn_cntr            <=  0;
      end else begin
        if(linear_pttrn_gen_en)
          pttrn_cntr            <= pttrn_cntr + 1;
        else begin
          if(pttrn_cntr  ==  0)  //Start from non-zero value
            pttrn_cntr            <=  {{(PTTRN_CNTR_W-1){1'b0}},1'b1};
          else  //Pseudo-random pattern
  ...
  generate
    ...
    begin : gen_pttrn_data
      if((i==NUM_SLICES-1)  &&  (LAST_SLICE_REMAINDER > 0))
      begin
        assign  pttrn_data[(i*PTTRN_CNTR_W) +:  LAST_SLICE_REMAINDER]   = pttrn_cntr[LAST_SLICE_REMAINDER-1:0];
      end
      else
      begin
        assign  pttrn_data[(i*PTTRN_CNTR_W) +:  PTTRN_CNTR_W] = pttrn_cntr;
      end
    end //gen_pttrn_data
  endgenerate

endmodule //IW_fpga_pttrn_gen
