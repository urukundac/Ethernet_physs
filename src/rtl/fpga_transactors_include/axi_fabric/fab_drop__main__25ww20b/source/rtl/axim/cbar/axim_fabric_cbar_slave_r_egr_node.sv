...
/*
 ...
 -- Module Name       : axim_fabric_cbar_slave_r_egr_node
 -- Author            : Gregory James
 -- Associated modules: 
 -- Function          : This block is the egress node which interfaces with
                        AXI4 Slave R channel
 --------------------------------------------------------------------------
*/
...
module axim_fabric_cbar_slave_r_egr_node #(
...
//----------------------- Internal Register Declarations ------------------
  ...
  axi4_r_struct_t             search_key_val;
...
//----------------------- Internal Wire Declarations ----------------------
  ...
  logic                     search_en;
  axi4_ingr2egr_r_struct_t  search_data;
  logic                     search_found;
  logic                     search_miss;
  axi4_ingr2egr_r_struct_t  search_result;
...
//----------------------- Start of Code -----------------------------------
  ...
  /*  Pack into struct  */
  assign  ff_wr_data.rid    = s_rid;
  assign  ff_wr_data.rdata  = s_rdata;
  assign  ff_wr_data.rresp  = s_rresp;
  assign  ff_wr_data.rlast  = s_rlast;
  assign  ff_wr_data.ruser  = s_ruser;

  assign  ff_wr_en  = s_rvalid  & s_rready;

  assign  s_rready  = ~ff_wr_full & ~master_id_search_ff_full;
  ...
  assign  ingr2egr_r_ready  = ~master_id_search_ff_full;

  assign  search_en             = s_rvalid & s_rready;
  assign  search_data.master_id = {MASTER_ID_W{1'b0}};
  assign  search_data.axi_id    = s_rid;
  ...
  begin
    if(~core_rst_n)
    begin
      core_r_valid              <=  {NUM_MASTERS{1'b0}};
      core_r_data               <=  {AXI4_R_STRUCT_W{1'b0}};

      current_master_id         <=  {MASTER_ID_W{1'b0}};
    end
    else
    begin
      if(core_r_valid) //Request is valid this cycle
      begin
        if(core_master_ready)  //Got grant
        begin
          core_r_valid          <=  ff_ready_w_m_decode_match  ? master_sel_sync : {NUM_MASTERS{1'b0}};
          core_r_data           <=  ff_ready_w_m_decode_match  ? ff_rd_data      : core_r_data;
          current_master_id     <=  ff_ready_w_m_decode_match  ? master_id_sync  : current_master_id;
        end
      end
      else
      begin
        core_r_valid            <=  ff_ready_w_m_decode_match  ? master_sel_sync : core_r_valid;
        core_r_data             <=  ff_ready_w_m_decode_match  ? ff_rd_data      : core_r_data;
        current_master_id       <=  ff_ready_w_m_decode_match  ? master_id_sync  : current_master_id;
      end
    end
  ...
  assign  core_master_ready = core_r_ready[current_master_id];


endmodule // axim_fabric_cbar_slave_r_egr_node
