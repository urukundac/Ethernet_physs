...
/*
 ...
 -- Module Name       : axim_fabric_cbar_independent_fifo_top
 -- Author            : Gregory James
 -- Associated modules: 
 -- Function          : The top level module for AXI4 Fabric Crossbar using
                        independent FIFOs foreach channel.
 --------------------------------------------------------------------------
*/
...
module axim_fabric_cbar_independent_fifo_top #(
...
//----------------------- Internal Wire Declarations ----------------------
  logic [NUM_SLAVES-1:0]      master_core_aw_valid [NUM_MASTERS-1:0];
  logic [NUM_SLAVES-1:0]      master_core_aw_ready [NUM_MASTERS-1:0];
  axi4_aw_struct_t            master_core_aw_data [NUM_MASTERS-1:0];

  logic [NUM_SLAVES-1:0]      master_core_w_valid [NUM_MASTERS-1:0];
  logic [NUM_SLAVES-1:0]      master_core_w_ready [NUM_MASTERS-1:0];
  axi4_w_struct_t             master_core_w_data [NUM_MASTERS-1:0];

  logic [NUM_SLAVES-1:0]      master_core_ar_valid [NUM_MASTERS-1:0];
  logic [NUM_SLAVES-1:0]      master_core_ar_ready [NUM_MASTERS-1:0];
  axi4_ar_struct_t            master_core_ar_data [NUM_MASTERS-1:0];


  logic [NUM_SLAVES-1:0]      master_core_b_valid [NUM_MASTERS-1:0];
  logic [NUM_SLAVES-1:0]      master_core_b_ready [NUM_MASTERS-1:0];
  axi4_b_struct_t             master_core_b_data [NUM_MASTERS-1:0] [NUM_SLAVES-1:0];

  logic [NUM_SLAVES-1:0]      master_core_r_valid [NUM_MASTERS-1:0];
  logic [NUM_SLAVES-1:0]      master_core_r_ready [NUM_MASTERS-1:0];
  axi4_r_struct_t             master_core_r_data [NUM_MASTERS-1:0] [NUM_SLAVES-1:0];



  logic [NUM_MASTERS-1:0]     slave_core_aw_valid [NUM_SLAVES-1:0];
  logic [NUM_MASTERS-1:0]     slave_core_aw_ready [NUM_SLAVES-1:0];
  axi4_aw_struct_t            slave_core_aw_data  [NUM_SLAVES-1:0]  [NUM_MASTERS-1:0];

  logic [NUM_MASTERS-1:0]     slave_core_w_valid [NUM_SLAVES-1:0];
  logic [NUM_MASTERS-1:0]     slave_core_w_ready [NUM_SLAVES-1:0];
  axi4_w_struct_t             slave_core_w_data  [NUM_SLAVES-1:0] [NUM_MASTERS-1:0];

  logic [NUM_MASTERS-1:0]     slave_core_ar_valid [NUM_SLAVES-1:0];
  logic [NUM_MASTERS-1:0]     slave_core_ar_ready [NUM_SLAVES-1:0];
  axi4_ar_struct_t            slave_core_ar_data  [NUM_SLAVES-1:0]  [NUM_MASTERS-1:0];

  logic [NUM_MASTERS-1:0]     slave_core_b_valid [NUM_SLAVES-1:0];
  logic [NUM_MASTERS-1:0]     slave_core_b_ready [NUM_SLAVES-1:0];
  axi4_b_struct_t             slave_core_b_data [NUM_SLAVES-1:0];

  logic [NUM_MASTERS-1:0]     slave_core_r_valid [NUM_SLAVES-1:0];
  logic [NUM_MASTERS-1:0]     slave_core_r_ready [NUM_SLAVES-1:0];
  axi4_r_struct_t             slave_core_r_data [NUM_SLAVES-1:0];
  ...
  generate
    ...
    begin : gen_masters
      axim_fabric_cbar_master_node #(
        ...
        ,.core_aw_valid     (master_core_aw_valid[i]     )
        ,.core_aw_ready     (master_core_aw_ready[i]     )
        ,.core_aw_data      (master_core_aw_data[i]      )

        ,.core_w_valid      (master_core_w_valid[i]      )
        ,.core_w_ready      (master_core_w_ready[i]      )
        ,.core_w_data       (master_core_w_data[i]       )

        ,.core_ar_valid     (master_core_ar_valid[i]     )
        ,.core_ar_ready     (master_core_ar_ready[i]     )
        ,.core_ar_data      (master_core_ar_data[i]      )


        ,.core_b_valid      (master_core_b_valid[i]      )
        ,.core_b_ready      (master_core_b_ready[i]      )
        ,.core_b_data       (master_core_b_data[i]       )

        ,.core_r_valid      (master_core_r_valid[i]      )
        ,.core_r_ready      (master_core_r_ready[i]      )
        ,.core_r_data       (master_core_r_data[i]       )
    ...
    begin : gen_slaves
      axim_fabric_cbar_slave_node #(
        ...
        ,.core_aw_valid     (slave_core_aw_valid[i]     )
        ,.core_aw_ready     (slave_core_aw_ready[i]     )
        ,.core_aw_data      (slave_core_aw_data[i]      )

        ,.core_w_valid      (slave_core_w_valid[i]      )
        ,.core_w_ready      (slave_core_w_ready[i]      )
        ,.core_w_data       (slave_core_w_data[i]       )

        ,.core_ar_valid     (slave_core_ar_valid[i]     )
        ,.core_ar_ready     (slave_core_ar_ready[i]     )
        ,.core_ar_data      (slave_core_ar_data[i]      )

        ,.core_b_valid      (slave_core_b_valid[i]      )
        ,.core_b_ready      (slave_core_b_ready[i]      )
        ,.core_b_data       (slave_core_b_data[i]       )

        ,.core_r_valid      (slave_core_r_valid[i]      )
        ,.core_r_ready      (slave_core_r_ready[i]      )
        ,.core_r_data       (slave_core_r_data[i]       )
    ...
    begin : gen_masters_interconnect
      ...
      begin : gen_slaves_interconnect
        assign  slave_core_aw_valid[j][i]     = master_core_aw_valid[i][j];
        assign  slave_core_aw_data[j][i]      = master_core_aw_data[i];
        assign  master_core_aw_ready[i][j]    = slave_core_aw_ready[j][i];

        assign  slave_core_w_valid[j][i]      = master_core_w_valid[i][j];
        assign  slave_core_w_data[j][i]       = master_core_w_data[i];
        assign  master_core_w_ready[i][j]     = slave_core_w_ready[j][i];

        assign  slave_core_ar_valid[j][i]     = master_core_ar_valid[i][j];
        assign  slave_core_ar_data[j][i]      = master_core_ar_data[i];
        assign  master_core_ar_ready[i][j]    = slave_core_ar_ready[j][i];


        assign  master_core_b_valid[i][j]     = slave_core_b_valid[j][i];
        assign  master_core_b_data[i][j]      = slave_core_b_data[j];
        assign  slave_core_b_ready[j][i]      = master_core_b_ready[i][j];

        assign  master_core_r_valid[i][j]     = slave_core_r_valid[j][i];
        assign  master_core_r_data[i][j]      = slave_core_r_data[j];
        assign  slave_core_r_ready[j][i]      = master_core_r_ready[i][j];
      end //gen_slaves_interconnect
    end //gen_masters_interconnect

  endgenerate

endmodule // axim_fabric_cbar_independent_fifo_top
