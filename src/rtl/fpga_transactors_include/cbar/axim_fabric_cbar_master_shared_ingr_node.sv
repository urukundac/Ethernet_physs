...
/*
 ...
 -- Module Name       : axim_fabric_cbar_master_shared_ingr_node
 -- Author            : Gregory James
 -- Associated modules: 
 -- Function          : This module receives B & R transactions from a
                        common FIFO & routes to the Master channels.
 --------------------------------------------------------------------------
*/
...
module axim_fabric_cbar_master_shared_ingr_node #(
...
//----------------------- Port Types -------------------------------------
  ...
  axi4_shared_core_stuct_t  core_ingr_data [NUM_SLAVES-1:0];
...
//----------------------- Start of Code -----------------------------------
  ...
  axi_fabric_arb #(
    ...
    /*  output logic [SLAVE_ID_W-1:0]         */  ,.gnt_agent_id    (gnt_agent_id)
    /*  output logic [NUM_AGENTS-1:0]         */  ,.gnt_agent_sel   (gnt_agents_sel)

  );

  assign  agent_req     = ff_wr_full  ? {NUM_SLAVES{1'b0}} : core_ingr_valid;
  ...
  assign  core_ingr_ready = gnt_agents_sel;
  ...
  //B Signals
  assign  m_bid   = ff_rd_b_data.bid;
  assign  m_bresp = ff_rd_b_data.bresp;
  assign  m_buser = ff_rd_b_data.buser;

  assign  m_bvalid  = ~ff_rd_empty  ? ((ff_rd_data.xtn == B_XTN) ? 1'b1 : 1'b0) : 1'b0;


  //R Signals
  assign  m_rid     = ff_rd_r_data.rid;
  assign  m_rdata   = ff_rd_r_data.rdata;
  assign  m_rresp   = ff_rd_r_data.rresp;
  assign  m_rlast   = ff_rd_r_data.rlast;
  assign  m_ruser   = ff_rd_r_data.ruser;

  assign  m_rvalid  = ~ff_rd_empty  ? ((ff_rd_data.xtn == R_XTN) ? 1'b1 : 1'b0) : 1'b0;
  ...
  begin
    ...
    begin
      ...
      begin
        ff_rd_en    = m_bready; //m_bvalid should get set high at this point as well
      end
      else if(ff_rd_data.xtn == R_XTN)
      begin
        ff_rd_en    = m_rready; //m_rvalid should get set high at this point as well
      end
    end
  end


endmodule // axim_fabric_cbar_master_shared_ingr_node
