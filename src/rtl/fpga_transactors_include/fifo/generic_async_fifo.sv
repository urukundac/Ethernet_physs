...
/*
 ...
 -- Module Name       : generic_async_fifo
 -- Author            : Gregory James
 -- Associated modules: 
 -- Function          : This is a parameterized Asynchronous FIFO with FWFT/
                        lookahead behaviour
 --------------------------------------------------------------------------
*/
...
//----------------------- Start of Code -----------------------------------
  ...
  /* Generate Combined Resets */
  assign rst_combined_int_n = wr_rst_n & rd_rst_n;

  always@(posedge wr_clk, negedge rst_combined_int_n)
  begin
    if(~rst_combined_int_n)
    begin
      wr_rst_int_n  <= 1'b0;
    end
    else
  ...
  always@(posedge rd_clk, negedge rst_combined_int_n)
  begin
    if(~rst_combined_int_n)
    begin
      rd_rst_int_n  <= 1'b0;
  ...
  generate
    if(WIDTH_RATIO  ==  1)
    begin
      generic_dd_sync#(.WIDTH(MEM_ADDR_W+1),.NUM_STAGES(NUM_SYNC_STAGES),.USE_RESET(1),.RESET_VAL({(MEM_ADDR_W+1){1'b0}}))  u_wrptr_sync (.clk(rd_clk),.rst_n(rd_rst_int_n),.in(wrptr),.out(wrptr_sync));

      generic_dd_sync#(.WIDTH(MEM_ADDR_W+1),.NUM_STAGES(NUM_SYNC_STAGES),.USE_RESET(1),.RESET_VAL({(MEM_ADDR_W+1){1'b0}}))  u_rdptr_sync (.clk(wr_clk),.rst_n(wr_rst_int_n),.in(rdptr),.out(rdptr_sync));
    end
    else if(WRITE_IS_MIN)
    begin
      generic_dd_sync#(.WIDTH(MEM_ADDR_W+1),.NUM_STAGES(NUM_SYNC_STAGES),.USE_RESET(1),.RESET_VAL({(MEM_ADDR_W+1){1'b0}}))  u_wrptr_sync (.clk(rd_clk),.rst_n(rd_rst_int_n),.in(wrptr_wadj),.out(wrptr_sync));
    ...
    begin
      ...
      generic_dd_sync#(.WIDTH(MEM_ADDR_W+1),.NUM_STAGES(NUM_SYNC_STAGES),.USE_RESET(1),.RESET_VAL({(MEM_ADDR_W+1){1'b0}}))  u_rdptr_sync (.clk(wr_clk),.rst_n(wr_rst_int_n),.in(rdptr_wadj),.out(rdptr_sync));
    end
  endgenerate
  ...
  assign  wrptr_inc = wr_en & ~wr_full; //protect FIFO from overflow

  always@(posedge wr_clk, negedge wr_rst_int_n)
  begin
    if(~wr_rst_int_n)
    begin
    ...
    begin
      ...
      wr_afull            <=  wr_full_next | ((wr_occupancy_mem_next >= AFULL_NUM_WORDS) ? 1'b1 : 1'b0);
  ...
  assign  wr_full_next    = (wrptr_next ==  {~rdptr_sync[MEM_ADDR_W:MEM_ADDR_W-1],rdptr_sync[MEM_ADDR_W-2:0]})  ? 1'b1  : 1'b0;
  ...
  generate
    if(MEM_RD_DELAY > 1)
    begin
      always@(posedge rd_clk, negedge rd_rst_int_n)
      begin
        if(~rd_rst_int_n)
        begin
  ...
  assign  mem_rd_data_valid = mem_rd_data_valid_pipe[MEM_RD_DELAY-1]; //This signal is not used & the fixed 1 cycle mem read delay will work. Dangerous.
  ...
  begin
    ...
    begin
      ...
      begin
        rd_aempty         <=  (rd_occupancy_mem_next <= AEMPTY_NUM_WORDS) ? 1'b1 : 1'b0;
      end
  ...
  assign  rd_empty_next = mem_empty_next;
