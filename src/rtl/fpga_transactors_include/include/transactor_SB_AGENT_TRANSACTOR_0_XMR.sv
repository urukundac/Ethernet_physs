...
`ifdef F2F_BRIDGE
    `define 	SB_FABRIC_PATH_0  				fpga_transactors_PAR_top	
`else
    `define 	SB_FABRIC_PATH_0  			`FPGA_TRANSACTORS_TOP		
...
`ifdef F2F_BRIDGE
assign `TRANSACTORS_PATH.side_clkack[0] 							= `SB_FABRIC_PATH_0.side_clkreq_0 	;
`else
...
`ifdef F2F_BRIDGE
assign `SB_FABRIC_PATH_0.side_clkack_0 								= `TRANSACTORS_PATH.side_clkreq[0];
`else
