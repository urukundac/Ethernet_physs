...
module  hlp_mod_wrap_tcam_mod_ms_sci_table_tcam_shell_512x80 #(
...
//clc        output        [*num_of_inst*-1:0]                      fscan_sout
        ...
        input wire MEM2_BIST_COLLAR_SI, output wire BIST_SO, 
        ...
        input wire BIST_COLLAR_EN0, input wire BIST_COLLAR_EN2, 
        input wire BIST_RUN_TO_COLLAR0, input wire BIST_RUN_TO_COLLAR2, 
        ...
        output wire tcam_row_0_col_1_bisr_inst_SO, input wire DFTSHIFTEN_ts1, 
...
// Disassembling the to_tcam bus
  ...
  wire FLS, FLS_ts1, TVBE, TVBE_ts1, TVBI, TVBI_ts1, BISTE, BISTE_ts1, 
       ...
       tcam_row_0_col_0_interface_inst_WE, tcam_row_0_col_1_interface_inst_WE, 
       TWE, TWE_ts1, tcam_row_0_col_0_interface_inst_RE, 
       tcam_row_0_col_1_interface_inst_RE, TRE, TRE_ts1, 
       tcam_row_0_col_0_interface_inst_ME, tcam_row_0_col_1_interface_inst_ME, 
...
localparam TCAM_SLICE_EN_MIN = TCAM_SLICE_EN_FACT < TCAM_SLICE_EN_WIDTH ? TCAM_SLICE_EN_FACT : TCAM_SLICE_EN_WIDTH;
    ...
    logic    [80-1:0]    tcam_row_wr_data;
    always_comb begin
            tcam_row_wr_data        = wr_data;
    end
    logic    [40-1:0]    tcam_wr_data_col[2-1:0];
    always_comb begin
    ...
    logic    [80-1:0]    tcam_chk_key;
    always_comb begin
            tcam_chk_key        = chk_key;
    end
    logic    [40-1:0]    tcam_chk_key_col[2-1:0];
    always_comb begin
        for (int i = 0; i < 2; i = i + 1) begin
    ...
    logic    [80-1:0]    tcam_chk_mask;
    always_comb begin
            tcam_chk_mask        = '0;
            tcam_chk_mask        = wr_en ? '1 : chk_mask_int;
    end
    logic    [40-1:0]    tcam_chk_mask_col[2-1:0];
    always_comb begin
        for (int i = 0; i < 2; i = i + 1) begin
    ...
    logic    [40-1:0]    tcam_rd_data_col[1-1:0][2-1:0];
    logic    [80-1:0]    tcam_rd_data_row;
    always_comb begin
        for (int i = 0; i < 2; i = i + 1) begin
    ...
    logic    [512-1:0]    tcam_col_hit_out[1-1:0][2-1:0];
    logic    [512-1:0]    tcam_hit_out;
    always_comb begin
        for (int i = 0; i < 1; i = i + 1) begin
                tcam_hit_out[i*512+:512]    =  tcam_col_hit_out[i][0]   & tcam_col_hit_out[i][1];
        end
    end
...
// Match In Array 
    ...
    // EBBs Instantiation
        ...
        wire    [TCAM_ANALOG_TUNE_CONFG_WIDTH-1:0]       tcam_row_0_col_1_cfg = analog_tune_confg;
        wire    [8-1:0]                                  tcam_row_0_col_1_slice_en = slice_en[0+:8];
        ...
        wire    [10-1:0]                               tcam_row_0_col_1_adr  = tcam_row_adr[0][10-1:0];
        wire    [40-1:0]                              tcam_row_0_col_1_data_in = tcam_row_wr_en[0] ? tcam_wr_data_col[1][40-1:0] : tcam_chk_key_col[1][40-1:0];
        wire    [40-1:0]                              tcam_row_0_col_1_mask_in = tcam_chk_mask_col[1][40-1:0]; // must be inverted // inverted back in DFT wrapper
        wire    [512-1:0]                               tcam_row_0_col_1_hit_in = tcam_raw_hit_in[0];
        wire    [40-1:0]                              tcam_row_0_col_1_data_out;
        wire    [512-1:0]                               tcam_row_0_col_1_hit_out;
        
        //needed for intel18A TCAM
        logic    [40-1:0]                              tcam_row_0_col_1_data_out_ff; // must delay data_out 1 cycle due to 1 cycle read within intel18A TCAM
        logic                                                    tcam_row_0_col_1_x_ybar_sel;
        
        logic  [40-1:0]                              tcam_row_0_col_1_data_in_inv;
        
        wire tcam_row_0_col_1_memen;
        wire tcam_row_0_col_1_dft_wrp_rst;
        
        assign tcam_row_0_col_1_data_in_inv = ~tcam_row_0_col_1_data_in; // must be inverted
        
        assign tcam_row_0_col_1_x_ybar_sel = ~tcam_row_0_col_1_adr[0:0]; // must be inverted
        
        
        assign tcam_row_0_col_1_memen = tcam_row_0_col_1_wr_en ^ tcam_row_0_col_1_rd_en; // disable mem if ren = 1 and wen = 1 and also wen ren = 0 and wen = 0;
        assign tcam_row_0_col_1_dft_wrp_rst = ~fary_output_reset; // changed in DFT wrp
        ...
        tcam_row_0_col_1  (       
                     ...
                     //------------------------------------------------------------
                       ...
                       .CLK                             (tcam_row_0_col_1_clk),
                       .DIN                             (DIN_ts1),
                       .RE                              (tcam_row_0_col_1_interface_inst_RE),
                       .WE                              (tcam_row_0_col_1_interface_inst_WE),
                       .ME                              (tcam_row_0_col_1_interface_inst_ME),
                       .MASK                            (tcam_row_0_col_1_mask_in),
                       .CMP                             (tcam_row_0_col_1_chk_en), //srchen
                       .ADR                             ({tcam_row_0_col_1_x_ybar_sel,tcam_row_0_col_1_adr[10-1:1]}), //reorder this because that's how they implemented it in dft_wrp
        
                       .VBI                               (1'b0), // dinvalidmask
                       .VBE                               (1'b1), // dinvalid
                       .RST                               (tcam_row_0_col_1_dft_wrp_rst), //output_reset 
                       .RVLD                              ('1), //matchin
                       .QOUT                              (tcam_row_0_col_1_data_out),
                       .QHR                               (tcam_row_0_col_1_hit_out),
                       .DSEL                              (tcam_row_0_col_1_x_ybar_sel), //x_ybar_sel
                       .FLS                               (FLS_ts1), //glbvalidbitrst
                       .QVLD                            (),
                  
                       //.RESET_N                          (tcam_row_0_col_1_reset_n),
                      //.FUNC_CM_SLICE_EN                 (tcam_row_0_col_1_slice_en),
                       //Test inputs
                       .TADR (TADR_ts1),
                       .TVBE(TVBE_ts1),
                       .TVBI (TVBI_ts1),
                 ...
                 //-------------------------------------------------------------      
                    ...
                    .FAF           (tcam_row_0_col_1_bisr_inst_Q[6:1]), // column repair addres? --- > F means faulty
                    .FAF_EN         (tcam_row_0_col_1_bisr_inst_Q[0]), //Column repair enable
        ...
        always_ff @(posedge clk) begin
           tcam_row_0_col_1_data_out_ff <= tcam_row_0_col_1_data_out;
        end
        
        assign          tcam_rd_data_col[0][1]      = ~tcam_row_0_col_1_data_out_ff; // invert data_out before sending back to controller // 
        assign          tcam_col_hit_out[0][1]       = tcam_row_0_col_1_hit_out; // DFT_WRP already delays this by one
...
`endif
  ...
  hlp_mod_apr_tcam_mems_rtl_tessent_mbist_TCAM_c1_interface_MEM1 tcam_row_0_col_0_interface_inst(
  ...
  hlp_mod_apr_tcam_mems_rtl_tessent_mbist_TCAM_c1_interface_MEM2 tcam_row_0_col_1_interface_inst(
      .BIST_CLEAR_BIRA(BIST_CLEAR_BIRA), .FLS_IN(1'b0), .TVBE_IN(1'b0), .TVBI_IN(1'b0), 
      .BIST_CM_MODE_IN(1'b0), .BIST_CM_EN_IN_IN(1'b0), .BIST_CM_MATCH_SEL0_IN(1'b0), 
      .BIST_CM_MATCH_SEL1_IN(1'b0), .BIST_ROTATE_SR_IN(1'b0), .BIST_ALL_MASK_IN(1'b0), 
      .BIST_EN_SR_DATA_B_IN(1'b0), .BIST_EN_SR_MASK_B_IN(1'b0), .BIST_MATCHIN_IN_IN(1'b0), 
      .BIST_FLS_IN(1'b0), .WE_IN(tcam_row_0_col_1_wr_en), .RE_IN(tcam_row_0_col_1_rd_en), 
      .ME_IN(tcam_row_0_col_1_memen), .QOUT(tcam_row_0_col_1_data_out), .DIN_IN(tcam_row_0_col_1_data_in_inv[39:0]), 
      .TCK(to_interfaces_tck), .BIST_CMP(BIST_CMP), .INCLUDE_MEM_RESULTS_REG(INCLUDE_MEM_RESULTS_REG), 
      .BIST_USER9(BIST_USER9), .BIST_USER10(BIST_USER10), .BIST_USER11(BIST_USER11), 
      .BIST_USER0(BIST_USER0), .BIST_USER1(BIST_USER1), .BIST_USER2(BIST_USER2), 
      .BIST_USER3(BIST_USER3), .BIST_USER4(BIST_USER4), .BIST_USER5(BIST_USER5), 
      ...
      .BIST_CLEAR(BIST_CLEAR), .BIST_SETUP2(BIST_SETUP_ts2), .BIST_SETUP1(BIST_SETUP_ts1), 
      .BIST_SETUP0(BIST_SETUP), .LV_TM(ltest_to_en), .FREEZE_STOP_ERROR(FREEZE_STOP_ERROR), 
      .GO_ID_REG_SEL(GO_ID_REG_SEL), .BIST_COLLAR_EN(BIST_COLLAR_EN2), .FROM_BISR_All_SCOL0_FUSE_REG(tcam_row_0_col_1_bisr_inst_Q[6:1]), 
      .FROM_BISR_All_SCOL0_ALLOC_REG(tcam_row_0_col_1_bisr_inst_Q[0]), .BIST_SHIFT_BIRA_COLLAR(BIST_SHIFT_BIRA_COLLAR), 
      .RESET_REG_SETUP2(MBISTPG_RESET_REG_SETUP2), .ERROR_CNT_ZERO(ERROR_CNT_ZERO), 
      ...
      .BIST_FLS(BIST_FLS_ts1), .TMASK(TMASK_ts1), .WE(tcam_row_0_col_1_interface_inst_WE), 
      .TWE(TWE_ts1), .RE(tcam_row_0_col_1_interface_inst_RE), .TRE(TRE_ts1), .ME(tcam_row_0_col_1_interface_inst_ME), 
  ...
  hlp_mod_apr_tcam_mems_rtl_tessent_mbisr_register_ip783tcam512x40s0c1_dft_wrp tcam_row_0_col_0_bisr_inst(
  ...
  hlp_mod_apr_tcam_mems_rtl_tessent_mbisr_register_ip783tcam512x40s0c1_dft_wrp tcam_row_0_col_1_bisr_inst(
      .CLK(bisr_clk_pd_vinf), .RSTB(bisr_reset_pd_vinf), .SI(tcam_row_0_col_0_bisr_inst_SO), 
      .SO(tcam_row_0_col_1_bisr_inst_SO), .SE(bisr_shift_en_pd_vinf), .D({
      All_SCOL0_FUSE_REG_ts1[5:0], All_SCOL0_ALLOC_REG_ts1}), .MSO(1'b0), .MSEL(1'b0), 
      .Q(tcam_row_0_col_1_bisr_inst_Q)
  );
endmodule
