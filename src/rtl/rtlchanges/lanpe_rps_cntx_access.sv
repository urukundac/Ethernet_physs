...
(
    ...
    input  logic                                                      i_rlan_event_q_cntx_req_vld,
    output logic                                                      o_rlan_event_q_cntx_req_rdy,
    input  lanpe_sl_package::lan_rx__rlan_rps_event_q_cntx_ftch_t     i_rlan_event_q_cntx_req_cmd,

    // RLAN read data interface
    output logic                                                      o_rlan_event_q_cntx_rslt_vld,
    input  logic                                                      i_rlan_event_q_cntx_rslt_rdy,
    output lanpe_sl_package::lan_rps_rlan_event_q_cntxt_complt_t      o_rlan_event_q_cntx_rslt_data,

    // Q ctrl queue enable interface
    lanpe_reg_intf.master_array                                       io_q_ctrl_q_en_intf,

    // CSR request to cntx_mem_ctrl & read data from q_cntx_early_static
    output logic                                                      o_mem_ctrl_csr_q_cntx_early_static_wr_en,
    output q_cntx_early_mem_data_t                                    o_mem_ctrl_csr_q_cntx_early_static_wr_data,
    output logic                                                      o_mem_ctrl_csr_q_cntx_early_static_rd_en,
    output q_cntx_early_mem_addr_t                                    o_mem_ctrl_csr_q_cntx_early_static_addr,
    input  logic                                                      i_mem_ctrl_csr_q_cntx_early_static_rdy,
    input  logic                                                      i_mem_ctrl_csr_q_cntx_early_static_rd_vld,
    input  q_cntx_early_mem_data_t                                    i_mem_ctrl_csr_q_cntx_early_static_rd_data,

    // CSR request to cntx_mem_ctrl & read data from q_cntx_static
    output logic                                                      o_mem_ctrl_csr_q_cntx_static_wr_en,
    output q_cntx_mem_data_t                                          o_mem_ctrl_csr_q_cntx_static_wr_data,
    output logic                                                      o_mem_ctrl_csr_q_cntx_static_rd_en,
    output q_cntx_mem_addr_t                                          o_mem_ctrl_csr_q_cntx_static_addr,
    input  logic                                                      i_mem_ctrl_csr_q_cntx_static_rdy,
    input  logic                                                      i_mem_ctrl_csr_q_cntx_static_rd_vld,
    input  q_cntx_mem_data_t                                          i_mem_ctrl_csr_q_cntx_static_rd_data,

    // CSR request to cntx_mem_ctrl & read data from q_cntx_buffer_static
    output logic                                                      o_mem_ctrl_csr_q_cntx_buffer_static_wr_en,
    output buffq_cntx_mem_data_t                                      o_mem_ctrl_csr_q_cntx_buffer_static_wr_data,
    output logic                                                      o_mem_ctrl_csr_q_cntx_buffer_static_rd_en,
    output buffq_cntx_mem_addr_t                                      o_mem_ctrl_csr_q_cntx_buffer_static_addr,
    input  logic                                                      i_mem_ctrl_csr_q_cntx_buffer_static_rdy,
    input  logic                                                      i_mem_ctrl_csr_q_cntx_buffer_static_rd_vld,
    input  buffq_cntx_mem_data_t                                      i_mem_ctrl_csr_q_cntx_buffer_static_rd_data,

    // CSR request to cntx_mem_ctrl & read data from q_cntx_first_pkt
    output logic                                                      o_mem_ctrl_csr_q_cntx_first_pkt_wr_en,
    output q_first_pkt_mem_addr_t                                     o_mem_ctrl_csr_q_cntx_first_pkt_wr_addr,
    output q_first_pkt_mem_data_t                                     o_mem_ctrl_csr_q_cntx_first_pkt_wr_data,
    input  logic                                                      i_mem_ctrl_csr_q_cntx_first_pkt_wr_rdy,
    output logic                                                      o_mem_ctrl_csr_q_cntx_first_pkt_rd_en,
    output q_first_pkt_mem_addr_t                                     o_mem_ctrl_csr_q_cntx_first_pkt_rd_addr,
    input  logic                                                      i_mem_ctrl_csr_q_cntx_first_pkt_rd_rdy,
    input  logic                                                      i_mem_ctrl_csr_q_cntx_first_pkt_rd_vld,
    input  q_first_pkt_mem_data_t                                     i_mem_ctrl_csr_q_cntx_first_pkt_rd_data,

    // CSR request to cntx_mem_ctrl & read data from q_cntx_buffer_first_pkt
    output logic                                                      o_mem_ctrl_csr_q_cntx_buffer_first_pkt_wr_en,
    output buffq_first_pkt_mem_addr_t                                 o_mem_ctrl_csr_q_cntx_buffer_first_pkt_wr_addr,
    output buffq_first_pkt_mem_data_t                                 o_mem_ctrl_csr_q_cntx_buffer_first_pkt_wr_data,
    input  logic                                                      i_mem_ctrl_csr_q_cntx_buffer_first_pkt_wr_rdy,
    output logic                                                      o_mem_ctrl_csr_q_cntx_buffer_first_pkt_rd_en,
    output buffq_first_pkt_mem_addr_t                                 o_mem_ctrl_csr_q_cntx_buffer_first_pkt_rd_addr,
    input  logic                                                      i_mem_ctrl_csr_q_cntx_buffer_first_pkt_rd_rdy,
    input  logic                                                      i_mem_ctrl_csr_q_cntx_buffer_first_pkt_rd_vld,
    input  buffq_first_pkt_mem_data_t                                 i_mem_ctrl_csr_q_cntx_buffer_first_pkt_rd_data,

    // CSR request to cntx_mem_ctrl & read data from q_cntx_ring_head
    output logic                                                      o_mem_ctrl_csr_q_cntx_ring_head_wr_en,
    output q_ring_ptr_mem_addr_t                                      o_mem_ctrl_csr_q_cntx_ring_head_wr_addr,
    output q_ring_ptr_mem_data_t                                      o_mem_ctrl_csr_q_cntx_ring_head_wr_data,
    input  logic                                                      i_mem_ctrl_csr_q_cntx_ring_head_wr_rdy,
    output logic                                                      o_mem_ctrl_csr_q_cntx_ring_head_rd_en,
    output q_ring_ptr_mem_addr_t                                      o_mem_ctrl_csr_q_cntx_ring_head_rd_addr,
    input  logic                                                      i_mem_ctrl_csr_q_cntx_ring_head_rd_rdy,
    input  logic                                                      i_mem_ctrl_csr_q_cntx_ring_head_rd_vld,
    input  q_ring_ptr_mem_data_t                                      i_mem_ctrl_csr_q_cntx_ring_head_rd_data,

    // CSR request to cntx_mem_ctrl & read data from q_cntx_ring_empty
    output logic                                                      o_mem_ctrl_csr_q_cntx_ring_empty_wr_en,
    output q_ring_empty_mem_addr_t                                    o_mem_ctrl_csr_q_cntx_ring_empty_wr_addr,
    output q_ring_empty_mem_data_t                                    o_mem_ctrl_csr_q_cntx_ring_empty_wr_data,
    input  logic                                                      i_mem_ctrl_csr_q_cntx_ring_empty_wr_rdy,
    output logic                                                      o_mem_ctrl_csr_q_cntx_ring_empty_rd_en,
    output q_ring_empty_mem_addr_t                                    o_mem_ctrl_csr_q_cntx_ring_empty_rd_addr,
    input  logic                                                      i_mem_ctrl_csr_q_cntx_ring_empty_rd_rdy,
    input  logic                                                      i_mem_ctrl_csr_q_cntx_ring_empty_rd_vld,
    input  q_ring_empty_mem_data_t                                    i_mem_ctrl_csr_q_cntx_ring_empty_rd_data,

    // RLAN request to cntx_mem_ctrl & read data from q_cntx_static
    output logic                                                      o_mem_ctrl_rlan_q_cntx_static_rd_en,
    output q_cntx_mem_addr_t                                          o_mem_ctrl_rlan_q_cntx_static_addr,
    input  logic                                                      i_mem_ctrl_rlan_q_cntx_static_rdy,
    input  logic                                                      i_mem_ctrl_rlan_q_cntx_static_rd_vld,
    input  q_cntx_mem_data_t                                          i_mem_ctrl_rlan_q_cntx_static_rd_data,
    ...
    typedef logic [$clog2(Q_FIRST_PKT_MEM_DATA_WDT)-1:0] first_pkt_mem_bit_t;
    localparam FIRST_PKT_MEM_QUEUES_PER_LINE_WDT = $clog2(Q_FIRST_PKT_QUEUES_PER_LINE);
    typedef logic [$clog2(BUFFQ_FIRST_PKT_MEM_DATA_WDT)-1:0] buffer_first_pkt_mem_bit_t;
    localparam BUFFER_FIRST_PKT_MEM_QUEUES_PER_LINE_WDT = $clog2(BUFFQ_FIRST_PKT_QUEUES_PER_LINE);
...
//    csr_cqlen_data_t                                                  csr_cqlen_rd_data_pre_l_ff;
    ...
    q_first_pkt_mem_addr_t                                            mem_ctrl_csr_q_cntx_first_pkt_addr;
    first_pkt_mem_bit_t                                               mem_ctrl_csr_q_cntx_first_pkt_bit;
    logic                                                             mem_ctrl_csr_q_cntx_first_pkt_wr_en;
    q_first_pkt_mem_data_t                                            mem_ctrl_csr_q_cntx_first_pkt_wr_data;
    logic                                                             mem_ctrl_csr_q_cntx_first_pkt_rd_en_nxt;
    reg                                                               mem_ctrl_csr_q_cntx_first_pkt_rd_en_ff;
    q_first_pkt_mem_data_t                                            mem_ctrl_csr_q_cntx_first_pkt_rd_data_sel;
    q_first_pkt_mem_data_t                                            mem_ctrl_csr_q_cntx_first_pkt_rd_data_l_ff;
    buffq_first_pkt_mem_addr_t                                        mem_ctrl_csr_q_cntx_buffer_first_pkt_addr;
    buffer_first_pkt_mem_bit_t                                        mem_ctrl_csr_q_cntx_buffer_first_pkt_bit;
    logic                                                             mem_ctrl_csr_q_cntx_buffer_first_pkt_wr_en;
    buffq_first_pkt_mem_data_t                                        mem_ctrl_csr_q_cntx_buffer_first_pkt_wr_data;
    logic                                                             mem_ctrl_csr_q_cntx_buffer_first_pkt_rd_en_nxt;
    reg                                                               mem_ctrl_csr_q_cntx_buffer_first_pkt_rd_en_ff;
    buffq_first_pkt_mem_data_t                                        mem_ctrl_csr_q_cntx_buffer_first_pkt_rd_data_sel;
    buffq_first_pkt_mem_data_t                                        mem_ctrl_csr_q_cntx_buffer_first_pkt_rd_data_l_ff;
    // QRX ring empty
    ...
    // Mem ctrl events
    assign mem_ctrl_csr_q_cntx_early_static_wr_ev     = o_mem_ctrl_csr_q_cntx_early_static_wr_en & i_mem_ctrl_csr_q_cntx_early_static_rdy;
    assign mem_ctrl_csr_q_cntx_early_static_rd_ev     = o_mem_ctrl_csr_q_cntx_early_static_rd_en & i_mem_ctrl_csr_q_cntx_early_static_rdy;
    assign mem_ctrl_csr_q_cntx_static_wr_ev           = o_mem_ctrl_csr_q_cntx_static_wr_en & i_mem_ctrl_csr_q_cntx_static_rdy;
    assign mem_ctrl_csr_q_cntx_static_rd_ev           = o_mem_ctrl_csr_q_cntx_static_rd_en & i_mem_ctrl_csr_q_cntx_static_rdy;
    assign mem_ctrl_csr_q_cntx_buffer_static_wr_ev    = o_mem_ctrl_csr_q_cntx_buffer_static_wr_en & i_mem_ctrl_csr_q_cntx_buffer_static_rdy;
    assign mem_ctrl_csr_q_cntx_buffer_static_rd_ev    = o_mem_ctrl_csr_q_cntx_buffer_static_rd_en & i_mem_ctrl_csr_q_cntx_buffer_static_rdy;
    assign mem_ctrl_csr_q_cntx_first_pkt_wr_ev        = o_mem_ctrl_csr_q_cntx_first_pkt_wr_en & i_mem_ctrl_csr_q_cntx_first_pkt_wr_rdy;
    assign mem_ctrl_csr_q_cntx_first_pkt_rd_ev        = o_mem_ctrl_csr_q_cntx_first_pkt_rd_en & i_mem_ctrl_csr_q_cntx_first_pkt_rd_rdy;
    assign mem_ctrl_csr_q_cntx_buffer_first_pkt_wr_ev = o_mem_ctrl_csr_q_cntx_buffer_first_pkt_wr_en & i_mem_ctrl_csr_q_cntx_buffer_first_pkt_wr_rdy;
    assign mem_ctrl_csr_q_cntx_buffer_first_pkt_rd_ev = o_mem_ctrl_csr_q_cntx_buffer_first_pkt_rd_en & i_mem_ctrl_csr_q_cntx_buffer_first_pkt_rd_rdy;
    assign mem_ctrl_csr_q_cntx_ring_head_wr_ev        = o_mem_ctrl_csr_q_cntx_ring_head_wr_en & i_mem_ctrl_csr_q_cntx_ring_head_wr_rdy;
    assign mem_ctrl_csr_q_cntx_ring_head_rd_ev        = o_mem_ctrl_csr_q_cntx_ring_head_rd_en & i_mem_ctrl_csr_q_cntx_ring_head_rd_rdy;
    ...
    assign csr_context_wr_req_waiting_rd_vld_nxt[0] = (i_mem_ctrl_csr_q_cntx_early_static_rd_vld) ? '0 : (csr_context_wr_req_rd_cmd_start & (~csr_context_ctl_wr_data_s_ff.buff_q)) ? '1 : csr_context_wr_req_waiting_rd_vld_ff[0];
    assign csr_context_wr_req_waiting_rd_vld_nxt[1] = (i_mem_ctrl_csr_q_cntx_static_rd_vld) ? '0 : (csr_context_wr_req_rd_cmd_start & (~csr_context_ctl_wr_data_s_ff.buff_q)) ? '1 : csr_context_wr_req_waiting_rd_vld_ff[1];
    assign csr_context_wr_req_waiting_rd_vld_nxt[2] = (i_mem_ctrl_csr_q_cntx_buffer_static_rd_vld) ? '0 : (csr_context_wr_req_rd_cmd_start & csr_context_ctl_wr_data_s_ff.buff_q) ? '1 : csr_context_wr_req_waiting_rd_vld_ff[2];
    ...
    assign csr_context_wr_req_init_stage_sel        = (csr_context_wr_req_wr_cmd_start & csr_context_wr_req_init_en) ? '0 :
                                                      (i_mem_ctrl_csr_q_cntx_first_pkt_rd_vld | i_mem_ctrl_csr_q_cntx_buffer_first_pkt_rd_vld) ? '1 : csr_context_wr_req_init_stage_ff;
    ...
    assign o_mem_ctrl_csr_q_cntx_early_static_wr_en   = mem_ctrl_csr_q_cntx_early_static_wr_en_ff;
    assign o_mem_ctrl_csr_q_cntx_early_static_wr_data = csr_context_table_to_early_static_mem;
    assign o_mem_ctrl_csr_q_cntx_early_static_rd_en   = mem_ctrl_csr_q_cntx_early_static_rd_en_ff;
    assign o_mem_ctrl_csr_q_cntx_early_static_addr    = (mem_ctrl_csr_q_cntx_early_static_wr_en_ff | mem_ctrl_csr_q_cntx_early_static_rd_en_ff) ? q_cntx_early_mem_addr_t'(csr_context_ctl_l_ff.queue_id) : '0;

    assign o_mem_ctrl_csr_q_cntx_static_wr_en         = mem_ctrl_csr_q_cntx_static_wr_en_ff | mem_ctrl_csr_cq_cntx_static_wr_en_ff;
    assign o_mem_ctrl_csr_q_cntx_static_wr_data       = (mem_ctrl_csr_q_cntx_static_wr_en_ff) ? csr_context_table_to_static_mem : (mem_ctrl_csr_cq_cntx_static_wr_en_ff) ? csr_cq_context_table_ff : '0;
    assign o_mem_ctrl_csr_q_cntx_static_rd_en         = mem_ctrl_csr_q_cntx_static_rd_en_ff | mem_ctrl_csr_cq_cntx_static_rd_en_ff;
    assign o_mem_ctrl_csr_q_cntx_static_addr          = (mem_ctrl_csr_q_cntx_static_wr_en_ff | mem_ctrl_csr_q_cntx_static_rd_en_ff) ? q_cntx_mem_addr_t'(csr_context_ctl_l_ff.queue_id) :
                                                        (mem_ctrl_csr_cq_cntx_static_wr_en_ff | mem_ctrl_csr_cq_cntx_static_rd_en_ff) ? csr_cq_idx_l_ff : '0;

    assign o_mem_ctrl_csr_q_cntx_buffer_static_wr_en  = mem_ctrl_csr_q_cntx_buffer_static_wr_en_ff;
    assign o_mem_ctrl_csr_q_cntx_buffer_static_wr_data= csr_context_table_to_buffer_static_mem;
    assign o_mem_ctrl_csr_q_cntx_buffer_static_rd_en  = mem_ctrl_csr_q_cntx_buffer_static_rd_en_ff;
    assign o_mem_ctrl_csr_q_cntx_buffer_static_addr   = csr_context_ctl_l_ff.queue_id;

    always_comb begin
        csr_context_table_from_static_mem                          = csr_context_table_ff;
        if (i_mem_ctrl_csr_q_cntx_static_rd_vld) begin
            csr_context_table_from_static_mem.ipf_divert_report_pf = i_mem_ctrl_csr_q_cntx_static_rd_data.report_pf;
            csr_context_table_from_static_mem.var_buff_size_mode   = i_mem_ctrl_csr_q_cntx_static_rd_data.var_buff_size_mode;
            csr_context_table_from_static_mem.func_mal_policy      = i_mem_ctrl_csr_q_cntx_static_rd_data.func_mal_policy;
            csr_context_table_from_static_mem.hostoff_func_num     = i_mem_ctrl_csr_q_cntx_static_rd_data.hostoff_func_num;
            csr_context_table_from_static_mem.hostoff_func_type    = i_mem_ctrl_csr_q_cntx_static_rd_data.hostoff_func_type;
            csr_context_table_from_static_mem.hostoff_pf_num       = i_mem_ctrl_csr_q_cntx_static_rd_data.hostoff_pf_num;
            csr_context_table_from_static_mem.hostoff_home         = i_mem_ctrl_csr_q_cntx_static_rd_data.hostoff_home;
            csr_context_table_from_static_mem.hostoff_pasid_valid  = i_mem_ctrl_csr_q_cntx_static_rd_data.hostoff_pasid_valid;
            csr_context_table_from_static_mem.hostoff_pasid        = i_mem_ctrl_csr_q_cntx_static_rd_data.hostoff_pasid;
            csr_context_table_from_static_mem.hsplit_pasid_select  = i_mem_ctrl_csr_q_cntx_static_rd_data.hsplit_pasid_select;
            csr_context_table_from_static_mem.data_pasid           = i_mem_ctrl_csr_q_cntx_static_rd_data.data_pasid;
            csr_context_table_from_static_mem.data_pasid_valid     = i_mem_ctrl_csr_q_cntx_static_rd_data.data_pasid_valid;
            csr_context_table_from_static_mem.desc_pasid_valid     = i_mem_ctrl_csr_q_cntx_static_rd_data.desc_pasid_valid;
            csr_context_table_from_static_mem.desc_pasid           = i_mem_ctrl_csr_q_cntx_static_rd_data.desc_pasid;
            csr_context_table_from_static_mem.flex_field4_decode   = i_mem_ctrl_csr_q_cntx_static_rd_data.flex_field4_decode;
            csr_context_table_from_static_mem.flex_field3_decode   = i_mem_ctrl_csr_q_cntx_static_rd_data.flex_field3_decode;
            csr_context_table_from_static_mem.flex_field2_decode   = i_mem_ctrl_csr_q_cntx_static_rd_data.flex_field2_decode;
            csr_context_table_from_static_mem.flex_field1_decode   = i_mem_ctrl_csr_q_cntx_static_rd_data.flex_field1_decode;
            csr_context_table_from_static_mem.no_expire            = i_mem_ctrl_csr_q_cntx_static_rd_data.no_expire;
            csr_context_table_from_static_mem.tph_hwb_en           = i_mem_ctrl_csr_q_cntx_static_rd_data.tph_hwb_en;
            csr_context_table_from_static_mem.tph_hdr_en           = i_mem_ctrl_csr_q_cntx_static_rd_data.tph_hdr_en;
            csr_context_table_from_static_mem.tph_data_en          = i_mem_ctrl_csr_q_cntx_static_rd_data.tph_data_en;
            csr_context_table_from_static_mem.tph_wr_desc_en       = i_mem_ctrl_csr_q_cntx_static_rd_data.tph_wr_desc_en;
            csr_context_table_from_static_mem.tph_rd_desc_en       = i_mem_ctrl_csr_q_cntx_static_rd_data.tph_rd_desc_en;
            csr_context_table_from_static_mem.p2p_tx_q_idx         = i_mem_ctrl_csr_q_cntx_static_rd_data.p2p_tx_q_idx;
            csr_context_table_from_static_mem.pref_en              = i_mem_ctrl_csr_q_cntx_static_rd_data.pref_en;
            csr_context_table_from_static_mem.low_rx_q_thr         = i_mem_ctrl_csr_q_cntx_static_rd_data.low_rx_q_thr;
            csr_context_table_from_static_mem.rx_q_len             = i_mem_ctrl_csr_q_cntx_static_rd_data.rx_q_len;
            csr_context_table_from_static_mem.base_high            = i_mem_ctrl_csr_q_cntx_static_rd_data.base_high;
            csr_context_table_from_static_mem.base_low             = i_mem_ctrl_csr_q_cntx_static_rd_data.base_low;
            csr_context_table_from_static_mem.queue_type           = i_mem_ctrl_csr_q_cntx_static_rd_data.queue_type;
            csr_context_table_from_static_mem.rd_dsize             = i_mem_ctrl_csr_q_cntx_static_rd_data.rd_dsize;
            csr_context_table_from_static_mem.wr_dsize             = i_mem_ctrl_csr_q_cntx_static_rd_data.wr_dsize;
        end
        if (i_mem_ctrl_csr_q_cntx_early_static_rd_vld) begin
            csr_context_table_from_static_mem.gen_sep_dbl          = i_mem_ctrl_csr_q_cntx_early_static_rd_data.gen_sep_dbl;
            csr_context_table_from_static_mem.send_addr_in_comp    = i_mem_ctrl_csr_q_cntx_early_static_rd_data.send_addr_in_comp;
            csr_context_table_from_static_mem.wb_aggr_size         = i_mem_ctrl_csr_q_cntx_early_static_rd_data.wb_aggr_size;
            csr_context_table_from_static_mem.requestor_id         = i_mem_ctrl_csr_q_cntx_early_static_rd_data.requestor_id;
            csr_context_table_from_static_mem.rxdid_recipe_idx     = i_mem_ctrl_csr_q_cntx_early_static_rd_data.rxdid_recipe_idx;
            csr_context_table_from_static_mem.rxdid_override       = i_mem_ctrl_csr_q_cntx_early_static_rd_data.rxdid_override;
            csr_context_table_from_static_mem.shift_units_num      = i_mem_ctrl_csr_q_cntx_early_static_rd_data.shift_units_num;
            csr_context_table_from_static_mem.shift_pkt_mode       = i_mem_ctrl_csr_q_cntx_early_static_rd_data.shift_pkt_mode;
            csr_context_table_from_static_mem.hostoff_home         = i_mem_ctrl_csr_q_cntx_early_static_rd_data.hostoff_home;
            csr_context_table_from_static_mem.rsc_q_context        = i_mem_ctrl_csr_q_cntx_early_static_rd_data.rsc_q_context;
            csr_context_table_from_static_mem.buffer_q1_en         = i_mem_ctrl_csr_q_cntx_early_static_rd_data.buffer_q1_en;
            csr_context_table_from_static_mem.buffer_q1_index      = i_mem_ctrl_csr_q_cntx_early_static_rd_data.buffer_q1_index;
            csr_context_table_from_static_mem.buffer_q0_index      = i_mem_ctrl_csr_q_cntx_early_static_rd_data.buffer_q0_index;
            csr_context_table_from_static_mem.rx_max_pkt_sz        = i_mem_ctrl_csr_q_cntx_early_static_rd_data.rx_max_pkt_sz;
            csr_context_table_from_static_mem.strip_crc            = i_mem_ctrl_csr_q_cntx_early_static_rd_data.strip_crc;
            csr_context_table_from_static_mem.hbuff                = i_mem_ctrl_csr_q_cntx_early_static_rd_data.hbuff;
            csr_context_table_from_static_mem.buffer_q1_dbuff      = i_mem_ctrl_csr_q_cntx_early_static_rd_data.buffer_q1_dbuff;
            csr_context_table_from_static_mem.dbuff                = i_mem_ctrl_csr_q_cntx_early_static_rd_data.dbuff;
            csr_context_table_from_static_mem.dtype                = i_mem_ctrl_csr_q_cntx_early_static_rd_data.dtype;
            csr_context_table_from_static_mem.queue_model          = i_mem_ctrl_csr_q_cntx_early_static_rd_data.queue_model;
        end
    end

    always_comb begin
        csr_context_table_from_buffer_static_mem                     = csr_context_table_ff;
        if (i_mem_ctrl_csr_q_cntx_buffer_static_rd_vld) begin
            csr_context_table_from_buffer_static_mem.head_wb_addr    = i_mem_ctrl_csr_q_cntx_buffer_static_rd_data.head_wb_addr;
            csr_context_table_from_buffer_static_mem.buff_ntf_dis    = i_mem_ctrl_csr_q_cntx_buffer_static_rd_data.buff_ntf_dis;
            csr_context_table_from_buffer_static_mem.buff_ntf_stride = i_mem_ctrl_csr_q_cntx_buffer_static_rd_data.buff_ntf_stride;
            csr_context_table_from_buffer_static_mem.tph_hwb_en      = i_mem_ctrl_csr_q_cntx_buffer_static_rd_data.tph_hwb_en;
            csr_context_table_from_buffer_static_mem.tph_rd_desc_en  = i_mem_ctrl_csr_q_cntx_buffer_static_rd_data.tph_rd_desc_en;
            csr_context_table_from_buffer_static_mem.pref_en         = i_mem_ctrl_csr_q_cntx_buffer_static_rd_data.pref_en;
            csr_context_table_from_buffer_static_mem.low_rx_q_thr    = i_mem_ctrl_csr_q_cntx_buffer_static_rd_data.low_rx_q_thr;
            csr_context_table_from_buffer_static_mem.rx_q_len        = i_mem_ctrl_csr_q_cntx_buffer_static_rd_data.rx_q_len;
            csr_context_table_from_buffer_static_mem.base_high       = i_mem_ctrl_csr_q_cntx_buffer_static_rd_data.base_high;
            csr_context_table_from_buffer_static_mem.base_low        = i_mem_ctrl_csr_q_cntx_buffer_static_rd_data.base_low;
        end
    end
    ...
    assign mem_ctrl_csr_q_cntx_first_pkt_wr_en     = i_mem_ctrl_csr_q_cntx_first_pkt_rd_vld & csr_context_wr_req_init_active_ff;
    ...
    assign mem_ctrl_csr_q_cntx_first_pkt_addr = target_q_idx_t'(csr_context_ctl_l_ff.queue_id) >> FIRST_PKT_MEM_QUEUES_PER_LINE_WDT;
    assign mem_ctrl_csr_q_cntx_first_pkt_bit  = (csr_context_wr_req_init_vm_mig_ff) ? {csr_context_ctl_l_ff.queue_id[FIRST_PKT_MEM_QUEUES_PER_LINE_WDT-1:0], 1'b1}:
                                                                                      {csr_context_ctl_l_ff.queue_id[FIRST_PKT_MEM_QUEUES_PER_LINE_WDT-1:0], 1'b0};

    assign mem_ctrl_csr_q_cntx_first_pkt_rd_data_sel = (i_mem_ctrl_csr_q_cntx_first_pkt_rd_vld) ? i_mem_ctrl_csr_q_cntx_first_pkt_rd_data : mem_ctrl_csr_q_cntx_first_pkt_rd_data_l_ff;
    ...
    always_comb begin : always_q_cntx_first_pkt_wr_data
        for (int bit_i = 0; bit_i < Q_FIRST_PKT_MEM_DATA_WDT; bit_i++) begin
    ...
    assign o_mem_ctrl_csr_q_cntx_first_pkt_wr_en   = mem_ctrl_csr_q_cntx_first_pkt_wr_en;
    assign o_mem_ctrl_csr_q_cntx_first_pkt_wr_addr = (mem_ctrl_csr_q_cntx_first_pkt_wr_en) ? mem_ctrl_csr_q_cntx_first_pkt_addr : '0;
    assign o_mem_ctrl_csr_q_cntx_first_pkt_wr_data = (mem_ctrl_csr_q_cntx_first_pkt_wr_en) ? mem_ctrl_csr_q_cntx_first_pkt_wr_data : '0;
    assign o_mem_ctrl_csr_q_cntx_first_pkt_rd_en   = mem_ctrl_csr_q_cntx_first_pkt_rd_en_ff;
    assign o_mem_ctrl_csr_q_cntx_first_pkt_rd_addr = (mem_ctrl_csr_q_cntx_first_pkt_rd_en_ff) ? mem_ctrl_csr_q_cntx_first_pkt_addr : '0;

    // QRX BUFFER FIRST PACKET MEMORY ACCESS

    // Write must be done at cycle of rd_vld to avoid collisions with other client
    assign mem_ctrl_csr_q_cntx_buffer_first_pkt_wr_en     = i_mem_ctrl_csr_q_cntx_buffer_first_pkt_rd_vld & csr_context_wr_req_init_active_ff;
    ...
    assign mem_ctrl_csr_q_cntx_buffer_first_pkt_addr = csr_context_ctl_l_ff.queue_id >> BUFFER_FIRST_PKT_MEM_QUEUES_PER_LINE_WDT;
    assign mem_ctrl_csr_q_cntx_buffer_first_pkt_bit  = (csr_context_wr_req_init_vm_mig_ff) ? {csr_context_ctl_l_ff.queue_id[BUFFER_FIRST_PKT_MEM_QUEUES_PER_LINE_WDT-1:0], 1'b1}:
                                                                                             {csr_context_ctl_l_ff.queue_id[BUFFER_FIRST_PKT_MEM_QUEUES_PER_LINE_WDT-1:0], 1'b0};

    assign mem_ctrl_csr_q_cntx_buffer_first_pkt_rd_data_sel = (i_mem_ctrl_csr_q_cntx_buffer_first_pkt_rd_vld) ? i_mem_ctrl_csr_q_cntx_buffer_first_pkt_rd_data : mem_ctrl_csr_q_cntx_buffer_first_pkt_rd_data_l_ff;
    ...
    always_comb begin : always_q_cntx_buffer_first_pkt_wr_data
        for (int bit_i = 0; bit_i < BUFFQ_FIRST_PKT_MEM_DATA_WDT; bit_i++) begin
    ...
    assign o_mem_ctrl_csr_q_cntx_buffer_first_pkt_wr_en   = mem_ctrl_csr_q_cntx_buffer_first_pkt_wr_en;
    assign o_mem_ctrl_csr_q_cntx_buffer_first_pkt_wr_addr = (mem_ctrl_csr_q_cntx_buffer_first_pkt_wr_en) ? mem_ctrl_csr_q_cntx_buffer_first_pkt_addr : '0;
    assign o_mem_ctrl_csr_q_cntx_buffer_first_pkt_wr_data = (mem_ctrl_csr_q_cntx_buffer_first_pkt_wr_en) ? mem_ctrl_csr_q_cntx_buffer_first_pkt_wr_data : '0;
    assign o_mem_ctrl_csr_q_cntx_buffer_first_pkt_rd_en   = mem_ctrl_csr_q_cntx_buffer_first_pkt_rd_en_ff;
    assign o_mem_ctrl_csr_q_cntx_buffer_first_pkt_rd_addr = (mem_ctrl_csr_q_cntx_buffer_first_pkt_rd_en_ff) ? mem_ctrl_csr_q_cntx_buffer_first_pkt_addr : '0;
    ...
    assign csr_q_head_ack_nxt     = (mem_ctrl_csr_q_cntx_ring_head_wr_ev | i_mem_ctrl_csr_q_cntx_ring_head_rd_vld) & csr_q_head_vld_l_ff;
    assign csr_q_head_rd_data_nxt = (i_mem_ctrl_csr_q_cntx_ring_head_rd_vld & csr_q_head_vld_l_ff) ? i_mem_ctrl_csr_q_cntx_ring_head_rd_data : '0;
    ...
    assign csr_buffq_head_ack_nxt     = (mem_ctrl_csr_q_cntx_ring_head_wr_ev | i_mem_ctrl_csr_q_cntx_ring_head_rd_vld) & csr_buffq_head_vld_l_ff;
    assign csr_buffq_head_rd_data_nxt = (i_mem_ctrl_csr_q_cntx_ring_head_rd_vld & csr_buffq_head_vld_l_ff) ? i_mem_ctrl_csr_q_cntx_ring_head_rd_data : '0;
    ...
    assign o_mem_ctrl_csr_q_cntx_ring_head_wr_en   = mem_ctrl_csr_q_cntx_ring_head_wr_en_ff;
    assign o_mem_ctrl_csr_q_cntx_ring_head_wr_addr = (mem_ctrl_csr_q_cntx_ring_head_wr_en_ff | mem_ctrl_csr_q_cntx_ring_head_rd_en_ff) ? mem_ctrl_csr_q_cntx_ring_head_addr : '0;
    assign o_mem_ctrl_csr_q_cntx_ring_head_wr_data = (mem_ctrl_csr_q_cntx_ring_head_wr_en_ff) ? mem_ctrl_csr_q_cntx_ring_head_wr_data : '0;
    assign o_mem_ctrl_csr_q_cntx_ring_head_rd_en   = mem_ctrl_csr_q_cntx_ring_head_rd_en_ff;
    assign o_mem_ctrl_csr_q_cntx_ring_head_rd_addr = (mem_ctrl_csr_q_cntx_ring_head_wr_en_ff | mem_ctrl_csr_q_cntx_ring_head_rd_en_ff) ? mem_ctrl_csr_q_cntx_ring_head_addr : '0;
    ...
    assign csr_cqbal_rd_data_nxt = (csr_cq_rd_cmd_done & (csr_cq_reg_l_ff == CQBAL)) ? i_mem_ctrl_csr_q_cntx_static_rd_data.base_low : '0;
    ...
    assign csr_cqbah_rd_data_nxt = (csr_cq_rd_cmd_done & (csr_cq_reg_l_ff == CQBAH)) ? i_mem_ctrl_csr_q_cntx_static_rd_data.base_high : '0;
...
//    assign csr_cqlen_rd_data_pre_l_nxt.length = ((csr_cq_reg_l_ff == CQLEN) & i_mem_ctrl_csr_q_cntx_static_rd_vld) ? i_mem_ctrl_csr_q_cntx_static_rd_data.rx_q_len : csr_cqlen_rd_data_pre_l_ff.length;
//    assign csr_cqlen_rd_data_nxt              = (csr_cq_rd_cmd_done & (csr_cq_reg_l_ff == CQLEN)) ? csr_cqlen_rd_data_pre_l_ff : '0;
    assign csr_cqlen_ack_nxt            = (csr_cq_wr_cmd_done | csr_cq_rd_cmd_done) & (csr_cq_reg_l_ff == CQLEN);
    assign csr_cqlen_rd_data_pre_l_nxt  = ((csr_cq_reg_l_ff == CQLEN) & i_mem_ctrl_csr_q_cntx_static_rd_vld) ? i_mem_ctrl_csr_q_cntx_static_rd_data.rx_q_len : csr_cqlen_rd_data_pre_l_ff;
    ...
    always_comb begin
        ...
        if (i_mem_ctrl_csr_q_cntx_static_rd_vld & (csr_cq_reg_l_ff != IDLE)) begin
            csr_cq_context_table_nxt                       = i_mem_ctrl_csr_q_cntx_static_rd_data;
    ...
    assign csr_cq_rd_cmd_done = ((csr_cq_reg_l_ff != CQLEN) & csr_cq_rd_cmd_active_ff & i_mem_ctrl_csr_q_cntx_static_rd_vld) |
    ...
    assign csr_cq_context_wr_access_start = (csr_cq_wr_cmd_active_ff & i_mem_ctrl_csr_q_cntx_static_rd_vld);
    ...
    assign csr_cq_q_ctrl_rd_req       = (csr_cq_reg_l_ff == CQLEN) & csr_cq_rd_cmd_active_ff & i_mem_ctrl_csr_q_cntx_static_rd_vld;
    ...
    assign csr_ring_empty_ack_nxt     = i_mem_ctrl_csr_q_cntx_ring_empty_rd_vld & csr_ring_empty_vld_l_ff;
    assign csr_ring_empty_rd_data_nxt = (i_mem_ctrl_csr_q_cntx_ring_empty_rd_vld & csr_ring_empty_vld_l_ff) ? i_mem_ctrl_csr_q_cntx_ring_empty_rd_data : '0;
    ...
    assign csr_ring_empty_rd_req_sent_nxt = (o_mem_ctrl_csr_q_cntx_ring_empty_rd_en & i_mem_ctrl_csr_q_cntx_ring_empty_rd_rdy) ? '1 : (o_mem_ctrl_csr_q_cntx_ring_empty_wr_en) ? '0 : csr_ring_empty_rd_req_sent_ff;
    ...
    always_comb begin : always_csr_ring_empty_mem_wr_data
        csr_ring_empty_mem_wr_data = i_mem_ctrl_csr_q_cntx_ring_empty_rd_data;
    ...
    assign o_mem_ctrl_csr_q_cntx_ring_empty_wr_en   = csr_ring_empty_vld_l_ff & i_mem_ctrl_csr_q_cntx_ring_empty_rd_vld;
    assign o_mem_ctrl_csr_q_cntx_ring_empty_wr_addr = (o_mem_ctrl_csr_q_cntx_ring_empty_wr_en) ? csr_ring_empty_idx_l_ff : '0;
    assign o_mem_ctrl_csr_q_cntx_ring_empty_wr_data = (o_mem_ctrl_csr_q_cntx_ring_empty_wr_en) ? csr_ring_empty_mem_wr_data : '0;
    assign o_mem_ctrl_csr_q_cntx_ring_empty_rd_en   = csr_ring_empty_vld_l_ff & (~csr_ring_empty_rd_req_sent_ff);
    assign o_mem_ctrl_csr_q_cntx_ring_empty_rd_addr = (o_mem_ctrl_csr_q_cntx_ring_empty_rd_en) ? csr_ring_empty_idx_l_ff : '0;
    ...
    assign event_q_cntx_req_fifo_wr_vld  = i_rlan_event_q_cntx_req_vld;
    assign event_q_cntx_req_fifo_wr_en   = i_rlan_event_q_cntx_req_vld & o_rlan_event_q_cntx_req_rdy;
    assign event_q_cntx_req_fifo_wr_data = i_rlan_event_q_cntx_req_cmd;
    assign event_q_cntx_req_fifo_rd_en   = event_q_cntx_req_fifo_rd_vld & o_mem_ctrl_rlan_q_cntx_static_rd_en & i_mem_ctrl_rlan_q_cntx_static_rdy;
    assign o_rlan_event_q_cntx_req_rdy   = event_q_cntx_req_fifo_wr_rdy;
    ...
    assign o_mem_ctrl_rlan_q_cntx_static_rd_en = event_q_cntx_req_fifo_rd_vld & event_q_cntx_rslt_fifo_wr_rdy & event_q_cntx_rslt_fifo_below_thr;
    assign o_mem_ctrl_rlan_q_cntx_static_addr  = event_q_cntx_req_fifo_rd_data.event_queue;
    ...
    assign event_q_cntx_rslt_fifo_wr_en     = i_mem_ctrl_rlan_q_cntx_static_rd_vld;
    assign event_q_cntx_rslt_fifo_wr_data   = convert_to_rlan_ev_q_cntx_cmpl_func(i_mem_ctrl_rlan_q_cntx_static_rd_data);
    assign event_q_cntx_rslt_fifo_rd_en     = event_q_cntx_rslt_fifo_rd_vld & i_rlan_event_q_cntx_rslt_rdy;
    assign event_q_cntx_rslt_fifo_below_thr = (event_q_cntx_rslt_fifo_used_space < csr_cntx_access_ctrl_ff.rslt_fifo_thr);
    assign o_rlan_event_q_cntx_rslt_vld     = event_q_cntx_rslt_fifo_rd_vld;
    assign o_rlan_event_q_cntx_rslt_data    = event_q_cntx_rslt_fifo_rd_data;
    ...
    assign sva_cond_q_cntx_ring_empty_mem_wr_en_bp = o_mem_ctrl_csr_q_cntx_ring_empty_wr_en & (~i_mem_ctrl_csr_q_cntx_ring_empty_wr_rdy);
    ...
    assign sva_cond_mem_rd_vld_when_rslt_fifo_not_rdy = i_mem_ctrl_rlan_q_cntx_static_rd_vld & (~event_q_cntx_rslt_fifo_wr_rdy);
    ...
    assign sva_cond_q_cntx_static_rd_data_unknown = i_mem_ctrl_rlan_q_cntx_static_rd_vld & $isunknown(i_mem_ctrl_rlan_q_cntx_static_rd_data);
    ...
    covergroup cg_backpressures @(posedge clk);
        cov_mem_cntx_early_static_wr_bp        : coverpoint (o_mem_ctrl_csr_q_cntx_early_static_wr_en & (~i_mem_ctrl_csr_q_cntx_early_static_rdy));
        cov_mem_cntx_early_static_rd_bp        : coverpoint (o_mem_ctrl_csr_q_cntx_early_static_rd_en & (~i_mem_ctrl_csr_q_cntx_early_static_rdy));
        cov_mem_cntx_static_wr_bp              : coverpoint (o_mem_ctrl_csr_q_cntx_static_wr_en & (~i_mem_ctrl_csr_q_cntx_static_rdy));
        cov_mem_cntx_static_rd_bp              : coverpoint (o_mem_ctrl_csr_q_cntx_static_rd_en & (~i_mem_ctrl_csr_q_cntx_static_rdy));
        cov_mem_cntx_buffer_static_wr_bp       : coverpoint (o_mem_ctrl_csr_q_cntx_buffer_static_wr_en & (~i_mem_ctrl_csr_q_cntx_buffer_static_rdy));
        cov_mem_cntx_buffer_static_rd_bp       : coverpoint (o_mem_ctrl_csr_q_cntx_buffer_static_rd_en & (~i_mem_ctrl_csr_q_cntx_buffer_static_rdy));
        ...
        // cov_mem_cntx_first_pkt_wr_bp        : coverpoint (o_mem_ctrl_csr_q_cntx_first_pkt_wr_en & (~i_mem_ctrl_csr_q_cntx_first_pkt_wr_rdy));
        cov_mem_cntx_first_pkt_rd_bp           : coverpoint (o_mem_ctrl_csr_q_cntx_first_pkt_rd_en & (~i_mem_ctrl_csr_q_cntx_first_pkt_rd_rdy));
        ...
        // cov_mem_cntx_buffer_first_pkt_wr_bp : coverpoint (o_mem_ctrl_csr_q_cntx_buffer_first_pkt_wr_en & (~i_mem_ctrl_csr_q_cntx_buffer_first_pkt_wr_rdy));
        cov_mem_cntx_buffer_first_pkt_rd_bp    : coverpoint (o_mem_ctrl_csr_q_cntx_buffer_first_pkt_rd_en & (~i_mem_ctrl_csr_q_cntx_buffer_first_pkt_rd_rdy));
        cov_mem_cntx_ring_head_wr_bp           : coverpoint (o_mem_ctrl_csr_q_cntx_ring_head_wr_en & (~i_mem_ctrl_csr_q_cntx_ring_head_wr_rdy));
        cov_mem_cntx_ring_head_rd_bp           : coverpoint (o_mem_ctrl_csr_q_cntx_ring_head_rd_en & (~i_mem_ctrl_csr_q_cntx_ring_head_rd_rdy));
        cov_rlan_event_q_cntx_req_in_bp        : coverpoint (i_rlan_event_q_cntx_req_vld & (~o_rlan_event_q_cntx_req_rdy));
        cov_rlan_event_q_cntx_rslt_out_bp      : coverpoint (o_rlan_event_q_cntx_rslt_vld & (~i_rlan_event_q_cntx_rslt_rdy));
