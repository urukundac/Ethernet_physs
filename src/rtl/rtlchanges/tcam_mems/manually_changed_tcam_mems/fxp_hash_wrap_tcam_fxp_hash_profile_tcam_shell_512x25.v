...
module  fxp_hash_wrap_tcam_fxp_hash_profile_tcam_shell_512x25 #(
       ...
       ,
        ...
        input wire bisr_reset_pd_vinf, input wire ram_row_1_col_0_bisr_inst_SO, 
...
`ifndef FXP_FXP_HASH_TCAM_BEHAVE_MEMS
    `define FXP_FXP_HASH_ASIC_MEMS
`endif

`ifdef  FXP_FXP_HASH_ASIC_MEMS
...
localparam TCAM_SLICE_EN_MIN = ((TCAM_SLICE_EN_FACT < TCAM_SLICE_EN_WIDTH ) ? TCAM_SLICE_EN_FACT : TCAM_SLICE_EN_WIDTH);
    ...
    logic    [28-1:0]    tcam_rd_data_col[1-1:0][1-1:0];
    logic    [28-1:0]    tcam_rd_data_row;
    always_comb begin
        for (int i = 0; i < 1; i = i + 1) begin
            tcam_rd_data_row[i*(28)+:28]        = tcam_rd_data_col[tcam_row_num_rd_en_delay[1-1:0]][i][28-1:0];
        end
    end
...
// Match In Array 
    ...
    // EBBs Instantiation
        ...
        //                             |_____________________________|
                         ...
                         fxp_hash_ip783tcam512x28s0c1_0_0_wrap #(        
                                 )               
        
                                 ip783tcam512x28s0c1_wrap_tcam_row_0_col_0 (
                   //-------------------------------------------------------------
...
parameter       MEM_SEG_PADDING_W = 3 ; 
    ...
    generate if (BCAM_N7==0 && TCAM_PROTECTION_TYPE==1 && BCAM_RELIEF==0) begin : protect    
       ...
       task automatic update_col_sweeper(input bit pattern, input logic[25-1:0] old_val, input logic[25-1:0] new_val);
           logic [25-1:0] temp;
    ...
    // ********** this task is valid for TCAM only *****
      task automatic write_tcam_raw_key (input logic[25-1:0] key, input int row_idx);
          reg temp1,temp2;
          logic [(1 * 28) - 1:0] local_key = '1;
          logic [(1 * 28) - 1:0] prev_key_data;
          local_key[25-1:0] = TCAM_18A ? ~key : ~key;
          ...
          `ifdef FXP_FXP_HASH_TCAM_BEHAVE_MEMS
             `ifndef FXP_FPGA_TCAM_MEMS
         ...
         for (int j = 0;j < 1 ; j = j + 1) 
             case (j)
                                    0: begin
                                            //write_mask
                                             gen_tcam_row_0_col_0_tcam.ip783tcam512x28s0c1_wrap_tcam_row_0_col_0.tcam_row_0_col_0.ip783tcam512x28s0c1_gen_wrp_inst.ip783tcam512x28s0c1_inst.ip783tcam512x28s0c1_array.array_x[row_idx % 512] = local_key[0*(28 -2*BCAM_N7) +: 28-2*BCAM_N7];
                                             gen_tcam_row_0_col_0_tcam.ip783tcam512x28s0c1_wrap_tcam_row_0_col_0.tcam_row_0_col_0.ip783tcam512x28s0c1_gen_wrp_inst.ip783tcam512x28s0c1_inst.ip783tcam512x28s0c1_array.array_v[row_idx % 512] = 1'b1;
                                    end
                                    default : begin
       ...
       // ********** backdoor DATA ***********
        task automatic write_tcam_raw_inv_key (input logic[25-1:0] inv_key, input int row_idx);
          reg temp1,temp2;
          logic [(1 * 28) - 1:0] local_inv_key = '0;
          logic [(1 * 28) - 1:0] prev_inv_key_data;
          local_inv_key[25-1:0] = TCAM_18A ? ~inv_key : ~inv_key;
         ...
         for (int j = 0;j < 1 ; j = j + 1) 
             case (j)
                                    0: begin
                                            //write_DATA
                                             gen_tcam_row_0_col_0_tcam.ip783tcam512x28s0c1_wrap_tcam_row_0_col_0.tcam_row_0_col_0.ip783tcam512x28s0c1_gen_wrp_inst.ip783tcam512x28s0c1_inst.ip783tcam512x28s0c1_array.array_y[row_idx % 512] = local_inv_key[0*(28 -2*BCAM_N7) +: 28-2*BCAM_N7];
                                             gen_tcam_row_0_col_0_tcam.ip783tcam512x28s0c1_wrap_tcam_row_0_col_0.tcam_row_0_col_0.ip783tcam512x28s0c1_gen_wrp_inst.ip783tcam512x28s0c1_inst.ip783tcam512x28s0c1_array.array_v[row_idx % 512] = 1'b1;
                                    end
                                    default : begin
                                       $error("SOMTING IS wrong with parameters given for col number %d ",j);
       ...
       task automatic write_tcam_raw_line (input logic[25-1:0] key, input logic [25-1:0] inv_key, input int row_idx);
       ...
       // ********** backdoor MASK ***********
       //****this task is valid for TCAM only*****
       task automatic read_tcam_raw_key (output logic[25-1:0] key, input int row_idx);
         ...
         for (int j = 0;j < 1 ; j = j + 1) 
             case (j)
                                    0: begin
                                            //READ_mask
                                             local_key[0*28 +: 28] = gen_tcam_row_0_col_0_tcam.ip783tcam512x28s0c1_wrap_tcam_row_0_col_0.tcam_row_0_col_0.ip783tcam512x28s0c1_gen_wrp_inst.ip783tcam512x28s0c1_inst.ip783tcam512x28s0c1_array.array_x[row_idx % 512];
                                    end
                                    default : begin
                                       $error("SOMTING IS wrong with parameters given for col number %d ",j);
                                     end
     ...
     // ********** backdoor DATA ***********
       task automatic read_tcam_raw_inv_key (output logic[25-1:0] inv_key, input int row_idx);
           reg temp1,valid_word;
         ...
         for (int j = 0;j < 1 ; j = j + 1) 
             case (j)
                                    0: begin
                                            //READ_data
                                             local_inv_key[0*28 +: 28] = gen_tcam_row_0_col_0_tcam.ip783tcam512x28s0c1_wrap_tcam_row_0_col_0.tcam_row_0_col_0.ip783tcam512x28s0c1_gen_wrp_inst.ip783tcam512x28s0c1_inst.ip783tcam512x28s0c1_array.array_y[row_idx % 512];
                                    end
                                    default : begin
                                       $error("SOMTING IS wrong with parameters given for col number %d ",j);
                                     end
       ...
       task automatic read_tcam_raw_line (output logic[25-1:0] key, output logic [25-1:0] inv_key, input int row_idx);
