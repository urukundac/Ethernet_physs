...
module  fxp_lpm_wrap_tcam_fxp_lpm_root_tcam_shell_2048x33 #(
       ...
       ,
        ...
        output wire [6:0] tcam_row_3_col_0_bisr_inst_Q, 
...
localparam PHYSICAL_ROW_WIDTH = TCAM_WIDTH + BCAM_N7*(1*36 - TCAM_WIDTH);
 wire tcam_row_0_col_0_bisr_inst_SO, tcam_row_1_col_0_bisr_inst_SO, 
      tcam_row_2_col_0_bisr_inst_SO;
...
`ifndef FXP_FXP_LPM_TCAM_BEHAVE_MEMS
    `define FXP_FXP_LPM_ASIC_MEMS
`endif

`ifdef  FXP_FXP_LPM_ASIC_MEMS
...
localparam TCAM_SLICE_EN_MIN = ((TCAM_SLICE_EN_FACT < TCAM_SLICE_EN_WIDTH ) ? TCAM_SLICE_EN_FACT : TCAM_SLICE_EN_WIDTH);
    ...
    wire    [4-1:0]                tcam_row_sel;
    assign                    tcam_row_sel[0]        = ((adr>=12'd0) && (adr<12'd1024));
    assign                    tcam_row_sel[1]        = ((adr>=12'd1024) && (adr<12'd2048));
    assign                    tcam_row_sel[2]        = ((adr>=12'd2048) && (adr<12'd3072));
    assign                    tcam_row_sel[3]        = (adr>=12'd3072);


    // TCAM Address Decoder

    wire    [10-1:0]        tcam_row_adr[4-1:0];
    ...
    reg            rd_en_nope_done;
    reg    [4-1:0]    rd_en_delay_inst[TCAM_RD_DELAY:0];
    logic   [TCAM_RD_DELAY:0]               rd_en_delay;
    always_comb
         begin
        rd_en_delay_inst[0][4-1:0]        = tcam_row_rd_en[4-1:0];
    ...
    reg            wr_en_nope_done;
    reg    [4-1:0]    wr_en_delay_inst[TCAM_WR_DELAY:0];
    logic    [TCAM_WR_DELAY:0]               wr_en_delay;
    always_comb
         begin
        wr_en_delay_inst[0][4-1:0]        = tcam_row_wr_en[4-1:0];
    ...
    logic    [36-1:0]    tcam_row_wr_data;
    always_comb begin
            tcam_row_wr_data = '1;
            tcam_row_wr_data[0+:PHYSICAL_ROW_WIDTH]    = wr_data;
    end
    logic    [36-1:0]    tcam_wr_data_col[1-1:0];
    always_comb begin
        for (int i = 0; i < 1; i = i + 1) begin
            tcam_wr_data_col[i][36-1:0]        = tcam_row_wr_data[i*36+:36];
        end
    end
    ...
    logic    [36-1:0]    tcam_chk_key;
    always_comb begin
            tcam_chk_key = '1;
            tcam_chk_key[0+:PHYSICAL_ROW_WIDTH]    = chk_key;
    end
    logic    [36-1:0]    tcam_chk_key_col[1-1:0];
    always_comb begin
        for (int i = 0; i < 1; i = i + 1) begin
            tcam_chk_key_col[i][36-1:0]        = tcam_chk_key[i*36+:36];
        end
    end
    ...
    logic    [36-1:0]    tcam_chk_mask;
    always_comb begin
            tcam_chk_mask        = '0;
            tcam_chk_mask        = wr_en ? '1 : chk_mask_int;
    end
    logic    [36-1:0]    tcam_chk_mask_col[1-1:0];
    always_comb begin
        for (int i = 0; i < 1; i = i + 1) begin
            tcam_chk_mask_col[i][36-1:0]        = tcam_chk_mask[i*36+:36];
        end
    end
    ...
    logic    [512-1:0]    tcam_raw_hit_in[4-1:0];
    always_comb begin
        for (int i = 0; i < 4; i = i + 1) begin
    ...
    logic    [36-1:0]    tcam_rd_data_col[4-1:0][1-1:0];
    logic    [36-1:0]    tcam_rd_data_row;
    always_comb begin
        for (int i = 0; i < 1; i = i + 1) begin
            tcam_rd_data_row[i*(36)+:36]        = tcam_rd_data_col[tcam_row_num_rd_en_delay[2-1:0]][i][36-1:0];
        end
    end
    ...
    logic    [512-1:0]    tcam_col_hit_out[4-1:0][1-1:0];
...
// Match In Array 
        ...
        logic    [512-1:0]      tcam_row_match_in[4-1:0];
        always_comb begin 
                for (int i = 0; i < 4; i = i + 1) begin
    ...
    // EBBs Instantiation
        ...
        wire    [36-1:0]                      tcam_row_0_col_0_data_in = tcam_row_wr_en[0] ? tcam_wr_data_col[0][36-1:0] : tcam_chk_key_col[0][36-1:0];
        wire    [36-1:0]                      tcam_row_0_col_0_data_in_pre = tcam_wr_data_col[0][36-1:0] ;
        wire    [36-1:0]                      tcam_row_0_col_0_key_in =  tcam_chk_key_col[0][36-1:0];
        wire    [36-1:0]                      tcam_row_0_col_0_mask_in = tcam_chk_mask_col[0][36-1:0];
        wire    [512-1:0]                       tcam_row_0_col_0_hit_in = tcam_raw_hit_in[0];
        wire    [512-1:0]                       tcam_row_0_col_0_tcam_match_in = tcam_row_match_in[0];
        wire    [36-1:0]                      tcam_row_0_col_0_data_out_inv;
        ...
        logic   [TCAM_SLICE_EN_FACT-1:0]                  tcam_row_0_col_0_slice_en_s1; 
             ...
             wire    [36-1:0]                     tcam_row_0_col_0_data_out ;
        ...
        //                             |_____________________________|
                         ...
                         fxp_lpm_ip783tcam512x36s0c1_0_0_wrap #(        
                                 )               
        
                                 ip783tcam512x36s0c1_wrap_tcam_row_0_col_0 (
                   //-------------------------------------------------------------
                   ...
                   //-------------------------------------------------------------   
                      ...
                      `endif
                        ...
                        .tcam_row_0_col_0_bisr_inst_Q_ts1(tcam_row_0_col_0_bisr_inst_Q[6:0]), 
        ...
        wire    [36-1:0]                      tcam_row_1_col_0_data_in = tcam_row_wr_en[1] ? tcam_wr_data_col[0][36-1:0] : tcam_chk_key_col[0][36-1:0];
        wire    [36-1:0]                      tcam_row_1_col_0_data_in_pre = tcam_wr_data_col[0][36-1:0] ;
        wire    [36-1:0]                      tcam_row_1_col_0_key_in =  tcam_chk_key_col[0][36-1:0];
        wire    [36-1:0]                      tcam_row_1_col_0_mask_in = tcam_chk_mask_col[0][36-1:0];
        wire    [512-1:0]                       tcam_row_1_col_0_hit_in = tcam_raw_hit_in[1];
        wire    [512-1:0]                       tcam_row_1_col_0_tcam_match_in = tcam_row_match_in[1];
        wire    [36-1:0]                      tcam_row_1_col_0_data_out_inv;
        ...
        logic   [TCAM_SLICE_EN_FACT-1:0]                  tcam_row_1_col_0_slice_en_s1; 
             ...
             wire    [36-1:0]                     tcam_row_1_col_0_data_out ;
        ...
        //                             |_____________________________|
                         ...
                         fxp_lpm_ip783tcam512x36s0c1_1_0_wrap #(        
                                 )               
        
                                 ip783tcam512x36s0c1_wrap_tcam_row_1_col_0 (
                   //-------------------------------------------------------------
                   // Power Management Interfaces
                   //-------------------------------------------------------------   
                      `ifndef INTEL_NO_PWR_PINS
                      ...
                      `endif
                        ...
                        .BIST_WRITEENABLE(BIST_WRITEENABLE), .BIST_READENABLE(BIST_READENABLE), 
                        .BIST_SELECT(BIST_SELECT), .BIST_COL_ADD(BIST_COL_ADD), 
                        .BIST_ROW_ADD(BIST_ROW_ADD[6:0]), .BIST_COLLAR_EN1(BIST_COLLAR_EN1), 
                        ...
                        .BIST_CLEAR_DEFAULT(BIST_CLEAR_DEFAULT), .BIST_CLEAR(BIST_CLEAR), 
                        .BIST_COLLAR_HOLD(BIST_COLLAR_HOLD), 
                        .BIST_DATA_FROM_MEM(BIST_DATA_FROM_MEM_ts1[35:0]), 
                        .MBISTPG_RESET_REG_SETUP2(MBISTPG_RESET_REG_SETUP2), 
                        .BIST_BANK_ADD(BIST_BANK_ADD[1:0]), 
                        .bisr_shift_en_pd_vinf(bisr_shift_en_pd_vinf), 
                        .bisr_clk_pd_vinf(bisr_clk_pd_vinf), 
                        ...
                        .tcam_row_1_col_0_bisr_inst_Q_ts1(tcam_row_1_col_0_bisr_inst_Q[6:0]), 
                        .fscan_clkungate(fscan_clkungate)
                     );
        ...
        wire    [36-1:0]                      tcam_row_2_col_0_data_in = tcam_row_wr_en[2] ? tcam_wr_data_col[0][36-1:0] : tcam_chk_key_col[0][36-1:0];
        wire    [36-1:0]                      tcam_row_2_col_0_data_in_pre = tcam_wr_data_col[0][36-1:0] ;
        wire    [36-1:0]                      tcam_row_2_col_0_key_in =  tcam_chk_key_col[0][36-1:0];
        wire    [36-1:0]                      tcam_row_2_col_0_mask_in = tcam_chk_mask_col[0][36-1:0];
        wire    [512-1:0]                       tcam_row_2_col_0_hit_in = tcam_raw_hit_in[2];
        wire    [512-1:0]                       tcam_row_2_col_0_tcam_match_in = tcam_row_match_in[2];
        wire    [36-1:0]                      tcam_row_2_col_0_data_out_inv;
        ...
        logic   [TCAM_SLICE_EN_FACT-1:0]                  tcam_row_2_col_0_slice_en_s1; 
             ...
             wire    [36-1:0]                     tcam_row_2_col_0_data_out ;
        ...
        //                             |_____________________________|
                         ...
                         fxp_lpm_ip783tcam512x36s0c1_2_0_wrap #(        
                                 )               
        
                                 ip783tcam512x36s0c1_wrap_tcam_row_2_col_0 (
                   //-------------------------------------------------------------
                   // Power Management Interfaces
                   //-------------------------------------------------------------   
                      `ifndef INTEL_NO_PWR_PINS
                      ...
                      `endif
                        ...
                        .BIST_WRITEENABLE(BIST_WRITEENABLE), .BIST_READENABLE(BIST_READENABLE), 
                        .BIST_SELECT(BIST_SELECT), .BIST_COL_ADD(BIST_COL_ADD), 
                        .BIST_ROW_ADD(BIST_ROW_ADD[6:0]), .BIST_COLLAR_EN2(BIST_COLLAR_EN2), 
                        ...
                        .BIST_CLEAR_DEFAULT(BIST_CLEAR_DEFAULT), .BIST_CLEAR(BIST_CLEAR), 
                        .BIST_COLLAR_HOLD(BIST_COLLAR_HOLD), 
                        .BIST_DATA_FROM_MEM(BIST_DATA_FROM_MEM_ts2[35:0]), 
                        .MBISTPG_RESET_REG_SETUP2(MBISTPG_RESET_REG_SETUP2), 
                        .BIST_BANK_ADD(BIST_BANK_ADD[1:0]), 
                        .bisr_shift_en_pd_vinf(bisr_shift_en_pd_vinf), 
                        .bisr_clk_pd_vinf(bisr_clk_pd_vinf), 
                        ...
                        .tcam_row_2_col_0_bisr_inst_Q_ts1(tcam_row_2_col_0_bisr_inst_Q[6:0]), 
                        .fscan_clkungate(fscan_clkungate)
                     );
        ...
        wire    [36-1:0]                      tcam_row_3_col_0_data_in = tcam_row_wr_en[3] ? tcam_wr_data_col[0][36-1:0] : tcam_chk_key_col[0][36-1:0];
        wire    [36-1:0]                      tcam_row_3_col_0_data_in_pre = tcam_wr_data_col[0][36-1:0] ;
        wire    [36-1:0]                      tcam_row_3_col_0_key_in =  tcam_chk_key_col[0][36-1:0];
        wire    [36-1:0]                      tcam_row_3_col_0_mask_in = tcam_chk_mask_col[0][36-1:0];
        wire    [512-1:0]                       tcam_row_3_col_0_hit_in = tcam_raw_hit_in[3];
        wire    [512-1:0]                       tcam_row_3_col_0_tcam_match_in = tcam_row_match_in[3];
        wire    [36-1:0]                      tcam_row_3_col_0_data_out_inv;
        ...
        logic   [TCAM_SLICE_EN_FACT-1:0]                  tcam_row_3_col_0_slice_en_s1; 
             ...
             wire    [36-1:0]                     tcam_row_3_col_0_data_out ;
        ...
        //                             |_____________________________|
                         ...
                         fxp_lpm_ip783tcam512x36s0c1_3_0_wrap #(        
                                 )               
        
                                 ip783tcam512x36s0c1_wrap_tcam_row_3_col_0 (
                   //-------------------------------------------------------------
                   // Power Management Interfaces
                   //-------------------------------------------------------------   
                      `ifndef INTEL_NO_PWR_PINS
                      ...
                      `endif
                        ...
                        .BIST_WRITEENABLE(BIST_WRITEENABLE), .BIST_READENABLE(BIST_READENABLE), 
                        .BIST_SELECT(BIST_SELECT), .BIST_COL_ADD(BIST_COL_ADD), 
                        .BIST_ROW_ADD(BIST_ROW_ADD[6:0]), .BIST_COLLAR_EN3(BIST_COLLAR_EN3), 
                        ...
                        .BIST_CLEAR_DEFAULT(BIST_CLEAR_DEFAULT), .BIST_CLEAR(BIST_CLEAR), 
                        .BIST_COLLAR_HOLD(BIST_COLLAR_HOLD), 
                        .BIST_DATA_FROM_MEM(BIST_DATA_FROM_MEM_ts3[35:0]), 
                        .MBISTPG_RESET_REG_SETUP2(MBISTPG_RESET_REG_SETUP2), 
                        .BIST_BANK_ADD(BIST_BANK_ADD[1:0]), 
                        .bisr_shift_en_pd_vinf(bisr_shift_en_pd_vinf), 
                        .bisr_clk_pd_vinf(bisr_clk_pd_vinf), 
                        ...
                        .tcam_row_3_col_0_bisr_inst_Q_ts1(tcam_row_3_col_0_bisr_inst_Q[6:0]), 
                        .fscan_clkungate(fscan_clkungate)
                     );
    ...
    if (BCAM_N7 == 1) 
        begin : bacm_prot_calc
            always_comb begin
                ...
                for (int i = 0; i < 1; i = i + 1) begin
                    rd_data[PHYSICAL_ROW_WIDTH-((1-i-1)*2)-2] = ^rd_data_before_prot_calc[(i*(36-2))+:((36-2)/2)];
                    rd_data[PHYSICAL_ROW_WIDTH-((1-i-1)*2)-1] = ^rd_data_before_prot_calc[(i*(36-2))+((36-2)/2)+:((36-2)/2)];
                end
            end
        end
    else
...
parameter       MEM_SEG_PADDING_W = 3 ; 
    ...
    generate if (BCAM_N7==0 && TCAM_PROTECTION_TYPE==1 && BCAM_RELIEF==0) begin : protect    
       ...
       task automatic update_col_sweeper(input bit pattern, input logic[33-1:0] old_val, input logic[33-1:0] new_val);
           logic [33-1:0] temp;
    ...
    // ********** this task is valid for TCAM only *****
      task automatic write_tcam_raw_key (input logic[33-1:0] key, input int row_idx);
          reg temp1,temp2;
          logic [(1 * 36) - 1:0] local_key = '1;
          logic [(1 * 36) - 1:0] prev_key_data;
          local_key[33-1:0] = TCAM_18A ? ~key : ~key;
          ...
          `ifdef FXP_FXP_LPM_TCAM_BEHAVE_MEMS
             `ifndef FXP_FPGA_TCAM_MEMS
         ...
         for (int j = 0;j < 1 ; j = j + 1) 
             case (j)
                                    0: begin
                                            //write_mask
                                             gen_tcam_row_0_col_0_tcam.ip783tcam512x36s0c1_wrap_tcam_row_0_col_0.tcam_row_0_col_0.ip783tcam512x36s0c1_gen_wrp_inst.ip783tcam512x36s0c1_inst.ip783tcam512x36s0c1_array.array_x[row_idx % 512] = local_key[0*(36 -2*BCAM_N7) +: 36-2*BCAM_N7];
                                             gen_tcam_row_0_col_0_tcam.ip783tcam512x36s0c1_wrap_tcam_row_0_col_0.tcam_row_0_col_0.ip783tcam512x36s0c1_gen_wrp_inst.ip783tcam512x36s0c1_inst.ip783tcam512x36s0c1_array.array_v[row_idx % 512] = 1'b1;
                                    end
                                    default : begin
         ...
         for (int j = 0;j < 1 ; j = j + 1) 
             case (j)
                                    0: begin
                                            //write_mask
                                             gen_tcam_row_1_col_0_tcam.ip783tcam512x36s0c1_wrap_tcam_row_1_col_0.tcam_row_1_col_0.ip783tcam512x36s0c1_gen_wrp_inst.ip783tcam512x36s0c1_inst.ip783tcam512x36s0c1_array.array_x[row_idx % 512] = local_key[0*(36 -2*BCAM_N7) +: 36-2*BCAM_N7];
                                             gen_tcam_row_1_col_0_tcam.ip783tcam512x36s0c1_wrap_tcam_row_1_col_0.tcam_row_1_col_0.ip783tcam512x36s0c1_gen_wrp_inst.ip783tcam512x36s0c1_inst.ip783tcam512x36s0c1_array.array_v[row_idx % 512] = 1'b1;
                                    end
                                    default : begin
                                       $error("SOMTING IS wrong with parameters given for col number %d ",j);
                                     end
         ...
         for (int j = 0;j < 1 ; j = j + 1) 
             case (j)
                                    0: begin
                                            //write_mask
                                             gen_tcam_row_2_col_0_tcam.ip783tcam512x36s0c1_wrap_tcam_row_2_col_0.tcam_row_2_col_0.ip783tcam512x36s0c1_gen_wrp_inst.ip783tcam512x36s0c1_inst.ip783tcam512x36s0c1_array.array_x[row_idx % 512] = local_key[0*(36 -2*BCAM_N7) +: 36-2*BCAM_N7];
                                             gen_tcam_row_2_col_0_tcam.ip783tcam512x36s0c1_wrap_tcam_row_2_col_0.tcam_row_2_col_0.ip783tcam512x36s0c1_gen_wrp_inst.ip783tcam512x36s0c1_inst.ip783tcam512x36s0c1_array.array_v[row_idx % 512] = 1'b1;
                                    end
                                    default : begin
                                       $error("SOMTING IS wrong with parameters given for col number %d ",j);
                                     end
         ...
         for (int j = 0;j < 1 ; j = j + 1) 
             case (j)
                                    0: begin
                                            //write_mask
                                             gen_tcam_row_3_col_0_tcam.ip783tcam512x36s0c1_wrap_tcam_row_3_col_0.tcam_row_3_col_0.ip783tcam512x36s0c1_gen_wrp_inst.ip783tcam512x36s0c1_inst.ip783tcam512x36s0c1_array.array_x[row_idx % 512] = local_key[0*(36 -2*BCAM_N7) +: 36-2*BCAM_N7];
                                             gen_tcam_row_3_col_0_tcam.ip783tcam512x36s0c1_wrap_tcam_row_3_col_0.tcam_row_3_col_0.ip783tcam512x36s0c1_gen_wrp_inst.ip783tcam512x36s0c1_inst.ip783tcam512x36s0c1_array.array_v[row_idx % 512] = 1'b1;
                                    end
                                    default : begin
                                       $error("SOMTING IS wrong with parameters given for col number %d ",j);
                                     end
       ...
       // ********** backdoor DATA ***********
        task automatic write_tcam_raw_inv_key (input logic[33-1:0] inv_key, input int row_idx);
          reg temp1,temp2;
          logic [(1 * 36) - 1:0] local_inv_key = '0;
          logic [(1 * 36) - 1:0] prev_inv_key_data;
          local_inv_key[33-1:0] = TCAM_18A ? ~inv_key : ~inv_key;
         ...
         for (int j = 0;j < 1 ; j = j + 1) 
             case (j)
                                    0: begin
                                            //write_DATA
                                             gen_tcam_row_0_col_0_tcam.ip783tcam512x36s0c1_wrap_tcam_row_0_col_0.tcam_row_0_col_0.ip783tcam512x36s0c1_gen_wrp_inst.ip783tcam512x36s0c1_inst.ip783tcam512x36s0c1_array.array_y[row_idx % 512] = local_inv_key[0*(36 -2*BCAM_N7) +: 36-2*BCAM_N7];
                                             gen_tcam_row_0_col_0_tcam.ip783tcam512x36s0c1_wrap_tcam_row_0_col_0.tcam_row_0_col_0.ip783tcam512x36s0c1_gen_wrp_inst.ip783tcam512x36s0c1_inst.ip783tcam512x36s0c1_array.array_v[row_idx % 512] = 1'b1;
                                    end
                                    default : begin
                                       $error("SOMTING IS wrong with parameters given for col number %d ",j);
         ...
         for (int j = 0;j < 1 ; j = j + 1) 
             case (j)
                                    0: begin
                                            //write_DATA
                                             gen_tcam_row_1_col_0_tcam.ip783tcam512x36s0c1_wrap_tcam_row_1_col_0.tcam_row_1_col_0.ip783tcam512x36s0c1_gen_wrp_inst.ip783tcam512x36s0c1_inst.ip783tcam512x36s0c1_array.array_y[row_idx % 512] = local_inv_key[0*(36 -2*BCAM_N7) +: 36-2*BCAM_N7];
                                             gen_tcam_row_1_col_0_tcam.ip783tcam512x36s0c1_wrap_tcam_row_1_col_0.tcam_row_1_col_0.ip783tcam512x36s0c1_gen_wrp_inst.ip783tcam512x36s0c1_inst.ip783tcam512x36s0c1_array.array_v[row_idx % 512] = 1'b1;
                                    end
                                    default : begin
                                       $error("SOMTING IS wrong with parameters given for col number %d ",j);
         ...
         for (int j = 0;j < 1 ; j = j + 1) 
             case (j)
                                    0: begin
                                            //write_DATA
                                             gen_tcam_row_2_col_0_tcam.ip783tcam512x36s0c1_wrap_tcam_row_2_col_0.tcam_row_2_col_0.ip783tcam512x36s0c1_gen_wrp_inst.ip783tcam512x36s0c1_inst.ip783tcam512x36s0c1_array.array_y[row_idx % 512] = local_inv_key[0*(36 -2*BCAM_N7) +: 36-2*BCAM_N7];
                                             gen_tcam_row_2_col_0_tcam.ip783tcam512x36s0c1_wrap_tcam_row_2_col_0.tcam_row_2_col_0.ip783tcam512x36s0c1_gen_wrp_inst.ip783tcam512x36s0c1_inst.ip783tcam512x36s0c1_array.array_v[row_idx % 512] = 1'b1;
                                    end
                                    default : begin
                                       $error("SOMTING IS wrong with parameters given for col number %d ",j);
         ...
         for (int j = 0;j < 1 ; j = j + 1) 
             case (j)
                                    0: begin
                                            //write_DATA
                                             gen_tcam_row_3_col_0_tcam.ip783tcam512x36s0c1_wrap_tcam_row_3_col_0.tcam_row_3_col_0.ip783tcam512x36s0c1_gen_wrp_inst.ip783tcam512x36s0c1_inst.ip783tcam512x36s0c1_array.array_y[row_idx % 512] = local_inv_key[0*(36 -2*BCAM_N7) +: 36-2*BCAM_N7];
                                             gen_tcam_row_3_col_0_tcam.ip783tcam512x36s0c1_wrap_tcam_row_3_col_0.tcam_row_3_col_0.ip783tcam512x36s0c1_gen_wrp_inst.ip783tcam512x36s0c1_inst.ip783tcam512x36s0c1_array.array_v[row_idx % 512] = 1'b1;
                                    end
                                    default : begin
                                       $error("SOMTING IS wrong with parameters given for col number %d ",j);
       ...
       task automatic write_tcam_raw_line (input logic[33-1:0] key, input logic [33-1:0] inv_key, input int row_idx);
       ...
       // ********** backdoor MASK ***********
       //****this task is valid for TCAM only*****
       task automatic read_tcam_raw_key (output logic[33-1:0] key, input int row_idx);
           reg temp1,valid_word;
           logic [(1 * 36) - 1:0] local_key;
           // check size out-of-bounds
           if (row_idx >= TCAM_RULES_NUM) begin
         ...
         for (int j = 0;j < 1 ; j = j + 1) 
             case (j)
                                    0: begin
                                            //READ_mask
                                             local_key[0*36 +: 36] = gen_tcam_row_0_col_0_tcam.ip783tcam512x36s0c1_wrap_tcam_row_0_col_0.tcam_row_0_col_0.ip783tcam512x36s0c1_gen_wrp_inst.ip783tcam512x36s0c1_inst.ip783tcam512x36s0c1_array.array_x[row_idx % 512];
                                    end
                                    default : begin
                                       $error("SOMTING IS wrong with parameters given for col number %d ",j);
                                     end
         ...
         for (int j = 0;j < 1 ; j = j + 1) 
             case (j)
                                    0: begin
                                            //READ_mask
                                             local_key[0*36 +: 36] = gen_tcam_row_1_col_0_tcam.ip783tcam512x36s0c1_wrap_tcam_row_1_col_0.tcam_row_1_col_0.ip783tcam512x36s0c1_gen_wrp_inst.ip783tcam512x36s0c1_inst.ip783tcam512x36s0c1_array.array_x[row_idx % 512];
                                    end
                                    default : begin
                                       $error("SOMTING IS wrong with parameters given for col number %d ",j);
                                     end
         ...
         for (int j = 0;j < 1 ; j = j + 1) 
             case (j)
                                    0: begin
                                            //READ_mask
                                             local_key[0*36 +: 36] = gen_tcam_row_2_col_0_tcam.ip783tcam512x36s0c1_wrap_tcam_row_2_col_0.tcam_row_2_col_0.ip783tcam512x36s0c1_gen_wrp_inst.ip783tcam512x36s0c1_inst.ip783tcam512x36s0c1_array.array_x[row_idx % 512];
                                    end
                                    default : begin
                                       $error("SOMTING IS wrong with parameters given for col number %d ",j);
                                     end
         ...
         for (int j = 0;j < 1 ; j = j + 1) 
             case (j)
                                    0: begin
                                            //READ_mask
                                             local_key[0*36 +: 36] = gen_tcam_row_3_col_0_tcam.ip783tcam512x36s0c1_wrap_tcam_row_3_col_0.tcam_row_3_col_0.ip783tcam512x36s0c1_gen_wrp_inst.ip783tcam512x36s0c1_inst.ip783tcam512x36s0c1_array.array_x[row_idx % 512];
                                    end
                                    default : begin
                                       $error("SOMTING IS wrong with parameters given for col number %d ",j);
                                     end
     ...
     // ********** backdoor DATA ***********
       task automatic read_tcam_raw_inv_key (output logic[33-1:0] inv_key, input int row_idx);
           reg temp1,valid_word;
           logic [(1 * 36) - 1:0] local_inv_key;
           // check size out-of-bounds
           if (row_idx >= TCAM_RULES_NUM) begin
         ...
         for (int j = 0;j < 1 ; j = j + 1) 
             case (j)
                                    0: begin
                                            //READ_data
                                             local_inv_key[0*36 +: 36] = gen_tcam_row_0_col_0_tcam.ip783tcam512x36s0c1_wrap_tcam_row_0_col_0.tcam_row_0_col_0.ip783tcam512x36s0c1_gen_wrp_inst.ip783tcam512x36s0c1_inst.ip783tcam512x36s0c1_array.array_y[row_idx % 512];
                                    end
                                    default : begin
                                       $error("SOMTING IS wrong with parameters given for col number %d ",j);
                                     end
         ...
         for (int j = 0;j < 1 ; j = j + 1) 
             case (j)
                                    0: begin
                                            //READ_data
                                             local_inv_key[0*36 +: 36] = gen_tcam_row_1_col_0_tcam.ip783tcam512x36s0c1_wrap_tcam_row_1_col_0.tcam_row_1_col_0.ip783tcam512x36s0c1_gen_wrp_inst.ip783tcam512x36s0c1_inst.ip783tcam512x36s0c1_array.array_y[row_idx % 512];
                                    end
                                    default : begin
                                       $error("SOMTING IS wrong with parameters given for col number %d ",j);
                                     end
         ...
         for (int j = 0;j < 1 ; j = j + 1) 
             case (j)
                                    0: begin
                                            //READ_data
                                             local_inv_key[0*36 +: 36] = gen_tcam_row_2_col_0_tcam.ip783tcam512x36s0c1_wrap_tcam_row_2_col_0.tcam_row_2_col_0.ip783tcam512x36s0c1_gen_wrp_inst.ip783tcam512x36s0c1_inst.ip783tcam512x36s0c1_array.array_y[row_idx % 512];
                                    end
                                    default : begin
                                       $error("SOMTING IS wrong with parameters given for col number %d ",j);
                                     end
         ...
         for (int j = 0;j < 1 ; j = j + 1) 
             case (j)
                                    0: begin
                                            //READ_data
                                             local_inv_key[0*36 +: 36] = gen_tcam_row_3_col_0_tcam.ip783tcam512x36s0c1_wrap_tcam_row_3_col_0.tcam_row_3_col_0.ip783tcam512x36s0c1_gen_wrp_inst.ip783tcam512x36s0c1_inst.ip783tcam512x36s0c1_array.array_y[row_idx % 512];
                                    end
                                    default : begin
                                       $error("SOMTING IS wrong with parameters given for col number %d ",j);
                                     end
       ...
       task automatic read_tcam_raw_line (output logic[33-1:0] key, output logic [33-1:0] inv_key, input int row_idx);
