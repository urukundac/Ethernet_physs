...
module  hif_pcie_hif_lme_r12_wrap_tcam_hif_lme_r12_lut_tcam_shell_2048x91 #(
...
localparam PHYSICAL_ROW_WIDTH = TCAM_WIDTH + BCAM_N7*(3*32 - TCAM_WIDTH);
...
`ifndef HIF_PCIE_HIF_LME_R12_TCAM_BEHAVE_MEMS
    `define HIF_PCIE_HIF_LME_R12_ASIC_MEMS
`endif

`ifdef  HIF_PCIE_HIF_LME_R12_ASIC_MEMS
...
localparam TCAM_SLICE_EN_MIN = ((TCAM_SLICE_EN_FACT < TCAM_SLICE_EN_WIDTH ) ? TCAM_SLICE_EN_FACT : TCAM_SLICE_EN_WIDTH);
    ...
    wire    [16-1:0]                tcam_row_sel;
    assign                    tcam_row_sel[0]        = ((adr>=12'd0) && (adr<12'd256));
    assign                    tcam_row_sel[1]        = ((adr>=12'd256) && (adr<12'd512));
    assign                    tcam_row_sel[2]        = ((adr>=12'd512) && (adr<12'd768));
    assign                    tcam_row_sel[3]        = ((adr>=12'd768) && (adr<12'd1024));
    assign                    tcam_row_sel[4]        = ((adr>=12'd1024) && (adr<12'd1280));
    assign                    tcam_row_sel[5]        = ((adr>=12'd1280) && (adr<12'd1536));
    assign                    tcam_row_sel[6]        = ((adr>=12'd1536) && (adr<12'd1792));
    assign                    tcam_row_sel[7]        = ((adr>=12'd1792) && (adr<12'd2048));
    assign                    tcam_row_sel[8]        = ((adr>=12'd2048) && (adr<12'd2304));
    assign                    tcam_row_sel[9]        = ((adr>=12'd2304) && (adr<12'd2560));
    assign                    tcam_row_sel[10]        = ((adr>=12'd2560) && (adr<12'd2816));
    assign                    tcam_row_sel[11]        = ((adr>=12'd2816) && (adr<12'd3072));
    assign                    tcam_row_sel[12]        = ((adr>=12'd3072) && (adr<12'd3328));
    assign                    tcam_row_sel[13]        = ((adr>=12'd3328) && (adr<12'd3584));
    assign                    tcam_row_sel[14]        = ((adr>=12'd3584) && (adr<12'd3840));
    assign                    tcam_row_sel[15]        = (adr>=12'd3840);


    // TCAM Address Decoder

    wire    [8-1:0]        tcam_row_adr[16-1:0];
    ...
    reg            rd_en_nope_done;
    reg    [16-1:0]    rd_en_delay_inst[TCAM_RD_DELAY:0];
    logic   [TCAM_RD_DELAY:0]               rd_en_delay;
    always_comb
         begin
        rd_en_delay_inst[0][16-1:0]        = tcam_row_rd_en[16-1:0];
    ...
    reg            wr_en_nope_done;
    reg    [16-1:0]    wr_en_delay_inst[TCAM_WR_DELAY:0];
    logic    [TCAM_WR_DELAY:0]               wr_en_delay;
    always_comb
         begin
        wr_en_delay_inst[0][16-1:0]        = tcam_row_wr_en[16-1:0];
    ...
    logic    [96-1:0]    tcam_row_wr_data;
    always_comb begin
            tcam_row_wr_data = '1;
            tcam_row_wr_data[0+:PHYSICAL_ROW_WIDTH]    = wr_data;
    end
    logic    [32-1:0]    tcam_wr_data_col[3-1:0];
    always_comb begin
    ...
    logic    [96-1:0]    tcam_chk_key;
    always_comb begin
            tcam_chk_key = '1;
            tcam_chk_key[0+:PHYSICAL_ROW_WIDTH]    = chk_key;
    end
    logic    [32-1:0]    tcam_chk_key_col[3-1:0];
    always_comb begin
        for (int i = 0; i < 3; i = i + 1) begin
    ...
    logic    [96-1:0]    tcam_chk_mask;
    always_comb begin
            tcam_chk_mask        = '0;
            tcam_chk_mask        = wr_en ? '1 : chk_mask_int;
    end
    logic    [32-1:0]    tcam_chk_mask_col[3-1:0];
    always_comb begin
        for (int i = 0; i < 3; i = i + 1) begin
    ...
    logic    [2048-1:0]    tcam_hit_in;
    always_comb begin
        tcam_hit_in                = {2048{1'b1}};
        tcam_hit_in[TCAM_RULES_NUM-1:0]    = raw_hit_in[TCAM_RULES_NUM-1:0];
    end
    logic    [128-1:0]    tcam_raw_hit_in[16-1:0];
    always_comb begin
        for (int i = 0; i < 16; i = i + 1) begin
    ...
    logic    [32-1:0]    tcam_rd_data_col[16-1:0][3-1:0];
    logic    [96-1:0]    tcam_rd_data_row;
    always_comb begin
        for (int i = 0; i < 3; i = i + 1) begin
            tcam_rd_data_row[i*(32)+:32]        = tcam_rd_data_col[tcam_row_num_rd_en_delay[4-1:0]][i][32-1:0];
        end
    end
    ...
    logic    [128-1:0]    tcam_col_hit_out[16-1:0][3-1:0];
    logic    [2048-1:0]    tcam_hit_out;
    always_comb begin
        for (int i = 0; i < 16; i = i + 1) begin
                tcam_hit_out[i*128+:128]    =  tcam_col_hit_out[i][0]   & tcam_col_hit_out[i][1] & tcam_col_hit_out[i][2];
        end
    end
    assign    raw_hit_out = tcam_hit_out[TCAM_RULES_NUM-1:0];
// Match In Array 

        logic    [128-1:0]      tcam_row_match_in[16-1:0];
        always_comb begin 
                for (int i = 0; i < 16; i = i + 1) begin
    ...
    // EBBs Instantiation
        ...
        //                             |_____________________________|
                         ...
                         hif_lme_r12_ip783tcam128x32s0c1_0_0_wrap #(        
                                 )               
        ...
        //                             |_____________________________|
                         ...
                         hif_lme_r12_ip783tcam128x32s0c1_0_1_wrap #(        
                                 )               
        ...
        //                             |_____________________________|
                         ...
                         hif_lme_r12_ip783tcam128x32s0c1_0_2_wrap #(        
                                 )               
        ...
        //                             |_____________________________|
                         ...
                         hif_lme_r12_ip783tcam128x32s0c1_1_0_wrap #(        
                                 )               
        ...
        wire    [8-1:0]                       tcam_row_1_col_1_adr  = tcam_row_adr[1][8-1:0];
        ...
        wire    [128-1:0]                       tcam_row_1_col_1_hit_in = tcam_raw_hit_in[1];
        wire    [128-1:0]                       tcam_row_1_col_1_tcam_match_in = tcam_row_match_in[1];
        wire    [32-1:0]                      tcam_row_1_col_1_data_out_inv;
        logic   [128-1:0]                     tcam_row_1_col_1_hit_out_int;
        logic   [128-1:0]                     tcam_row_1_col_1_hit_out_latch;
        logic   [128-1:0]                     tcam_row_1_col_1_hit_out;
        ...
        wire      [8-1:0]     tcam_row_1_col_1_tadr = 'b0;
        ...
        //                             |_____________________________|
                         ...
                         hif_lme_r12_ip783tcam128x32s0c1_1_1_wrap #(        
                                 )               
        
                                 ip783tcam128x32s0c1_wrap_tcam_row_1_col_1 (
                   //-------------------------------------------------------------
                   // Power Management Interfaces
                   //-------------------------------------------------------------   
                      `ifndef INTEL_NO_PWR_PINS
                      ...
                      `endif
                        .vddp             (1'b1),
                      `endif
                         .ADR               (tcam_row_1_col_1_adr[8-1:1]), //Input adr_intess
                         .ME                (ce),  
        ...
        wire                                            tcam_row_1_col_2_clk         = clk;
        
        wire    [TCAM_SLICE_EN_FACT-1:0]                tcam_row_1_col_2_slice_en = TCAM_SLICE_EN_FACT'(slice_en[(1 * TCAM_SLICE_EN_FACT)+:TCAM_SLICE_EN_MIN]);
        
        wire                                            tcam_row_1_col_2_rd_en       = tcam_row_rd_en[1];
        wire                                            tcam_row_1_col_2_wr_en       = tcam_row_wr_en[1];
        wire                                            tcam_row_1_col_2_chk_en      = chk_en;
        wire                                            tcam_row_1_col_2_reset_n = reset_n;
        wire    [8-1:0]                       tcam_row_1_col_2_adr  = tcam_row_adr[1][8-1:0];
        wire    [32-1:0]                      tcam_row_1_col_2_data_in = tcam_row_wr_en[1] ? tcam_wr_data_col[2][32-1:0] : tcam_chk_key_col[2][32-1:0];
        wire    [32-1:0]                      tcam_row_1_col_2_data_in_pre = tcam_wr_data_col[2][32-1:0] ;
        wire    [32-1:0]                      tcam_row_1_col_2_key_in =  tcam_chk_key_col[2][32-1:0];
        wire    [32-1:0]                      tcam_row_1_col_2_mask_in = tcam_chk_mask_col[2][32-1:0];
        wire    [128-1:0]                       tcam_row_1_col_2_hit_in = tcam_raw_hit_in[1];
        wire    [128-1:0]                       tcam_row_1_col_2_tcam_match_in = tcam_row_match_in[1];
        wire    [32-1:0]                      tcam_row_1_col_2_data_out_inv;
        logic   [128-1:0]                     tcam_row_1_col_2_hit_out_int;
        logic   [128-1:0]                     tcam_row_1_col_2_hit_out_latch;
        logic   [128-1:0]                     tcam_row_1_col_2_hit_out;
        
        `ifdef HIF_PCIE_DFX_DISABLE
        wire                          tcam_row_1_col_2_biste = 'b0; 
        wire     [5-1:0]                  tcam_row_1_col_2_fca = 'b0;
        wire                            tcam_row_1_col_2_cre = 'b0;
        wire                            tcam_row_1_col_2_dftshiften = 'b0;
        wire                            tcam_row_1_col_2_dftmask = 'b0;
        wire                            tcam_row_1_col_2_dft_array_freeze = 'b0;
        wire                            tcam_row_1_col_2_dft_afd_reset_b = 'b0;
        wire      [32-1:0]    tcam_row_1_col_2_tdin = 'b0;
        wire      [8-1:0]     tcam_row_1_col_2_tadr = 'b0;
        wire                            tcam_row_1_col_2_tvbe = 'b0;
        wire                            tcam_row_1_col_2_tvbi = 'b0;
        wire      [32-1:0]    tcam_row_1_col_2_tmask = 'b0;
        wire                            tcam_row_1_col_2_twe = 'b0;
        wire                            tcam_row_1_col_2_tre = 'b0;
        wire                            tcam_row_1_col_2_tme = 'b0;
        wire                            tcam_row_1_col_2_bist_rotate_sr = 'b0;
        wire                            tcam_row_1_col_2_bist_en_sr_mask_b = 'b0;
        wire                            tcam_row_1_col_2_bist_all_mask = 'b0;
        wire                            tcam_row_1_col_2_bist_en_sr_data_b = 'b0;
        wire                            tcam_row_1_col_2_bist_cm_mode = 'b0;
        wire                            tcam_row_1_col_2_bist_cm_en_in = 'b0;
        wire                            tcam_row_1_col_2_bist_cm_match_sel0 = 'b0;
        wire                            tcam_row_1_col_2_bist_cm_match_sel1 = 'b0;
        wire                            tcam_row_1_col_2_bist_matchin_in = 'b0;
        wire                            tcam_row_1_col_2_bist_fls = 'b0;
        
        `else
        wire                            tcam_row_1_col_2_biste = 'b0;
        wire     [5-1:0]                  tcam_row_1_col_2_fca = 'b0;
        wire                            tcam_row_1_col_2_cre = 'b0;
        wire                            tcam_row_1_col_2_dftshiften = dftshiften;
        wire                            tcam_row_1_col_2_dftmask = dftmask;
        wire                            tcam_row_1_col_2_dft_array_freeze = dft_array_freeze;
        wire                            tcam_row_1_col_2_dft_afd_reset_b = dft_afd_reset_b;
        wire      [32-1:0]    tcam_row_1_col_2_tdin = 'b0;
        wire      [8-1:0]     tcam_row_1_col_2_tadr = 'b0;
        wire                            tcam_row_1_col_2_tvbe = 'b0;
        wire                            tcam_row_1_col_2_tvbi = 'b0;
        ...
        // Outputs
        logic                           tcam_row_1_col_2_wr_bira_fail;
        logic                           tcam_row_1_col_2_wr_so;
        logic                           tcam_row_1_col_2_curerrout;
        logic                           tcam_row_1_col_2_rscout;
        
        
        
        logic   [TCAM_SLICE_EN_FACT-1:0]                  tcam_row_1_col_2_slice_en_s0; 
        logic   [TCAM_SLICE_EN_FACT-1:0]                  tcam_row_1_col_2_slice_en_s1; 
        
        
        
             wire    [32-1:0]                     tcam_row_1_col_2_data_out ;
        
        
        
         generate
            if (BCAM_N7 == 0) 
              begin : gen_tcam_row_1_col_2_tcam
        ...
        // slice_en kept for TCAM only for backward compatability with Intel process
                ...
                begin
                  for (int i = 0; i < TCAM_SLICE_EN_FACT; i++)
                      if (tcam_row_1_col_2_slice_en_s1[i])
                           tcam_row_1_col_2_hit_out[ i*64 +: 64]  = tcam_row_1_col_2_hit_out_int  [ i*64 +: 64]; 
                      else
                           tcam_row_1_col_2_hit_out[ i*64 +: 64]  = tcam_row_1_col_2_hit_out_latch[ i*64 +: 64]; 
                end
        
        
        
                always_ff  @(posedge tcam_row_1_col_2_clk)
                begin
                  tcam_row_1_col_2_slice_en_s0 <= tcam_row_1_col_2_slice_en; 
                  tcam_row_1_col_2_slice_en_s1 <= tcam_row_1_col_2_slice_en_s0; 
        
                  for (int i = 0; i < TCAM_SLICE_EN_FACT; i++)
                    begin
                      if (tcam_row_1_col_2_slice_en_s1[i])
                         tcam_row_1_col_2_hit_out_latch[ i*64 +: 64] <=  tcam_row_1_col_2_hit_out_int[ i*64 +: 64];
                    end
                end
        ...
        //                             |_____________________________|
                         ...
                         assign tcam_row_1_col_2_data_out  = ~tcam_row_1_col_2_data_out_inv;
                         hif_lme_r12_ip783tcam128x32s0c1_1_2_wrap #(        
                                 )               
        
                                 ip783tcam128x32s0c1_wrap_tcam_row_1_col_2 (
                   //-------------------------------------------------------------
                   // Power Management Interfaces
                   //-------------------------------------------------------------   
                      `ifndef INTEL_NO_PWR_PINS
                      ...
                      `endif
                        .vddp             (1'b1),
                      `endif
                         .ADR               (tcam_row_1_col_2_adr[8-1:1]), //Input adr_intess
                         .ME                (ce),  
        
                         .CLK               (tcam_row_1_col_2_clk), //Clock
                         .CMP               (tcam_row_1_col_2_chk_en), //Control Enable
                         .DIN               (~tcam_row_1_col_2_data_in), 
                         .DSEL              (~tcam_row_1_col_2_adr[0]), //dncen enables access of the TCAM Arrays adr_intessed
                         .FLS               (flush), //flush initiates a Flush operation that resets all Valid
                         .RVLD              (tcam_row_1_col_2_hit_in ), //Row validation bus. Each match_in input controls 1
                         .MASK              (tcam_row_1_col_2_mask_in), //Input msk
                         .QOUT              (tcam_row_1_col_2_data_out_inv), //Output vben
                         .QHR               (tcam_row_1_col_2_hit_out_int),
                         .QVLD              (), 
                         .RE                (tcam_row_1_col_2_rd_en), //Read Enable
                         .RST               (tcam_row_1_col_2_reset_n), //Asynchronous Active-High Reset
                         .VBE               (1'b1), //vben enables the Valid Bit addressed by adr[a-1:0] to
                         .VBI               (1'b0),//  Valid Bit Input
                         .WE                (tcam_row_1_col_2_wr_en), //Write Enable
                         .FAF               (tcam_row_1_col_2_fca),
                         .FAF_EN            (tcam_row_1_col_2_cre),
                         .DFTSHIFTEN        (tcam_row_1_col_2_dftshiften),
                         .DFTMASK           (tcam_row_1_col_2_dftmask),
                         .DFT_ARRAY_FREEZE  (tcam_row_1_col_2_dft_array_freeze),
                         .DFT_AFD_RESET_B   (tcam_row_1_col_2_dft_afd_reset_b),
                         .BISTE           (tcam_row_1_col_2_biste),
                         .TDIN (tcam_row_1_col_2_tdin),
                         .TADR (tcam_row_1_col_2_tadr),
                         .TVBE (tcam_row_1_col_2_tvbe),
                         .TVBI (tcam_row_1_col_2_tvbi),
                         .TMASK (tcam_row_1_col_2_tmask),
                         .TWE (tcam_row_1_col_2_twe),
                         .TRE (tcam_row_1_col_2_tre),
                         .TME (tcam_row_1_col_2_tme),
                         .BIST_ROTATE_SR (tcam_row_1_col_2_bist_rotate_sr),
                         .BIST_EN_SR_MASK_B (tcam_row_1_col_2_bist_en_sr_mask_b),
                         .BIST_ALL_MASK (tcam_row_1_col_2_bist_all_mask),
                         .BIST_EN_SR_DATA_B (tcam_row_1_col_2_bist_en_sr_data_b),
                         .BIST_CM_MODE (tcam_row_1_col_2_bist_cm_mode),
                         .BIST_CM_EN_IN (tcam_row_1_col_2_bist_cm_en_in),
                         .BIST_CM_MATCH_SEL0 (tcam_row_1_col_2_bist_cm_match_sel0),
                         .BIST_CM_MATCH_SEL1 (tcam_row_1_col_2_bist_cm_match_sel1),
                         .BIST_MATCHIN_IN (tcam_row_1_col_2_bist_matchin_in),
                         .BIST_FLS (tcam_row_1_col_2_bist_fls),
                         .MDST_tcam (fuse_tcam)
                     );
              ...
              begin : gen_tcam_row_1_col_2_bcam
              end
        ...
        always_ff @(posedge clk) begin
                   tcam_rd_data_col[1][2]      <= tcam_row_1_col_2_data_out;
                   tcam_col_hit_out[1][2]       <= tcam_row_1_col_2_hit_out;
        end
        ...
        //                             |_____________________________|
                         ...
                         hif_lme_r12_ip783tcam128x32s0c1_2_0_wrap #(        
                                 )               
        ...
        wire    [8-1:0]                       tcam_row_2_col_1_adr  = tcam_row_adr[2][8-1:0];
        ...
        wire    [128-1:0]                       tcam_row_2_col_1_hit_in = tcam_raw_hit_in[2];
        wire    [128-1:0]                       tcam_row_2_col_1_tcam_match_in = tcam_row_match_in[2];
        wire    [32-1:0]                      tcam_row_2_col_1_data_out_inv;
        logic   [128-1:0]                     tcam_row_2_col_1_hit_out_int;
        logic   [128-1:0]                     tcam_row_2_col_1_hit_out_latch;
        logic   [128-1:0]                     tcam_row_2_col_1_hit_out;
        ...
        wire      [8-1:0]     tcam_row_2_col_1_tadr = 'b0;
        ...
        //                             |_____________________________|
                         ...
                         hif_lme_r12_ip783tcam128x32s0c1_2_1_wrap #(        
                                 )               
        
                                 ip783tcam128x32s0c1_wrap_tcam_row_2_col_1 (
                   //-------------------------------------------------------------
                   // Power Management Interfaces
                   //-------------------------------------------------------------   
                      `ifndef INTEL_NO_PWR_PINS
                      ...
                      `endif
                        .vddp             (1'b1),
                      `endif
                         .ADR               (tcam_row_2_col_1_adr[8-1:1]), //Input adr_intess
                         .ME                (ce),  
        ...
        wire                                            tcam_row_2_col_2_clk         = clk;
        
        wire    [TCAM_SLICE_EN_FACT-1:0]                tcam_row_2_col_2_slice_en = TCAM_SLICE_EN_FACT'(slice_en[(2 * TCAM_SLICE_EN_FACT)+:TCAM_SLICE_EN_MIN]);
        
        wire                                            tcam_row_2_col_2_rd_en       = tcam_row_rd_en[2];
        wire                                            tcam_row_2_col_2_wr_en       = tcam_row_wr_en[2];
        wire                                            tcam_row_2_col_2_chk_en      = chk_en;
        wire                                            tcam_row_2_col_2_reset_n = reset_n;
        wire    [8-1:0]                       tcam_row_2_col_2_adr  = tcam_row_adr[2][8-1:0];
        wire    [32-1:0]                      tcam_row_2_col_2_data_in = tcam_row_wr_en[2] ? tcam_wr_data_col[2][32-1:0] : tcam_chk_key_col[2][32-1:0];
        wire    [32-1:0]                      tcam_row_2_col_2_data_in_pre = tcam_wr_data_col[2][32-1:0] ;
        wire    [32-1:0]                      tcam_row_2_col_2_key_in =  tcam_chk_key_col[2][32-1:0];
        wire    [32-1:0]                      tcam_row_2_col_2_mask_in = tcam_chk_mask_col[2][32-1:0];
        wire    [128-1:0]                       tcam_row_2_col_2_hit_in = tcam_raw_hit_in[2];
        wire    [128-1:0]                       tcam_row_2_col_2_tcam_match_in = tcam_row_match_in[2];
        wire    [32-1:0]                      tcam_row_2_col_2_data_out_inv;
        logic   [128-1:0]                     tcam_row_2_col_2_hit_out_int;
        logic   [128-1:0]                     tcam_row_2_col_2_hit_out_latch;
        logic   [128-1:0]                     tcam_row_2_col_2_hit_out;
        
        `ifdef HIF_PCIE_DFX_DISABLE
        wire                          tcam_row_2_col_2_biste = 'b0; 
        wire     [5-1:0]                  tcam_row_2_col_2_fca = 'b0;
        wire                            tcam_row_2_col_2_cre = 'b0;
        wire                            tcam_row_2_col_2_dftshiften = 'b0;
        wire                            tcam_row_2_col_2_dftmask = 'b0;
        wire                            tcam_row_2_col_2_dft_array_freeze = 'b0;
        wire                            tcam_row_2_col_2_dft_afd_reset_b = 'b0;
        wire      [32-1:0]    tcam_row_2_col_2_tdin = 'b0;
        wire      [8-1:0]     tcam_row_2_col_2_tadr = 'b0;
        wire                            tcam_row_2_col_2_tvbe = 'b0;
        wire                            tcam_row_2_col_2_tvbi = 'b0;
        wire      [32-1:0]    tcam_row_2_col_2_tmask = 'b0;
        wire                            tcam_row_2_col_2_twe = 'b0;
        wire                            tcam_row_2_col_2_tre = 'b0;
        wire                            tcam_row_2_col_2_tme = 'b0;
        wire                            tcam_row_2_col_2_bist_rotate_sr = 'b0;
        wire                            tcam_row_2_col_2_bist_en_sr_mask_b = 'b0;
        wire                            tcam_row_2_col_2_bist_all_mask = 'b0;
        wire                            tcam_row_2_col_2_bist_en_sr_data_b = 'b0;
        wire                            tcam_row_2_col_2_bist_cm_mode = 'b0;
        wire                            tcam_row_2_col_2_bist_cm_en_in = 'b0;
        wire                            tcam_row_2_col_2_bist_cm_match_sel0 = 'b0;
        wire                            tcam_row_2_col_2_bist_cm_match_sel1 = 'b0;
        wire                            tcam_row_2_col_2_bist_matchin_in = 'b0;
        wire                            tcam_row_2_col_2_bist_fls = 'b0;
        
        `else
        wire                            tcam_row_2_col_2_biste = 'b0;
        wire     [5-1:0]                  tcam_row_2_col_2_fca = 'b0;
        wire                            tcam_row_2_col_2_cre = 'b0;
        wire                            tcam_row_2_col_2_dftshiften = dftshiften;
        wire                            tcam_row_2_col_2_dftmask = dftmask;
        wire                            tcam_row_2_col_2_dft_array_freeze = dft_array_freeze;
        wire                            tcam_row_2_col_2_dft_afd_reset_b = dft_afd_reset_b;
        wire      [32-1:0]    tcam_row_2_col_2_tdin = 'b0;
        wire      [8-1:0]     tcam_row_2_col_2_tadr = 'b0;
        wire                            tcam_row_2_col_2_tvbe = 'b0;
        wire                            tcam_row_2_col_2_tvbi = 'b0;
        ...
        // Outputs
        logic                           tcam_row_2_col_2_wr_bira_fail;
        logic                           tcam_row_2_col_2_wr_so;
        logic                           tcam_row_2_col_2_curerrout;
        logic                           tcam_row_2_col_2_rscout;
        
        
        
        logic   [TCAM_SLICE_EN_FACT-1:0]                  tcam_row_2_col_2_slice_en_s0; 
        logic   [TCAM_SLICE_EN_FACT-1:0]                  tcam_row_2_col_2_slice_en_s1; 
        
        
        
             wire    [32-1:0]                     tcam_row_2_col_2_data_out ;
        
        
        
         generate
            if (BCAM_N7 == 0) 
              begin : gen_tcam_row_2_col_2_tcam
        ...
        // slice_en kept for TCAM only for backward compatability with Intel process
                ...
                begin
                  for (int i = 0; i < TCAM_SLICE_EN_FACT; i++)
                      if (tcam_row_2_col_2_slice_en_s1[i])
                           tcam_row_2_col_2_hit_out[ i*64 +: 64]  = tcam_row_2_col_2_hit_out_int  [ i*64 +: 64]; 
                      else
                           tcam_row_2_col_2_hit_out[ i*64 +: 64]  = tcam_row_2_col_2_hit_out_latch[ i*64 +: 64]; 
                end
        
        
        
                always_ff  @(posedge tcam_row_2_col_2_clk)
                begin
                  tcam_row_2_col_2_slice_en_s0 <= tcam_row_2_col_2_slice_en; 
                  tcam_row_2_col_2_slice_en_s1 <= tcam_row_2_col_2_slice_en_s0; 
        
                  for (int i = 0; i < TCAM_SLICE_EN_FACT; i++)
                    begin
                      if (tcam_row_2_col_2_slice_en_s1[i])
                         tcam_row_2_col_2_hit_out_latch[ i*64 +: 64] <=  tcam_row_2_col_2_hit_out_int[ i*64 +: 64];
                    end
                end
        ...
        //                             |_____________________________|
                         ...
                         assign tcam_row_2_col_2_data_out  = ~tcam_row_2_col_2_data_out_inv;
                         hif_lme_r12_ip783tcam128x32s0c1_2_2_wrap #(        
                                 )               
        
                                 ip783tcam128x32s0c1_wrap_tcam_row_2_col_2 (
                   //-------------------------------------------------------------
                   // Power Management Interfaces
                   //-------------------------------------------------------------   
                      `ifndef INTEL_NO_PWR_PINS
                      ...
                      `endif
                        .vddp             (1'b1),
                      `endif
                         .ADR               (tcam_row_2_col_2_adr[8-1:1]), //Input adr_intess
                         .ME                (ce),  
        
                         .CLK               (tcam_row_2_col_2_clk), //Clock
                         .CMP               (tcam_row_2_col_2_chk_en), //Control Enable
                         .DIN               (~tcam_row_2_col_2_data_in), 
                         .DSEL              (~tcam_row_2_col_2_adr[0]), //dncen enables access of the TCAM Arrays adr_intessed
                         .FLS               (flush), //flush initiates a Flush operation that resets all Valid
                         .RVLD              (tcam_row_2_col_2_hit_in ), //Row validation bus. Each match_in input controls 1
                         .MASK              (tcam_row_2_col_2_mask_in), //Input msk
                         .QOUT              (tcam_row_2_col_2_data_out_inv), //Output vben
                         .QHR               (tcam_row_2_col_2_hit_out_int),
                         .QVLD              (), 
                         .RE                (tcam_row_2_col_2_rd_en), //Read Enable
                         .RST               (tcam_row_2_col_2_reset_n), //Asynchronous Active-High Reset
                         .VBE               (1'b1), //vben enables the Valid Bit addressed by adr[a-1:0] to
                         .VBI               (1'b0),//  Valid Bit Input
                         .WE                (tcam_row_2_col_2_wr_en), //Write Enable
                         .FAF               (tcam_row_2_col_2_fca),
                         .FAF_EN            (tcam_row_2_col_2_cre),
                         .DFTSHIFTEN        (tcam_row_2_col_2_dftshiften),
                         .DFTMASK           (tcam_row_2_col_2_dftmask),
                         .DFT_ARRAY_FREEZE  (tcam_row_2_col_2_dft_array_freeze),
                         .DFT_AFD_RESET_B   (tcam_row_2_col_2_dft_afd_reset_b),
                         .BISTE           (tcam_row_2_col_2_biste),
                         .TDIN (tcam_row_2_col_2_tdin),
                         .TADR (tcam_row_2_col_2_tadr),
                         .TVBE (tcam_row_2_col_2_tvbe),
                         .TVBI (tcam_row_2_col_2_tvbi),
                         .TMASK (tcam_row_2_col_2_tmask),
                         .TWE (tcam_row_2_col_2_twe),
                         .TRE (tcam_row_2_col_2_tre),
                         .TME (tcam_row_2_col_2_tme),
                         .BIST_ROTATE_SR (tcam_row_2_col_2_bist_rotate_sr),
                         .BIST_EN_SR_MASK_B (tcam_row_2_col_2_bist_en_sr_mask_b),
                         .BIST_ALL_MASK (tcam_row_2_col_2_bist_all_mask),
                         .BIST_EN_SR_DATA_B (tcam_row_2_col_2_bist_en_sr_data_b),
                         .BIST_CM_MODE (tcam_row_2_col_2_bist_cm_mode),
                         .BIST_CM_EN_IN (tcam_row_2_col_2_bist_cm_en_in),
                         .BIST_CM_MATCH_SEL0 (tcam_row_2_col_2_bist_cm_match_sel0),
                         .BIST_CM_MATCH_SEL1 (tcam_row_2_col_2_bist_cm_match_sel1),
                         .BIST_MATCHIN_IN (tcam_row_2_col_2_bist_matchin_in),
                         .BIST_FLS (tcam_row_2_col_2_bist_fls),
                         .MDST_tcam (fuse_tcam)
                     );
              ...
              begin : gen_tcam_row_2_col_2_bcam
              end
        ...
        always_ff @(posedge clk) begin
                   tcam_rd_data_col[2][2]      <= tcam_row_2_col_2_data_out;
                   tcam_col_hit_out[2][2]       <= tcam_row_2_col_2_hit_out;
        end
        ...
        //                             |_____________________________|
                         ...
                         hif_lme_r12_ip783tcam128x32s0c1_3_0_wrap #(        
                                 )               
        ...
        wire    [8-1:0]                       tcam_row_3_col_1_adr  = tcam_row_adr[3][8-1:0];
        ...
        wire    [128-1:0]                       tcam_row_3_col_1_hit_in = tcam_raw_hit_in[3];
        wire    [128-1:0]                       tcam_row_3_col_1_tcam_match_in = tcam_row_match_in[3];
        wire    [32-1:0]                      tcam_row_3_col_1_data_out_inv;
        logic   [128-1:0]                     tcam_row_3_col_1_hit_out_int;
        logic   [128-1:0]                     tcam_row_3_col_1_hit_out_latch;
        logic   [128-1:0]                     tcam_row_3_col_1_hit_out;
        ...
        wire      [8-1:0]     tcam_row_3_col_1_tadr = 'b0;
        ...
        //                             |_____________________________|
                         ...
                         hif_lme_r12_ip783tcam128x32s0c1_3_1_wrap #(        
                                 )               
        
                                 ip783tcam128x32s0c1_wrap_tcam_row_3_col_1 (
                   //-------------------------------------------------------------
                   // Power Management Interfaces
                   //-------------------------------------------------------------   
                      `ifndef INTEL_NO_PWR_PINS
                      ...
                      `endif
                        .vddp             (1'b1),
                      `endif
                         .ADR               (tcam_row_3_col_1_adr[8-1:1]), //Input adr_intess
                         .ME                (ce),  
        ...
        wire                                            tcam_row_3_col_2_clk         = clk;
        
        wire    [TCAM_SLICE_EN_FACT-1:0]                tcam_row_3_col_2_slice_en = TCAM_SLICE_EN_FACT'(slice_en[(3 * TCAM_SLICE_EN_FACT)+:TCAM_SLICE_EN_MIN]);
        
        wire                                            tcam_row_3_col_2_rd_en       = tcam_row_rd_en[3];
        wire                                            tcam_row_3_col_2_wr_en       = tcam_row_wr_en[3];
        wire                                            tcam_row_3_col_2_chk_en      = chk_en;
        wire                                            tcam_row_3_col_2_reset_n = reset_n;
        wire    [8-1:0]                       tcam_row_3_col_2_adr  = tcam_row_adr[3][8-1:0];
        wire    [32-1:0]                      tcam_row_3_col_2_data_in = tcam_row_wr_en[3] ? tcam_wr_data_col[2][32-1:0] : tcam_chk_key_col[2][32-1:0];
        wire    [32-1:0]                      tcam_row_3_col_2_data_in_pre = tcam_wr_data_col[2][32-1:0] ;
        wire    [32-1:0]                      tcam_row_3_col_2_key_in =  tcam_chk_key_col[2][32-1:0];
        wire    [32-1:0]                      tcam_row_3_col_2_mask_in = tcam_chk_mask_col[2][32-1:0];
        wire    [128-1:0]                       tcam_row_3_col_2_hit_in = tcam_raw_hit_in[3];
        wire    [128-1:0]                       tcam_row_3_col_2_tcam_match_in = tcam_row_match_in[3];
        wire    [32-1:0]                      tcam_row_3_col_2_data_out_inv;
        logic   [128-1:0]                     tcam_row_3_col_2_hit_out_int;
        logic   [128-1:0]                     tcam_row_3_col_2_hit_out_latch;
        logic   [128-1:0]                     tcam_row_3_col_2_hit_out;
        
        `ifdef HIF_PCIE_DFX_DISABLE
        wire                          tcam_row_3_col_2_biste = 'b0; 
        wire     [5-1:0]                  tcam_row_3_col_2_fca = 'b0;
        wire                            tcam_row_3_col_2_cre = 'b0;
        wire                            tcam_row_3_col_2_dftshiften = 'b0;
        wire                            tcam_row_3_col_2_dftmask = 'b0;
        wire                            tcam_row_3_col_2_dft_array_freeze = 'b0;
        wire                            tcam_row_3_col_2_dft_afd_reset_b = 'b0;
        wire      [32-1:0]    tcam_row_3_col_2_tdin = 'b0;
        wire      [8-1:0]     tcam_row_3_col_2_tadr = 'b0;
        wire                            tcam_row_3_col_2_tvbe = 'b0;
        wire                            tcam_row_3_col_2_tvbi = 'b0;
        wire      [32-1:0]    tcam_row_3_col_2_tmask = 'b0;
        wire                            tcam_row_3_col_2_twe = 'b0;
        wire                            tcam_row_3_col_2_tre = 'b0;
        wire                            tcam_row_3_col_2_tme = 'b0;
        wire                            tcam_row_3_col_2_bist_rotate_sr = 'b0;
        wire                            tcam_row_3_col_2_bist_en_sr_mask_b = 'b0;
        wire                            tcam_row_3_col_2_bist_all_mask = 'b0;
        wire                            tcam_row_3_col_2_bist_en_sr_data_b = 'b0;
        wire                            tcam_row_3_col_2_bist_cm_mode = 'b0;
        wire                            tcam_row_3_col_2_bist_cm_en_in = 'b0;
        wire                            tcam_row_3_col_2_bist_cm_match_sel0 = 'b0;
        wire                            tcam_row_3_col_2_bist_cm_match_sel1 = 'b0;
        wire                            tcam_row_3_col_2_bist_matchin_in = 'b0;
        wire                            tcam_row_3_col_2_bist_fls = 'b0;
        
        `else
        wire                            tcam_row_3_col_2_biste = 'b0;
        wire     [5-1:0]                  tcam_row_3_col_2_fca = 'b0;
        wire                            tcam_row_3_col_2_cre = 'b0;
        wire                            tcam_row_3_col_2_dftshiften = dftshiften;
        wire                            tcam_row_3_col_2_dftmask = dftmask;
        wire                            tcam_row_3_col_2_dft_array_freeze = dft_array_freeze;
        wire                            tcam_row_3_col_2_dft_afd_reset_b = dft_afd_reset_b;
        wire      [32-1:0]    tcam_row_3_col_2_tdin = 'b0;
        wire      [8-1:0]     tcam_row_3_col_2_tadr = 'b0;
        wire                            tcam_row_3_col_2_tvbe = 'b0;
        wire                            tcam_row_3_col_2_tvbi = 'b0;
        ...
        // Outputs
        logic                           tcam_row_3_col_2_wr_bira_fail;
        logic                           tcam_row_3_col_2_wr_so;
        logic                           tcam_row_3_col_2_curerrout;
        logic                           tcam_row_3_col_2_rscout;
        
        
        
        logic   [TCAM_SLICE_EN_FACT-1:0]                  tcam_row_3_col_2_slice_en_s0; 
        logic   [TCAM_SLICE_EN_FACT-1:0]                  tcam_row_3_col_2_slice_en_s1; 
        
        
        
             wire    [32-1:0]                     tcam_row_3_col_2_data_out ;
        
        
        
         generate
            if (BCAM_N7 == 0) 
              begin : gen_tcam_row_3_col_2_tcam
        ...
        // slice_en kept for TCAM only for backward compatability with Intel process
                ...
                begin
                  for (int i = 0; i < TCAM_SLICE_EN_FACT; i++)
                      if (tcam_row_3_col_2_slice_en_s1[i])
                           tcam_row_3_col_2_hit_out[ i*64 +: 64]  = tcam_row_3_col_2_hit_out_int  [ i*64 +: 64]; 
                      else
                           tcam_row_3_col_2_hit_out[ i*64 +: 64]  = tcam_row_3_col_2_hit_out_latch[ i*64 +: 64]; 
                end
        
        
        
                always_ff  @(posedge tcam_row_3_col_2_clk)
                begin
                  tcam_row_3_col_2_slice_en_s0 <= tcam_row_3_col_2_slice_en; 
                  tcam_row_3_col_2_slice_en_s1 <= tcam_row_3_col_2_slice_en_s0; 
        
                  for (int i = 0; i < TCAM_SLICE_EN_FACT; i++)
                    begin
                      if (tcam_row_3_col_2_slice_en_s1[i])
                         tcam_row_3_col_2_hit_out_latch[ i*64 +: 64] <=  tcam_row_3_col_2_hit_out_int[ i*64 +: 64];
                    end
                end
        ...
        //                             |_____________________________|
                         ...
                         assign tcam_row_3_col_2_data_out  = ~tcam_row_3_col_2_data_out_inv;
                         hif_lme_r12_ip783tcam128x32s0c1_3_2_wrap #(        
                                 )               
        
                                 ip783tcam128x32s0c1_wrap_tcam_row_3_col_2 (
                   //-------------------------------------------------------------
                   // Power Management Interfaces
                   //-------------------------------------------------------------   
                      `ifndef INTEL_NO_PWR_PINS
                      ...
                      `endif
                        .vddp             (1'b1),
                      `endif
                         .ADR               (tcam_row_3_col_2_adr[8-1:1]), //Input adr_intess
                         .ME                (ce),  
        
                         .CLK               (tcam_row_3_col_2_clk), //Clock
                         .CMP               (tcam_row_3_col_2_chk_en), //Control Enable
                         .DIN               (~tcam_row_3_col_2_data_in), 
                         .DSEL              (~tcam_row_3_col_2_adr[0]), //dncen enables access of the TCAM Arrays adr_intessed
                         .FLS               (flush), //flush initiates a Flush operation that resets all Valid
                         .RVLD              (tcam_row_3_col_2_hit_in ), //Row validation bus. Each match_in input controls 1
                         .MASK              (tcam_row_3_col_2_mask_in), //Input msk
                         .QOUT              (tcam_row_3_col_2_data_out_inv), //Output vben
                         .QHR               (tcam_row_3_col_2_hit_out_int),
                         .QVLD              (), 
                         .RE                (tcam_row_3_col_2_rd_en), //Read Enable
                         .RST               (tcam_row_3_col_2_reset_n), //Asynchronous Active-High Reset
                         .VBE               (1'b1), //vben enables the Valid Bit addressed by adr[a-1:0] to
                         .VBI               (1'b0),//  Valid Bit Input
                         .WE                (tcam_row_3_col_2_wr_en), //Write Enable
                         .FAF               (tcam_row_3_col_2_fca),
                         .FAF_EN            (tcam_row_3_col_2_cre),
                         .DFTSHIFTEN        (tcam_row_3_col_2_dftshiften),
                         .DFTMASK           (tcam_row_3_col_2_dftmask),
                         .DFT_ARRAY_FREEZE  (tcam_row_3_col_2_dft_array_freeze),
                         .DFT_AFD_RESET_B   (tcam_row_3_col_2_dft_afd_reset_b),
                         .BISTE           (tcam_row_3_col_2_biste),
                         .TDIN (tcam_row_3_col_2_tdin),
                         .TADR (tcam_row_3_col_2_tadr),
                         .TVBE (tcam_row_3_col_2_tvbe),
                         .TVBI (tcam_row_3_col_2_tvbi),
                         .TMASK (tcam_row_3_col_2_tmask),
                         .TWE (tcam_row_3_col_2_twe),
                         .TRE (tcam_row_3_col_2_tre),
                         .TME (tcam_row_3_col_2_tme),
                         .BIST_ROTATE_SR (tcam_row_3_col_2_bist_rotate_sr),
                         .BIST_EN_SR_MASK_B (tcam_row_3_col_2_bist_en_sr_mask_b),
                         .BIST_ALL_MASK (tcam_row_3_col_2_bist_all_mask),
                         .BIST_EN_SR_DATA_B (tcam_row_3_col_2_bist_en_sr_data_b),
                         .BIST_CM_MODE (tcam_row_3_col_2_bist_cm_mode),
                         .BIST_CM_EN_IN (tcam_row_3_col_2_bist_cm_en_in),
                         .BIST_CM_MATCH_SEL0 (tcam_row_3_col_2_bist_cm_match_sel0),
                         .BIST_CM_MATCH_SEL1 (tcam_row_3_col_2_bist_cm_match_sel1),
                         .BIST_MATCHIN_IN (tcam_row_3_col_2_bist_matchin_in),
                         .BIST_FLS (tcam_row_3_col_2_bist_fls),
                         .MDST_tcam (fuse_tcam)
                     );
              ...
              begin : gen_tcam_row_3_col_2_bcam
              end
        ...
        always_ff @(posedge clk) begin
                   tcam_rd_data_col[3][2]      <= tcam_row_3_col_2_data_out;
                   tcam_col_hit_out[3][2]       <= tcam_row_3_col_2_hit_out;
        end
        ...
        //                             |_____________________________|
                         ...
                         hif_lme_r12_ip783tcam128x32s0c1_4_0_wrap #(        
                                 )               
        ...
        wire    [8-1:0]                       tcam_row_4_col_1_adr  = tcam_row_adr[4][8-1:0];
        ...
        wire    [128-1:0]                       tcam_row_4_col_1_hit_in = tcam_raw_hit_in[4];
        wire    [128-1:0]                       tcam_row_4_col_1_tcam_match_in = tcam_row_match_in[4];
        wire    [32-1:0]                      tcam_row_4_col_1_data_out_inv;
        logic   [128-1:0]                     tcam_row_4_col_1_hit_out_int;
        logic   [128-1:0]                     tcam_row_4_col_1_hit_out_latch;
        logic   [128-1:0]                     tcam_row_4_col_1_hit_out;
        ...
        wire      [8-1:0]     tcam_row_4_col_1_tadr = 'b0;
        ...
        //                             |_____________________________|
                         ...
                         hif_lme_r12_ip783tcam128x32s0c1_4_1_wrap #(        
                                 )               
        
                                 ip783tcam128x32s0c1_wrap_tcam_row_4_col_1 (
                   //-------------------------------------------------------------
                   // Power Management Interfaces
                   //-------------------------------------------------------------   
                      `ifndef INTEL_NO_PWR_PINS
                      ...
                      `endif
                        .vddp             (1'b1),
                      `endif
                         .ADR               (tcam_row_4_col_1_adr[8-1:1]), //Input adr_intess
                         .ME                (ce),  
        ...
        wire                                            tcam_row_4_col_2_clk         = clk;
        
        wire    [TCAM_SLICE_EN_FACT-1:0]                tcam_row_4_col_2_slice_en = TCAM_SLICE_EN_FACT'(slice_en[(4 * TCAM_SLICE_EN_FACT)+:TCAM_SLICE_EN_MIN]);
        
        wire                                            tcam_row_4_col_2_rd_en       = tcam_row_rd_en[4];
        wire                                            tcam_row_4_col_2_wr_en       = tcam_row_wr_en[4];
        wire                                            tcam_row_4_col_2_chk_en      = chk_en;
        wire                                            tcam_row_4_col_2_reset_n = reset_n;
        wire    [8-1:0]                       tcam_row_4_col_2_adr  = tcam_row_adr[4][8-1:0];
        wire    [32-1:0]                      tcam_row_4_col_2_data_in = tcam_row_wr_en[4] ? tcam_wr_data_col[2][32-1:0] : tcam_chk_key_col[2][32-1:0];
        wire    [32-1:0]                      tcam_row_4_col_2_data_in_pre = tcam_wr_data_col[2][32-1:0] ;
        wire    [32-1:0]                      tcam_row_4_col_2_key_in =  tcam_chk_key_col[2][32-1:0];
        wire    [32-1:0]                      tcam_row_4_col_2_mask_in = tcam_chk_mask_col[2][32-1:0];
        wire    [128-1:0]                       tcam_row_4_col_2_hit_in = tcam_raw_hit_in[4];
        wire    [128-1:0]                       tcam_row_4_col_2_tcam_match_in = tcam_row_match_in[4];
        wire    [32-1:0]                      tcam_row_4_col_2_data_out_inv;
        logic   [128-1:0]                     tcam_row_4_col_2_hit_out_int;
        logic   [128-1:0]                     tcam_row_4_col_2_hit_out_latch;
        logic   [128-1:0]                     tcam_row_4_col_2_hit_out;
        
        `ifdef HIF_PCIE_DFX_DISABLE
        wire                          tcam_row_4_col_2_biste = 'b0; 
        wire     [5-1:0]                  tcam_row_4_col_2_fca = 'b0;
        wire                            tcam_row_4_col_2_cre = 'b0;
        wire                            tcam_row_4_col_2_dftshiften = 'b0;
        wire                            tcam_row_4_col_2_dftmask = 'b0;
        wire                            tcam_row_4_col_2_dft_array_freeze = 'b0;
        wire                            tcam_row_4_col_2_dft_afd_reset_b = 'b0;
        wire      [32-1:0]    tcam_row_4_col_2_tdin = 'b0;
        wire      [8-1:0]     tcam_row_4_col_2_tadr = 'b0;
        wire                            tcam_row_4_col_2_tvbe = 'b0;
        wire                            tcam_row_4_col_2_tvbi = 'b0;
        wire      [32-1:0]    tcam_row_4_col_2_tmask = 'b0;
        wire                            tcam_row_4_col_2_twe = 'b0;
        wire                            tcam_row_4_col_2_tre = 'b0;
        wire                            tcam_row_4_col_2_tme = 'b0;
        wire                            tcam_row_4_col_2_bist_rotate_sr = 'b0;
        wire                            tcam_row_4_col_2_bist_en_sr_mask_b = 'b0;
        wire                            tcam_row_4_col_2_bist_all_mask = 'b0;
        wire                            tcam_row_4_col_2_bist_en_sr_data_b = 'b0;
        wire                            tcam_row_4_col_2_bist_cm_mode = 'b0;
        wire                            tcam_row_4_col_2_bist_cm_en_in = 'b0;
        wire                            tcam_row_4_col_2_bist_cm_match_sel0 = 'b0;
        wire                            tcam_row_4_col_2_bist_cm_match_sel1 = 'b0;
        wire                            tcam_row_4_col_2_bist_matchin_in = 'b0;
        wire                            tcam_row_4_col_2_bist_fls = 'b0;
        
        `else
        wire                            tcam_row_4_col_2_biste = 'b0;
        wire     [5-1:0]                  tcam_row_4_col_2_fca = 'b0;
        wire                            tcam_row_4_col_2_cre = 'b0;
        wire                            tcam_row_4_col_2_dftshiften = dftshiften;
        wire                            tcam_row_4_col_2_dftmask = dftmask;
        wire                            tcam_row_4_col_2_dft_array_freeze = dft_array_freeze;
        wire                            tcam_row_4_col_2_dft_afd_reset_b = dft_afd_reset_b;
        wire      [32-1:0]    tcam_row_4_col_2_tdin = 'b0;
        wire      [8-1:0]     tcam_row_4_col_2_tadr = 'b0;
        wire                            tcam_row_4_col_2_tvbe = 'b0;
        wire                            tcam_row_4_col_2_tvbi = 'b0;
        ...
        // Outputs
        logic                           tcam_row_4_col_2_wr_bira_fail;
        logic                           tcam_row_4_col_2_wr_so;
        logic                           tcam_row_4_col_2_curerrout;
        logic                           tcam_row_4_col_2_rscout;
        
        
        
        logic   [TCAM_SLICE_EN_FACT-1:0]                  tcam_row_4_col_2_slice_en_s0; 
        logic   [TCAM_SLICE_EN_FACT-1:0]                  tcam_row_4_col_2_slice_en_s1; 
        
        
        
             wire    [32-1:0]                     tcam_row_4_col_2_data_out ;
        
        
        
         generate
            if (BCAM_N7 == 0) 
              begin : gen_tcam_row_4_col_2_tcam
        ...
        // slice_en kept for TCAM only for backward compatability with Intel process
                ...
                begin
                  for (int i = 0; i < TCAM_SLICE_EN_FACT; i++)
                      if (tcam_row_4_col_2_slice_en_s1[i])
                           tcam_row_4_col_2_hit_out[ i*64 +: 64]  = tcam_row_4_col_2_hit_out_int  [ i*64 +: 64]; 
                      else
                           tcam_row_4_col_2_hit_out[ i*64 +: 64]  = tcam_row_4_col_2_hit_out_latch[ i*64 +: 64]; 
                end
        
        
        
                always_ff  @(posedge tcam_row_4_col_2_clk)
                begin
                  tcam_row_4_col_2_slice_en_s0 <= tcam_row_4_col_2_slice_en; 
                  tcam_row_4_col_2_slice_en_s1 <= tcam_row_4_col_2_slice_en_s0; 
        
                  for (int i = 0; i < TCAM_SLICE_EN_FACT; i++)
                    begin
                      if (tcam_row_4_col_2_slice_en_s1[i])
                         tcam_row_4_col_2_hit_out_latch[ i*64 +: 64] <=  tcam_row_4_col_2_hit_out_int[ i*64 +: 64];
                    end
                end
        ...
        //                             |_____________________________|
                         ...
                         assign tcam_row_4_col_2_data_out  = ~tcam_row_4_col_2_data_out_inv;
                         hif_lme_r12_ip783tcam128x32s0c1_4_2_wrap #(        
                                 )               
        
                                 ip783tcam128x32s0c1_wrap_tcam_row_4_col_2 (
                   //-------------------------------------------------------------
                   // Power Management Interfaces
                   //-------------------------------------------------------------   
                      `ifndef INTEL_NO_PWR_PINS
                      ...
                      `endif
                        .vddp             (1'b1),
                      `endif
                         .ADR               (tcam_row_4_col_2_adr[8-1:1]), //Input adr_intess
                         .ME                (ce),  
        
                         .CLK               (tcam_row_4_col_2_clk), //Clock
                         .CMP               (tcam_row_4_col_2_chk_en), //Control Enable
                         .DIN               (~tcam_row_4_col_2_data_in), 
                         .DSEL              (~tcam_row_4_col_2_adr[0]), //dncen enables access of the TCAM Arrays adr_intessed
                         .FLS               (flush), //flush initiates a Flush operation that resets all Valid
                         .RVLD              (tcam_row_4_col_2_hit_in ), //Row validation bus. Each match_in input controls 1
                         .MASK              (tcam_row_4_col_2_mask_in), //Input msk
                         .QOUT              (tcam_row_4_col_2_data_out_inv), //Output vben
                         .QHR               (tcam_row_4_col_2_hit_out_int),
                         .QVLD              (), 
                         .RE                (tcam_row_4_col_2_rd_en), //Read Enable
                         .RST               (tcam_row_4_col_2_reset_n), //Asynchronous Active-High Reset
                         .VBE               (1'b1), //vben enables the Valid Bit addressed by adr[a-1:0] to
                         .VBI               (1'b0),//  Valid Bit Input
                         .WE                (tcam_row_4_col_2_wr_en), //Write Enable
                         .FAF               (tcam_row_4_col_2_fca),
                         .FAF_EN            (tcam_row_4_col_2_cre),
                         .DFTSHIFTEN        (tcam_row_4_col_2_dftshiften),
                         .DFTMASK           (tcam_row_4_col_2_dftmask),
                         .DFT_ARRAY_FREEZE  (tcam_row_4_col_2_dft_array_freeze),
                         .DFT_AFD_RESET_B   (tcam_row_4_col_2_dft_afd_reset_b),
                         .BISTE           (tcam_row_4_col_2_biste),
                         .TDIN (tcam_row_4_col_2_tdin),
                         .TADR (tcam_row_4_col_2_tadr),
                         .TVBE (tcam_row_4_col_2_tvbe),
                         .TVBI (tcam_row_4_col_2_tvbi),
                         .TMASK (tcam_row_4_col_2_tmask),
                         .TWE (tcam_row_4_col_2_twe),
                         .TRE (tcam_row_4_col_2_tre),
                         .TME (tcam_row_4_col_2_tme),
                         .BIST_ROTATE_SR (tcam_row_4_col_2_bist_rotate_sr),
                         .BIST_EN_SR_MASK_B (tcam_row_4_col_2_bist_en_sr_mask_b),
                         .BIST_ALL_MASK (tcam_row_4_col_2_bist_all_mask),
                         .BIST_EN_SR_DATA_B (tcam_row_4_col_2_bist_en_sr_data_b),
                         .BIST_CM_MODE (tcam_row_4_col_2_bist_cm_mode),
                         .BIST_CM_EN_IN (tcam_row_4_col_2_bist_cm_en_in),
                         .BIST_CM_MATCH_SEL0 (tcam_row_4_col_2_bist_cm_match_sel0),
                         .BIST_CM_MATCH_SEL1 (tcam_row_4_col_2_bist_cm_match_sel1),
                         .BIST_MATCHIN_IN (tcam_row_4_col_2_bist_matchin_in),
                         .BIST_FLS (tcam_row_4_col_2_bist_fls),
                         .MDST_tcam (fuse_tcam)
                     );
              ...
              begin : gen_tcam_row_4_col_2_bcam
              end
        ...
        always_ff @(posedge clk) begin
                   tcam_rd_data_col[4][2]      <= tcam_row_4_col_2_data_out;
                   tcam_col_hit_out[4][2]       <= tcam_row_4_col_2_hit_out;
        end
        ...
        //                             |_____________________________|
                         ...
                         hif_lme_r12_ip783tcam128x32s0c1_5_0_wrap #(        
                                 )               
        ...
        wire    [8-1:0]                       tcam_row_5_col_1_adr  = tcam_row_adr[5][8-1:0];
        ...
        wire    [128-1:0]                       tcam_row_5_col_1_hit_in = tcam_raw_hit_in[5];
        wire    [128-1:0]                       tcam_row_5_col_1_tcam_match_in = tcam_row_match_in[5];
        wire    [32-1:0]                      tcam_row_5_col_1_data_out_inv;
        logic   [128-1:0]                     tcam_row_5_col_1_hit_out_int;
        logic   [128-1:0]                     tcam_row_5_col_1_hit_out_latch;
        logic   [128-1:0]                     tcam_row_5_col_1_hit_out;
        ...
        wire      [8-1:0]     tcam_row_5_col_1_tadr = 'b0;
        ...
        //                             |_____________________________|
                         ...
                         hif_lme_r12_ip783tcam128x32s0c1_5_1_wrap #(        
                                 )               
        
                                 ip783tcam128x32s0c1_wrap_tcam_row_5_col_1 (
                   //-------------------------------------------------------------
                   // Power Management Interfaces
                   //-------------------------------------------------------------   
                      `ifndef INTEL_NO_PWR_PINS
                      ...
                      `endif
                        .vddp             (1'b1),
                      `endif
                         .ADR               (tcam_row_5_col_1_adr[8-1:1]), //Input adr_intess
                         .ME                (ce),  
        ...
        wire                                            tcam_row_5_col_2_clk         = clk;
        
        wire    [TCAM_SLICE_EN_FACT-1:0]                tcam_row_5_col_2_slice_en = TCAM_SLICE_EN_FACT'(slice_en[(5 * TCAM_SLICE_EN_FACT)+:TCAM_SLICE_EN_MIN]);
        
        wire                                            tcam_row_5_col_2_rd_en       = tcam_row_rd_en[5];
        wire                                            tcam_row_5_col_2_wr_en       = tcam_row_wr_en[5];
        wire                                            tcam_row_5_col_2_chk_en      = chk_en;
        wire                                            tcam_row_5_col_2_reset_n = reset_n;
        wire    [8-1:0]                       tcam_row_5_col_2_adr  = tcam_row_adr[5][8-1:0];
        wire    [32-1:0]                      tcam_row_5_col_2_data_in = tcam_row_wr_en[5] ? tcam_wr_data_col[2][32-1:0] : tcam_chk_key_col[2][32-1:0];
        wire    [32-1:0]                      tcam_row_5_col_2_data_in_pre = tcam_wr_data_col[2][32-1:0] ;
        wire    [32-1:0]                      tcam_row_5_col_2_key_in =  tcam_chk_key_col[2][32-1:0];
        wire    [32-1:0]                      tcam_row_5_col_2_mask_in = tcam_chk_mask_col[2][32-1:0];
        wire    [128-1:0]                       tcam_row_5_col_2_hit_in = tcam_raw_hit_in[5];
        wire    [128-1:0]                       tcam_row_5_col_2_tcam_match_in = tcam_row_match_in[5];
        wire    [32-1:0]                      tcam_row_5_col_2_data_out_inv;
        logic   [128-1:0]                     tcam_row_5_col_2_hit_out_int;
        logic   [128-1:0]                     tcam_row_5_col_2_hit_out_latch;
        logic   [128-1:0]                     tcam_row_5_col_2_hit_out;
        
        `ifdef HIF_PCIE_DFX_DISABLE
        wire                          tcam_row_5_col_2_biste = 'b0; 
        wire     [5-1:0]                  tcam_row_5_col_2_fca = 'b0;
        wire                            tcam_row_5_col_2_cre = 'b0;
        wire                            tcam_row_5_col_2_dftshiften = 'b0;
        wire                            tcam_row_5_col_2_dftmask = 'b0;
        wire                            tcam_row_5_col_2_dft_array_freeze = 'b0;
        wire                            tcam_row_5_col_2_dft_afd_reset_b = 'b0;
        wire      [32-1:0]    tcam_row_5_col_2_tdin = 'b0;
        wire      [8-1:0]     tcam_row_5_col_2_tadr = 'b0;
        wire                            tcam_row_5_col_2_tvbe = 'b0;
        wire                            tcam_row_5_col_2_tvbi = 'b0;
        wire      [32-1:0]    tcam_row_5_col_2_tmask = 'b0;
        wire                            tcam_row_5_col_2_twe = 'b0;
        wire                            tcam_row_5_col_2_tre = 'b0;
        wire                            tcam_row_5_col_2_tme = 'b0;
        wire                            tcam_row_5_col_2_bist_rotate_sr = 'b0;
        wire                            tcam_row_5_col_2_bist_en_sr_mask_b = 'b0;
        wire                            tcam_row_5_col_2_bist_all_mask = 'b0;
        wire                            tcam_row_5_col_2_bist_en_sr_data_b = 'b0;
        wire                            tcam_row_5_col_2_bist_cm_mode = 'b0;
        wire                            tcam_row_5_col_2_bist_cm_en_in = 'b0;
        wire                            tcam_row_5_col_2_bist_cm_match_sel0 = 'b0;
        wire                            tcam_row_5_col_2_bist_cm_match_sel1 = 'b0;
        wire                            tcam_row_5_col_2_bist_matchin_in = 'b0;
        wire                            tcam_row_5_col_2_bist_fls = 'b0;
        
        `else
        wire                            tcam_row_5_col_2_biste = 'b0;
        wire     [5-1:0]                  tcam_row_5_col_2_fca = 'b0;
        wire                            tcam_row_5_col_2_cre = 'b0;
        wire                            tcam_row_5_col_2_dftshiften = dftshiften;
        wire                            tcam_row_5_col_2_dftmask = dftmask;
        wire                            tcam_row_5_col_2_dft_array_freeze = dft_array_freeze;
        wire                            tcam_row_5_col_2_dft_afd_reset_b = dft_afd_reset_b;
        wire      [32-1:0]    tcam_row_5_col_2_tdin = 'b0;
        wire      [8-1:0]     tcam_row_5_col_2_tadr = 'b0;
        wire                            tcam_row_5_col_2_tvbe = 'b0;
        wire                            tcam_row_5_col_2_tvbi = 'b0;
        ...
        // Outputs
        logic                           tcam_row_5_col_2_wr_bira_fail;
        logic                           tcam_row_5_col_2_wr_so;
        logic                           tcam_row_5_col_2_curerrout;
        logic                           tcam_row_5_col_2_rscout;
        
        
        
        logic   [TCAM_SLICE_EN_FACT-1:0]                  tcam_row_5_col_2_slice_en_s0; 
        logic   [TCAM_SLICE_EN_FACT-1:0]                  tcam_row_5_col_2_slice_en_s1; 
        
        
        
             wire    [32-1:0]                     tcam_row_5_col_2_data_out ;
        
        
        
         generate
            if (BCAM_N7 == 0) 
              begin : gen_tcam_row_5_col_2_tcam
        ...
        // slice_en kept for TCAM only for backward compatability with Intel process
                ...
                begin
                  for (int i = 0; i < TCAM_SLICE_EN_FACT; i++)
                      if (tcam_row_5_col_2_slice_en_s1[i])
                           tcam_row_5_col_2_hit_out[ i*64 +: 64]  = tcam_row_5_col_2_hit_out_int  [ i*64 +: 64]; 
                      else
                           tcam_row_5_col_2_hit_out[ i*64 +: 64]  = tcam_row_5_col_2_hit_out_latch[ i*64 +: 64]; 
                end
        
        
        
                always_ff  @(posedge tcam_row_5_col_2_clk)
                begin
                  tcam_row_5_col_2_slice_en_s0 <= tcam_row_5_col_2_slice_en; 
                  tcam_row_5_col_2_slice_en_s1 <= tcam_row_5_col_2_slice_en_s0; 
        
                  for (int i = 0; i < TCAM_SLICE_EN_FACT; i++)
                    begin
                      if (tcam_row_5_col_2_slice_en_s1[i])
                         tcam_row_5_col_2_hit_out_latch[ i*64 +: 64] <=  tcam_row_5_col_2_hit_out_int[ i*64 +: 64];
                    end
                end
        ...
        //                             |_____________________________|
                         ...
                         assign tcam_row_5_col_2_data_out  = ~tcam_row_5_col_2_data_out_inv;
                         hif_lme_r12_ip783tcam128x32s0c1_5_2_wrap #(        
                                 )               
        
                                 ip783tcam128x32s0c1_wrap_tcam_row_5_col_2 (
                   //-------------------------------------------------------------
                   // Power Management Interfaces
                   //-------------------------------------------------------------   
                      `ifndef INTEL_NO_PWR_PINS
                      ...
                      `endif
                        .vddp             (1'b1),
                      `endif
                         .ADR               (tcam_row_5_col_2_adr[8-1:1]), //Input adr_intess
                         .ME                (ce),  
        
                         .CLK               (tcam_row_5_col_2_clk), //Clock
                         .CMP               (tcam_row_5_col_2_chk_en), //Control Enable
                         .DIN               (~tcam_row_5_col_2_data_in), 
                         .DSEL              (~tcam_row_5_col_2_adr[0]), //dncen enables access of the TCAM Arrays adr_intessed
                         .FLS               (flush), //flush initiates a Flush operation that resets all Valid
                         .RVLD              (tcam_row_5_col_2_hit_in ), //Row validation bus. Each match_in input controls 1
                         .MASK              (tcam_row_5_col_2_mask_in), //Input msk
                         .QOUT              (tcam_row_5_col_2_data_out_inv), //Output vben
                         .QHR               (tcam_row_5_col_2_hit_out_int),
                         .QVLD              (), 
                         .RE                (tcam_row_5_col_2_rd_en), //Read Enable
                         .RST               (tcam_row_5_col_2_reset_n), //Asynchronous Active-High Reset
                         .VBE               (1'b1), //vben enables the Valid Bit addressed by adr[a-1:0] to
                         .VBI               (1'b0),//  Valid Bit Input
                         .WE                (tcam_row_5_col_2_wr_en), //Write Enable
                         .FAF               (tcam_row_5_col_2_fca),
                         .FAF_EN            (tcam_row_5_col_2_cre),
                         .DFTSHIFTEN        (tcam_row_5_col_2_dftshiften),
                         .DFTMASK           (tcam_row_5_col_2_dftmask),
                         .DFT_ARRAY_FREEZE  (tcam_row_5_col_2_dft_array_freeze),
                         .DFT_AFD_RESET_B   (tcam_row_5_col_2_dft_afd_reset_b),
                         .BISTE           (tcam_row_5_col_2_biste),
                         .TDIN (tcam_row_5_col_2_tdin),
                         .TADR (tcam_row_5_col_2_tadr),
                         .TVBE (tcam_row_5_col_2_tvbe),
                         .TVBI (tcam_row_5_col_2_tvbi),
                         .TMASK (tcam_row_5_col_2_tmask),
                         .TWE (tcam_row_5_col_2_twe),
                         .TRE (tcam_row_5_col_2_tre),
                         .TME (tcam_row_5_col_2_tme),
                         .BIST_ROTATE_SR (tcam_row_5_col_2_bist_rotate_sr),
                         .BIST_EN_SR_MASK_B (tcam_row_5_col_2_bist_en_sr_mask_b),
                         .BIST_ALL_MASK (tcam_row_5_col_2_bist_all_mask),
                         .BIST_EN_SR_DATA_B (tcam_row_5_col_2_bist_en_sr_data_b),
                         .BIST_CM_MODE (tcam_row_5_col_2_bist_cm_mode),
                         .BIST_CM_EN_IN (tcam_row_5_col_2_bist_cm_en_in),
                         .BIST_CM_MATCH_SEL0 (tcam_row_5_col_2_bist_cm_match_sel0),
                         .BIST_CM_MATCH_SEL1 (tcam_row_5_col_2_bist_cm_match_sel1),
                         .BIST_MATCHIN_IN (tcam_row_5_col_2_bist_matchin_in),
                         .BIST_FLS (tcam_row_5_col_2_bist_fls),
                         .MDST_tcam (fuse_tcam)
                     );
              ...
              begin : gen_tcam_row_5_col_2_bcam
              end
        ...
        always_ff @(posedge clk) begin
                   tcam_rd_data_col[5][2]      <= tcam_row_5_col_2_data_out;
                   tcam_col_hit_out[5][2]       <= tcam_row_5_col_2_hit_out;
        end
        ...
        //                             |_____________________________|
                         ...
                         hif_lme_r12_ip783tcam128x32s0c1_6_0_wrap #(        
                                 )               
        ...
        wire    [8-1:0]                       tcam_row_6_col_1_adr  = tcam_row_adr[6][8-1:0];
        ...
        wire    [128-1:0]                       tcam_row_6_col_1_hit_in = tcam_raw_hit_in[6];
        wire    [128-1:0]                       tcam_row_6_col_1_tcam_match_in = tcam_row_match_in[6];
        wire    [32-1:0]                      tcam_row_6_col_1_data_out_inv;
        logic   [128-1:0]                     tcam_row_6_col_1_hit_out_int;
        logic   [128-1:0]                     tcam_row_6_col_1_hit_out_latch;
        logic   [128-1:0]                     tcam_row_6_col_1_hit_out;
        ...
        wire      [8-1:0]     tcam_row_6_col_1_tadr = 'b0;
        ...
        //                             |_____________________________|
                         ...
                         hif_lme_r12_ip783tcam128x32s0c1_6_1_wrap #(        
                                 )               
        
                                 ip783tcam128x32s0c1_wrap_tcam_row_6_col_1 (
                   //-------------------------------------------------------------
                   // Power Management Interfaces
                   //-------------------------------------------------------------   
                      `ifndef INTEL_NO_PWR_PINS
                      ...
                      `endif
                        .vddp             (1'b1),
                      `endif
                         .ADR               (tcam_row_6_col_1_adr[8-1:1]), //Input adr_intess
                         .ME                (ce),  
        ...
        wire                                            tcam_row_6_col_2_clk         = clk;
        
        wire    [TCAM_SLICE_EN_FACT-1:0]                tcam_row_6_col_2_slice_en = TCAM_SLICE_EN_FACT'(slice_en[(6 * TCAM_SLICE_EN_FACT)+:TCAM_SLICE_EN_MIN]);
        
        wire                                            tcam_row_6_col_2_rd_en       = tcam_row_rd_en[6];
        wire                                            tcam_row_6_col_2_wr_en       = tcam_row_wr_en[6];
        wire                                            tcam_row_6_col_2_chk_en      = chk_en;
        wire                                            tcam_row_6_col_2_reset_n = reset_n;
        wire    [8-1:0]                       tcam_row_6_col_2_adr  = tcam_row_adr[6][8-1:0];
        wire    [32-1:0]                      tcam_row_6_col_2_data_in = tcam_row_wr_en[6] ? tcam_wr_data_col[2][32-1:0] : tcam_chk_key_col[2][32-1:0];
        wire    [32-1:0]                      tcam_row_6_col_2_data_in_pre = tcam_wr_data_col[2][32-1:0] ;
        wire    [32-1:0]                      tcam_row_6_col_2_key_in =  tcam_chk_key_col[2][32-1:0];
        wire    [32-1:0]                      tcam_row_6_col_2_mask_in = tcam_chk_mask_col[2][32-1:0];
        wire    [128-1:0]                       tcam_row_6_col_2_hit_in = tcam_raw_hit_in[6];
        wire    [128-1:0]                       tcam_row_6_col_2_tcam_match_in = tcam_row_match_in[6];
        wire    [32-1:0]                      tcam_row_6_col_2_data_out_inv;
        logic   [128-1:0]                     tcam_row_6_col_2_hit_out_int;
        logic   [128-1:0]                     tcam_row_6_col_2_hit_out_latch;
        logic   [128-1:0]                     tcam_row_6_col_2_hit_out;
        
        `ifdef HIF_PCIE_DFX_DISABLE
        wire                          tcam_row_6_col_2_biste = 'b0; 
        wire     [5-1:0]                  tcam_row_6_col_2_fca = 'b0;
        wire                            tcam_row_6_col_2_cre = 'b0;
        wire                            tcam_row_6_col_2_dftshiften = 'b0;
        wire                            tcam_row_6_col_2_dftmask = 'b0;
        wire                            tcam_row_6_col_2_dft_array_freeze = 'b0;
        wire                            tcam_row_6_col_2_dft_afd_reset_b = 'b0;
        wire      [32-1:0]    tcam_row_6_col_2_tdin = 'b0;
        wire      [8-1:0]     tcam_row_6_col_2_tadr = 'b0;
        wire                            tcam_row_6_col_2_tvbe = 'b0;
        wire                            tcam_row_6_col_2_tvbi = 'b0;
        wire      [32-1:0]    tcam_row_6_col_2_tmask = 'b0;
        wire                            tcam_row_6_col_2_twe = 'b0;
        wire                            tcam_row_6_col_2_tre = 'b0;
        wire                            tcam_row_6_col_2_tme = 'b0;
        wire                            tcam_row_6_col_2_bist_rotate_sr = 'b0;
        wire                            tcam_row_6_col_2_bist_en_sr_mask_b = 'b0;
        wire                            tcam_row_6_col_2_bist_all_mask = 'b0;
        wire                            tcam_row_6_col_2_bist_en_sr_data_b = 'b0;
        wire                            tcam_row_6_col_2_bist_cm_mode = 'b0;
        wire                            tcam_row_6_col_2_bist_cm_en_in = 'b0;
        wire                            tcam_row_6_col_2_bist_cm_match_sel0 = 'b0;
        wire                            tcam_row_6_col_2_bist_cm_match_sel1 = 'b0;
        wire                            tcam_row_6_col_2_bist_matchin_in = 'b0;
        wire                            tcam_row_6_col_2_bist_fls = 'b0;
        
        `else
        wire                            tcam_row_6_col_2_biste = 'b0;
        wire     [5-1:0]                  tcam_row_6_col_2_fca = 'b0;
        wire                            tcam_row_6_col_2_cre = 'b0;
        wire                            tcam_row_6_col_2_dftshiften = dftshiften;
        wire                            tcam_row_6_col_2_dftmask = dftmask;
        wire                            tcam_row_6_col_2_dft_array_freeze = dft_array_freeze;
        wire                            tcam_row_6_col_2_dft_afd_reset_b = dft_afd_reset_b;
        wire      [32-1:0]    tcam_row_6_col_2_tdin = 'b0;
        wire      [8-1:0]     tcam_row_6_col_2_tadr = 'b0;
        wire                            tcam_row_6_col_2_tvbe = 'b0;
        wire                            tcam_row_6_col_2_tvbi = 'b0;
        ...
        // Outputs
        logic                           tcam_row_6_col_2_wr_bira_fail;
        logic                           tcam_row_6_col_2_wr_so;
        logic                           tcam_row_6_col_2_curerrout;
        logic                           tcam_row_6_col_2_rscout;
        
        
        
        logic   [TCAM_SLICE_EN_FACT-1:0]                  tcam_row_6_col_2_slice_en_s0; 
        logic   [TCAM_SLICE_EN_FACT-1:0]                  tcam_row_6_col_2_slice_en_s1; 
        
        
        
             wire    [32-1:0]                     tcam_row_6_col_2_data_out ;
        
        
        
         generate
            if (BCAM_N7 == 0) 
              begin : gen_tcam_row_6_col_2_tcam
        ...
        // slice_en kept for TCAM only for backward compatability with Intel process
                ...
                begin
                  for (int i = 0; i < TCAM_SLICE_EN_FACT; i++)
                      if (tcam_row_6_col_2_slice_en_s1[i])
                           tcam_row_6_col_2_hit_out[ i*64 +: 64]  = tcam_row_6_col_2_hit_out_int  [ i*64 +: 64]; 
                      else
                           tcam_row_6_col_2_hit_out[ i*64 +: 64]  = tcam_row_6_col_2_hit_out_latch[ i*64 +: 64]; 
                end
        
        
        
                always_ff  @(posedge tcam_row_6_col_2_clk)
                begin
                  tcam_row_6_col_2_slice_en_s0 <= tcam_row_6_col_2_slice_en; 
                  tcam_row_6_col_2_slice_en_s1 <= tcam_row_6_col_2_slice_en_s0; 
        
                  for (int i = 0; i < TCAM_SLICE_EN_FACT; i++)
                    begin
                      if (tcam_row_6_col_2_slice_en_s1[i])
                         tcam_row_6_col_2_hit_out_latch[ i*64 +: 64] <=  tcam_row_6_col_2_hit_out_int[ i*64 +: 64];
                    end
                end
        ...
        //                             |_____________________________|
                         ...
                         assign tcam_row_6_col_2_data_out  = ~tcam_row_6_col_2_data_out_inv;
                         hif_lme_r12_ip783tcam128x32s0c1_6_2_wrap #(        
                                 )               
        
                                 ip783tcam128x32s0c1_wrap_tcam_row_6_col_2 (
                   //-------------------------------------------------------------
                   // Power Management Interfaces
                   //-------------------------------------------------------------   
                      `ifndef INTEL_NO_PWR_PINS
                      ...
                      `endif
                        .vddp             (1'b1),
                      `endif
                         .ADR               (tcam_row_6_col_2_adr[8-1:1]), //Input adr_intess
                         .ME                (ce),  
        
                         .CLK               (tcam_row_6_col_2_clk), //Clock
                         .CMP               (tcam_row_6_col_2_chk_en), //Control Enable
                         .DIN               (~tcam_row_6_col_2_data_in), 
                         .DSEL              (~tcam_row_6_col_2_adr[0]), //dncen enables access of the TCAM Arrays adr_intessed
                         .FLS               (flush), //flush initiates a Flush operation that resets all Valid
                         .RVLD              (tcam_row_6_col_2_hit_in ), //Row validation bus. Each match_in input controls 1
                         .MASK              (tcam_row_6_col_2_mask_in), //Input msk
                         .QOUT              (tcam_row_6_col_2_data_out_inv), //Output vben
                         .QHR               (tcam_row_6_col_2_hit_out_int),
                         .QVLD              (), 
                         .RE                (tcam_row_6_col_2_rd_en), //Read Enable
                         .RST               (tcam_row_6_col_2_reset_n), //Asynchronous Active-High Reset
                         .VBE               (1'b1), //vben enables the Valid Bit addressed by adr[a-1:0] to
                         .VBI               (1'b0),//  Valid Bit Input
                         .WE                (tcam_row_6_col_2_wr_en), //Write Enable
                         .FAF               (tcam_row_6_col_2_fca),
                         .FAF_EN            (tcam_row_6_col_2_cre),
                         .DFTSHIFTEN        (tcam_row_6_col_2_dftshiften),
                         .DFTMASK           (tcam_row_6_col_2_dftmask),
                         .DFT_ARRAY_FREEZE  (tcam_row_6_col_2_dft_array_freeze),
                         .DFT_AFD_RESET_B   (tcam_row_6_col_2_dft_afd_reset_b),
                         .BISTE           (tcam_row_6_col_2_biste),
                         .TDIN (tcam_row_6_col_2_tdin),
                         .TADR (tcam_row_6_col_2_tadr),
                         .TVBE (tcam_row_6_col_2_tvbe),
                         .TVBI (tcam_row_6_col_2_tvbi),
                         .TMASK (tcam_row_6_col_2_tmask),
                         .TWE (tcam_row_6_col_2_twe),
                         .TRE (tcam_row_6_col_2_tre),
                         .TME (tcam_row_6_col_2_tme),
                         .BIST_ROTATE_SR (tcam_row_6_col_2_bist_rotate_sr),
                         .BIST_EN_SR_MASK_B (tcam_row_6_col_2_bist_en_sr_mask_b),
                         .BIST_ALL_MASK (tcam_row_6_col_2_bist_all_mask),
                         .BIST_EN_SR_DATA_B (tcam_row_6_col_2_bist_en_sr_data_b),
                         .BIST_CM_MODE (tcam_row_6_col_2_bist_cm_mode),
                         .BIST_CM_EN_IN (tcam_row_6_col_2_bist_cm_en_in),
                         .BIST_CM_MATCH_SEL0 (tcam_row_6_col_2_bist_cm_match_sel0),
                         .BIST_CM_MATCH_SEL1 (tcam_row_6_col_2_bist_cm_match_sel1),
                         .BIST_MATCHIN_IN (tcam_row_6_col_2_bist_matchin_in),
                         .BIST_FLS (tcam_row_6_col_2_bist_fls),
                         .MDST_tcam (fuse_tcam)
                     );
              ...
              begin : gen_tcam_row_6_col_2_bcam
              end
        ...
        always_ff @(posedge clk) begin
                   tcam_rd_data_col[6][2]      <= tcam_row_6_col_2_data_out;
                   tcam_col_hit_out[6][2]       <= tcam_row_6_col_2_hit_out;
        end
        ...
        //                             |_____________________________|
                         ...
                         hif_lme_r12_ip783tcam128x32s0c1_7_0_wrap #(        
                                 )               
        ...
        wire    [8-1:0]                       tcam_row_7_col_1_adr  = tcam_row_adr[7][8-1:0];
        ...
        wire    [128-1:0]                       tcam_row_7_col_1_hit_in = tcam_raw_hit_in[7];
        wire    [128-1:0]                       tcam_row_7_col_1_tcam_match_in = tcam_row_match_in[7];
        wire    [32-1:0]                      tcam_row_7_col_1_data_out_inv;
        logic   [128-1:0]                     tcam_row_7_col_1_hit_out_int;
        logic   [128-1:0]                     tcam_row_7_col_1_hit_out_latch;
        logic   [128-1:0]                     tcam_row_7_col_1_hit_out;
        ...
        wire      [8-1:0]     tcam_row_7_col_1_tadr = 'b0;
        ...
        //                             |_____________________________|
                         ...
                         hif_lme_r12_ip783tcam128x32s0c1_7_1_wrap #(        
                                 )               
        
                                 ip783tcam128x32s0c1_wrap_tcam_row_7_col_1 (
                   //-------------------------------------------------------------
                   // Power Management Interfaces
                   //-------------------------------------------------------------   
                      `ifndef INTEL_NO_PWR_PINS
                      ...
                      `endif
                        .vddp             (1'b1),
                      `endif
                         .ADR               (tcam_row_7_col_1_adr[8-1:1]), //Input adr_intess
                         .ME                (ce),  
        ...
        wire                                            tcam_row_7_col_2_clk         = clk;
        
        wire    [TCAM_SLICE_EN_FACT-1:0]                tcam_row_7_col_2_slice_en = TCAM_SLICE_EN_FACT'(slice_en[(7 * TCAM_SLICE_EN_FACT)+:TCAM_SLICE_EN_MIN]);
        
        wire                                            tcam_row_7_col_2_rd_en       = tcam_row_rd_en[7];
        wire                                            tcam_row_7_col_2_wr_en       = tcam_row_wr_en[7];
        wire                                            tcam_row_7_col_2_chk_en      = chk_en;
        wire                                            tcam_row_7_col_2_reset_n = reset_n;
        wire    [8-1:0]                       tcam_row_7_col_2_adr  = tcam_row_adr[7][8-1:0];
        wire    [32-1:0]                      tcam_row_7_col_2_data_in = tcam_row_wr_en[7] ? tcam_wr_data_col[2][32-1:0] : tcam_chk_key_col[2][32-1:0];
        wire    [32-1:0]                      tcam_row_7_col_2_data_in_pre = tcam_wr_data_col[2][32-1:0] ;
        wire    [32-1:0]                      tcam_row_7_col_2_key_in =  tcam_chk_key_col[2][32-1:0];
        wire    [32-1:0]                      tcam_row_7_col_2_mask_in = tcam_chk_mask_col[2][32-1:0];
        wire    [128-1:0]                       tcam_row_7_col_2_hit_in = tcam_raw_hit_in[7];
        wire    [128-1:0]                       tcam_row_7_col_2_tcam_match_in = tcam_row_match_in[7];
        wire    [32-1:0]                      tcam_row_7_col_2_data_out_inv;
        logic   [128-1:0]                     tcam_row_7_col_2_hit_out_int;
        logic   [128-1:0]                     tcam_row_7_col_2_hit_out_latch;
        logic   [128-1:0]                     tcam_row_7_col_2_hit_out;
        
        `ifdef HIF_PCIE_DFX_DISABLE
        wire                          tcam_row_7_col_2_biste = 'b0; 
        wire     [5-1:0]                  tcam_row_7_col_2_fca = 'b0;
        wire                            tcam_row_7_col_2_cre = 'b0;
        wire                            tcam_row_7_col_2_dftshiften = 'b0;
        wire                            tcam_row_7_col_2_dftmask = 'b0;
        wire                            tcam_row_7_col_2_dft_array_freeze = 'b0;
        wire                            tcam_row_7_col_2_dft_afd_reset_b = 'b0;
        wire      [32-1:0]    tcam_row_7_col_2_tdin = 'b0;
        wire      [8-1:0]     tcam_row_7_col_2_tadr = 'b0;
        wire                            tcam_row_7_col_2_tvbe = 'b0;
        wire                            tcam_row_7_col_2_tvbi = 'b0;
        wire      [32-1:0]    tcam_row_7_col_2_tmask = 'b0;
        wire                            tcam_row_7_col_2_twe = 'b0;
        wire                            tcam_row_7_col_2_tre = 'b0;
        wire                            tcam_row_7_col_2_tme = 'b0;
        wire                            tcam_row_7_col_2_bist_rotate_sr = 'b0;
        wire                            tcam_row_7_col_2_bist_en_sr_mask_b = 'b0;
        wire                            tcam_row_7_col_2_bist_all_mask = 'b0;
        wire                            tcam_row_7_col_2_bist_en_sr_data_b = 'b0;
        wire                            tcam_row_7_col_2_bist_cm_mode = 'b0;
        wire                            tcam_row_7_col_2_bist_cm_en_in = 'b0;
        wire                            tcam_row_7_col_2_bist_cm_match_sel0 = 'b0;
        wire                            tcam_row_7_col_2_bist_cm_match_sel1 = 'b0;
        wire                            tcam_row_7_col_2_bist_matchin_in = 'b0;
        wire                            tcam_row_7_col_2_bist_fls = 'b0;
        
        `else
        wire                            tcam_row_7_col_2_biste = 'b0;
        wire     [5-1:0]                  tcam_row_7_col_2_fca = 'b0;
        wire                            tcam_row_7_col_2_cre = 'b0;
        wire                            tcam_row_7_col_2_dftshiften = dftshiften;
        wire                            tcam_row_7_col_2_dftmask = dftmask;
        wire                            tcam_row_7_col_2_dft_array_freeze = dft_array_freeze;
        wire                            tcam_row_7_col_2_dft_afd_reset_b = dft_afd_reset_b;
        wire      [32-1:0]    tcam_row_7_col_2_tdin = 'b0;
        wire      [8-1:0]     tcam_row_7_col_2_tadr = 'b0;
        wire                            tcam_row_7_col_2_tvbe = 'b0;
        wire                            tcam_row_7_col_2_tvbi = 'b0;
        ...
        // Outputs
        logic                           tcam_row_7_col_2_wr_bira_fail;
        logic                           tcam_row_7_col_2_wr_so;
        logic                           tcam_row_7_col_2_curerrout;
        logic                           tcam_row_7_col_2_rscout;
        
        
        
        logic   [TCAM_SLICE_EN_FACT-1:0]                  tcam_row_7_col_2_slice_en_s0; 
        logic   [TCAM_SLICE_EN_FACT-1:0]                  tcam_row_7_col_2_slice_en_s1; 
        
        
        
             wire    [32-1:0]                     tcam_row_7_col_2_data_out ;
        
        
        
         generate
            if (BCAM_N7 == 0) 
              begin : gen_tcam_row_7_col_2_tcam
        ...
        // slice_en kept for TCAM only for backward compatability with Intel process
                ...
                begin
                  for (int i = 0; i < TCAM_SLICE_EN_FACT; i++)
                      if (tcam_row_7_col_2_slice_en_s1[i])
                           tcam_row_7_col_2_hit_out[ i*64 +: 64]  = tcam_row_7_col_2_hit_out_int  [ i*64 +: 64]; 
                      else
                           tcam_row_7_col_2_hit_out[ i*64 +: 64]  = tcam_row_7_col_2_hit_out_latch[ i*64 +: 64]; 
                end
        
        
        
                always_ff  @(posedge tcam_row_7_col_2_clk)
                begin
                  tcam_row_7_col_2_slice_en_s0 <= tcam_row_7_col_2_slice_en; 
                  tcam_row_7_col_2_slice_en_s1 <= tcam_row_7_col_2_slice_en_s0; 
        
                  for (int i = 0; i < TCAM_SLICE_EN_FACT; i++)
                    begin
                      if (tcam_row_7_col_2_slice_en_s1[i])
                         tcam_row_7_col_2_hit_out_latch[ i*64 +: 64] <=  tcam_row_7_col_2_hit_out_int[ i*64 +: 64];
                    end
                end
        ...
        //                             |_____________________________|
                         ...
                         assign tcam_row_7_col_2_data_out  = ~tcam_row_7_col_2_data_out_inv;
                         hif_lme_r12_ip783tcam128x32s0c1_7_2_wrap #(        
                                 )               
        
                                 ip783tcam128x32s0c1_wrap_tcam_row_7_col_2 (
                   //-------------------------------------------------------------
                   // Power Management Interfaces
                   //-------------------------------------------------------------   
                      `ifndef INTEL_NO_PWR_PINS
                      ...
                      `endif
                        .vddp             (1'b1),
                      `endif
                         .ADR               (tcam_row_7_col_2_adr[8-1:1]), //Input adr_intess
                         .ME                (ce),  
        
                         .CLK               (tcam_row_7_col_2_clk), //Clock
                         .CMP               (tcam_row_7_col_2_chk_en), //Control Enable
                         .DIN               (~tcam_row_7_col_2_data_in), 
                         .DSEL              (~tcam_row_7_col_2_adr[0]), //dncen enables access of the TCAM Arrays adr_intessed
                         .FLS               (flush), //flush initiates a Flush operation that resets all Valid
                         .RVLD              (tcam_row_7_col_2_hit_in ), //Row validation bus. Each match_in input controls 1
                         .MASK              (tcam_row_7_col_2_mask_in), //Input msk
                         .QOUT              (tcam_row_7_col_2_data_out_inv), //Output vben
                         .QHR               (tcam_row_7_col_2_hit_out_int),
                         .QVLD              (), 
                         .RE                (tcam_row_7_col_2_rd_en), //Read Enable
                         .RST               (tcam_row_7_col_2_reset_n), //Asynchronous Active-High Reset
                         .VBE               (1'b1), //vben enables the Valid Bit addressed by adr[a-1:0] to
                         .VBI               (1'b0),//  Valid Bit Input
                         .WE                (tcam_row_7_col_2_wr_en), //Write Enable
                         .FAF               (tcam_row_7_col_2_fca),
                         .FAF_EN            (tcam_row_7_col_2_cre),
                         .DFTSHIFTEN        (tcam_row_7_col_2_dftshiften),
                         .DFTMASK           (tcam_row_7_col_2_dftmask),
                         .DFT_ARRAY_FREEZE  (tcam_row_7_col_2_dft_array_freeze),
                         .DFT_AFD_RESET_B   (tcam_row_7_col_2_dft_afd_reset_b),
                         .BISTE           (tcam_row_7_col_2_biste),
                         .TDIN (tcam_row_7_col_2_tdin),
                         .TADR (tcam_row_7_col_2_tadr),
                         .TVBE (tcam_row_7_col_2_tvbe),
                         .TVBI (tcam_row_7_col_2_tvbi),
                         .TMASK (tcam_row_7_col_2_tmask),
                         .TWE (tcam_row_7_col_2_twe),
                         .TRE (tcam_row_7_col_2_tre),
                         .TME (tcam_row_7_col_2_tme),
                         .BIST_ROTATE_SR (tcam_row_7_col_2_bist_rotate_sr),
                         .BIST_EN_SR_MASK_B (tcam_row_7_col_2_bist_en_sr_mask_b),
                         .BIST_ALL_MASK (tcam_row_7_col_2_bist_all_mask),
                         .BIST_EN_SR_DATA_B (tcam_row_7_col_2_bist_en_sr_data_b),
                         .BIST_CM_MODE (tcam_row_7_col_2_bist_cm_mode),
                         .BIST_CM_EN_IN (tcam_row_7_col_2_bist_cm_en_in),
                         .BIST_CM_MATCH_SEL0 (tcam_row_7_col_2_bist_cm_match_sel0),
                         .BIST_CM_MATCH_SEL1 (tcam_row_7_col_2_bist_cm_match_sel1),
                         .BIST_MATCHIN_IN (tcam_row_7_col_2_bist_matchin_in),
                         .BIST_FLS (tcam_row_7_col_2_bist_fls),
                         .MDST_tcam (fuse_tcam)
                     );
              ...
              begin : gen_tcam_row_7_col_2_bcam
              end
        ...
        always_ff @(posedge clk) begin
                   tcam_rd_data_col[7][2]      <= tcam_row_7_col_2_data_out;
                   tcam_col_hit_out[7][2]       <= tcam_row_7_col_2_hit_out;
        end
        ...
        //                             |_____________________________|
                         ...
                         hif_lme_r12_ip783tcam128x32s0c1_8_0_wrap #(        
                                 )               
        ...
        wire    [8-1:0]                       tcam_row_8_col_1_adr  = tcam_row_adr[8][8-1:0];
        ...
        wire    [128-1:0]                       tcam_row_8_col_1_hit_in = tcam_raw_hit_in[8];
        wire    [128-1:0]                       tcam_row_8_col_1_tcam_match_in = tcam_row_match_in[8];
        wire    [32-1:0]                      tcam_row_8_col_1_data_out_inv;
        logic   [128-1:0]                     tcam_row_8_col_1_hit_out_int;
        logic   [128-1:0]                     tcam_row_8_col_1_hit_out_latch;
        logic   [128-1:0]                     tcam_row_8_col_1_hit_out;
        ...
        wire      [8-1:0]     tcam_row_8_col_1_tadr = 'b0;
        ...
        //                             |_____________________________|
                         ...
                         hif_lme_r12_ip783tcam128x32s0c1_8_1_wrap #(        
                                 )               
        
                                 ip783tcam128x32s0c1_wrap_tcam_row_8_col_1 (
                   //-------------------------------------------------------------
                   // Power Management Interfaces
                   //-------------------------------------------------------------   
                      `ifndef INTEL_NO_PWR_PINS
                      ...
                      `endif
                        .vddp             (1'b1),
                      `endif
                         .ADR               (tcam_row_8_col_1_adr[8-1:1]), //Input adr_intess
                         .ME                (ce),  
        ...
        wire                                            tcam_row_8_col_2_clk         = clk;
        
        wire    [TCAM_SLICE_EN_FACT-1:0]                tcam_row_8_col_2_slice_en = TCAM_SLICE_EN_FACT'(slice_en[(8 * TCAM_SLICE_EN_FACT)+:TCAM_SLICE_EN_MIN]);
        
        wire                                            tcam_row_8_col_2_rd_en       = tcam_row_rd_en[8];
        wire                                            tcam_row_8_col_2_wr_en       = tcam_row_wr_en[8];
        wire                                            tcam_row_8_col_2_chk_en      = chk_en;
        wire                                            tcam_row_8_col_2_reset_n = reset_n;
        wire    [8-1:0]                       tcam_row_8_col_2_adr  = tcam_row_adr[8][8-1:0];
        wire    [32-1:0]                      tcam_row_8_col_2_data_in = tcam_row_wr_en[8] ? tcam_wr_data_col[2][32-1:0] : tcam_chk_key_col[2][32-1:0];
        wire    [32-1:0]                      tcam_row_8_col_2_data_in_pre = tcam_wr_data_col[2][32-1:0] ;
        wire    [32-1:0]                      tcam_row_8_col_2_key_in =  tcam_chk_key_col[2][32-1:0];
        wire    [32-1:0]                      tcam_row_8_col_2_mask_in = tcam_chk_mask_col[2][32-1:0];
        wire    [128-1:0]                       tcam_row_8_col_2_hit_in = tcam_raw_hit_in[8];
        wire    [128-1:0]                       tcam_row_8_col_2_tcam_match_in = tcam_row_match_in[8];
        wire    [32-1:0]                      tcam_row_8_col_2_data_out_inv;
        logic   [128-1:0]                     tcam_row_8_col_2_hit_out_int;
        logic   [128-1:0]                     tcam_row_8_col_2_hit_out_latch;
        logic   [128-1:0]                     tcam_row_8_col_2_hit_out;
        
        `ifdef HIF_PCIE_DFX_DISABLE
        wire                          tcam_row_8_col_2_biste = 'b0; 
        wire     [5-1:0]                  tcam_row_8_col_2_fca = 'b0;
        wire                            tcam_row_8_col_2_cre = 'b0;
        wire                            tcam_row_8_col_2_dftshiften = 'b0;
        wire                            tcam_row_8_col_2_dftmask = 'b0;
        wire                            tcam_row_8_col_2_dft_array_freeze = 'b0;
        wire                            tcam_row_8_col_2_dft_afd_reset_b = 'b0;
        wire      [32-1:0]    tcam_row_8_col_2_tdin = 'b0;
        wire      [8-1:0]     tcam_row_8_col_2_tadr = 'b0;
        wire                            tcam_row_8_col_2_tvbe = 'b0;
        wire                            tcam_row_8_col_2_tvbi = 'b0;
        wire      [32-1:0]    tcam_row_8_col_2_tmask = 'b0;
        wire                            tcam_row_8_col_2_twe = 'b0;
        wire                            tcam_row_8_col_2_tre = 'b0;
        wire                            tcam_row_8_col_2_tme = 'b0;
        wire                            tcam_row_8_col_2_bist_rotate_sr = 'b0;
        wire                            tcam_row_8_col_2_bist_en_sr_mask_b = 'b0;
        wire                            tcam_row_8_col_2_bist_all_mask = 'b0;
        wire                            tcam_row_8_col_2_bist_en_sr_data_b = 'b0;
        wire                            tcam_row_8_col_2_bist_cm_mode = 'b0;
        wire                            tcam_row_8_col_2_bist_cm_en_in = 'b0;
        wire                            tcam_row_8_col_2_bist_cm_match_sel0 = 'b0;
        wire                            tcam_row_8_col_2_bist_cm_match_sel1 = 'b0;
        wire                            tcam_row_8_col_2_bist_matchin_in = 'b0;
        wire                            tcam_row_8_col_2_bist_fls = 'b0;
        
        `else
        wire                            tcam_row_8_col_2_biste = 'b0;
        wire     [5-1:0]                  tcam_row_8_col_2_fca = 'b0;
        wire                            tcam_row_8_col_2_cre = 'b0;
        wire                            tcam_row_8_col_2_dftshiften = dftshiften;
        wire                            tcam_row_8_col_2_dftmask = dftmask;
        wire                            tcam_row_8_col_2_dft_array_freeze = dft_array_freeze;
        wire                            tcam_row_8_col_2_dft_afd_reset_b = dft_afd_reset_b;
        wire      [32-1:0]    tcam_row_8_col_2_tdin = 'b0;
        wire      [8-1:0]     tcam_row_8_col_2_tadr = 'b0;
        wire                            tcam_row_8_col_2_tvbe = 'b0;
        wire                            tcam_row_8_col_2_tvbi = 'b0;
        ...
        // Outputs
        logic                           tcam_row_8_col_2_wr_bira_fail;
        logic                           tcam_row_8_col_2_wr_so;
        logic                           tcam_row_8_col_2_curerrout;
        logic                           tcam_row_8_col_2_rscout;
        
        
        
        logic   [TCAM_SLICE_EN_FACT-1:0]                  tcam_row_8_col_2_slice_en_s0; 
        logic   [TCAM_SLICE_EN_FACT-1:0]                  tcam_row_8_col_2_slice_en_s1; 
        
        
        
             wire    [32-1:0]                     tcam_row_8_col_2_data_out ;
        
        
        
         generate
            if (BCAM_N7 == 0) 
              begin : gen_tcam_row_8_col_2_tcam
        ...
        // slice_en kept for TCAM only for backward compatability with Intel process
                ...
                begin
                  for (int i = 0; i < TCAM_SLICE_EN_FACT; i++)
                      if (tcam_row_8_col_2_slice_en_s1[i])
                           tcam_row_8_col_2_hit_out[ i*64 +: 64]  = tcam_row_8_col_2_hit_out_int  [ i*64 +: 64]; 
                      else
                           tcam_row_8_col_2_hit_out[ i*64 +: 64]  = tcam_row_8_col_2_hit_out_latch[ i*64 +: 64]; 
                end
        
        
        
                always_ff  @(posedge tcam_row_8_col_2_clk)
                begin
                  tcam_row_8_col_2_slice_en_s0 <= tcam_row_8_col_2_slice_en; 
                  tcam_row_8_col_2_slice_en_s1 <= tcam_row_8_col_2_slice_en_s0; 
        
                  for (int i = 0; i < TCAM_SLICE_EN_FACT; i++)
                    begin
                      if (tcam_row_8_col_2_slice_en_s1[i])
                         tcam_row_8_col_2_hit_out_latch[ i*64 +: 64] <=  tcam_row_8_col_2_hit_out_int[ i*64 +: 64];
                    end
                end
        ...
        //                             |_____________________________|
                         ...
                         assign tcam_row_8_col_2_data_out  = ~tcam_row_8_col_2_data_out_inv;
                         hif_lme_r12_ip783tcam128x32s0c1_8_2_wrap #(        
                                 )               
        
                                 ip783tcam128x32s0c1_wrap_tcam_row_8_col_2 (
                   //-------------------------------------------------------------
                   // Power Management Interfaces
                   //-------------------------------------------------------------   
                      `ifndef INTEL_NO_PWR_PINS
                      ...
                      `endif
                        .vddp             (1'b1),
                      `endif
                         .ADR               (tcam_row_8_col_2_adr[8-1:1]), //Input adr_intess
                         .ME                (ce),  
        
                         .CLK               (tcam_row_8_col_2_clk), //Clock
                         .CMP               (tcam_row_8_col_2_chk_en), //Control Enable
                         .DIN               (~tcam_row_8_col_2_data_in), 
                         .DSEL              (~tcam_row_8_col_2_adr[0]), //dncen enables access of the TCAM Arrays adr_intessed
                         .FLS               (flush), //flush initiates a Flush operation that resets all Valid
                         .RVLD              (tcam_row_8_col_2_hit_in ), //Row validation bus. Each match_in input controls 1
                         .MASK              (tcam_row_8_col_2_mask_in), //Input msk
                         .QOUT              (tcam_row_8_col_2_data_out_inv), //Output vben
                         .QHR               (tcam_row_8_col_2_hit_out_int),
                         .QVLD              (), 
                         .RE                (tcam_row_8_col_2_rd_en), //Read Enable
                         .RST               (tcam_row_8_col_2_reset_n), //Asynchronous Active-High Reset
                         .VBE               (1'b1), //vben enables the Valid Bit addressed by adr[a-1:0] to
                         .VBI               (1'b0),//  Valid Bit Input
                         .WE                (tcam_row_8_col_2_wr_en), //Write Enable
                         .FAF               (tcam_row_8_col_2_fca),
                         .FAF_EN            (tcam_row_8_col_2_cre),
                         .DFTSHIFTEN        (tcam_row_8_col_2_dftshiften),
                         .DFTMASK           (tcam_row_8_col_2_dftmask),
                         .DFT_ARRAY_FREEZE  (tcam_row_8_col_2_dft_array_freeze),
                         .DFT_AFD_RESET_B   (tcam_row_8_col_2_dft_afd_reset_b),
                         .BISTE           (tcam_row_8_col_2_biste),
                         .TDIN (tcam_row_8_col_2_tdin),
                         .TADR (tcam_row_8_col_2_tadr),
                         .TVBE (tcam_row_8_col_2_tvbe),
                         .TVBI (tcam_row_8_col_2_tvbi),
                         .TMASK (tcam_row_8_col_2_tmask),
                         .TWE (tcam_row_8_col_2_twe),
                         .TRE (tcam_row_8_col_2_tre),
                         .TME (tcam_row_8_col_2_tme),
                         .BIST_ROTATE_SR (tcam_row_8_col_2_bist_rotate_sr),
                         .BIST_EN_SR_MASK_B (tcam_row_8_col_2_bist_en_sr_mask_b),
                         .BIST_ALL_MASK (tcam_row_8_col_2_bist_all_mask),
                         .BIST_EN_SR_DATA_B (tcam_row_8_col_2_bist_en_sr_data_b),
                         .BIST_CM_MODE (tcam_row_8_col_2_bist_cm_mode),
                         .BIST_CM_EN_IN (tcam_row_8_col_2_bist_cm_en_in),
                         .BIST_CM_MATCH_SEL0 (tcam_row_8_col_2_bist_cm_match_sel0),
                         .BIST_CM_MATCH_SEL1 (tcam_row_8_col_2_bist_cm_match_sel1),
                         .BIST_MATCHIN_IN (tcam_row_8_col_2_bist_matchin_in),
                         .BIST_FLS (tcam_row_8_col_2_bist_fls),
                         .MDST_tcam (fuse_tcam)
                     );
              ...
              begin : gen_tcam_row_8_col_2_bcam
              end
        ...
        always_ff @(posedge clk) begin
                   tcam_rd_data_col[8][2]      <= tcam_row_8_col_2_data_out;
                   tcam_col_hit_out[8][2]       <= tcam_row_8_col_2_hit_out;
        end
        ...
        //                             |_____________________________|
                         ...
                         hif_lme_r12_ip783tcam128x32s0c1_9_0_wrap #(        
                                 )               
        ...
        wire    [8-1:0]                       tcam_row_9_col_1_adr  = tcam_row_adr[9][8-1:0];
        ...
        wire    [128-1:0]                       tcam_row_9_col_1_hit_in = tcam_raw_hit_in[9];
        wire    [128-1:0]                       tcam_row_9_col_1_tcam_match_in = tcam_row_match_in[9];
        wire    [32-1:0]                      tcam_row_9_col_1_data_out_inv;
        logic   [128-1:0]                     tcam_row_9_col_1_hit_out_int;
        logic   [128-1:0]                     tcam_row_9_col_1_hit_out_latch;
        logic   [128-1:0]                     tcam_row_9_col_1_hit_out;
        ...
        wire      [8-1:0]     tcam_row_9_col_1_tadr = 'b0;
        ...
        //                             |_____________________________|
                         ...
                         hif_lme_r12_ip783tcam128x32s0c1_9_1_wrap #(        
                                 )               
        
                                 ip783tcam128x32s0c1_wrap_tcam_row_9_col_1 (
                   //-------------------------------------------------------------
                   // Power Management Interfaces
                   //-------------------------------------------------------------   
                      `ifndef INTEL_NO_PWR_PINS
                      ...
                      `endif
                        .vddp             (1'b1),
                      `endif
                         .ADR               (tcam_row_9_col_1_adr[8-1:1]), //Input adr_intess
                         .ME                (ce),  
        ...
        wire                                            tcam_row_9_col_2_clk         = clk;
        
        wire    [TCAM_SLICE_EN_FACT-1:0]                tcam_row_9_col_2_slice_en = TCAM_SLICE_EN_FACT'(slice_en[(9 * TCAM_SLICE_EN_FACT)+:TCAM_SLICE_EN_MIN]);
        
        wire                                            tcam_row_9_col_2_rd_en       = tcam_row_rd_en[9];
        wire                                            tcam_row_9_col_2_wr_en       = tcam_row_wr_en[9];
        wire                                            tcam_row_9_col_2_chk_en      = chk_en;
        wire                                            tcam_row_9_col_2_reset_n = reset_n;
        wire    [8-1:0]                       tcam_row_9_col_2_adr  = tcam_row_adr[9][8-1:0];
        wire    [32-1:0]                      tcam_row_9_col_2_data_in = tcam_row_wr_en[9] ? tcam_wr_data_col[2][32-1:0] : tcam_chk_key_col[2][32-1:0];
        wire    [32-1:0]                      tcam_row_9_col_2_data_in_pre = tcam_wr_data_col[2][32-1:0] ;
        wire    [32-1:0]                      tcam_row_9_col_2_key_in =  tcam_chk_key_col[2][32-1:0];
        wire    [32-1:0]                      tcam_row_9_col_2_mask_in = tcam_chk_mask_col[2][32-1:0];
        wire    [128-1:0]                       tcam_row_9_col_2_hit_in = tcam_raw_hit_in[9];
        wire    [128-1:0]                       tcam_row_9_col_2_tcam_match_in = tcam_row_match_in[9];
        wire    [32-1:0]                      tcam_row_9_col_2_data_out_inv;
        logic   [128-1:0]                     tcam_row_9_col_2_hit_out_int;
        logic   [128-1:0]                     tcam_row_9_col_2_hit_out_latch;
        logic   [128-1:0]                     tcam_row_9_col_2_hit_out;
        
        `ifdef HIF_PCIE_DFX_DISABLE
        wire                          tcam_row_9_col_2_biste = 'b0; 
        wire     [5-1:0]                  tcam_row_9_col_2_fca = 'b0;
        wire                            tcam_row_9_col_2_cre = 'b0;
        wire                            tcam_row_9_col_2_dftshiften = 'b0;
        wire                            tcam_row_9_col_2_dftmask = 'b0;
        wire                            tcam_row_9_col_2_dft_array_freeze = 'b0;
        wire                            tcam_row_9_col_2_dft_afd_reset_b = 'b0;
        wire      [32-1:0]    tcam_row_9_col_2_tdin = 'b0;
        wire      [8-1:0]     tcam_row_9_col_2_tadr = 'b0;
        wire                            tcam_row_9_col_2_tvbe = 'b0;
        wire                            tcam_row_9_col_2_tvbi = 'b0;
        wire      [32-1:0]    tcam_row_9_col_2_tmask = 'b0;
        wire                            tcam_row_9_col_2_twe = 'b0;
        wire                            tcam_row_9_col_2_tre = 'b0;
        wire                            tcam_row_9_col_2_tme = 'b0;
        wire                            tcam_row_9_col_2_bist_rotate_sr = 'b0;
        wire                            tcam_row_9_col_2_bist_en_sr_mask_b = 'b0;
        wire                            tcam_row_9_col_2_bist_all_mask = 'b0;
        wire                            tcam_row_9_col_2_bist_en_sr_data_b = 'b0;
        wire                            tcam_row_9_col_2_bist_cm_mode = 'b0;
        wire                            tcam_row_9_col_2_bist_cm_en_in = 'b0;
        wire                            tcam_row_9_col_2_bist_cm_match_sel0 = 'b0;
        wire                            tcam_row_9_col_2_bist_cm_match_sel1 = 'b0;
        wire                            tcam_row_9_col_2_bist_matchin_in = 'b0;
        wire                            tcam_row_9_col_2_bist_fls = 'b0;
        
        `else
        wire                            tcam_row_9_col_2_biste = 'b0;
        wire     [5-1:0]                  tcam_row_9_col_2_fca = 'b0;
        wire                            tcam_row_9_col_2_cre = 'b0;
        wire                            tcam_row_9_col_2_dftshiften = dftshiften;
        wire                            tcam_row_9_col_2_dftmask = dftmask;
        wire                            tcam_row_9_col_2_dft_array_freeze = dft_array_freeze;
        wire                            tcam_row_9_col_2_dft_afd_reset_b = dft_afd_reset_b;
        wire      [32-1:0]    tcam_row_9_col_2_tdin = 'b0;
        wire      [8-1:0]     tcam_row_9_col_2_tadr = 'b0;
        wire                            tcam_row_9_col_2_tvbe = 'b0;
        wire                            tcam_row_9_col_2_tvbi = 'b0;
        ...
        // Outputs
        logic                           tcam_row_9_col_2_wr_bira_fail;
        logic                           tcam_row_9_col_2_wr_so;
        logic                           tcam_row_9_col_2_curerrout;
        logic                           tcam_row_9_col_2_rscout;
        
        
        
        logic   [TCAM_SLICE_EN_FACT-1:0]                  tcam_row_9_col_2_slice_en_s0; 
        logic   [TCAM_SLICE_EN_FACT-1:0]                  tcam_row_9_col_2_slice_en_s1; 
        
        
        
             wire    [32-1:0]                     tcam_row_9_col_2_data_out ;
        
        
        
         generate
            if (BCAM_N7 == 0) 
              begin : gen_tcam_row_9_col_2_tcam
        ...
        // slice_en kept for TCAM only for backward compatability with Intel process
                ...
                begin
                  for (int i = 0; i < TCAM_SLICE_EN_FACT; i++)
                      if (tcam_row_9_col_2_slice_en_s1[i])
                           tcam_row_9_col_2_hit_out[ i*64 +: 64]  = tcam_row_9_col_2_hit_out_int  [ i*64 +: 64]; 
                      else
                           tcam_row_9_col_2_hit_out[ i*64 +: 64]  = tcam_row_9_col_2_hit_out_latch[ i*64 +: 64]; 
                end
        
        
        
                always_ff  @(posedge tcam_row_9_col_2_clk)
                begin
                  tcam_row_9_col_2_slice_en_s0 <= tcam_row_9_col_2_slice_en; 
                  tcam_row_9_col_2_slice_en_s1 <= tcam_row_9_col_2_slice_en_s0; 
        
                  for (int i = 0; i < TCAM_SLICE_EN_FACT; i++)
                    begin
                      if (tcam_row_9_col_2_slice_en_s1[i])
                         tcam_row_9_col_2_hit_out_latch[ i*64 +: 64] <=  tcam_row_9_col_2_hit_out_int[ i*64 +: 64];
                    end
                end
        ...
        //                             |_____________________________|
                         ...
                         assign tcam_row_9_col_2_data_out  = ~tcam_row_9_col_2_data_out_inv;
                         hif_lme_r12_ip783tcam128x32s0c1_9_2_wrap #(        
                                 )               
        
                                 ip783tcam128x32s0c1_wrap_tcam_row_9_col_2 (
                   //-------------------------------------------------------------
                   // Power Management Interfaces
                   //-------------------------------------------------------------   
                      `ifndef INTEL_NO_PWR_PINS
                      ...
                      `endif
                        .vddp             (1'b1),
                      `endif
                         .ADR               (tcam_row_9_col_2_adr[8-1:1]), //Input adr_intess
                         .ME                (ce),  
        
                         .CLK               (tcam_row_9_col_2_clk), //Clock
                         .CMP               (tcam_row_9_col_2_chk_en), //Control Enable
                         .DIN               (~tcam_row_9_col_2_data_in), 
                         .DSEL              (~tcam_row_9_col_2_adr[0]), //dncen enables access of the TCAM Arrays adr_intessed
                         .FLS               (flush), //flush initiates a Flush operation that resets all Valid
                         .RVLD              (tcam_row_9_col_2_hit_in ), //Row validation bus. Each match_in input controls 1
                         .MASK              (tcam_row_9_col_2_mask_in), //Input msk
                         .QOUT              (tcam_row_9_col_2_data_out_inv), //Output vben
                         .QHR               (tcam_row_9_col_2_hit_out_int),
                         .QVLD              (), 
                         .RE                (tcam_row_9_col_2_rd_en), //Read Enable
                         .RST               (tcam_row_9_col_2_reset_n), //Asynchronous Active-High Reset
                         .VBE               (1'b1), //vben enables the Valid Bit addressed by adr[a-1:0] to
                         .VBI               (1'b0),//  Valid Bit Input
                         .WE                (tcam_row_9_col_2_wr_en), //Write Enable
                         .FAF               (tcam_row_9_col_2_fca),
                         .FAF_EN            (tcam_row_9_col_2_cre),
                         .DFTSHIFTEN        (tcam_row_9_col_2_dftshiften),
                         .DFTMASK           (tcam_row_9_col_2_dftmask),
                         .DFT_ARRAY_FREEZE  (tcam_row_9_col_2_dft_array_freeze),
                         .DFT_AFD_RESET_B   (tcam_row_9_col_2_dft_afd_reset_b),
                         .BISTE           (tcam_row_9_col_2_biste),
                         .TDIN (tcam_row_9_col_2_tdin),
                         .TADR (tcam_row_9_col_2_tadr),
                         .TVBE (tcam_row_9_col_2_tvbe),
                         .TVBI (tcam_row_9_col_2_tvbi),
                         .TMASK (tcam_row_9_col_2_tmask),
                         .TWE (tcam_row_9_col_2_twe),
                         .TRE (tcam_row_9_col_2_tre),
                         .TME (tcam_row_9_col_2_tme),
                         .BIST_ROTATE_SR (tcam_row_9_col_2_bist_rotate_sr),
                         .BIST_EN_SR_MASK_B (tcam_row_9_col_2_bist_en_sr_mask_b),
                         .BIST_ALL_MASK (tcam_row_9_col_2_bist_all_mask),
                         .BIST_EN_SR_DATA_B (tcam_row_9_col_2_bist_en_sr_data_b),
                         .BIST_CM_MODE (tcam_row_9_col_2_bist_cm_mode),
                         .BIST_CM_EN_IN (tcam_row_9_col_2_bist_cm_en_in),
                         .BIST_CM_MATCH_SEL0 (tcam_row_9_col_2_bist_cm_match_sel0),
                         .BIST_CM_MATCH_SEL1 (tcam_row_9_col_2_bist_cm_match_sel1),
                         .BIST_MATCHIN_IN (tcam_row_9_col_2_bist_matchin_in),
                         .BIST_FLS (tcam_row_9_col_2_bist_fls),
                         .MDST_tcam (fuse_tcam)
                     );
              ...
              begin : gen_tcam_row_9_col_2_bcam
              end
        ...
        always_ff @(posedge clk) begin
                   tcam_rd_data_col[9][2]      <= tcam_row_9_col_2_data_out;
                   tcam_col_hit_out[9][2]       <= tcam_row_9_col_2_hit_out;
        end
        ...
        //                             |_____________________________|
                         ...
                         hif_lme_r12_ip783tcam128x32s0c1_10_0_wrap #(        
                                 )               
        ...
        wire    [8-1:0]                       tcam_row_10_col_1_adr  = tcam_row_adr[10][8-1:0];
        ...
        wire    [128-1:0]                       tcam_row_10_col_1_hit_in = tcam_raw_hit_in[10];
        wire    [128-1:0]                       tcam_row_10_col_1_tcam_match_in = tcam_row_match_in[10];
        wire    [32-1:0]                      tcam_row_10_col_1_data_out_inv;
        logic   [128-1:0]                     tcam_row_10_col_1_hit_out_int;
        logic   [128-1:0]                     tcam_row_10_col_1_hit_out_latch;
        logic   [128-1:0]                     tcam_row_10_col_1_hit_out;
        ...
        wire      [8-1:0]     tcam_row_10_col_1_tadr = 'b0;
        ...
        //                             |_____________________________|
                         ...
                         hif_lme_r12_ip783tcam128x32s0c1_10_1_wrap #(        
                                 )               
        
                                 ip783tcam128x32s0c1_wrap_tcam_row_10_col_1 (
                   //-------------------------------------------------------------
                   // Power Management Interfaces
                   //-------------------------------------------------------------   
                      `ifndef INTEL_NO_PWR_PINS
                      ...
                      `endif
                        .vddp             (1'b1),
                      `endif
                         .ADR               (tcam_row_10_col_1_adr[8-1:1]), //Input adr_intess
                         .ME                (ce),  
        ...
        wire                                            tcam_row_10_col_2_clk         = clk;
        
        wire    [TCAM_SLICE_EN_FACT-1:0]                tcam_row_10_col_2_slice_en = TCAM_SLICE_EN_FACT'(slice_en[(10 * TCAM_SLICE_EN_FACT)+:TCAM_SLICE_EN_MIN]);
        
        wire                                            tcam_row_10_col_2_rd_en       = tcam_row_rd_en[10];
        wire                                            tcam_row_10_col_2_wr_en       = tcam_row_wr_en[10];
        wire                                            tcam_row_10_col_2_chk_en      = chk_en;
        wire                                            tcam_row_10_col_2_reset_n = reset_n;
        wire    [8-1:0]                       tcam_row_10_col_2_adr  = tcam_row_adr[10][8-1:0];
        wire    [32-1:0]                      tcam_row_10_col_2_data_in = tcam_row_wr_en[10] ? tcam_wr_data_col[2][32-1:0] : tcam_chk_key_col[2][32-1:0];
        wire    [32-1:0]                      tcam_row_10_col_2_data_in_pre = tcam_wr_data_col[2][32-1:0] ;
        wire    [32-1:0]                      tcam_row_10_col_2_key_in =  tcam_chk_key_col[2][32-1:0];
        wire    [32-1:0]                      tcam_row_10_col_2_mask_in = tcam_chk_mask_col[2][32-1:0];
        wire    [128-1:0]                       tcam_row_10_col_2_hit_in = tcam_raw_hit_in[10];
        wire    [128-1:0]                       tcam_row_10_col_2_tcam_match_in = tcam_row_match_in[10];
        wire    [32-1:0]                      tcam_row_10_col_2_data_out_inv;
        logic   [128-1:0]                     tcam_row_10_col_2_hit_out_int;
        logic   [128-1:0]                     tcam_row_10_col_2_hit_out_latch;
        logic   [128-1:0]                     tcam_row_10_col_2_hit_out;
        
        `ifdef HIF_PCIE_DFX_DISABLE
        wire                          tcam_row_10_col_2_biste = 'b0; 
        wire     [5-1:0]                  tcam_row_10_col_2_fca = 'b0;
        wire                            tcam_row_10_col_2_cre = 'b0;
        wire                            tcam_row_10_col_2_dftshiften = 'b0;
        wire                            tcam_row_10_col_2_dftmask = 'b0;
        wire                            tcam_row_10_col_2_dft_array_freeze = 'b0;
        wire                            tcam_row_10_col_2_dft_afd_reset_b = 'b0;
        wire      [32-1:0]    tcam_row_10_col_2_tdin = 'b0;
        wire      [8-1:0]     tcam_row_10_col_2_tadr = 'b0;
        wire                            tcam_row_10_col_2_tvbe = 'b0;
        wire                            tcam_row_10_col_2_tvbi = 'b0;
        wire      [32-1:0]    tcam_row_10_col_2_tmask = 'b0;
        wire                            tcam_row_10_col_2_twe = 'b0;
        wire                            tcam_row_10_col_2_tre = 'b0;
        wire                            tcam_row_10_col_2_tme = 'b0;
        wire                            tcam_row_10_col_2_bist_rotate_sr = 'b0;
        wire                            tcam_row_10_col_2_bist_en_sr_mask_b = 'b0;
        wire                            tcam_row_10_col_2_bist_all_mask = 'b0;
        wire                            tcam_row_10_col_2_bist_en_sr_data_b = 'b0;
        wire                            tcam_row_10_col_2_bist_cm_mode = 'b0;
        wire                            tcam_row_10_col_2_bist_cm_en_in = 'b0;
        wire                            tcam_row_10_col_2_bist_cm_match_sel0 = 'b0;
        wire                            tcam_row_10_col_2_bist_cm_match_sel1 = 'b0;
        wire                            tcam_row_10_col_2_bist_matchin_in = 'b0;
        wire                            tcam_row_10_col_2_bist_fls = 'b0;
        
        `else
        wire                            tcam_row_10_col_2_biste = 'b0;
        wire     [5-1:0]                  tcam_row_10_col_2_fca = 'b0;
        wire                            tcam_row_10_col_2_cre = 'b0;
        wire                            tcam_row_10_col_2_dftshiften = dftshiften;
        wire                            tcam_row_10_col_2_dftmask = dftmask;
        wire                            tcam_row_10_col_2_dft_array_freeze = dft_array_freeze;
        wire                            tcam_row_10_col_2_dft_afd_reset_b = dft_afd_reset_b;
        wire      [32-1:0]    tcam_row_10_col_2_tdin = 'b0;
        wire      [8-1:0]     tcam_row_10_col_2_tadr = 'b0;
        wire                            tcam_row_10_col_2_tvbe = 'b0;
        wire                            tcam_row_10_col_2_tvbi = 'b0;
        ...
        // Outputs
        logic                           tcam_row_10_col_2_wr_bira_fail;
        logic                           tcam_row_10_col_2_wr_so;
        logic                           tcam_row_10_col_2_curerrout;
        logic                           tcam_row_10_col_2_rscout;
        
        
        
        logic   [TCAM_SLICE_EN_FACT-1:0]                  tcam_row_10_col_2_slice_en_s0; 
        logic   [TCAM_SLICE_EN_FACT-1:0]                  tcam_row_10_col_2_slice_en_s1; 
        
        
        
             wire    [32-1:0]                     tcam_row_10_col_2_data_out ;
        
        
        
         generate
            if (BCAM_N7 == 0) 
              begin : gen_tcam_row_10_col_2_tcam
        ...
        // slice_en kept for TCAM only for backward compatability with Intel process
                ...
                begin
                  for (int i = 0; i < TCAM_SLICE_EN_FACT; i++)
                      if (tcam_row_10_col_2_slice_en_s1[i])
                           tcam_row_10_col_2_hit_out[ i*64 +: 64]  = tcam_row_10_col_2_hit_out_int  [ i*64 +: 64]; 
                      else
                           tcam_row_10_col_2_hit_out[ i*64 +: 64]  = tcam_row_10_col_2_hit_out_latch[ i*64 +: 64]; 
                end
        
        
        
                always_ff  @(posedge tcam_row_10_col_2_clk)
                begin
                  tcam_row_10_col_2_slice_en_s0 <= tcam_row_10_col_2_slice_en; 
                  tcam_row_10_col_2_slice_en_s1 <= tcam_row_10_col_2_slice_en_s0; 
        
                  for (int i = 0; i < TCAM_SLICE_EN_FACT; i++)
                    begin
                      if (tcam_row_10_col_2_slice_en_s1[i])
                         tcam_row_10_col_2_hit_out_latch[ i*64 +: 64] <=  tcam_row_10_col_2_hit_out_int[ i*64 +: 64];
                    end
                end
        ...
        //                             |_____________________________|
                         ...
                         assign tcam_row_10_col_2_data_out  = ~tcam_row_10_col_2_data_out_inv;
                         hif_lme_r12_ip783tcam128x32s0c1_10_2_wrap #(        
                                 )               
        
                                 ip783tcam128x32s0c1_wrap_tcam_row_10_col_2 (
                   //-------------------------------------------------------------
                   // Power Management Interfaces
                   //-------------------------------------------------------------   
                      `ifndef INTEL_NO_PWR_PINS
                      ...
                      `endif
                        .vddp             (1'b1),
                      `endif
                         .ADR               (tcam_row_10_col_2_adr[8-1:1]), //Input adr_intess
                         .ME                (ce),  
        
                         .CLK               (tcam_row_10_col_2_clk), //Clock
                         .CMP               (tcam_row_10_col_2_chk_en), //Control Enable
                         .DIN               (~tcam_row_10_col_2_data_in), 
                         .DSEL              (~tcam_row_10_col_2_adr[0]), //dncen enables access of the TCAM Arrays adr_intessed
                         .FLS               (flush), //flush initiates a Flush operation that resets all Valid
                         .RVLD              (tcam_row_10_col_2_hit_in ), //Row validation bus. Each match_in input controls 1
                         .MASK              (tcam_row_10_col_2_mask_in), //Input msk
                         .QOUT              (tcam_row_10_col_2_data_out_inv), //Output vben
                         .QHR               (tcam_row_10_col_2_hit_out_int),
                         .QVLD              (), 
                         .RE                (tcam_row_10_col_2_rd_en), //Read Enable
                         .RST               (tcam_row_10_col_2_reset_n), //Asynchronous Active-High Reset
                         .VBE               (1'b1), //vben enables the Valid Bit addressed by adr[a-1:0] to
                         .VBI               (1'b0),//  Valid Bit Input
                         .WE                (tcam_row_10_col_2_wr_en), //Write Enable
                         .FAF               (tcam_row_10_col_2_fca),
                         .FAF_EN            (tcam_row_10_col_2_cre),
                         .DFTSHIFTEN        (tcam_row_10_col_2_dftshiften),
                         .DFTMASK           (tcam_row_10_col_2_dftmask),
                         .DFT_ARRAY_FREEZE  (tcam_row_10_col_2_dft_array_freeze),
                         .DFT_AFD_RESET_B   (tcam_row_10_col_2_dft_afd_reset_b),
                         .BISTE           (tcam_row_10_col_2_biste),
                         .TDIN (tcam_row_10_col_2_tdin),
                         .TADR (tcam_row_10_col_2_tadr),
                         .TVBE (tcam_row_10_col_2_tvbe),
                         .TVBI (tcam_row_10_col_2_tvbi),
                         .TMASK (tcam_row_10_col_2_tmask),
                         .TWE (tcam_row_10_col_2_twe),
                         .TRE (tcam_row_10_col_2_tre),
                         .TME (tcam_row_10_col_2_tme),
                         .BIST_ROTATE_SR (tcam_row_10_col_2_bist_rotate_sr),
                         .BIST_EN_SR_MASK_B (tcam_row_10_col_2_bist_en_sr_mask_b),
                         .BIST_ALL_MASK (tcam_row_10_col_2_bist_all_mask),
                         .BIST_EN_SR_DATA_B (tcam_row_10_col_2_bist_en_sr_data_b),
                         .BIST_CM_MODE (tcam_row_10_col_2_bist_cm_mode),
                         .BIST_CM_EN_IN (tcam_row_10_col_2_bist_cm_en_in),
                         .BIST_CM_MATCH_SEL0 (tcam_row_10_col_2_bist_cm_match_sel0),
                         .BIST_CM_MATCH_SEL1 (tcam_row_10_col_2_bist_cm_match_sel1),
                         .BIST_MATCHIN_IN (tcam_row_10_col_2_bist_matchin_in),
                         .BIST_FLS (tcam_row_10_col_2_bist_fls),
                         .MDST_tcam (fuse_tcam)
                     );
              ...
              begin : gen_tcam_row_10_col_2_bcam
              end
        ...
        always_ff @(posedge clk) begin
                   tcam_rd_data_col[10][2]      <= tcam_row_10_col_2_data_out;
                   tcam_col_hit_out[10][2]       <= tcam_row_10_col_2_hit_out;
        end
        ...
        //                             |_____________________________|
                         ...
                         hif_lme_r12_ip783tcam128x32s0c1_11_0_wrap #(        
                                 )               
        ...
        wire    [8-1:0]                       tcam_row_11_col_1_adr  = tcam_row_adr[11][8-1:0];
        ...
        wire    [128-1:0]                       tcam_row_11_col_1_hit_in = tcam_raw_hit_in[11];
        wire    [128-1:0]                       tcam_row_11_col_1_tcam_match_in = tcam_row_match_in[11];
        wire    [32-1:0]                      tcam_row_11_col_1_data_out_inv;
        logic   [128-1:0]                     tcam_row_11_col_1_hit_out_int;
        logic   [128-1:0]                     tcam_row_11_col_1_hit_out_latch;
        logic   [128-1:0]                     tcam_row_11_col_1_hit_out;
        ...
        wire      [8-1:0]     tcam_row_11_col_1_tadr = 'b0;
        ...
        //                             |_____________________________|
                         ...
                         hif_lme_r12_ip783tcam128x32s0c1_11_1_wrap #(        
                                 )               
        
                                 ip783tcam128x32s0c1_wrap_tcam_row_11_col_1 (
                   //-------------------------------------------------------------
                   // Power Management Interfaces
                   //-------------------------------------------------------------   
                      `ifndef INTEL_NO_PWR_PINS
                      ...
                      `endif
                        .vddp             (1'b1),
                      `endif
                         .ADR               (tcam_row_11_col_1_adr[8-1:1]), //Input adr_intess
                         .ME                (ce),  
        ...
        wire                                            tcam_row_11_col_2_clk         = clk;
        
        wire    [TCAM_SLICE_EN_FACT-1:0]                tcam_row_11_col_2_slice_en = TCAM_SLICE_EN_FACT'(slice_en[(11 * TCAM_SLICE_EN_FACT)+:TCAM_SLICE_EN_MIN]);
        
        wire                                            tcam_row_11_col_2_rd_en       = tcam_row_rd_en[11];
        wire                                            tcam_row_11_col_2_wr_en       = tcam_row_wr_en[11];
        wire                                            tcam_row_11_col_2_chk_en      = chk_en;
        wire                                            tcam_row_11_col_2_reset_n = reset_n;
        wire    [8-1:0]                       tcam_row_11_col_2_adr  = tcam_row_adr[11][8-1:0];
        wire    [32-1:0]                      tcam_row_11_col_2_data_in = tcam_row_wr_en[11] ? tcam_wr_data_col[2][32-1:0] : tcam_chk_key_col[2][32-1:0];
        wire    [32-1:0]                      tcam_row_11_col_2_data_in_pre = tcam_wr_data_col[2][32-1:0] ;
        wire    [32-1:0]                      tcam_row_11_col_2_key_in =  tcam_chk_key_col[2][32-1:0];
        wire    [32-1:0]                      tcam_row_11_col_2_mask_in = tcam_chk_mask_col[2][32-1:0];
        wire    [128-1:0]                       tcam_row_11_col_2_hit_in = tcam_raw_hit_in[11];
        wire    [128-1:0]                       tcam_row_11_col_2_tcam_match_in = tcam_row_match_in[11];
        wire    [32-1:0]                      tcam_row_11_col_2_data_out_inv;
        logic   [128-1:0]                     tcam_row_11_col_2_hit_out_int;
        logic   [128-1:0]                     tcam_row_11_col_2_hit_out_latch;
        logic   [128-1:0]                     tcam_row_11_col_2_hit_out;
        
        `ifdef HIF_PCIE_DFX_DISABLE
        wire                          tcam_row_11_col_2_biste = 'b0; 
        wire     [5-1:0]                  tcam_row_11_col_2_fca = 'b0;
        wire                            tcam_row_11_col_2_cre = 'b0;
        wire                            tcam_row_11_col_2_dftshiften = 'b0;
        wire                            tcam_row_11_col_2_dftmask = 'b0;
        wire                            tcam_row_11_col_2_dft_array_freeze = 'b0;
        wire                            tcam_row_11_col_2_dft_afd_reset_b = 'b0;
        wire      [32-1:0]    tcam_row_11_col_2_tdin = 'b0;
        wire      [8-1:0]     tcam_row_11_col_2_tadr = 'b0;
        wire                            tcam_row_11_col_2_tvbe = 'b0;
        wire                            tcam_row_11_col_2_tvbi = 'b0;
        wire      [32-1:0]    tcam_row_11_col_2_tmask = 'b0;
        wire                            tcam_row_11_col_2_twe = 'b0;
        wire                            tcam_row_11_col_2_tre = 'b0;
        wire                            tcam_row_11_col_2_tme = 'b0;
        wire                            tcam_row_11_col_2_bist_rotate_sr = 'b0;
        wire                            tcam_row_11_col_2_bist_en_sr_mask_b = 'b0;
        wire                            tcam_row_11_col_2_bist_all_mask = 'b0;
        wire                            tcam_row_11_col_2_bist_en_sr_data_b = 'b0;
        wire                            tcam_row_11_col_2_bist_cm_mode = 'b0;
        wire                            tcam_row_11_col_2_bist_cm_en_in = 'b0;
        wire                            tcam_row_11_col_2_bist_cm_match_sel0 = 'b0;
        wire                            tcam_row_11_col_2_bist_cm_match_sel1 = 'b0;
        wire                            tcam_row_11_col_2_bist_matchin_in = 'b0;
        wire                            tcam_row_11_col_2_bist_fls = 'b0;
        
        `else
        wire                            tcam_row_11_col_2_biste = 'b0;
        wire     [5-1:0]                  tcam_row_11_col_2_fca = 'b0;
        wire                            tcam_row_11_col_2_cre = 'b0;
        wire                            tcam_row_11_col_2_dftshiften = dftshiften;
        wire                            tcam_row_11_col_2_dftmask = dftmask;
        wire                            tcam_row_11_col_2_dft_array_freeze = dft_array_freeze;
        wire                            tcam_row_11_col_2_dft_afd_reset_b = dft_afd_reset_b;
        wire      [32-1:0]    tcam_row_11_col_2_tdin = 'b0;
        wire      [8-1:0]     tcam_row_11_col_2_tadr = 'b0;
        wire                            tcam_row_11_col_2_tvbe = 'b0;
        wire                            tcam_row_11_col_2_tvbi = 'b0;
        ...
        // Outputs
        logic                           tcam_row_11_col_2_wr_bira_fail;
        logic                           tcam_row_11_col_2_wr_so;
        logic                           tcam_row_11_col_2_curerrout;
        logic                           tcam_row_11_col_2_rscout;
        
        
        
        logic   [TCAM_SLICE_EN_FACT-1:0]                  tcam_row_11_col_2_slice_en_s0; 
        logic   [TCAM_SLICE_EN_FACT-1:0]                  tcam_row_11_col_2_slice_en_s1; 
        
        
        
             wire    [32-1:0]                     tcam_row_11_col_2_data_out ;
        
        
        
         generate
            if (BCAM_N7 == 0) 
              begin : gen_tcam_row_11_col_2_tcam
        ...
        // slice_en kept for TCAM only for backward compatability with Intel process
                ...
                begin
                  for (int i = 0; i < TCAM_SLICE_EN_FACT; i++)
                      if (tcam_row_11_col_2_slice_en_s1[i])
                           tcam_row_11_col_2_hit_out[ i*64 +: 64]  = tcam_row_11_col_2_hit_out_int  [ i*64 +: 64]; 
                      else
                           tcam_row_11_col_2_hit_out[ i*64 +: 64]  = tcam_row_11_col_2_hit_out_latch[ i*64 +: 64]; 
                end
        
        
        
                always_ff  @(posedge tcam_row_11_col_2_clk)
                begin
                  tcam_row_11_col_2_slice_en_s0 <= tcam_row_11_col_2_slice_en; 
                  tcam_row_11_col_2_slice_en_s1 <= tcam_row_11_col_2_slice_en_s0; 
        
                  for (int i = 0; i < TCAM_SLICE_EN_FACT; i++)
                    begin
                      if (tcam_row_11_col_2_slice_en_s1[i])
                         tcam_row_11_col_2_hit_out_latch[ i*64 +: 64] <=  tcam_row_11_col_2_hit_out_int[ i*64 +: 64];
                    end
                end
        ...
        //                             |_____________________________|
                         ...
                         assign tcam_row_11_col_2_data_out  = ~tcam_row_11_col_2_data_out_inv;
                         hif_lme_r12_ip783tcam128x32s0c1_11_2_wrap #(        
                                 )               
        
                                 ip783tcam128x32s0c1_wrap_tcam_row_11_col_2 (
                   //-------------------------------------------------------------
                   // Power Management Interfaces
                   //-------------------------------------------------------------   
                      `ifndef INTEL_NO_PWR_PINS
                      ...
                      `endif
                        .vddp             (1'b1),
                      `endif
                         .ADR               (tcam_row_11_col_2_adr[8-1:1]), //Input adr_intess
                         .ME                (ce),  
        
                         .CLK               (tcam_row_11_col_2_clk), //Clock
                         .CMP               (tcam_row_11_col_2_chk_en), //Control Enable
                         .DIN               (~tcam_row_11_col_2_data_in), 
                         .DSEL              (~tcam_row_11_col_2_adr[0]), //dncen enables access of the TCAM Arrays adr_intessed
                         .FLS               (flush), //flush initiates a Flush operation that resets all Valid
                         .RVLD              (tcam_row_11_col_2_hit_in ), //Row validation bus. Each match_in input controls 1
                         .MASK              (tcam_row_11_col_2_mask_in), //Input msk
                         .QOUT              (tcam_row_11_col_2_data_out_inv), //Output vben
                         .QHR               (tcam_row_11_col_2_hit_out_int),
                         .QVLD              (), 
                         .RE                (tcam_row_11_col_2_rd_en), //Read Enable
                         .RST               (tcam_row_11_col_2_reset_n), //Asynchronous Active-High Reset
                         .VBE               (1'b1), //vben enables the Valid Bit addressed by adr[a-1:0] to
                         .VBI               (1'b0),//  Valid Bit Input
                         .WE                (tcam_row_11_col_2_wr_en), //Write Enable
                         .FAF               (tcam_row_11_col_2_fca),
                         .FAF_EN            (tcam_row_11_col_2_cre),
                         .DFTSHIFTEN        (tcam_row_11_col_2_dftshiften),
                         .DFTMASK           (tcam_row_11_col_2_dftmask),
                         .DFT_ARRAY_FREEZE  (tcam_row_11_col_2_dft_array_freeze),
                         .DFT_AFD_RESET_B   (tcam_row_11_col_2_dft_afd_reset_b),
                         .BISTE           (tcam_row_11_col_2_biste),
                         .TDIN (tcam_row_11_col_2_tdin),
                         .TADR (tcam_row_11_col_2_tadr),
                         .TVBE (tcam_row_11_col_2_tvbe),
                         .TVBI (tcam_row_11_col_2_tvbi),
                         .TMASK (tcam_row_11_col_2_tmask),
                         .TWE (tcam_row_11_col_2_twe),
                         .TRE (tcam_row_11_col_2_tre),
                         .TME (tcam_row_11_col_2_tme),
                         .BIST_ROTATE_SR (tcam_row_11_col_2_bist_rotate_sr),
                         .BIST_EN_SR_MASK_B (tcam_row_11_col_2_bist_en_sr_mask_b),
                         .BIST_ALL_MASK (tcam_row_11_col_2_bist_all_mask),
                         .BIST_EN_SR_DATA_B (tcam_row_11_col_2_bist_en_sr_data_b),
                         .BIST_CM_MODE (tcam_row_11_col_2_bist_cm_mode),
                         .BIST_CM_EN_IN (tcam_row_11_col_2_bist_cm_en_in),
                         .BIST_CM_MATCH_SEL0 (tcam_row_11_col_2_bist_cm_match_sel0),
                         .BIST_CM_MATCH_SEL1 (tcam_row_11_col_2_bist_cm_match_sel1),
                         .BIST_MATCHIN_IN (tcam_row_11_col_2_bist_matchin_in),
                         .BIST_FLS (tcam_row_11_col_2_bist_fls),
                         .MDST_tcam (fuse_tcam)
                     );
              ...
              begin : gen_tcam_row_11_col_2_bcam
              end
        ...
        always_ff @(posedge clk) begin
                   tcam_rd_data_col[11][2]      <= tcam_row_11_col_2_data_out;
                   tcam_col_hit_out[11][2]       <= tcam_row_11_col_2_hit_out;
        end
        ...
        //                             |_____________________________|
                         ...
                         hif_lme_r12_ip783tcam128x32s0c1_12_0_wrap #(        
                                 )               
        ...
        wire    [8-1:0]                       tcam_row_12_col_1_adr  = tcam_row_adr[12][8-1:0];
        ...
        wire    [128-1:0]                       tcam_row_12_col_1_hit_in = tcam_raw_hit_in[12];
        wire    [128-1:0]                       tcam_row_12_col_1_tcam_match_in = tcam_row_match_in[12];
        wire    [32-1:0]                      tcam_row_12_col_1_data_out_inv;
        logic   [128-1:0]                     tcam_row_12_col_1_hit_out_int;
        logic   [128-1:0]                     tcam_row_12_col_1_hit_out_latch;
        logic   [128-1:0]                     tcam_row_12_col_1_hit_out;
        ...
        wire      [8-1:0]     tcam_row_12_col_1_tadr = 'b0;
        ...
        //                             |_____________________________|
                         ...
                         hif_lme_r12_ip783tcam128x32s0c1_12_1_wrap #(        
                                 )               
        
                                 ip783tcam128x32s0c1_wrap_tcam_row_12_col_1 (
                   //-------------------------------------------------------------
                   // Power Management Interfaces
                   //-------------------------------------------------------------   
                      `ifndef INTEL_NO_PWR_PINS
                      ...
                      `endif
                        .vddp             (1'b1),
                      `endif
                         .ADR               (tcam_row_12_col_1_adr[8-1:1]), //Input adr_intess
                         .ME                (ce),  
        ...
        wire                                            tcam_row_12_col_2_clk         = clk;
        
        wire    [TCAM_SLICE_EN_FACT-1:0]                tcam_row_12_col_2_slice_en = TCAM_SLICE_EN_FACT'(slice_en[(12 * TCAM_SLICE_EN_FACT)+:TCAM_SLICE_EN_MIN]);
        
        wire                                            tcam_row_12_col_2_rd_en       = tcam_row_rd_en[12];
        wire                                            tcam_row_12_col_2_wr_en       = tcam_row_wr_en[12];
        wire                                            tcam_row_12_col_2_chk_en      = chk_en;
        wire                                            tcam_row_12_col_2_reset_n = reset_n;
        wire    [8-1:0]                       tcam_row_12_col_2_adr  = tcam_row_adr[12][8-1:0];
        wire    [32-1:0]                      tcam_row_12_col_2_data_in = tcam_row_wr_en[12] ? tcam_wr_data_col[2][32-1:0] : tcam_chk_key_col[2][32-1:0];
        wire    [32-1:0]                      tcam_row_12_col_2_data_in_pre = tcam_wr_data_col[2][32-1:0] ;
        wire    [32-1:0]                      tcam_row_12_col_2_key_in =  tcam_chk_key_col[2][32-1:0];
        wire    [32-1:0]                      tcam_row_12_col_2_mask_in = tcam_chk_mask_col[2][32-1:0];
        wire    [128-1:0]                       tcam_row_12_col_2_hit_in = tcam_raw_hit_in[12];
        wire    [128-1:0]                       tcam_row_12_col_2_tcam_match_in = tcam_row_match_in[12];
        wire    [32-1:0]                      tcam_row_12_col_2_data_out_inv;
        logic   [128-1:0]                     tcam_row_12_col_2_hit_out_int;
        logic   [128-1:0]                     tcam_row_12_col_2_hit_out_latch;
        logic   [128-1:0]                     tcam_row_12_col_2_hit_out;
        
        `ifdef HIF_PCIE_DFX_DISABLE
        wire                          tcam_row_12_col_2_biste = 'b0; 
        wire     [5-1:0]                  tcam_row_12_col_2_fca = 'b0;
        wire                            tcam_row_12_col_2_cre = 'b0;
        wire                            tcam_row_12_col_2_dftshiften = 'b0;
        wire                            tcam_row_12_col_2_dftmask = 'b0;
        wire                            tcam_row_12_col_2_dft_array_freeze = 'b0;
        wire                            tcam_row_12_col_2_dft_afd_reset_b = 'b0;
        wire      [32-1:0]    tcam_row_12_col_2_tdin = 'b0;
        wire      [8-1:0]     tcam_row_12_col_2_tadr = 'b0;
        wire                            tcam_row_12_col_2_tvbe = 'b0;
        wire                            tcam_row_12_col_2_tvbi = 'b0;
        wire      [32-1:0]    tcam_row_12_col_2_tmask = 'b0;
        wire                            tcam_row_12_col_2_twe = 'b0;
        wire                            tcam_row_12_col_2_tre = 'b0;
        wire                            tcam_row_12_col_2_tme = 'b0;
        wire                            tcam_row_12_col_2_bist_rotate_sr = 'b0;
        wire                            tcam_row_12_col_2_bist_en_sr_mask_b = 'b0;
        wire                            tcam_row_12_col_2_bist_all_mask = 'b0;
        wire                            tcam_row_12_col_2_bist_en_sr_data_b = 'b0;
        wire                            tcam_row_12_col_2_bist_cm_mode = 'b0;
        wire                            tcam_row_12_col_2_bist_cm_en_in = 'b0;
        wire                            tcam_row_12_col_2_bist_cm_match_sel0 = 'b0;
        wire                            tcam_row_12_col_2_bist_cm_match_sel1 = 'b0;
        wire                            tcam_row_12_col_2_bist_matchin_in = 'b0;
        wire                            tcam_row_12_col_2_bist_fls = 'b0;
        
        `else
        wire                            tcam_row_12_col_2_biste = 'b0;
        wire     [5-1:0]                  tcam_row_12_col_2_fca = 'b0;
        wire                            tcam_row_12_col_2_cre = 'b0;
        wire                            tcam_row_12_col_2_dftshiften = dftshiften;
        wire                            tcam_row_12_col_2_dftmask = dftmask;
        wire                            tcam_row_12_col_2_dft_array_freeze = dft_array_freeze;
        wire                            tcam_row_12_col_2_dft_afd_reset_b = dft_afd_reset_b;
        wire      [32-1:0]    tcam_row_12_col_2_tdin = 'b0;
        wire      [8-1:0]     tcam_row_12_col_2_tadr = 'b0;
        wire                            tcam_row_12_col_2_tvbe = 'b0;
        wire                            tcam_row_12_col_2_tvbi = 'b0;
        ...
        // Outputs
        logic                           tcam_row_12_col_2_wr_bira_fail;
        logic                           tcam_row_12_col_2_wr_so;
        logic                           tcam_row_12_col_2_curerrout;
        logic                           tcam_row_12_col_2_rscout;
        
        
        
        logic   [TCAM_SLICE_EN_FACT-1:0]                  tcam_row_12_col_2_slice_en_s0; 
        logic   [TCAM_SLICE_EN_FACT-1:0]                  tcam_row_12_col_2_slice_en_s1; 
        
        
        
             wire    [32-1:0]                     tcam_row_12_col_2_data_out ;
        
        
        
         generate
            if (BCAM_N7 == 0) 
              begin : gen_tcam_row_12_col_2_tcam
        ...
        // slice_en kept for TCAM only for backward compatability with Intel process
                ...
                begin
                  for (int i = 0; i < TCAM_SLICE_EN_FACT; i++)
                      if (tcam_row_12_col_2_slice_en_s1[i])
                           tcam_row_12_col_2_hit_out[ i*64 +: 64]  = tcam_row_12_col_2_hit_out_int  [ i*64 +: 64]; 
                      else
                           tcam_row_12_col_2_hit_out[ i*64 +: 64]  = tcam_row_12_col_2_hit_out_latch[ i*64 +: 64]; 
                end
        
        
        
                always_ff  @(posedge tcam_row_12_col_2_clk)
                begin
                  tcam_row_12_col_2_slice_en_s0 <= tcam_row_12_col_2_slice_en; 
                  tcam_row_12_col_2_slice_en_s1 <= tcam_row_12_col_2_slice_en_s0; 
        
                  for (int i = 0; i < TCAM_SLICE_EN_FACT; i++)
                    begin
                      if (tcam_row_12_col_2_slice_en_s1[i])
                         tcam_row_12_col_2_hit_out_latch[ i*64 +: 64] <=  tcam_row_12_col_2_hit_out_int[ i*64 +: 64];
                    end
                end
        ...
        //                             |_____________________________|
                         ...
                         assign tcam_row_12_col_2_data_out  = ~tcam_row_12_col_2_data_out_inv;
                         hif_lme_r12_ip783tcam128x32s0c1_12_2_wrap #(        
                                 )               
        
                                 ip783tcam128x32s0c1_wrap_tcam_row_12_col_2 (
                   //-------------------------------------------------------------
                   // Power Management Interfaces
                   //-------------------------------------------------------------   
                      `ifndef INTEL_NO_PWR_PINS
                      ...
                      `endif
                        .vddp             (1'b1),
                      `endif
                         .ADR               (tcam_row_12_col_2_adr[8-1:1]), //Input adr_intess
                         .ME                (ce),  
        
                         .CLK               (tcam_row_12_col_2_clk), //Clock
                         .CMP               (tcam_row_12_col_2_chk_en), //Control Enable
                         .DIN               (~tcam_row_12_col_2_data_in), 
                         .DSEL              (~tcam_row_12_col_2_adr[0]), //dncen enables access of the TCAM Arrays adr_intessed
                         .FLS               (flush), //flush initiates a Flush operation that resets all Valid
                         .RVLD              (tcam_row_12_col_2_hit_in ), //Row validation bus. Each match_in input controls 1
                         .MASK              (tcam_row_12_col_2_mask_in), //Input msk
                         .QOUT              (tcam_row_12_col_2_data_out_inv), //Output vben
                         .QHR               (tcam_row_12_col_2_hit_out_int),
                         .QVLD              (), 
                         .RE                (tcam_row_12_col_2_rd_en), //Read Enable
                         .RST               (tcam_row_12_col_2_reset_n), //Asynchronous Active-High Reset
                         .VBE               (1'b1), //vben enables the Valid Bit addressed by adr[a-1:0] to
                         .VBI               (1'b0),//  Valid Bit Input
                         .WE                (tcam_row_12_col_2_wr_en), //Write Enable
                         .FAF               (tcam_row_12_col_2_fca),
                         .FAF_EN            (tcam_row_12_col_2_cre),
                         .DFTSHIFTEN        (tcam_row_12_col_2_dftshiften),
                         .DFTMASK           (tcam_row_12_col_2_dftmask),
                         .DFT_ARRAY_FREEZE  (tcam_row_12_col_2_dft_array_freeze),
                         .DFT_AFD_RESET_B   (tcam_row_12_col_2_dft_afd_reset_b),
                         .BISTE           (tcam_row_12_col_2_biste),
                         .TDIN (tcam_row_12_col_2_tdin),
                         .TADR (tcam_row_12_col_2_tadr),
                         .TVBE (tcam_row_12_col_2_tvbe),
                         .TVBI (tcam_row_12_col_2_tvbi),
                         .TMASK (tcam_row_12_col_2_tmask),
                         .TWE (tcam_row_12_col_2_twe),
                         .TRE (tcam_row_12_col_2_tre),
                         .TME (tcam_row_12_col_2_tme),
                         .BIST_ROTATE_SR (tcam_row_12_col_2_bist_rotate_sr),
                         .BIST_EN_SR_MASK_B (tcam_row_12_col_2_bist_en_sr_mask_b),
                         .BIST_ALL_MASK (tcam_row_12_col_2_bist_all_mask),
                         .BIST_EN_SR_DATA_B (tcam_row_12_col_2_bist_en_sr_data_b),
                         .BIST_CM_MODE (tcam_row_12_col_2_bist_cm_mode),
                         .BIST_CM_EN_IN (tcam_row_12_col_2_bist_cm_en_in),
                         .BIST_CM_MATCH_SEL0 (tcam_row_12_col_2_bist_cm_match_sel0),
                         .BIST_CM_MATCH_SEL1 (tcam_row_12_col_2_bist_cm_match_sel1),
                         .BIST_MATCHIN_IN (tcam_row_12_col_2_bist_matchin_in),
                         .BIST_FLS (tcam_row_12_col_2_bist_fls),
                         .MDST_tcam (fuse_tcam)
                     );
              ...
              begin : gen_tcam_row_12_col_2_bcam
              end
        ...
        always_ff @(posedge clk) begin
                   tcam_rd_data_col[12][2]      <= tcam_row_12_col_2_data_out;
                   tcam_col_hit_out[12][2]       <= tcam_row_12_col_2_hit_out;
        end
        ...
        //                             |_____________________________|
                         ...
                         hif_lme_r12_ip783tcam128x32s0c1_13_0_wrap #(        
                                 )               
        ...
        wire    [8-1:0]                       tcam_row_13_col_1_adr  = tcam_row_adr[13][8-1:0];
        ...
        wire    [128-1:0]                       tcam_row_13_col_1_hit_in = tcam_raw_hit_in[13];
        wire    [128-1:0]                       tcam_row_13_col_1_tcam_match_in = tcam_row_match_in[13];
        wire    [32-1:0]                      tcam_row_13_col_1_data_out_inv;
        logic   [128-1:0]                     tcam_row_13_col_1_hit_out_int;
        logic   [128-1:0]                     tcam_row_13_col_1_hit_out_latch;
        logic   [128-1:0]                     tcam_row_13_col_1_hit_out;
        ...
        wire      [8-1:0]     tcam_row_13_col_1_tadr = 'b0;
        ...
        //                             |_____________________________|
                         ...
                         hif_lme_r12_ip783tcam128x32s0c1_13_1_wrap #(        
                                 )               
        
                                 ip783tcam128x32s0c1_wrap_tcam_row_13_col_1 (
                   //-------------------------------------------------------------
                   // Power Management Interfaces
                   //-------------------------------------------------------------   
                      `ifndef INTEL_NO_PWR_PINS
                      ...
                      `endif
                        .vddp             (1'b1),
                      `endif
                         .ADR               (tcam_row_13_col_1_adr[8-1:1]), //Input adr_intess
                         .ME                (ce),  
        ...
        wire                                            tcam_row_13_col_2_clk         = clk;
        
        wire    [TCAM_SLICE_EN_FACT-1:0]                tcam_row_13_col_2_slice_en = TCAM_SLICE_EN_FACT'(slice_en[(13 * TCAM_SLICE_EN_FACT)+:TCAM_SLICE_EN_MIN]);
        
        wire                                            tcam_row_13_col_2_rd_en       = tcam_row_rd_en[13];
        wire                                            tcam_row_13_col_2_wr_en       = tcam_row_wr_en[13];
        wire                                            tcam_row_13_col_2_chk_en      = chk_en;
        wire                                            tcam_row_13_col_2_reset_n = reset_n;
        wire    [8-1:0]                       tcam_row_13_col_2_adr  = tcam_row_adr[13][8-1:0];
        wire    [32-1:0]                      tcam_row_13_col_2_data_in = tcam_row_wr_en[13] ? tcam_wr_data_col[2][32-1:0] : tcam_chk_key_col[2][32-1:0];
        wire    [32-1:0]                      tcam_row_13_col_2_data_in_pre = tcam_wr_data_col[2][32-1:0] ;
        wire    [32-1:0]                      tcam_row_13_col_2_key_in =  tcam_chk_key_col[2][32-1:0];
        wire    [32-1:0]                      tcam_row_13_col_2_mask_in = tcam_chk_mask_col[2][32-1:0];
        wire    [128-1:0]                       tcam_row_13_col_2_hit_in = tcam_raw_hit_in[13];
        wire    [128-1:0]                       tcam_row_13_col_2_tcam_match_in = tcam_row_match_in[13];
        wire    [32-1:0]                      tcam_row_13_col_2_data_out_inv;
        logic   [128-1:0]                     tcam_row_13_col_2_hit_out_int;
        logic   [128-1:0]                     tcam_row_13_col_2_hit_out_latch;
        logic   [128-1:0]                     tcam_row_13_col_2_hit_out;
        
        `ifdef HIF_PCIE_DFX_DISABLE
        wire                          tcam_row_13_col_2_biste = 'b0; 
        wire     [5-1:0]                  tcam_row_13_col_2_fca = 'b0;
        wire                            tcam_row_13_col_2_cre = 'b0;
        wire                            tcam_row_13_col_2_dftshiften = 'b0;
        wire                            tcam_row_13_col_2_dftmask = 'b0;
        wire                            tcam_row_13_col_2_dft_array_freeze = 'b0;
        wire                            tcam_row_13_col_2_dft_afd_reset_b = 'b0;
        wire      [32-1:0]    tcam_row_13_col_2_tdin = 'b0;
        wire      [8-1:0]     tcam_row_13_col_2_tadr = 'b0;
        wire                            tcam_row_13_col_2_tvbe = 'b0;
        wire                            tcam_row_13_col_2_tvbi = 'b0;
        wire      [32-1:0]    tcam_row_13_col_2_tmask = 'b0;
        wire                            tcam_row_13_col_2_twe = 'b0;
        wire                            tcam_row_13_col_2_tre = 'b0;
        wire                            tcam_row_13_col_2_tme = 'b0;
        wire                            tcam_row_13_col_2_bist_rotate_sr = 'b0;
        wire                            tcam_row_13_col_2_bist_en_sr_mask_b = 'b0;
        wire                            tcam_row_13_col_2_bist_all_mask = 'b0;
        wire                            tcam_row_13_col_2_bist_en_sr_data_b = 'b0;
        wire                            tcam_row_13_col_2_bist_cm_mode = 'b0;
        wire                            tcam_row_13_col_2_bist_cm_en_in = 'b0;
        wire                            tcam_row_13_col_2_bist_cm_match_sel0 = 'b0;
        wire                            tcam_row_13_col_2_bist_cm_match_sel1 = 'b0;
        wire                            tcam_row_13_col_2_bist_matchin_in = 'b0;
        wire                            tcam_row_13_col_2_bist_fls = 'b0;
        
        `else
        wire                            tcam_row_13_col_2_biste = 'b0;
        wire     [5-1:0]                  tcam_row_13_col_2_fca = 'b0;
        wire                            tcam_row_13_col_2_cre = 'b0;
        wire                            tcam_row_13_col_2_dftshiften = dftshiften;
        wire                            tcam_row_13_col_2_dftmask = dftmask;
        wire                            tcam_row_13_col_2_dft_array_freeze = dft_array_freeze;
        wire                            tcam_row_13_col_2_dft_afd_reset_b = dft_afd_reset_b;
        wire      [32-1:0]    tcam_row_13_col_2_tdin = 'b0;
        wire      [8-1:0]     tcam_row_13_col_2_tadr = 'b0;
        wire                            tcam_row_13_col_2_tvbe = 'b0;
        wire                            tcam_row_13_col_2_tvbi = 'b0;
        ...
        // Outputs
        logic                           tcam_row_13_col_2_wr_bira_fail;
        logic                           tcam_row_13_col_2_wr_so;
        logic                           tcam_row_13_col_2_curerrout;
        logic                           tcam_row_13_col_2_rscout;
        
        
        
        logic   [TCAM_SLICE_EN_FACT-1:0]                  tcam_row_13_col_2_slice_en_s0; 
        logic   [TCAM_SLICE_EN_FACT-1:0]                  tcam_row_13_col_2_slice_en_s1; 
        
        
        
             wire    [32-1:0]                     tcam_row_13_col_2_data_out ;
        
        
        
         generate
            if (BCAM_N7 == 0) 
              begin : gen_tcam_row_13_col_2_tcam
        ...
        // slice_en kept for TCAM only for backward compatability with Intel process
                ...
                begin
                  for (int i = 0; i < TCAM_SLICE_EN_FACT; i++)
                      if (tcam_row_13_col_2_slice_en_s1[i])
                           tcam_row_13_col_2_hit_out[ i*64 +: 64]  = tcam_row_13_col_2_hit_out_int  [ i*64 +: 64]; 
                      else
                           tcam_row_13_col_2_hit_out[ i*64 +: 64]  = tcam_row_13_col_2_hit_out_latch[ i*64 +: 64]; 
                end
        
        
        
                always_ff  @(posedge tcam_row_13_col_2_clk)
                begin
                  tcam_row_13_col_2_slice_en_s0 <= tcam_row_13_col_2_slice_en; 
                  tcam_row_13_col_2_slice_en_s1 <= tcam_row_13_col_2_slice_en_s0; 
        
                  for (int i = 0; i < TCAM_SLICE_EN_FACT; i++)
                    begin
                      if (tcam_row_13_col_2_slice_en_s1[i])
                         tcam_row_13_col_2_hit_out_latch[ i*64 +: 64] <=  tcam_row_13_col_2_hit_out_int[ i*64 +: 64];
                    end
                end
        ...
        //                             |_____________________________|
                         ...
                         assign tcam_row_13_col_2_data_out  = ~tcam_row_13_col_2_data_out_inv;
                         hif_lme_r12_ip783tcam128x32s0c1_13_2_wrap #(        
                                 )               
        
                                 ip783tcam128x32s0c1_wrap_tcam_row_13_col_2 (
                   //-------------------------------------------------------------
                   // Power Management Interfaces
                   //-------------------------------------------------------------   
                      `ifndef INTEL_NO_PWR_PINS
                      ...
                      `endif
                        .vddp             (1'b1),
                      `endif
                         .ADR               (tcam_row_13_col_2_adr[8-1:1]), //Input adr_intess
                         .ME                (ce),  
        
                         .CLK               (tcam_row_13_col_2_clk), //Clock
                         .CMP               (tcam_row_13_col_2_chk_en), //Control Enable
                         .DIN               (~tcam_row_13_col_2_data_in), 
                         .DSEL              (~tcam_row_13_col_2_adr[0]), //dncen enables access of the TCAM Arrays adr_intessed
                         .FLS               (flush), //flush initiates a Flush operation that resets all Valid
                         .RVLD              (tcam_row_13_col_2_hit_in ), //Row validation bus. Each match_in input controls 1
                         .MASK              (tcam_row_13_col_2_mask_in), //Input msk
                         .QOUT              (tcam_row_13_col_2_data_out_inv), //Output vben
                         .QHR               (tcam_row_13_col_2_hit_out_int),
                         .QVLD              (), 
                         .RE                (tcam_row_13_col_2_rd_en), //Read Enable
                         .RST               (tcam_row_13_col_2_reset_n), //Asynchronous Active-High Reset
                         .VBE               (1'b1), //vben enables the Valid Bit addressed by adr[a-1:0] to
                         .VBI               (1'b0),//  Valid Bit Input
                         .WE                (tcam_row_13_col_2_wr_en), //Write Enable
                         .FAF               (tcam_row_13_col_2_fca),
                         .FAF_EN            (tcam_row_13_col_2_cre),
                         .DFTSHIFTEN        (tcam_row_13_col_2_dftshiften),
                         .DFTMASK           (tcam_row_13_col_2_dftmask),
                         .DFT_ARRAY_FREEZE  (tcam_row_13_col_2_dft_array_freeze),
                         .DFT_AFD_RESET_B   (tcam_row_13_col_2_dft_afd_reset_b),
                         .BISTE           (tcam_row_13_col_2_biste),
                         .TDIN (tcam_row_13_col_2_tdin),
                         .TADR (tcam_row_13_col_2_tadr),
                         .TVBE (tcam_row_13_col_2_tvbe),
                         .TVBI (tcam_row_13_col_2_tvbi),
                         .TMASK (tcam_row_13_col_2_tmask),
                         .TWE (tcam_row_13_col_2_twe),
                         .TRE (tcam_row_13_col_2_tre),
                         .TME (tcam_row_13_col_2_tme),
                         .BIST_ROTATE_SR (tcam_row_13_col_2_bist_rotate_sr),
                         .BIST_EN_SR_MASK_B (tcam_row_13_col_2_bist_en_sr_mask_b),
                         .BIST_ALL_MASK (tcam_row_13_col_2_bist_all_mask),
                         .BIST_EN_SR_DATA_B (tcam_row_13_col_2_bist_en_sr_data_b),
                         .BIST_CM_MODE (tcam_row_13_col_2_bist_cm_mode),
                         .BIST_CM_EN_IN (tcam_row_13_col_2_bist_cm_en_in),
                         .BIST_CM_MATCH_SEL0 (tcam_row_13_col_2_bist_cm_match_sel0),
                         .BIST_CM_MATCH_SEL1 (tcam_row_13_col_2_bist_cm_match_sel1),
                         .BIST_MATCHIN_IN (tcam_row_13_col_2_bist_matchin_in),
                         .BIST_FLS (tcam_row_13_col_2_bist_fls),
                         .MDST_tcam (fuse_tcam)
                     );
              ...
              begin : gen_tcam_row_13_col_2_bcam
              end
        ...
        always_ff @(posedge clk) begin
                   tcam_rd_data_col[13][2]      <= tcam_row_13_col_2_data_out;
                   tcam_col_hit_out[13][2]       <= tcam_row_13_col_2_hit_out;
        end
        ...
        //                             |_____________________________|
                         ...
                         hif_lme_r12_ip783tcam128x32s0c1_14_0_wrap #(        
                                 )               
        ...
        wire    [8-1:0]                       tcam_row_14_col_1_adr  = tcam_row_adr[14][8-1:0];
        ...
        wire    [128-1:0]                       tcam_row_14_col_1_hit_in = tcam_raw_hit_in[14];
        wire    [128-1:0]                       tcam_row_14_col_1_tcam_match_in = tcam_row_match_in[14];
        wire    [32-1:0]                      tcam_row_14_col_1_data_out_inv;
        logic   [128-1:0]                     tcam_row_14_col_1_hit_out_int;
        logic   [128-1:0]                     tcam_row_14_col_1_hit_out_latch;
        logic   [128-1:0]                     tcam_row_14_col_1_hit_out;
        ...
        wire      [8-1:0]     tcam_row_14_col_1_tadr = 'b0;
        ...
        //                             |_____________________________|
                         ...
                         hif_lme_r12_ip783tcam128x32s0c1_14_1_wrap #(        
                                 )               
        
                                 ip783tcam128x32s0c1_wrap_tcam_row_14_col_1 (
                   //-------------------------------------------------------------
                   // Power Management Interfaces
                   //-------------------------------------------------------------   
                      `ifndef INTEL_NO_PWR_PINS
                      ...
                      `endif
                        .vddp             (1'b1),
                      `endif
                         .ADR               (tcam_row_14_col_1_adr[8-1:1]), //Input adr_intess
                         .ME                (ce),  
        ...
        wire                                            tcam_row_14_col_2_clk         = clk;
        
        wire    [TCAM_SLICE_EN_FACT-1:0]                tcam_row_14_col_2_slice_en = TCAM_SLICE_EN_FACT'(slice_en[(14 * TCAM_SLICE_EN_FACT)+:TCAM_SLICE_EN_MIN]);
        
        wire                                            tcam_row_14_col_2_rd_en       = tcam_row_rd_en[14];
        wire                                            tcam_row_14_col_2_wr_en       = tcam_row_wr_en[14];
        wire                                            tcam_row_14_col_2_chk_en      = chk_en;
        wire                                            tcam_row_14_col_2_reset_n = reset_n;
        wire    [8-1:0]                       tcam_row_14_col_2_adr  = tcam_row_adr[14][8-1:0];
        wire    [32-1:0]                      tcam_row_14_col_2_data_in = tcam_row_wr_en[14] ? tcam_wr_data_col[2][32-1:0] : tcam_chk_key_col[2][32-1:0];
        wire    [32-1:0]                      tcam_row_14_col_2_data_in_pre = tcam_wr_data_col[2][32-1:0] ;
        wire    [32-1:0]                      tcam_row_14_col_2_key_in =  tcam_chk_key_col[2][32-1:0];
        wire    [32-1:0]                      tcam_row_14_col_2_mask_in = tcam_chk_mask_col[2][32-1:0];
        wire    [128-1:0]                       tcam_row_14_col_2_hit_in = tcam_raw_hit_in[14];
        wire    [128-1:0]                       tcam_row_14_col_2_tcam_match_in = tcam_row_match_in[14];
        wire    [32-1:0]                      tcam_row_14_col_2_data_out_inv;
        logic   [128-1:0]                     tcam_row_14_col_2_hit_out_int;
        logic   [128-1:0]                     tcam_row_14_col_2_hit_out_latch;
        logic   [128-1:0]                     tcam_row_14_col_2_hit_out;
        
        `ifdef HIF_PCIE_DFX_DISABLE
        wire                          tcam_row_14_col_2_biste = 'b0; 
        wire     [5-1:0]                  tcam_row_14_col_2_fca = 'b0;
        wire                            tcam_row_14_col_2_cre = 'b0;
        wire                            tcam_row_14_col_2_dftshiften = 'b0;
        wire                            tcam_row_14_col_2_dftmask = 'b0;
        wire                            tcam_row_14_col_2_dft_array_freeze = 'b0;
        wire                            tcam_row_14_col_2_dft_afd_reset_b = 'b0;
        wire      [32-1:0]    tcam_row_14_col_2_tdin = 'b0;
        wire      [8-1:0]     tcam_row_14_col_2_tadr = 'b0;
        wire                            tcam_row_14_col_2_tvbe = 'b0;
        wire                            tcam_row_14_col_2_tvbi = 'b0;
        wire      [32-1:0]    tcam_row_14_col_2_tmask = 'b0;
        wire                            tcam_row_14_col_2_twe = 'b0;
        wire                            tcam_row_14_col_2_tre = 'b0;
        wire                            tcam_row_14_col_2_tme = 'b0;
        wire                            tcam_row_14_col_2_bist_rotate_sr = 'b0;
        wire                            tcam_row_14_col_2_bist_en_sr_mask_b = 'b0;
        wire                            tcam_row_14_col_2_bist_all_mask = 'b0;
        wire                            tcam_row_14_col_2_bist_en_sr_data_b = 'b0;
        wire                            tcam_row_14_col_2_bist_cm_mode = 'b0;
        wire                            tcam_row_14_col_2_bist_cm_en_in = 'b0;
        wire                            tcam_row_14_col_2_bist_cm_match_sel0 = 'b0;
        wire                            tcam_row_14_col_2_bist_cm_match_sel1 = 'b0;
        wire                            tcam_row_14_col_2_bist_matchin_in = 'b0;
        wire                            tcam_row_14_col_2_bist_fls = 'b0;
        
        `else
        wire                            tcam_row_14_col_2_biste = 'b0;
        wire     [5-1:0]                  tcam_row_14_col_2_fca = 'b0;
        wire                            tcam_row_14_col_2_cre = 'b0;
        wire                            tcam_row_14_col_2_dftshiften = dftshiften;
        wire                            tcam_row_14_col_2_dftmask = dftmask;
        wire                            tcam_row_14_col_2_dft_array_freeze = dft_array_freeze;
        wire                            tcam_row_14_col_2_dft_afd_reset_b = dft_afd_reset_b;
        wire      [32-1:0]    tcam_row_14_col_2_tdin = 'b0;
        wire      [8-1:0]     tcam_row_14_col_2_tadr = 'b0;
        wire                            tcam_row_14_col_2_tvbe = 'b0;
        wire                            tcam_row_14_col_2_tvbi = 'b0;
        ...
        // Outputs
        logic                           tcam_row_14_col_2_wr_bira_fail;
        logic                           tcam_row_14_col_2_wr_so;
        logic                           tcam_row_14_col_2_curerrout;
        logic                           tcam_row_14_col_2_rscout;
        
        
        
        logic   [TCAM_SLICE_EN_FACT-1:0]                  tcam_row_14_col_2_slice_en_s0; 
        logic   [TCAM_SLICE_EN_FACT-1:0]                  tcam_row_14_col_2_slice_en_s1; 
        
        
        
             wire    [32-1:0]                     tcam_row_14_col_2_data_out ;
        
        
        
         generate
            if (BCAM_N7 == 0) 
              begin : gen_tcam_row_14_col_2_tcam
        ...
        // slice_en kept for TCAM only for backward compatability with Intel process
                ...
                begin
                  for (int i = 0; i < TCAM_SLICE_EN_FACT; i++)
                      if (tcam_row_14_col_2_slice_en_s1[i])
                           tcam_row_14_col_2_hit_out[ i*64 +: 64]  = tcam_row_14_col_2_hit_out_int  [ i*64 +: 64]; 
                      else
                           tcam_row_14_col_2_hit_out[ i*64 +: 64]  = tcam_row_14_col_2_hit_out_latch[ i*64 +: 64]; 
                end
        
        
        
                always_ff  @(posedge tcam_row_14_col_2_clk)
                begin
                  tcam_row_14_col_2_slice_en_s0 <= tcam_row_14_col_2_slice_en; 
                  tcam_row_14_col_2_slice_en_s1 <= tcam_row_14_col_2_slice_en_s0; 
        
                  for (int i = 0; i < TCAM_SLICE_EN_FACT; i++)
                    begin
                      if (tcam_row_14_col_2_slice_en_s1[i])
                         tcam_row_14_col_2_hit_out_latch[ i*64 +: 64] <=  tcam_row_14_col_2_hit_out_int[ i*64 +: 64];
                    end
                end
        ...
        //                             |_____________________________|
                         ...
                         assign tcam_row_14_col_2_data_out  = ~tcam_row_14_col_2_data_out_inv;
                         hif_lme_r12_ip783tcam128x32s0c1_14_2_wrap #(        
                                 )               
        
                                 ip783tcam128x32s0c1_wrap_tcam_row_14_col_2 (
                   //-------------------------------------------------------------
                   // Power Management Interfaces
                   //-------------------------------------------------------------   
                      `ifndef INTEL_NO_PWR_PINS
                      ...
                      `endif
                        .vddp             (1'b1),
                      `endif
                         .ADR               (tcam_row_14_col_2_adr[8-1:1]), //Input adr_intess
                         .ME                (ce),  
        
                         .CLK               (tcam_row_14_col_2_clk), //Clock
                         .CMP               (tcam_row_14_col_2_chk_en), //Control Enable
                         .DIN               (~tcam_row_14_col_2_data_in), 
                         .DSEL              (~tcam_row_14_col_2_adr[0]), //dncen enables access of the TCAM Arrays adr_intessed
                         .FLS               (flush), //flush initiates a Flush operation that resets all Valid
                         .RVLD              (tcam_row_14_col_2_hit_in ), //Row validation bus. Each match_in input controls 1
                         .MASK              (tcam_row_14_col_2_mask_in), //Input msk
                         .QOUT              (tcam_row_14_col_2_data_out_inv), //Output vben
                         .QHR               (tcam_row_14_col_2_hit_out_int),
                         .QVLD              (), 
                         .RE                (tcam_row_14_col_2_rd_en), //Read Enable
                         .RST               (tcam_row_14_col_2_reset_n), //Asynchronous Active-High Reset
                         .VBE               (1'b1), //vben enables the Valid Bit addressed by adr[a-1:0] to
                         .VBI               (1'b0),//  Valid Bit Input
                         .WE                (tcam_row_14_col_2_wr_en), //Write Enable
                         .FAF               (tcam_row_14_col_2_fca),
                         .FAF_EN            (tcam_row_14_col_2_cre),
                         .DFTSHIFTEN        (tcam_row_14_col_2_dftshiften),
                         .DFTMASK           (tcam_row_14_col_2_dftmask),
                         .DFT_ARRAY_FREEZE  (tcam_row_14_col_2_dft_array_freeze),
                         .DFT_AFD_RESET_B   (tcam_row_14_col_2_dft_afd_reset_b),
                         .BISTE           (tcam_row_14_col_2_biste),
                         .TDIN (tcam_row_14_col_2_tdin),
                         .TADR (tcam_row_14_col_2_tadr),
                         .TVBE (tcam_row_14_col_2_tvbe),
                         .TVBI (tcam_row_14_col_2_tvbi),
                         .TMASK (tcam_row_14_col_2_tmask),
                         .TWE (tcam_row_14_col_2_twe),
                         .TRE (tcam_row_14_col_2_tre),
                         .TME (tcam_row_14_col_2_tme),
                         .BIST_ROTATE_SR (tcam_row_14_col_2_bist_rotate_sr),
                         .BIST_EN_SR_MASK_B (tcam_row_14_col_2_bist_en_sr_mask_b),
                         .BIST_ALL_MASK (tcam_row_14_col_2_bist_all_mask),
                         .BIST_EN_SR_DATA_B (tcam_row_14_col_2_bist_en_sr_data_b),
                         .BIST_CM_MODE (tcam_row_14_col_2_bist_cm_mode),
                         .BIST_CM_EN_IN (tcam_row_14_col_2_bist_cm_en_in),
                         .BIST_CM_MATCH_SEL0 (tcam_row_14_col_2_bist_cm_match_sel0),
                         .BIST_CM_MATCH_SEL1 (tcam_row_14_col_2_bist_cm_match_sel1),
                         .BIST_MATCHIN_IN (tcam_row_14_col_2_bist_matchin_in),
                         .BIST_FLS (tcam_row_14_col_2_bist_fls),
                         .MDST_tcam (fuse_tcam)
                     );
              ...
              begin : gen_tcam_row_14_col_2_bcam
              end
        ...
        always_ff @(posedge clk) begin
                   tcam_rd_data_col[14][2]      <= tcam_row_14_col_2_data_out;
                   tcam_col_hit_out[14][2]       <= tcam_row_14_col_2_hit_out;
        end
        ...
        //                             |_____________________________|
                         ...
                         hif_lme_r12_ip783tcam128x32s0c1_15_0_wrap #(        
                                 )               
        ...
        wire    [8-1:0]                       tcam_row_15_col_1_adr  = tcam_row_adr[15][8-1:0];
        ...
        wire    [128-1:0]                       tcam_row_15_col_1_hit_in = tcam_raw_hit_in[15];
        wire    [128-1:0]                       tcam_row_15_col_1_tcam_match_in = tcam_row_match_in[15];
        wire    [32-1:0]                      tcam_row_15_col_1_data_out_inv;
        logic   [128-1:0]                     tcam_row_15_col_1_hit_out_int;
        logic   [128-1:0]                     tcam_row_15_col_1_hit_out_latch;
        logic   [128-1:0]                     tcam_row_15_col_1_hit_out;
        ...
        wire      [8-1:0]     tcam_row_15_col_1_tadr = 'b0;
        ...
        //                             |_____________________________|
                         ...
                         hif_lme_r12_ip783tcam128x32s0c1_15_1_wrap #(        
                                 )               
        
                                 ip783tcam128x32s0c1_wrap_tcam_row_15_col_1 (
                   //-------------------------------------------------------------
                   // Power Management Interfaces
                   //-------------------------------------------------------------   
                      `ifndef INTEL_NO_PWR_PINS
                      ...
                      `endif
                        .vddp             (1'b1),
                      `endif
                         .ADR               (tcam_row_15_col_1_adr[8-1:1]), //Input adr_intess
                         .ME                (ce),  
        ...
        wire                                            tcam_row_15_col_2_clk         = clk;
        
        wire    [TCAM_SLICE_EN_FACT-1:0]                tcam_row_15_col_2_slice_en = TCAM_SLICE_EN_FACT'(slice_en[(15 * TCAM_SLICE_EN_FACT)+:TCAM_SLICE_EN_MIN]);
        
        wire                                            tcam_row_15_col_2_rd_en       = tcam_row_rd_en[15];
        wire                                            tcam_row_15_col_2_wr_en       = tcam_row_wr_en[15];
        wire                                            tcam_row_15_col_2_chk_en      = chk_en;
        wire                                            tcam_row_15_col_2_reset_n = reset_n;
        wire    [8-1:0]                       tcam_row_15_col_2_adr  = tcam_row_adr[15][8-1:0];
        wire    [32-1:0]                      tcam_row_15_col_2_data_in = tcam_row_wr_en[15] ? tcam_wr_data_col[2][32-1:0] : tcam_chk_key_col[2][32-1:0];
        wire    [32-1:0]                      tcam_row_15_col_2_data_in_pre = tcam_wr_data_col[2][32-1:0] ;
        wire    [32-1:0]                      tcam_row_15_col_2_key_in =  tcam_chk_key_col[2][32-1:0];
        wire    [32-1:0]                      tcam_row_15_col_2_mask_in = tcam_chk_mask_col[2][32-1:0];
        wire    [128-1:0]                       tcam_row_15_col_2_hit_in = tcam_raw_hit_in[15];
        wire    [128-1:0]                       tcam_row_15_col_2_tcam_match_in = tcam_row_match_in[15];
        wire    [32-1:0]                      tcam_row_15_col_2_data_out_inv;
        logic   [128-1:0]                     tcam_row_15_col_2_hit_out_int;
        logic   [128-1:0]                     tcam_row_15_col_2_hit_out_latch;
        logic   [128-1:0]                     tcam_row_15_col_2_hit_out;
        
        `ifdef HIF_PCIE_DFX_DISABLE
        wire                          tcam_row_15_col_2_biste = 'b0; 
        wire     [5-1:0]                  tcam_row_15_col_2_fca = 'b0;
        wire                            tcam_row_15_col_2_cre = 'b0;
        wire                            tcam_row_15_col_2_dftshiften = 'b0;
        wire                            tcam_row_15_col_2_dftmask = 'b0;
        wire                            tcam_row_15_col_2_dft_array_freeze = 'b0;
        wire                            tcam_row_15_col_2_dft_afd_reset_b = 'b0;
        wire      [32-1:0]    tcam_row_15_col_2_tdin = 'b0;
        wire      [8-1:0]     tcam_row_15_col_2_tadr = 'b0;
        wire                            tcam_row_15_col_2_tvbe = 'b0;
        wire                            tcam_row_15_col_2_tvbi = 'b0;
        wire      [32-1:0]    tcam_row_15_col_2_tmask = 'b0;
        wire                            tcam_row_15_col_2_twe = 'b0;
        wire                            tcam_row_15_col_2_tre = 'b0;
        wire                            tcam_row_15_col_2_tme = 'b0;
        wire                            tcam_row_15_col_2_bist_rotate_sr = 'b0;
        wire                            tcam_row_15_col_2_bist_en_sr_mask_b = 'b0;
        wire                            tcam_row_15_col_2_bist_all_mask = 'b0;
        wire                            tcam_row_15_col_2_bist_en_sr_data_b = 'b0;
        wire                            tcam_row_15_col_2_bist_cm_mode = 'b0;
        wire                            tcam_row_15_col_2_bist_cm_en_in = 'b0;
        wire                            tcam_row_15_col_2_bist_cm_match_sel0 = 'b0;
        wire                            tcam_row_15_col_2_bist_cm_match_sel1 = 'b0;
        wire                            tcam_row_15_col_2_bist_matchin_in = 'b0;
        wire                            tcam_row_15_col_2_bist_fls = 'b0;
        
        `else
        wire                            tcam_row_15_col_2_biste = 'b0;
        wire     [5-1:0]                  tcam_row_15_col_2_fca = 'b0;
        wire                            tcam_row_15_col_2_cre = 'b0;
        wire                            tcam_row_15_col_2_dftshiften = dftshiften;
        wire                            tcam_row_15_col_2_dftmask = dftmask;
        wire                            tcam_row_15_col_2_dft_array_freeze = dft_array_freeze;
        wire                            tcam_row_15_col_2_dft_afd_reset_b = dft_afd_reset_b;
        wire      [32-1:0]    tcam_row_15_col_2_tdin = 'b0;
        wire      [8-1:0]     tcam_row_15_col_2_tadr = 'b0;
        wire                            tcam_row_15_col_2_tvbe = 'b0;
        wire                            tcam_row_15_col_2_tvbi = 'b0;
        ...
        // Outputs
        logic                           tcam_row_15_col_2_wr_bira_fail;
        logic                           tcam_row_15_col_2_wr_so;
        logic                           tcam_row_15_col_2_curerrout;
        logic                           tcam_row_15_col_2_rscout;
        
        
        
        logic   [TCAM_SLICE_EN_FACT-1:0]                  tcam_row_15_col_2_slice_en_s0; 
        logic   [TCAM_SLICE_EN_FACT-1:0]                  tcam_row_15_col_2_slice_en_s1; 
        
        
        
             wire    [32-1:0]                     tcam_row_15_col_2_data_out ;
        
        
        
         generate
            if (BCAM_N7 == 0) 
              begin : gen_tcam_row_15_col_2_tcam
        ...
        // slice_en kept for TCAM only for backward compatability with Intel process
                ...
                begin
                  for (int i = 0; i < TCAM_SLICE_EN_FACT; i++)
                      if (tcam_row_15_col_2_slice_en_s1[i])
                           tcam_row_15_col_2_hit_out[ i*64 +: 64]  = tcam_row_15_col_2_hit_out_int  [ i*64 +: 64]; 
                      else
                           tcam_row_15_col_2_hit_out[ i*64 +: 64]  = tcam_row_15_col_2_hit_out_latch[ i*64 +: 64]; 
                end
        
        
        
                always_ff  @(posedge tcam_row_15_col_2_clk)
                begin
                  tcam_row_15_col_2_slice_en_s0 <= tcam_row_15_col_2_slice_en; 
                  tcam_row_15_col_2_slice_en_s1 <= tcam_row_15_col_2_slice_en_s0; 
        
                  for (int i = 0; i < TCAM_SLICE_EN_FACT; i++)
                    begin
                      if (tcam_row_15_col_2_slice_en_s1[i])
                         tcam_row_15_col_2_hit_out_latch[ i*64 +: 64] <=  tcam_row_15_col_2_hit_out_int[ i*64 +: 64];
                    end
                end
        ...
        //                             |_____________________________|
                         ...
                         assign tcam_row_15_col_2_data_out  = ~tcam_row_15_col_2_data_out_inv;
                         hif_lme_r12_ip783tcam128x32s0c1_15_2_wrap #(        
                                 )               
        
                                 ip783tcam128x32s0c1_wrap_tcam_row_15_col_2 (
                   //-------------------------------------------------------------
                   // Power Management Interfaces
                   //-------------------------------------------------------------   
                      `ifndef INTEL_NO_PWR_PINS
                      ...
                      `endif
                        .vddp             (1'b1),
                      `endif
                         .ADR               (tcam_row_15_col_2_adr[8-1:1]), //Input adr_intess
                         .ME                (ce),  
        
                         .CLK               (tcam_row_15_col_2_clk), //Clock
                         .CMP               (tcam_row_15_col_2_chk_en), //Control Enable
                         .DIN               (~tcam_row_15_col_2_data_in), 
                         .DSEL              (~tcam_row_15_col_2_adr[0]), //dncen enables access of the TCAM Arrays adr_intessed
                         .FLS               (flush), //flush initiates a Flush operation that resets all Valid
                         .RVLD              (tcam_row_15_col_2_hit_in ), //Row validation bus. Each match_in input controls 1
                         .MASK              (tcam_row_15_col_2_mask_in), //Input msk
                         .QOUT              (tcam_row_15_col_2_data_out_inv), //Output vben
                         .QHR               (tcam_row_15_col_2_hit_out_int),
                         .QVLD              (), 
                         .RE                (tcam_row_15_col_2_rd_en), //Read Enable
                         .RST               (tcam_row_15_col_2_reset_n), //Asynchronous Active-High Reset
                         .VBE               (1'b1), //vben enables the Valid Bit addressed by adr[a-1:0] to
                         .VBI               (1'b0),//  Valid Bit Input
                         .WE                (tcam_row_15_col_2_wr_en), //Write Enable
                         .FAF               (tcam_row_15_col_2_fca),
                         .FAF_EN            (tcam_row_15_col_2_cre),
                         .DFTSHIFTEN        (tcam_row_15_col_2_dftshiften),
                         .DFTMASK           (tcam_row_15_col_2_dftmask),
                         .DFT_ARRAY_FREEZE  (tcam_row_15_col_2_dft_array_freeze),
                         .DFT_AFD_RESET_B   (tcam_row_15_col_2_dft_afd_reset_b),
                         .BISTE           (tcam_row_15_col_2_biste),
                         .TDIN (tcam_row_15_col_2_tdin),
                         .TADR (tcam_row_15_col_2_tadr),
                         .TVBE (tcam_row_15_col_2_tvbe),
                         .TVBI (tcam_row_15_col_2_tvbi),
                         .TMASK (tcam_row_15_col_2_tmask),
                         .TWE (tcam_row_15_col_2_twe),
                         .TRE (tcam_row_15_col_2_tre),
                         .TME (tcam_row_15_col_2_tme),
                         .BIST_ROTATE_SR (tcam_row_15_col_2_bist_rotate_sr),
                         .BIST_EN_SR_MASK_B (tcam_row_15_col_2_bist_en_sr_mask_b),
                         .BIST_ALL_MASK (tcam_row_15_col_2_bist_all_mask),
                         .BIST_EN_SR_DATA_B (tcam_row_15_col_2_bist_en_sr_data_b),
                         .BIST_CM_MODE (tcam_row_15_col_2_bist_cm_mode),
                         .BIST_CM_EN_IN (tcam_row_15_col_2_bist_cm_en_in),
                         .BIST_CM_MATCH_SEL0 (tcam_row_15_col_2_bist_cm_match_sel0),
                         .BIST_CM_MATCH_SEL1 (tcam_row_15_col_2_bist_cm_match_sel1),
                         .BIST_MATCHIN_IN (tcam_row_15_col_2_bist_matchin_in),
                         .BIST_FLS (tcam_row_15_col_2_bist_fls),
                         .MDST_tcam (fuse_tcam)
                     );
              ...
              begin : gen_tcam_row_15_col_2_bcam
              end
        ...
        always_ff @(posedge clk) begin
                   tcam_rd_data_col[15][2]      <= tcam_row_15_col_2_data_out;
                   tcam_col_hit_out[15][2]       <= tcam_row_15_col_2_hit_out;
        end
    ...
    if (BCAM_N7 == 1) 
        begin : bacm_prot_calc
            always_comb begin
                ...
                for (int i = 0; i < 3; i = i + 1) begin
                    rd_data[PHYSICAL_ROW_WIDTH-((3-i-1)*2)-2] = ^rd_data_before_prot_calc[(i*(32-2))+:((32-2)/2)];
                    rd_data[PHYSICAL_ROW_WIDTH-((3-i-1)*2)-1] = ^rd_data_before_prot_calc[(i*(32-2))+((32-2)/2)+:((32-2)/2)];
                end
            end
        end
    else
...
parameter       MEM_SEG_PADDING_W = 5 ; 
    ...
    generate if (BCAM_N7==0 && TCAM_PROTECTION_TYPE==1 && BCAM_RELIEF==0) begin : protect    
       ...
       task automatic update_col_sweeper(input bit pattern, input logic[91-1:0] old_val, input logic[91-1:0] new_val);
           logic [91-1:0] temp;
    ...
    // ********** this task is valid for TCAM only *****
      task automatic write_tcam_raw_key (input logic[91-1:0] key, input int row_idx);
          reg temp1,temp2;
          logic [(3 * 32) - 1:0] local_key = '1;
          logic [(3 * 32) - 1:0] prev_key_data;
          local_key[91-1:0] = TCAM_18A ? ~key : ~key;
          ...
          `ifdef HIF_PCIE_HIF_LME_R12_TCAM_BEHAVE_MEMS
             `ifndef HIF_PCIE_FPGA_TCAM_MEMS
         ...
         for (int j = 0;j < 3 ; j = j + 1) 
             case (j)
                                    ...
                                    1: begin
                                            //write_mask
                                             gen_tcam_row_1_col_1_tcam.ip783tcam128x32s0c1_wrap_tcam_row_1_col_1.tcam_row_1_col_1.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_x[row_idx % 128] = local_key[1*(32 -2*BCAM_N7) +: 32-2*BCAM_N7];
                                             gen_tcam_row_1_col_1_tcam.ip783tcam128x32s0c1_wrap_tcam_row_1_col_1.tcam_row_1_col_1.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_v[row_idx % 128] = 1'b1;
                                    end
                                    2: begin
                                            //write_mask
                                             gen_tcam_row_1_col_2_tcam.ip783tcam128x32s0c1_wrap_tcam_row_1_col_2.tcam_row_1_col_2.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_x[row_idx % 128] = local_key[2*(32 -2*BCAM_N7) +: 32-2*BCAM_N7];
                                             gen_tcam_row_1_col_2_tcam.ip783tcam128x32s0c1_wrap_tcam_row_1_col_2.tcam_row_1_col_2.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_v[row_idx % 128] = 1'b1;
                                    end
                                    default : begin
                                       $error("SOMTING IS wrong with parameters given for col number %d ",j);
                                     end
         ...
         for (int j = 0;j < 3 ; j = j + 1) 
             case (j)
                                    ...
                                    1: begin
                                            //write_mask
                                             gen_tcam_row_2_col_1_tcam.ip783tcam128x32s0c1_wrap_tcam_row_2_col_1.tcam_row_2_col_1.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_x[row_idx % 128] = local_key[1*(32 -2*BCAM_N7) +: 32-2*BCAM_N7];
                                             gen_tcam_row_2_col_1_tcam.ip783tcam128x32s0c1_wrap_tcam_row_2_col_1.tcam_row_2_col_1.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_v[row_idx % 128] = 1'b1;
                                    end
                                    2: begin
                                            //write_mask
                                             gen_tcam_row_2_col_2_tcam.ip783tcam128x32s0c1_wrap_tcam_row_2_col_2.tcam_row_2_col_2.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_x[row_idx % 128] = local_key[2*(32 -2*BCAM_N7) +: 32-2*BCAM_N7];
                                             gen_tcam_row_2_col_2_tcam.ip783tcam128x32s0c1_wrap_tcam_row_2_col_2.tcam_row_2_col_2.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_v[row_idx % 128] = 1'b1;
                                    end
                                    default : begin
                                       $error("SOMTING IS wrong with parameters given for col number %d ",j);
                                     end
         ...
         for (int j = 0;j < 3 ; j = j + 1) 
             case (j)
                                    ...
                                    1: begin
                                            //write_mask
                                             gen_tcam_row_3_col_1_tcam.ip783tcam128x32s0c1_wrap_tcam_row_3_col_1.tcam_row_3_col_1.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_x[row_idx % 128] = local_key[1*(32 -2*BCAM_N7) +: 32-2*BCAM_N7];
                                             gen_tcam_row_3_col_1_tcam.ip783tcam128x32s0c1_wrap_tcam_row_3_col_1.tcam_row_3_col_1.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_v[row_idx % 128] = 1'b1;
                                    end
                                    2: begin
                                            //write_mask
                                             gen_tcam_row_3_col_2_tcam.ip783tcam128x32s0c1_wrap_tcam_row_3_col_2.tcam_row_3_col_2.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_x[row_idx % 128] = local_key[2*(32 -2*BCAM_N7) +: 32-2*BCAM_N7];
                                             gen_tcam_row_3_col_2_tcam.ip783tcam128x32s0c1_wrap_tcam_row_3_col_2.tcam_row_3_col_2.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_v[row_idx % 128] = 1'b1;
                                    end
                                    default : begin
                                       $error("SOMTING IS wrong with parameters given for col number %d ",j);
                                     end
         ...
         for (int j = 0;j < 3 ; j = j + 1) 
             case (j)
                                    ...
                                    1: begin
                                            //write_mask
                                             gen_tcam_row_4_col_1_tcam.ip783tcam128x32s0c1_wrap_tcam_row_4_col_1.tcam_row_4_col_1.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_x[row_idx % 128] = local_key[1*(32 -2*BCAM_N7) +: 32-2*BCAM_N7];
                                             gen_tcam_row_4_col_1_tcam.ip783tcam128x32s0c1_wrap_tcam_row_4_col_1.tcam_row_4_col_1.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_v[row_idx % 128] = 1'b1;
                                    end
                                    2: begin
                                            //write_mask
                                             gen_tcam_row_4_col_2_tcam.ip783tcam128x32s0c1_wrap_tcam_row_4_col_2.tcam_row_4_col_2.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_x[row_idx % 128] = local_key[2*(32 -2*BCAM_N7) +: 32-2*BCAM_N7];
                                             gen_tcam_row_4_col_2_tcam.ip783tcam128x32s0c1_wrap_tcam_row_4_col_2.tcam_row_4_col_2.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_v[row_idx % 128] = 1'b1;
                                    end
                                    default : begin
                                       $error("SOMTING IS wrong with parameters given for col number %d ",j);
                                     end
         ...
         for (int j = 0;j < 3 ; j = j + 1) 
             case (j)
                                    ...
                                    1: begin
                                            //write_mask
                                             gen_tcam_row_5_col_1_tcam.ip783tcam128x32s0c1_wrap_tcam_row_5_col_1.tcam_row_5_col_1.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_x[row_idx % 128] = local_key[1*(32 -2*BCAM_N7) +: 32-2*BCAM_N7];
                                             gen_tcam_row_5_col_1_tcam.ip783tcam128x32s0c1_wrap_tcam_row_5_col_1.tcam_row_5_col_1.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_v[row_idx % 128] = 1'b1;
                                    end
                                    2: begin
                                            //write_mask
                                             gen_tcam_row_5_col_2_tcam.ip783tcam128x32s0c1_wrap_tcam_row_5_col_2.tcam_row_5_col_2.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_x[row_idx % 128] = local_key[2*(32 -2*BCAM_N7) +: 32-2*BCAM_N7];
                                             gen_tcam_row_5_col_2_tcam.ip783tcam128x32s0c1_wrap_tcam_row_5_col_2.tcam_row_5_col_2.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_v[row_idx % 128] = 1'b1;
                                    end
                                    default : begin
                                       $error("SOMTING IS wrong with parameters given for col number %d ",j);
                                     end
         ...
         for (int j = 0;j < 3 ; j = j + 1) 
             case (j)
                                    ...
                                    1: begin
                                            //write_mask
                                             gen_tcam_row_6_col_1_tcam.ip783tcam128x32s0c1_wrap_tcam_row_6_col_1.tcam_row_6_col_1.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_x[row_idx % 128] = local_key[1*(32 -2*BCAM_N7) +: 32-2*BCAM_N7];
                                             gen_tcam_row_6_col_1_tcam.ip783tcam128x32s0c1_wrap_tcam_row_6_col_1.tcam_row_6_col_1.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_v[row_idx % 128] = 1'b1;
                                    end
                                    2: begin
                                            //write_mask
                                             gen_tcam_row_6_col_2_tcam.ip783tcam128x32s0c1_wrap_tcam_row_6_col_2.tcam_row_6_col_2.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_x[row_idx % 128] = local_key[2*(32 -2*BCAM_N7) +: 32-2*BCAM_N7];
                                             gen_tcam_row_6_col_2_tcam.ip783tcam128x32s0c1_wrap_tcam_row_6_col_2.tcam_row_6_col_2.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_v[row_idx % 128] = 1'b1;
                                    end
                                    default : begin
                                       $error("SOMTING IS wrong with parameters given for col number %d ",j);
                                     end
         ...
         for (int j = 0;j < 3 ; j = j + 1) 
             case (j)
                                    ...
                                    1: begin
                                            //write_mask
                                             gen_tcam_row_7_col_1_tcam.ip783tcam128x32s0c1_wrap_tcam_row_7_col_1.tcam_row_7_col_1.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_x[row_idx % 128] = local_key[1*(32 -2*BCAM_N7) +: 32-2*BCAM_N7];
                                             gen_tcam_row_7_col_1_tcam.ip783tcam128x32s0c1_wrap_tcam_row_7_col_1.tcam_row_7_col_1.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_v[row_idx % 128] = 1'b1;
                                    end
                                    2: begin
                                            //write_mask
                                             gen_tcam_row_7_col_2_tcam.ip783tcam128x32s0c1_wrap_tcam_row_7_col_2.tcam_row_7_col_2.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_x[row_idx % 128] = local_key[2*(32 -2*BCAM_N7) +: 32-2*BCAM_N7];
                                             gen_tcam_row_7_col_2_tcam.ip783tcam128x32s0c1_wrap_tcam_row_7_col_2.tcam_row_7_col_2.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_v[row_idx % 128] = 1'b1;
                                    end
                                    default : begin
                                       $error("SOMTING IS wrong with parameters given for col number %d ",j);
                                     end
         ...
         for (int j = 0;j < 3 ; j = j + 1) 
             case (j)
                                    ...
                                    1: begin
                                            //write_mask
                                             gen_tcam_row_8_col_1_tcam.ip783tcam128x32s0c1_wrap_tcam_row_8_col_1.tcam_row_8_col_1.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_x[row_idx % 128] = local_key[1*(32 -2*BCAM_N7) +: 32-2*BCAM_N7];
                                             gen_tcam_row_8_col_1_tcam.ip783tcam128x32s0c1_wrap_tcam_row_8_col_1.tcam_row_8_col_1.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_v[row_idx % 128] = 1'b1;
                                    end
                                    2: begin
                                            //write_mask
                                             gen_tcam_row_8_col_2_tcam.ip783tcam128x32s0c1_wrap_tcam_row_8_col_2.tcam_row_8_col_2.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_x[row_idx % 128] = local_key[2*(32 -2*BCAM_N7) +: 32-2*BCAM_N7];
                                             gen_tcam_row_8_col_2_tcam.ip783tcam128x32s0c1_wrap_tcam_row_8_col_2.tcam_row_8_col_2.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_v[row_idx % 128] = 1'b1;
                                    end
                                    default : begin
                                       $error("SOMTING IS wrong with parameters given for col number %d ",j);
                                     end
         ...
         for (int j = 0;j < 3 ; j = j + 1) 
             case (j)
                                    ...
                                    1: begin
                                            //write_mask
                                             gen_tcam_row_9_col_1_tcam.ip783tcam128x32s0c1_wrap_tcam_row_9_col_1.tcam_row_9_col_1.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_x[row_idx % 128] = local_key[1*(32 -2*BCAM_N7) +: 32-2*BCAM_N7];
                                             gen_tcam_row_9_col_1_tcam.ip783tcam128x32s0c1_wrap_tcam_row_9_col_1.tcam_row_9_col_1.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_v[row_idx % 128] = 1'b1;
                                    end
                                    2: begin
                                            //write_mask
                                             gen_tcam_row_9_col_2_tcam.ip783tcam128x32s0c1_wrap_tcam_row_9_col_2.tcam_row_9_col_2.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_x[row_idx % 128] = local_key[2*(32 -2*BCAM_N7) +: 32-2*BCAM_N7];
                                             gen_tcam_row_9_col_2_tcam.ip783tcam128x32s0c1_wrap_tcam_row_9_col_2.tcam_row_9_col_2.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_v[row_idx % 128] = 1'b1;
                                    end
                                    default : begin
                                       $error("SOMTING IS wrong with parameters given for col number %d ",j);
                                     end
         ...
         for (int j = 0;j < 3 ; j = j + 1) 
             case (j)
                                    ...
                                    1: begin
                                            //write_mask
                                             gen_tcam_row_10_col_1_tcam.ip783tcam128x32s0c1_wrap_tcam_row_10_col_1.tcam_row_10_col_1.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_x[row_idx % 128] = local_key[1*(32 -2*BCAM_N7) +: 32-2*BCAM_N7];
                                             gen_tcam_row_10_col_1_tcam.ip783tcam128x32s0c1_wrap_tcam_row_10_col_1.tcam_row_10_col_1.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_v[row_idx % 128] = 1'b1;
                                    end
                                    2: begin
                                            //write_mask
                                             gen_tcam_row_10_col_2_tcam.ip783tcam128x32s0c1_wrap_tcam_row_10_col_2.tcam_row_10_col_2.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_x[row_idx % 128] = local_key[2*(32 -2*BCAM_N7) +: 32-2*BCAM_N7];
                                             gen_tcam_row_10_col_2_tcam.ip783tcam128x32s0c1_wrap_tcam_row_10_col_2.tcam_row_10_col_2.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_v[row_idx % 128] = 1'b1;
                                    end
                                    default : begin
                                       $error("SOMTING IS wrong with parameters given for col number %d ",j);
                                     end
         ...
         for (int j = 0;j < 3 ; j = j + 1) 
             case (j)
                                    ...
                                    1: begin
                                            //write_mask
                                             gen_tcam_row_11_col_1_tcam.ip783tcam128x32s0c1_wrap_tcam_row_11_col_1.tcam_row_11_col_1.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_x[row_idx % 128] = local_key[1*(32 -2*BCAM_N7) +: 32-2*BCAM_N7];
                                             gen_tcam_row_11_col_1_tcam.ip783tcam128x32s0c1_wrap_tcam_row_11_col_1.tcam_row_11_col_1.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_v[row_idx % 128] = 1'b1;
                                    end
                                    2: begin
                                            //write_mask
                                             gen_tcam_row_11_col_2_tcam.ip783tcam128x32s0c1_wrap_tcam_row_11_col_2.tcam_row_11_col_2.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_x[row_idx % 128] = local_key[2*(32 -2*BCAM_N7) +: 32-2*BCAM_N7];
                                             gen_tcam_row_11_col_2_tcam.ip783tcam128x32s0c1_wrap_tcam_row_11_col_2.tcam_row_11_col_2.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_v[row_idx % 128] = 1'b1;
                                    end
                                    default : begin
                                       $error("SOMTING IS wrong with parameters given for col number %d ",j);
                                     end
         ...
         for (int j = 0;j < 3 ; j = j + 1) 
             case (j)
                                    ...
                                    1: begin
                                            //write_mask
                                             gen_tcam_row_12_col_1_tcam.ip783tcam128x32s0c1_wrap_tcam_row_12_col_1.tcam_row_12_col_1.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_x[row_idx % 128] = local_key[1*(32 -2*BCAM_N7) +: 32-2*BCAM_N7];
                                             gen_tcam_row_12_col_1_tcam.ip783tcam128x32s0c1_wrap_tcam_row_12_col_1.tcam_row_12_col_1.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_v[row_idx % 128] = 1'b1;
                                    end
                                    2: begin
                                            //write_mask
                                             gen_tcam_row_12_col_2_tcam.ip783tcam128x32s0c1_wrap_tcam_row_12_col_2.tcam_row_12_col_2.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_x[row_idx % 128] = local_key[2*(32 -2*BCAM_N7) +: 32-2*BCAM_N7];
                                             gen_tcam_row_12_col_2_tcam.ip783tcam128x32s0c1_wrap_tcam_row_12_col_2.tcam_row_12_col_2.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_v[row_idx % 128] = 1'b1;
                                    end
                                    default : begin
                                       $error("SOMTING IS wrong with parameters given for col number %d ",j);
                                     end
         ...
         for (int j = 0;j < 3 ; j = j + 1) 
             case (j)
                                    ...
                                    1: begin
                                            //write_mask
                                             gen_tcam_row_13_col_1_tcam.ip783tcam128x32s0c1_wrap_tcam_row_13_col_1.tcam_row_13_col_1.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_x[row_idx % 128] = local_key[1*(32 -2*BCAM_N7) +: 32-2*BCAM_N7];
                                             gen_tcam_row_13_col_1_tcam.ip783tcam128x32s0c1_wrap_tcam_row_13_col_1.tcam_row_13_col_1.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_v[row_idx % 128] = 1'b1;
                                    end
                                    2: begin
                                            //write_mask
                                             gen_tcam_row_13_col_2_tcam.ip783tcam128x32s0c1_wrap_tcam_row_13_col_2.tcam_row_13_col_2.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_x[row_idx % 128] = local_key[2*(32 -2*BCAM_N7) +: 32-2*BCAM_N7];
                                             gen_tcam_row_13_col_2_tcam.ip783tcam128x32s0c1_wrap_tcam_row_13_col_2.tcam_row_13_col_2.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_v[row_idx % 128] = 1'b1;
                                    end
                                    default : begin
                                       $error("SOMTING IS wrong with parameters given for col number %d ",j);
                                     end
         ...
         for (int j = 0;j < 3 ; j = j + 1) 
             case (j)
                                    ...
                                    1: begin
                                            //write_mask
                                             gen_tcam_row_14_col_1_tcam.ip783tcam128x32s0c1_wrap_tcam_row_14_col_1.tcam_row_14_col_1.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_x[row_idx % 128] = local_key[1*(32 -2*BCAM_N7) +: 32-2*BCAM_N7];
                                             gen_tcam_row_14_col_1_tcam.ip783tcam128x32s0c1_wrap_tcam_row_14_col_1.tcam_row_14_col_1.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_v[row_idx % 128] = 1'b1;
                                    end
                                    2: begin
                                            //write_mask
                                             gen_tcam_row_14_col_2_tcam.ip783tcam128x32s0c1_wrap_tcam_row_14_col_2.tcam_row_14_col_2.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_x[row_idx % 128] = local_key[2*(32 -2*BCAM_N7) +: 32-2*BCAM_N7];
                                             gen_tcam_row_14_col_2_tcam.ip783tcam128x32s0c1_wrap_tcam_row_14_col_2.tcam_row_14_col_2.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_v[row_idx % 128] = 1'b1;
                                    end
                                    default : begin
                                       $error("SOMTING IS wrong with parameters given for col number %d ",j);
                                     end
         ...
         for (int j = 0;j < 3 ; j = j + 1) 
             case (j)
                                    ...
                                    1: begin
                                            //write_mask
                                             gen_tcam_row_15_col_1_tcam.ip783tcam128x32s0c1_wrap_tcam_row_15_col_1.tcam_row_15_col_1.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_x[row_idx % 128] = local_key[1*(32 -2*BCAM_N7) +: 32-2*BCAM_N7];
                                             gen_tcam_row_15_col_1_tcam.ip783tcam128x32s0c1_wrap_tcam_row_15_col_1.tcam_row_15_col_1.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_v[row_idx % 128] = 1'b1;
                                    end
                                    2: begin
                                            //write_mask
                                             gen_tcam_row_15_col_2_tcam.ip783tcam128x32s0c1_wrap_tcam_row_15_col_2.tcam_row_15_col_2.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_x[row_idx % 128] = local_key[2*(32 -2*BCAM_N7) +: 32-2*BCAM_N7];
                                             gen_tcam_row_15_col_2_tcam.ip783tcam128x32s0c1_wrap_tcam_row_15_col_2.tcam_row_15_col_2.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_v[row_idx % 128] = 1'b1;
                                    end
                                    default : begin
                                       $error("SOMTING IS wrong with parameters given for col number %d ",j);
                                     end
       ...
       // ********** backdoor DATA ***********
        task automatic write_tcam_raw_inv_key (input logic[91-1:0] inv_key, input int row_idx);
          reg temp1,temp2;
          logic [(3 * 32) - 1:0] local_inv_key = '0;
          logic [(3 * 32) - 1:0] prev_inv_key_data;
          local_inv_key[91-1:0] = TCAM_18A ? ~inv_key : ~inv_key;
         ...
         for (int j = 0;j < 3 ; j = j + 1) 
             case (j)
                                    ...
                                    1: begin
                                            //write_DATA
                                             gen_tcam_row_1_col_1_tcam.ip783tcam128x32s0c1_wrap_tcam_row_1_col_1.tcam_row_1_col_1.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_y[row_idx % 128] = local_inv_key[1*(32 -2*BCAM_N7) +: 32-2*BCAM_N7];
                                             gen_tcam_row_1_col_1_tcam.ip783tcam128x32s0c1_wrap_tcam_row_1_col_1.tcam_row_1_col_1.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_v[row_idx % 128] = 1'b1;
                                    end
                                    2: begin
                                            //write_DATA
                                             gen_tcam_row_1_col_2_tcam.ip783tcam128x32s0c1_wrap_tcam_row_1_col_2.tcam_row_1_col_2.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_y[row_idx % 128] = local_inv_key[2*(32 -2*BCAM_N7) +: 32-2*BCAM_N7];
                                             gen_tcam_row_1_col_2_tcam.ip783tcam128x32s0c1_wrap_tcam_row_1_col_2.tcam_row_1_col_2.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_v[row_idx % 128] = 1'b1;
                                    end
                                    default : begin
                                       $error("SOMTING IS wrong with parameters given for col number %d ",j);
         ...
         for (int j = 0;j < 3 ; j = j + 1) 
             case (j)
                                    ...
                                    1: begin
                                            //write_DATA
                                             gen_tcam_row_2_col_1_tcam.ip783tcam128x32s0c1_wrap_tcam_row_2_col_1.tcam_row_2_col_1.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_y[row_idx % 128] = local_inv_key[1*(32 -2*BCAM_N7) +: 32-2*BCAM_N7];
                                             gen_tcam_row_2_col_1_tcam.ip783tcam128x32s0c1_wrap_tcam_row_2_col_1.tcam_row_2_col_1.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_v[row_idx % 128] = 1'b1;
                                    end
                                    2: begin
                                            //write_DATA
                                             gen_tcam_row_2_col_2_tcam.ip783tcam128x32s0c1_wrap_tcam_row_2_col_2.tcam_row_2_col_2.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_y[row_idx % 128] = local_inv_key[2*(32 -2*BCAM_N7) +: 32-2*BCAM_N7];
                                             gen_tcam_row_2_col_2_tcam.ip783tcam128x32s0c1_wrap_tcam_row_2_col_2.tcam_row_2_col_2.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_v[row_idx % 128] = 1'b1;
                                    end
                                    default : begin
                                       $error("SOMTING IS wrong with parameters given for col number %d ",j);
         ...
         for (int j = 0;j < 3 ; j = j + 1) 
             case (j)
                                    ...
                                    1: begin
                                            //write_DATA
                                             gen_tcam_row_3_col_1_tcam.ip783tcam128x32s0c1_wrap_tcam_row_3_col_1.tcam_row_3_col_1.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_y[row_idx % 128] = local_inv_key[1*(32 -2*BCAM_N7) +: 32-2*BCAM_N7];
                                             gen_tcam_row_3_col_1_tcam.ip783tcam128x32s0c1_wrap_tcam_row_3_col_1.tcam_row_3_col_1.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_v[row_idx % 128] = 1'b1;
                                    end
                                    2: begin
                                            //write_DATA
                                             gen_tcam_row_3_col_2_tcam.ip783tcam128x32s0c1_wrap_tcam_row_3_col_2.tcam_row_3_col_2.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_y[row_idx % 128] = local_inv_key[2*(32 -2*BCAM_N7) +: 32-2*BCAM_N7];
                                             gen_tcam_row_3_col_2_tcam.ip783tcam128x32s0c1_wrap_tcam_row_3_col_2.tcam_row_3_col_2.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_v[row_idx % 128] = 1'b1;
                                    end
                                    default : begin
                                       $error("SOMTING IS wrong with parameters given for col number %d ",j);
         ...
         for (int j = 0;j < 3 ; j = j + 1) 
             case (j)
                                    ...
                                    1: begin
                                            //write_DATA
                                             gen_tcam_row_4_col_1_tcam.ip783tcam128x32s0c1_wrap_tcam_row_4_col_1.tcam_row_4_col_1.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_y[row_idx % 128] = local_inv_key[1*(32 -2*BCAM_N7) +: 32-2*BCAM_N7];
                                             gen_tcam_row_4_col_1_tcam.ip783tcam128x32s0c1_wrap_tcam_row_4_col_1.tcam_row_4_col_1.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_v[row_idx % 128] = 1'b1;
                                    end
                                    2: begin
                                            //write_DATA
                                             gen_tcam_row_4_col_2_tcam.ip783tcam128x32s0c1_wrap_tcam_row_4_col_2.tcam_row_4_col_2.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_y[row_idx % 128] = local_inv_key[2*(32 -2*BCAM_N7) +: 32-2*BCAM_N7];
                                             gen_tcam_row_4_col_2_tcam.ip783tcam128x32s0c1_wrap_tcam_row_4_col_2.tcam_row_4_col_2.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_v[row_idx % 128] = 1'b1;
                                    end
                                    default : begin
                                       $error("SOMTING IS wrong with parameters given for col number %d ",j);
         ...
         for (int j = 0;j < 3 ; j = j + 1) 
             case (j)
                                    ...
                                    1: begin
                                            //write_DATA
                                             gen_tcam_row_5_col_1_tcam.ip783tcam128x32s0c1_wrap_tcam_row_5_col_1.tcam_row_5_col_1.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_y[row_idx % 128] = local_inv_key[1*(32 -2*BCAM_N7) +: 32-2*BCAM_N7];
                                             gen_tcam_row_5_col_1_tcam.ip783tcam128x32s0c1_wrap_tcam_row_5_col_1.tcam_row_5_col_1.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_v[row_idx % 128] = 1'b1;
                                    end
                                    2: begin
                                            //write_DATA
                                             gen_tcam_row_5_col_2_tcam.ip783tcam128x32s0c1_wrap_tcam_row_5_col_2.tcam_row_5_col_2.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_y[row_idx % 128] = local_inv_key[2*(32 -2*BCAM_N7) +: 32-2*BCAM_N7];
                                             gen_tcam_row_5_col_2_tcam.ip783tcam128x32s0c1_wrap_tcam_row_5_col_2.tcam_row_5_col_2.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_v[row_idx % 128] = 1'b1;
                                    end
                                    default : begin
                                       $error("SOMTING IS wrong with parameters given for col number %d ",j);
         ...
         for (int j = 0;j < 3 ; j = j + 1) 
             case (j)
                                    ...
                                    1: begin
                                            //write_DATA
                                             gen_tcam_row_6_col_1_tcam.ip783tcam128x32s0c1_wrap_tcam_row_6_col_1.tcam_row_6_col_1.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_y[row_idx % 128] = local_inv_key[1*(32 -2*BCAM_N7) +: 32-2*BCAM_N7];
                                             gen_tcam_row_6_col_1_tcam.ip783tcam128x32s0c1_wrap_tcam_row_6_col_1.tcam_row_6_col_1.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_v[row_idx % 128] = 1'b1;
                                    end
                                    2: begin
                                            //write_DATA
                                             gen_tcam_row_6_col_2_tcam.ip783tcam128x32s0c1_wrap_tcam_row_6_col_2.tcam_row_6_col_2.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_y[row_idx % 128] = local_inv_key[2*(32 -2*BCAM_N7) +: 32-2*BCAM_N7];
                                             gen_tcam_row_6_col_2_tcam.ip783tcam128x32s0c1_wrap_tcam_row_6_col_2.tcam_row_6_col_2.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_v[row_idx % 128] = 1'b1;
                                    end
                                    default : begin
                                       $error("SOMTING IS wrong with parameters given for col number %d ",j);
         ...
         for (int j = 0;j < 3 ; j = j + 1) 
             case (j)
                                    ...
                                    1: begin
                                            //write_DATA
                                             gen_tcam_row_7_col_1_tcam.ip783tcam128x32s0c1_wrap_tcam_row_7_col_1.tcam_row_7_col_1.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_y[row_idx % 128] = local_inv_key[1*(32 -2*BCAM_N7) +: 32-2*BCAM_N7];
                                             gen_tcam_row_7_col_1_tcam.ip783tcam128x32s0c1_wrap_tcam_row_7_col_1.tcam_row_7_col_1.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_v[row_idx % 128] = 1'b1;
                                    end
                                    2: begin
                                            //write_DATA
                                             gen_tcam_row_7_col_2_tcam.ip783tcam128x32s0c1_wrap_tcam_row_7_col_2.tcam_row_7_col_2.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_y[row_idx % 128] = local_inv_key[2*(32 -2*BCAM_N7) +: 32-2*BCAM_N7];
                                             gen_tcam_row_7_col_2_tcam.ip783tcam128x32s0c1_wrap_tcam_row_7_col_2.tcam_row_7_col_2.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_v[row_idx % 128] = 1'b1;
                                    end
                                    default : begin
                                       $error("SOMTING IS wrong with parameters given for col number %d ",j);
         ...
         for (int j = 0;j < 3 ; j = j + 1) 
             case (j)
                                    ...
                                    1: begin
                                            //write_DATA
                                             gen_tcam_row_8_col_1_tcam.ip783tcam128x32s0c1_wrap_tcam_row_8_col_1.tcam_row_8_col_1.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_y[row_idx % 128] = local_inv_key[1*(32 -2*BCAM_N7) +: 32-2*BCAM_N7];
                                             gen_tcam_row_8_col_1_tcam.ip783tcam128x32s0c1_wrap_tcam_row_8_col_1.tcam_row_8_col_1.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_v[row_idx % 128] = 1'b1;
                                    end
                                    2: begin
                                            //write_DATA
                                             gen_tcam_row_8_col_2_tcam.ip783tcam128x32s0c1_wrap_tcam_row_8_col_2.tcam_row_8_col_2.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_y[row_idx % 128] = local_inv_key[2*(32 -2*BCAM_N7) +: 32-2*BCAM_N7];
                                             gen_tcam_row_8_col_2_tcam.ip783tcam128x32s0c1_wrap_tcam_row_8_col_2.tcam_row_8_col_2.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_v[row_idx % 128] = 1'b1;
                                    end
                                    default : begin
                                       $error("SOMTING IS wrong with parameters given for col number %d ",j);
         ...
         for (int j = 0;j < 3 ; j = j + 1) 
             case (j)
                                    ...
                                    1: begin
                                            //write_DATA
                                             gen_tcam_row_9_col_1_tcam.ip783tcam128x32s0c1_wrap_tcam_row_9_col_1.tcam_row_9_col_1.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_y[row_idx % 128] = local_inv_key[1*(32 -2*BCAM_N7) +: 32-2*BCAM_N7];
                                             gen_tcam_row_9_col_1_tcam.ip783tcam128x32s0c1_wrap_tcam_row_9_col_1.tcam_row_9_col_1.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_v[row_idx % 128] = 1'b1;
                                    end
                                    2: begin
                                            //write_DATA
                                             gen_tcam_row_9_col_2_tcam.ip783tcam128x32s0c1_wrap_tcam_row_9_col_2.tcam_row_9_col_2.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_y[row_idx % 128] = local_inv_key[2*(32 -2*BCAM_N7) +: 32-2*BCAM_N7];
                                             gen_tcam_row_9_col_2_tcam.ip783tcam128x32s0c1_wrap_tcam_row_9_col_2.tcam_row_9_col_2.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_v[row_idx % 128] = 1'b1;
                                    end
                                    default : begin
                                       $error("SOMTING IS wrong with parameters given for col number %d ",j);
         ...
         for (int j = 0;j < 3 ; j = j + 1) 
             case (j)
                                    ...
                                    1: begin
                                            //write_DATA
                                             gen_tcam_row_10_col_1_tcam.ip783tcam128x32s0c1_wrap_tcam_row_10_col_1.tcam_row_10_col_1.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_y[row_idx % 128] = local_inv_key[1*(32 -2*BCAM_N7) +: 32-2*BCAM_N7];
                                             gen_tcam_row_10_col_1_tcam.ip783tcam128x32s0c1_wrap_tcam_row_10_col_1.tcam_row_10_col_1.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_v[row_idx % 128] = 1'b1;
                                    end
                                    2: begin
                                            //write_DATA
                                             gen_tcam_row_10_col_2_tcam.ip783tcam128x32s0c1_wrap_tcam_row_10_col_2.tcam_row_10_col_2.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_y[row_idx % 128] = local_inv_key[2*(32 -2*BCAM_N7) +: 32-2*BCAM_N7];
                                             gen_tcam_row_10_col_2_tcam.ip783tcam128x32s0c1_wrap_tcam_row_10_col_2.tcam_row_10_col_2.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_v[row_idx % 128] = 1'b1;
                                    end
                                    default : begin
                                       $error("SOMTING IS wrong with parameters given for col number %d ",j);
         ...
         for (int j = 0;j < 3 ; j = j + 1) 
             case (j)
                                    ...
                                    1: begin
                                            //write_DATA
                                             gen_tcam_row_11_col_1_tcam.ip783tcam128x32s0c1_wrap_tcam_row_11_col_1.tcam_row_11_col_1.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_y[row_idx % 128] = local_inv_key[1*(32 -2*BCAM_N7) +: 32-2*BCAM_N7];
                                             gen_tcam_row_11_col_1_tcam.ip783tcam128x32s0c1_wrap_tcam_row_11_col_1.tcam_row_11_col_1.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_v[row_idx % 128] = 1'b1;
                                    end
                                    2: begin
                                            //write_DATA
                                             gen_tcam_row_11_col_2_tcam.ip783tcam128x32s0c1_wrap_tcam_row_11_col_2.tcam_row_11_col_2.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_y[row_idx % 128] = local_inv_key[2*(32 -2*BCAM_N7) +: 32-2*BCAM_N7];
                                             gen_tcam_row_11_col_2_tcam.ip783tcam128x32s0c1_wrap_tcam_row_11_col_2.tcam_row_11_col_2.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_v[row_idx % 128] = 1'b1;
                                    end
                                    default : begin
                                       $error("SOMTING IS wrong with parameters given for col number %d ",j);
         ...
         for (int j = 0;j < 3 ; j = j + 1) 
             case (j)
                                    ...
                                    1: begin
                                            //write_DATA
                                             gen_tcam_row_12_col_1_tcam.ip783tcam128x32s0c1_wrap_tcam_row_12_col_1.tcam_row_12_col_1.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_y[row_idx % 128] = local_inv_key[1*(32 -2*BCAM_N7) +: 32-2*BCAM_N7];
                                             gen_tcam_row_12_col_1_tcam.ip783tcam128x32s0c1_wrap_tcam_row_12_col_1.tcam_row_12_col_1.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_v[row_idx % 128] = 1'b1;
                                    end
                                    2: begin
                                            //write_DATA
                                             gen_tcam_row_12_col_2_tcam.ip783tcam128x32s0c1_wrap_tcam_row_12_col_2.tcam_row_12_col_2.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_y[row_idx % 128] = local_inv_key[2*(32 -2*BCAM_N7) +: 32-2*BCAM_N7];
                                             gen_tcam_row_12_col_2_tcam.ip783tcam128x32s0c1_wrap_tcam_row_12_col_2.tcam_row_12_col_2.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_v[row_idx % 128] = 1'b1;
                                    end
                                    default : begin
                                       $error("SOMTING IS wrong with parameters given for col number %d ",j);
         ...
         for (int j = 0;j < 3 ; j = j + 1) 
             case (j)
                                    ...
                                    1: begin
                                            //write_DATA
                                             gen_tcam_row_13_col_1_tcam.ip783tcam128x32s0c1_wrap_tcam_row_13_col_1.tcam_row_13_col_1.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_y[row_idx % 128] = local_inv_key[1*(32 -2*BCAM_N7) +: 32-2*BCAM_N7];
                                             gen_tcam_row_13_col_1_tcam.ip783tcam128x32s0c1_wrap_tcam_row_13_col_1.tcam_row_13_col_1.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_v[row_idx % 128] = 1'b1;
                                    end
                                    2: begin
                                            //write_DATA
                                             gen_tcam_row_13_col_2_tcam.ip783tcam128x32s0c1_wrap_tcam_row_13_col_2.tcam_row_13_col_2.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_y[row_idx % 128] = local_inv_key[2*(32 -2*BCAM_N7) +: 32-2*BCAM_N7];
                                             gen_tcam_row_13_col_2_tcam.ip783tcam128x32s0c1_wrap_tcam_row_13_col_2.tcam_row_13_col_2.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_v[row_idx % 128] = 1'b1;
                                    end
                                    default : begin
                                       $error("SOMTING IS wrong with parameters given for col number %d ",j);
         ...
         for (int j = 0;j < 3 ; j = j + 1) 
             case (j)
                                    ...
                                    1: begin
                                            //write_DATA
                                             gen_tcam_row_14_col_1_tcam.ip783tcam128x32s0c1_wrap_tcam_row_14_col_1.tcam_row_14_col_1.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_y[row_idx % 128] = local_inv_key[1*(32 -2*BCAM_N7) +: 32-2*BCAM_N7];
                                             gen_tcam_row_14_col_1_tcam.ip783tcam128x32s0c1_wrap_tcam_row_14_col_1.tcam_row_14_col_1.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_v[row_idx % 128] = 1'b1;
                                    end
                                    2: begin
                                            //write_DATA
                                             gen_tcam_row_14_col_2_tcam.ip783tcam128x32s0c1_wrap_tcam_row_14_col_2.tcam_row_14_col_2.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_y[row_idx % 128] = local_inv_key[2*(32 -2*BCAM_N7) +: 32-2*BCAM_N7];
                                             gen_tcam_row_14_col_2_tcam.ip783tcam128x32s0c1_wrap_tcam_row_14_col_2.tcam_row_14_col_2.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_v[row_idx % 128] = 1'b1;
                                    end
                                    default : begin
                                       $error("SOMTING IS wrong with parameters given for col number %d ",j);
         ...
         for (int j = 0;j < 3 ; j = j + 1) 
             case (j)
                                    ...
                                    1: begin
                                            //write_DATA
                                             gen_tcam_row_15_col_1_tcam.ip783tcam128x32s0c1_wrap_tcam_row_15_col_1.tcam_row_15_col_1.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_y[row_idx % 128] = local_inv_key[1*(32 -2*BCAM_N7) +: 32-2*BCAM_N7];
                                             gen_tcam_row_15_col_1_tcam.ip783tcam128x32s0c1_wrap_tcam_row_15_col_1.tcam_row_15_col_1.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_v[row_idx % 128] = 1'b1;
                                    end
                                    2: begin
                                            //write_DATA
                                             gen_tcam_row_15_col_2_tcam.ip783tcam128x32s0c1_wrap_tcam_row_15_col_2.tcam_row_15_col_2.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_y[row_idx % 128] = local_inv_key[2*(32 -2*BCAM_N7) +: 32-2*BCAM_N7];
                                             gen_tcam_row_15_col_2_tcam.ip783tcam128x32s0c1_wrap_tcam_row_15_col_2.tcam_row_15_col_2.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_v[row_idx % 128] = 1'b1;
                                    end
                                    default : begin
                                       $error("SOMTING IS wrong with parameters given for col number %d ",j);
       ...
       task automatic write_tcam_raw_line (input logic[91-1:0] key, input logic [91-1:0] inv_key, input int row_idx);
       ...
       // ********** backdoor MASK ***********
       //****this task is valid for TCAM only*****
       task automatic read_tcam_raw_key (output logic[91-1:0] key, input int row_idx);
           reg temp1,valid_word;
           logic [(3 * 32) - 1:0] local_key;
           // check size out-of-bounds
           if (row_idx >= TCAM_RULES_NUM) begin
         ...
         for (int j = 0;j < 3 ; j = j + 1) 
             case (j)
                                    ...
                                    1: begin
                                            //READ_mask
                                             local_key[1*32 +: 32] = gen_tcam_row_1_col_1_tcam.ip783tcam128x32s0c1_wrap_tcam_row_1_col_1.tcam_row_1_col_1.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_x[row_idx % 128];
                                    end
                                    2: begin
                                            //READ_mask
                                             local_key[2*32 +: 32] = gen_tcam_row_1_col_2_tcam.ip783tcam128x32s0c1_wrap_tcam_row_1_col_2.tcam_row_1_col_2.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_x[row_idx % 128];
                                    end
                                    default : begin
                                       $error("SOMTING IS wrong with parameters given for col number %d ",j);
                                     end
         ...
         for (int j = 0;j < 3 ; j = j + 1) 
             case (j)
                                    ...
                                    1: begin
                                            //READ_mask
                                             local_key[1*32 +: 32] = gen_tcam_row_2_col_1_tcam.ip783tcam128x32s0c1_wrap_tcam_row_2_col_1.tcam_row_2_col_1.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_x[row_idx % 128];
                                    end
                                    2: begin
                                            //READ_mask
                                             local_key[2*32 +: 32] = gen_tcam_row_2_col_2_tcam.ip783tcam128x32s0c1_wrap_tcam_row_2_col_2.tcam_row_2_col_2.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_x[row_idx % 128];
                                    end
                                    default : begin
                                       $error("SOMTING IS wrong with parameters given for col number %d ",j);
                                     end
         ...
         for (int j = 0;j < 3 ; j = j + 1) 
             case (j)
                                    ...
                                    1: begin
                                            //READ_mask
                                             local_key[1*32 +: 32] = gen_tcam_row_3_col_1_tcam.ip783tcam128x32s0c1_wrap_tcam_row_3_col_1.tcam_row_3_col_1.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_x[row_idx % 128];
                                    end
                                    2: begin
                                            //READ_mask
                                             local_key[2*32 +: 32] = gen_tcam_row_3_col_2_tcam.ip783tcam128x32s0c1_wrap_tcam_row_3_col_2.tcam_row_3_col_2.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_x[row_idx % 128];
                                    end
                                    default : begin
                                       $error("SOMTING IS wrong with parameters given for col number %d ",j);
                                     end
         ...
         for (int j = 0;j < 3 ; j = j + 1) 
             case (j)
                                    ...
                                    1: begin
                                            //READ_mask
                                             local_key[1*32 +: 32] = gen_tcam_row_4_col_1_tcam.ip783tcam128x32s0c1_wrap_tcam_row_4_col_1.tcam_row_4_col_1.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_x[row_idx % 128];
                                    end
                                    2: begin
                                            //READ_mask
                                             local_key[2*32 +: 32] = gen_tcam_row_4_col_2_tcam.ip783tcam128x32s0c1_wrap_tcam_row_4_col_2.tcam_row_4_col_2.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_x[row_idx % 128];
                                    end
                                    default : begin
                                       $error("SOMTING IS wrong with parameters given for col number %d ",j);
                                     end
         ...
         for (int j = 0;j < 3 ; j = j + 1) 
             case (j)
                                    ...
                                    1: begin
                                            //READ_mask
                                             local_key[1*32 +: 32] = gen_tcam_row_5_col_1_tcam.ip783tcam128x32s0c1_wrap_tcam_row_5_col_1.tcam_row_5_col_1.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_x[row_idx % 128];
                                    end
                                    2: begin
                                            //READ_mask
                                             local_key[2*32 +: 32] = gen_tcam_row_5_col_2_tcam.ip783tcam128x32s0c1_wrap_tcam_row_5_col_2.tcam_row_5_col_2.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_x[row_idx % 128];
                                    end
                                    default : begin
                                       $error("SOMTING IS wrong with parameters given for col number %d ",j);
                                     end
         ...
         for (int j = 0;j < 3 ; j = j + 1) 
             case (j)
                                    ...
                                    1: begin
                                            //READ_mask
                                             local_key[1*32 +: 32] = gen_tcam_row_6_col_1_tcam.ip783tcam128x32s0c1_wrap_tcam_row_6_col_1.tcam_row_6_col_1.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_x[row_idx % 128];
                                    end
                                    2: begin
                                            //READ_mask
                                             local_key[2*32 +: 32] = gen_tcam_row_6_col_2_tcam.ip783tcam128x32s0c1_wrap_tcam_row_6_col_2.tcam_row_6_col_2.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_x[row_idx % 128];
                                    end
                                    default : begin
                                       $error("SOMTING IS wrong with parameters given for col number %d ",j);
                                     end
         ...
         for (int j = 0;j < 3 ; j = j + 1) 
             case (j)
                                    ...
                                    1: begin
                                            //READ_mask
                                             local_key[1*32 +: 32] = gen_tcam_row_7_col_1_tcam.ip783tcam128x32s0c1_wrap_tcam_row_7_col_1.tcam_row_7_col_1.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_x[row_idx % 128];
                                    end
                                    2: begin
                                            //READ_mask
                                             local_key[2*32 +: 32] = gen_tcam_row_7_col_2_tcam.ip783tcam128x32s0c1_wrap_tcam_row_7_col_2.tcam_row_7_col_2.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_x[row_idx % 128];
                                    end
                                    default : begin
                                       $error("SOMTING IS wrong with parameters given for col number %d ",j);
                                     end
         ...
         for (int j = 0;j < 3 ; j = j + 1) 
             case (j)
                                    ...
                                    1: begin
                                            //READ_mask
                                             local_key[1*32 +: 32] = gen_tcam_row_8_col_1_tcam.ip783tcam128x32s0c1_wrap_tcam_row_8_col_1.tcam_row_8_col_1.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_x[row_idx % 128];
                                    end
                                    2: begin
                                            //READ_mask
                                             local_key[2*32 +: 32] = gen_tcam_row_8_col_2_tcam.ip783tcam128x32s0c1_wrap_tcam_row_8_col_2.tcam_row_8_col_2.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_x[row_idx % 128];
                                    end
                                    default : begin
                                       $error("SOMTING IS wrong with parameters given for col number %d ",j);
                                     end
         ...
         for (int j = 0;j < 3 ; j = j + 1) 
             case (j)
                                    ...
                                    1: begin
                                            //READ_mask
                                             local_key[1*32 +: 32] = gen_tcam_row_9_col_1_tcam.ip783tcam128x32s0c1_wrap_tcam_row_9_col_1.tcam_row_9_col_1.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_x[row_idx % 128];
                                    end
                                    2: begin
                                            //READ_mask
                                             local_key[2*32 +: 32] = gen_tcam_row_9_col_2_tcam.ip783tcam128x32s0c1_wrap_tcam_row_9_col_2.tcam_row_9_col_2.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_x[row_idx % 128];
                                    end
                                    default : begin
                                       $error("SOMTING IS wrong with parameters given for col number %d ",j);
                                     end
         ...
         for (int j = 0;j < 3 ; j = j + 1) 
             case (j)
                                    ...
                                    1: begin
                                            //READ_mask
                                             local_key[1*32 +: 32] = gen_tcam_row_10_col_1_tcam.ip783tcam128x32s0c1_wrap_tcam_row_10_col_1.tcam_row_10_col_1.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_x[row_idx % 128];
                                    end
                                    2: begin
                                            //READ_mask
                                             local_key[2*32 +: 32] = gen_tcam_row_10_col_2_tcam.ip783tcam128x32s0c1_wrap_tcam_row_10_col_2.tcam_row_10_col_2.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_x[row_idx % 128];
                                    end
                                    default : begin
                                       $error("SOMTING IS wrong with parameters given for col number %d ",j);
                                     end
         ...
         for (int j = 0;j < 3 ; j = j + 1) 
             case (j)
                                    ...
                                    1: begin
                                            //READ_mask
                                             local_key[1*32 +: 32] = gen_tcam_row_11_col_1_tcam.ip783tcam128x32s0c1_wrap_tcam_row_11_col_1.tcam_row_11_col_1.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_x[row_idx % 128];
                                    end
                                    2: begin
                                            //READ_mask
                                             local_key[2*32 +: 32] = gen_tcam_row_11_col_2_tcam.ip783tcam128x32s0c1_wrap_tcam_row_11_col_2.tcam_row_11_col_2.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_x[row_idx % 128];
                                    end
                                    default : begin
                                       $error("SOMTING IS wrong with parameters given for col number %d ",j);
                                     end
         ...
         for (int j = 0;j < 3 ; j = j + 1) 
             case (j)
                                    ...
                                    1: begin
                                            //READ_mask
                                             local_key[1*32 +: 32] = gen_tcam_row_12_col_1_tcam.ip783tcam128x32s0c1_wrap_tcam_row_12_col_1.tcam_row_12_col_1.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_x[row_idx % 128];
                                    end
                                    2: begin
                                            //READ_mask
                                             local_key[2*32 +: 32] = gen_tcam_row_12_col_2_tcam.ip783tcam128x32s0c1_wrap_tcam_row_12_col_2.tcam_row_12_col_2.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_x[row_idx % 128];
                                    end
                                    default : begin
                                       $error("SOMTING IS wrong with parameters given for col number %d ",j);
                                     end
         ...
         for (int j = 0;j < 3 ; j = j + 1) 
             case (j)
                                    ...
                                    1: begin
                                            //READ_mask
                                             local_key[1*32 +: 32] = gen_tcam_row_13_col_1_tcam.ip783tcam128x32s0c1_wrap_tcam_row_13_col_1.tcam_row_13_col_1.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_x[row_idx % 128];
                                    end
                                    2: begin
                                            //READ_mask
                                             local_key[2*32 +: 32] = gen_tcam_row_13_col_2_tcam.ip783tcam128x32s0c1_wrap_tcam_row_13_col_2.tcam_row_13_col_2.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_x[row_idx % 128];
                                    end
                                    default : begin
                                       $error("SOMTING IS wrong with parameters given for col number %d ",j);
                                     end
         ...
         for (int j = 0;j < 3 ; j = j + 1) 
             case (j)
                                    ...
                                    1: begin
                                            //READ_mask
                                             local_key[1*32 +: 32] = gen_tcam_row_14_col_1_tcam.ip783tcam128x32s0c1_wrap_tcam_row_14_col_1.tcam_row_14_col_1.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_x[row_idx % 128];
                                    end
                                    2: begin
                                            //READ_mask
                                             local_key[2*32 +: 32] = gen_tcam_row_14_col_2_tcam.ip783tcam128x32s0c1_wrap_tcam_row_14_col_2.tcam_row_14_col_2.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_x[row_idx % 128];
                                    end
                                    default : begin
                                       $error("SOMTING IS wrong with parameters given for col number %d ",j);
                                     end
         ...
         for (int j = 0;j < 3 ; j = j + 1) 
             case (j)
                                    ...
                                    1: begin
                                            //READ_mask
                                             local_key[1*32 +: 32] = gen_tcam_row_15_col_1_tcam.ip783tcam128x32s0c1_wrap_tcam_row_15_col_1.tcam_row_15_col_1.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_x[row_idx % 128];
                                    end
                                    2: begin
                                            //READ_mask
                                             local_key[2*32 +: 32] = gen_tcam_row_15_col_2_tcam.ip783tcam128x32s0c1_wrap_tcam_row_15_col_2.tcam_row_15_col_2.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_x[row_idx % 128];
                                    end
                                    default : begin
                                       $error("SOMTING IS wrong with parameters given for col number %d ",j);
                                     end
     ...
     // ********** backdoor DATA ***********
       task automatic read_tcam_raw_inv_key (output logic[91-1:0] inv_key, input int row_idx);
           reg temp1,valid_word;
           logic [(3 * 32) - 1:0] local_inv_key;
           // check size out-of-bounds
           if (row_idx >= TCAM_RULES_NUM) begin
         ...
         for (int j = 0;j < 3 ; j = j + 1) 
             case (j)
                                    ...
                                    1: begin
                                            //READ_data
                                             local_inv_key[1*32 +: 32] = gen_tcam_row_1_col_1_tcam.ip783tcam128x32s0c1_wrap_tcam_row_1_col_1.tcam_row_1_col_1.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_y[row_idx % 128];
                                    end
                                    2: begin
                                            //READ_data
                                             local_inv_key[2*32 +: 32] = gen_tcam_row_1_col_2_tcam.ip783tcam128x32s0c1_wrap_tcam_row_1_col_2.tcam_row_1_col_2.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_y[row_idx % 128];
                                    end
                                    default : begin
                                       $error("SOMTING IS wrong with parameters given for col number %d ",j);
                                     end
         ...
         for (int j = 0;j < 3 ; j = j + 1) 
             case (j)
                                    ...
                                    1: begin
                                            //READ_data
                                             local_inv_key[1*32 +: 32] = gen_tcam_row_2_col_1_tcam.ip783tcam128x32s0c1_wrap_tcam_row_2_col_1.tcam_row_2_col_1.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_y[row_idx % 128];
                                    end
                                    2: begin
                                            //READ_data
                                             local_inv_key[2*32 +: 32] = gen_tcam_row_2_col_2_tcam.ip783tcam128x32s0c1_wrap_tcam_row_2_col_2.tcam_row_2_col_2.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_y[row_idx % 128];
                                    end
                                    default : begin
                                       $error("SOMTING IS wrong with parameters given for col number %d ",j);
                                     end
         ...
         for (int j = 0;j < 3 ; j = j + 1) 
             case (j)
                                    ...
                                    1: begin
                                            //READ_data
                                             local_inv_key[1*32 +: 32] = gen_tcam_row_3_col_1_tcam.ip783tcam128x32s0c1_wrap_tcam_row_3_col_1.tcam_row_3_col_1.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_y[row_idx % 128];
                                    end
                                    2: begin
                                            //READ_data
                                             local_inv_key[2*32 +: 32] = gen_tcam_row_3_col_2_tcam.ip783tcam128x32s0c1_wrap_tcam_row_3_col_2.tcam_row_3_col_2.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_y[row_idx % 128];
                                    end
                                    default : begin
                                       $error("SOMTING IS wrong with parameters given for col number %d ",j);
                                     end
         ...
         for (int j = 0;j < 3 ; j = j + 1) 
             case (j)
                                    ...
                                    1: begin
                                            //READ_data
                                             local_inv_key[1*32 +: 32] = gen_tcam_row_4_col_1_tcam.ip783tcam128x32s0c1_wrap_tcam_row_4_col_1.tcam_row_4_col_1.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_y[row_idx % 128];
                                    end
                                    2: begin
                                            //READ_data
                                             local_inv_key[2*32 +: 32] = gen_tcam_row_4_col_2_tcam.ip783tcam128x32s0c1_wrap_tcam_row_4_col_2.tcam_row_4_col_2.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_y[row_idx % 128];
                                    end
                                    default : begin
                                       $error("SOMTING IS wrong with parameters given for col number %d ",j);
                                     end
         ...
         for (int j = 0;j < 3 ; j = j + 1) 
             case (j)
                                    ...
                                    1: begin
                                            //READ_data
                                             local_inv_key[1*32 +: 32] = gen_tcam_row_5_col_1_tcam.ip783tcam128x32s0c1_wrap_tcam_row_5_col_1.tcam_row_5_col_1.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_y[row_idx % 128];
                                    end
                                    2: begin
                                            //READ_data
                                             local_inv_key[2*32 +: 32] = gen_tcam_row_5_col_2_tcam.ip783tcam128x32s0c1_wrap_tcam_row_5_col_2.tcam_row_5_col_2.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_y[row_idx % 128];
                                    end
                                    default : begin
                                       $error("SOMTING IS wrong with parameters given for col number %d ",j);
                                     end
         ...
         for (int j = 0;j < 3 ; j = j + 1) 
             case (j)
                                    ...
                                    1: begin
                                            //READ_data
                                             local_inv_key[1*32 +: 32] = gen_tcam_row_6_col_1_tcam.ip783tcam128x32s0c1_wrap_tcam_row_6_col_1.tcam_row_6_col_1.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_y[row_idx % 128];
                                    end
                                    2: begin
                                            //READ_data
                                             local_inv_key[2*32 +: 32] = gen_tcam_row_6_col_2_tcam.ip783tcam128x32s0c1_wrap_tcam_row_6_col_2.tcam_row_6_col_2.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_y[row_idx % 128];
                                    end
                                    default : begin
                                       $error("SOMTING IS wrong with parameters given for col number %d ",j);
                                     end
         ...
         for (int j = 0;j < 3 ; j = j + 1) 
             case (j)
                                    ...
                                    1: begin
                                            //READ_data
                                             local_inv_key[1*32 +: 32] = gen_tcam_row_7_col_1_tcam.ip783tcam128x32s0c1_wrap_tcam_row_7_col_1.tcam_row_7_col_1.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_y[row_idx % 128];
                                    end
                                    2: begin
                                            //READ_data
                                             local_inv_key[2*32 +: 32] = gen_tcam_row_7_col_2_tcam.ip783tcam128x32s0c1_wrap_tcam_row_7_col_2.tcam_row_7_col_2.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_y[row_idx % 128];
                                    end
                                    default : begin
                                       $error("SOMTING IS wrong with parameters given for col number %d ",j);
                                     end
         ...
         for (int j = 0;j < 3 ; j = j + 1) 
             case (j)
                                    ...
                                    1: begin
                                            //READ_data
                                             local_inv_key[1*32 +: 32] = gen_tcam_row_8_col_1_tcam.ip783tcam128x32s0c1_wrap_tcam_row_8_col_1.tcam_row_8_col_1.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_y[row_idx % 128];
                                    end
                                    2: begin
                                            //READ_data
                                             local_inv_key[2*32 +: 32] = gen_tcam_row_8_col_2_tcam.ip783tcam128x32s0c1_wrap_tcam_row_8_col_2.tcam_row_8_col_2.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_y[row_idx % 128];
                                    end
                                    default : begin
                                       $error("SOMTING IS wrong with parameters given for col number %d ",j);
                                     end
         ...
         for (int j = 0;j < 3 ; j = j + 1) 
             case (j)
                                    ...
                                    1: begin
                                            //READ_data
                                             local_inv_key[1*32 +: 32] = gen_tcam_row_9_col_1_tcam.ip783tcam128x32s0c1_wrap_tcam_row_9_col_1.tcam_row_9_col_1.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_y[row_idx % 128];
                                    end
                                    2: begin
                                            //READ_data
                                             local_inv_key[2*32 +: 32] = gen_tcam_row_9_col_2_tcam.ip783tcam128x32s0c1_wrap_tcam_row_9_col_2.tcam_row_9_col_2.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_y[row_idx % 128];
                                    end
                                    default : begin
                                       $error("SOMTING IS wrong with parameters given for col number %d ",j);
                                     end
         ...
         for (int j = 0;j < 3 ; j = j + 1) 
             case (j)
                                    ...
                                    1: begin
                                            //READ_data
                                             local_inv_key[1*32 +: 32] = gen_tcam_row_10_col_1_tcam.ip783tcam128x32s0c1_wrap_tcam_row_10_col_1.tcam_row_10_col_1.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_y[row_idx % 128];
                                    end
                                    2: begin
                                            //READ_data
                                             local_inv_key[2*32 +: 32] = gen_tcam_row_10_col_2_tcam.ip783tcam128x32s0c1_wrap_tcam_row_10_col_2.tcam_row_10_col_2.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_y[row_idx % 128];
                                    end
                                    default : begin
                                       $error("SOMTING IS wrong with parameters given for col number %d ",j);
                                     end
         ...
         for (int j = 0;j < 3 ; j = j + 1) 
             case (j)
                                    ...
                                    1: begin
                                            //READ_data
                                             local_inv_key[1*32 +: 32] = gen_tcam_row_11_col_1_tcam.ip783tcam128x32s0c1_wrap_tcam_row_11_col_1.tcam_row_11_col_1.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_y[row_idx % 128];
                                    end
                                    2: begin
                                            //READ_data
                                             local_inv_key[2*32 +: 32] = gen_tcam_row_11_col_2_tcam.ip783tcam128x32s0c1_wrap_tcam_row_11_col_2.tcam_row_11_col_2.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_y[row_idx % 128];
                                    end
                                    default : begin
                                       $error("SOMTING IS wrong with parameters given for col number %d ",j);
                                     end
         ...
         for (int j = 0;j < 3 ; j = j + 1) 
             case (j)
                                    ...
                                    1: begin
                                            //READ_data
                                             local_inv_key[1*32 +: 32] = gen_tcam_row_12_col_1_tcam.ip783tcam128x32s0c1_wrap_tcam_row_12_col_1.tcam_row_12_col_1.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_y[row_idx % 128];
                                    end
                                    2: begin
                                            //READ_data
                                             local_inv_key[2*32 +: 32] = gen_tcam_row_12_col_2_tcam.ip783tcam128x32s0c1_wrap_tcam_row_12_col_2.tcam_row_12_col_2.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_y[row_idx % 128];
                                    end
                                    default : begin
                                       $error("SOMTING IS wrong with parameters given for col number %d ",j);
                                     end
         ...
         for (int j = 0;j < 3 ; j = j + 1) 
             case (j)
                                    ...
                                    1: begin
                                            //READ_data
                                             local_inv_key[1*32 +: 32] = gen_tcam_row_13_col_1_tcam.ip783tcam128x32s0c1_wrap_tcam_row_13_col_1.tcam_row_13_col_1.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_y[row_idx % 128];
                                    end
                                    2: begin
                                            //READ_data
                                             local_inv_key[2*32 +: 32] = gen_tcam_row_13_col_2_tcam.ip783tcam128x32s0c1_wrap_tcam_row_13_col_2.tcam_row_13_col_2.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_y[row_idx % 128];
                                    end
                                    default : begin
                                       $error("SOMTING IS wrong with parameters given for col number %d ",j);
                                     end
         ...
         for (int j = 0;j < 3 ; j = j + 1) 
             case (j)
                                    ...
                                    1: begin
                                            //READ_data
                                             local_inv_key[1*32 +: 32] = gen_tcam_row_14_col_1_tcam.ip783tcam128x32s0c1_wrap_tcam_row_14_col_1.tcam_row_14_col_1.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_y[row_idx % 128];
                                    end
                                    2: begin
                                            //READ_data
                                             local_inv_key[2*32 +: 32] = gen_tcam_row_14_col_2_tcam.ip783tcam128x32s0c1_wrap_tcam_row_14_col_2.tcam_row_14_col_2.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_y[row_idx % 128];
                                    end
                                    default : begin
                                       $error("SOMTING IS wrong with parameters given for col number %d ",j);
                                     end
         ...
         for (int j = 0;j < 3 ; j = j + 1) 
             case (j)
                                    ...
                                    1: begin
                                            //READ_data
                                             local_inv_key[1*32 +: 32] = gen_tcam_row_15_col_1_tcam.ip783tcam128x32s0c1_wrap_tcam_row_15_col_1.tcam_row_15_col_1.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_y[row_idx % 128];
                                    end
                                    2: begin
                                            //READ_data
                                             local_inv_key[2*32 +: 32] = gen_tcam_row_15_col_2_tcam.ip783tcam128x32s0c1_wrap_tcam_row_15_col_2.tcam_row_15_col_2.ip783tcam128x32s0c1_gen_wrp_inst.ip783tcam128x32s0c1_inst.ip783tcam128x32s0c1_array.array_y[row_idx % 128];
                                    end
                                    default : begin
                                       $error("SOMTING IS wrong with parameters given for col number %d ",j);
                                     end
       ...
       task automatic read_tcam_raw_line (output logic[91-1:0] key, output logic [91-1:0] inv_key, input int row_idx);
