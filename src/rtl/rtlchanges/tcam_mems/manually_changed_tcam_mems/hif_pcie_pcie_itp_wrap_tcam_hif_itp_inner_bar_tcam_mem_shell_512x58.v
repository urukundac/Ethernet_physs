...
module  hif_pcie_pcie_itp_wrap_tcam_hif_itp_inner_bar_tcam_mem_shell_512x58 #(
...
localparam TCAM_SLICE_EN_MIN = ((TCAM_SLICE_EN_FACT < TCAM_SLICE_EN_WIDTH ) ? TCAM_SLICE_EN_FACT : TCAM_SLICE_EN_WIDTH);
    ...
    assign                    tcam_row_sel[0]        = ((adr>=10'd0) && (adr<10'd256));
    assign                    tcam_row_sel[1]        = ((adr>=10'd256) && (adr<10'd512));
    assign                    tcam_row_sel[2]        = ((adr>=10'd512) && (adr<10'd768));
    assign                    tcam_row_sel[3]        = (adr>=10'd768);


    // TCAM Address Decoder

    wire    [8-1:0]        tcam_row_adr[4-1:0];
    ...
    logic    [128-1:0]    tcam_raw_hit_in[4-1:0];
    always_comb begin
        for (int i = 0; i < 4; i = i + 1) begin
    ...
    logic    [32-1:0]    tcam_rd_data_col[4-1:0][2-1:0];
    logic    [64-1:0]    tcam_rd_data_row;
    always_comb begin
        for (int i = 0; i < 2; i = i + 1) begin
            tcam_rd_data_row[i*(32)+:32]        = tcam_rd_data_col[tcam_row_num_rd_en_delay[2-1:0]][i][32-1:0];
        end
    end
    ...
    logic    [128-1:0]    tcam_col_hit_out[4-1:0][2-1:0];
    logic    [512-1:0]    tcam_hit_out;
    always_comb begin
        for (int i = 0; i < 4; i = i + 1) begin
                tcam_hit_out[i*128+:128]    =  tcam_col_hit_out[i][0]   & tcam_col_hit_out[i][1];
        end
    end
    assign    raw_hit_out = tcam_hit_out[TCAM_RULES_NUM-1:0];
// Match In Array 

        logic    [128-1:0]      tcam_row_match_in[4-1:0];
        always_comb begin 
                for (int i = 0; i < 4; i = i + 1) begin
    ...
    // EBBs Instantiation
        ...
        //                             |_____________________________|
                         ...
                         pcie_itp_ip783tcam128x32s0c1_0_0_wrap #(        
                                 )               
        ...
        //                             |_____________________________|
                         ...
                         pcie_itp_ip783tcam128x32s0c1_0_1_wrap #(        
                                 )               
        ...
        //                             |_____________________________|
                         ...
                         pcie_itp_ip783tcam128x32s0c1_1_0_wrap #(        
                                 )               
        ...
        //                             |_____________________________|
                         ...
                         pcie_itp_ip783tcam128x32s0c1_1_1_wrap #(        
                                 )               
        ...
        //                             |_____________________________|
                         ...
                         pcie_itp_ip783tcam128x32s0c1_2_0_wrap #(        
                                 )               
        ...
        //                             |_____________________________|
                         ...
                         pcie_itp_ip783tcam128x32s0c1_2_1_wrap #(        
                                 )               
        ...
        //                             |_____________________________|
                         ...
                         pcie_itp_ip783tcam128x32s0c1_3_0_wrap #(        
                                 )               
        ...
        //                             |_____________________________|
                         ...
                         pcie_itp_ip783tcam128x32s0c1_3_1_wrap #(        
                                 )               
...
parameter       MEM_SEG_PADDING_W = 6 ; 
    ...
    generate if (BCAM_N7==0 && TCAM_PROTECTION_TYPE==1 && BCAM_RELIEF==0) begin : protect    
       ...
       task automatic update_col_sweeper(input bit pattern, input logic[58-1:0] old_val, input logic[58-1:0] new_val);
           logic [58-1:0] temp;
    ...
    // ********** this task is valid for TCAM only *****
      task automatic write_tcam_raw_key (input logic[58-1:0] key, input int row_idx);
          reg temp1,temp2;
          logic [(2 * 32) - 1:0] local_key = '1;
          logic [(2 * 32) - 1:0] prev_key_data;
          local_key[58-1:0] = TCAM_18A ? ~key : ~key;
       ...
       // ********** backdoor DATA ***********
        task automatic write_tcam_raw_inv_key (input logic[58-1:0] inv_key, input int row_idx);
          reg temp1,temp2;
          logic [(2 * 32) - 1:0] local_inv_key = '0;
          logic [(2 * 32) - 1:0] prev_inv_key_data;
          local_inv_key[58-1:0] = TCAM_18A ? ~inv_key : ~inv_key;
       ...
       task automatic write_tcam_raw_line (input logic[58-1:0] key, input logic [58-1:0] inv_key, input int row_idx);
       ...
       // ********** backdoor MASK ***********
       //****this task is valid for TCAM only*****
       task automatic read_tcam_raw_key (output logic[58-1:0] key, input int row_idx);
     ...
     // ********** backdoor DATA ***********
       task automatic read_tcam_raw_inv_key (output logic[58-1:0] inv_key, input int row_idx);
           reg temp1,valid_word;
       ...
       task automatic read_tcam_raw_line (output logic[58-1:0] key, output logic [58-1:0] inv_key, input int row_idx);
