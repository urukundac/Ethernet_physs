...
module  hif_pcie_pcie_upp_wrap_tcam_hif_keepout_tcam_shell_128x72 #(
       ...
       ,
        ...
        input wire BIST_COLLAR_EN1, input wire BIST_RUN_TO_COLLAR0, 
        input wire BIST_RUN_TO_COLLAR1, input wire BIST_ASYNC_RESET, 
...
localparam PHYSICAL_ROW_WIDTH = TCAM_WIDTH + BCAM_N7*(2*36 - TCAM_WIDTH);
...
`ifndef HIF_PCIE_PCIE_UPP_TCAM_BEHAVE_MEMS
    `define HIF_PCIE_PCIE_UPP_ASIC_MEMS
`endif

`ifdef  HIF_PCIE_PCIE_UPP_ASIC_MEMS
...
localparam TCAM_SLICE_EN_MIN = ((TCAM_SLICE_EN_FACT < TCAM_SLICE_EN_WIDTH ) ? TCAM_SLICE_EN_FACT : TCAM_SLICE_EN_WIDTH);
    ...
    logic    [72-1:0]    tcam_row_wr_data;
    always_comb begin
            tcam_row_wr_data        = wr_data;
    end
    logic    [36-1:0]    tcam_wr_data_col[2-1:0];
    always_comb begin
    ...
    logic    [72-1:0]    tcam_chk_key;
    always_comb begin
            tcam_chk_key        = chk_key;
    end
    logic    [36-1:0]    tcam_chk_key_col[2-1:0];
    always_comb begin
        for (int i = 0; i < 2; i = i + 1) begin
    ...
    logic    [72-1:0]    tcam_chk_mask;
    always_comb begin
            tcam_chk_mask        = '0;
            tcam_chk_mask        = wr_en ? '1 : chk_mask_int;
    end
    logic    [36-1:0]    tcam_chk_mask_col[2-1:0];
    always_comb begin
        for (int i = 0; i < 2; i = i + 1) begin
    ...
    logic    [36-1:0]    tcam_rd_data_col[1-1:0][2-1:0];
    logic    [72-1:0]    tcam_rd_data_row;
    always_comb begin
        for (int i = 0; i < 2; i = i + 1) begin
            tcam_rd_data_row[i*(36)+:36]        = tcam_rd_data_col[tcam_row_num_rd_en_delay[1-1:0]][i][36-1:0];
        end
    end
    ...
    logic    [128-1:0]    tcam_col_hit_out[1-1:0][2-1:0];
...
// Match In Array 
    ...
    // EBBs Instantiation
        ...
        //                             |_____________________________|
                         ...
                         pcie_upp_ip783tcam128x36s0c1_0_0_wrap #(        
                                 )               
        
                                 ip783tcam128x36s0c1_wrap_tcam_row_0_col_0 (
                   //-------------------------------------------------------------
        ...
        wire    [36-1:0]                      tcam_row_0_col_1_data_in = tcam_row_wr_en[0] ? tcam_wr_data_col[1][36-1:0] : tcam_chk_key_col[1][36-1:0];
        wire    [36-1:0]                      tcam_row_0_col_1_data_in_pre = tcam_wr_data_col[1][36-1:0] ;
        wire    [36-1:0]                      tcam_row_0_col_1_key_in =  tcam_chk_key_col[1][36-1:0];
        wire    [36-1:0]                      tcam_row_0_col_1_mask_in = tcam_chk_mask_col[1][36-1:0];
        wire    [128-1:0]                       tcam_row_0_col_1_hit_in = tcam_raw_hit_in[0];
        wire    [128-1:0]                       tcam_row_0_col_1_tcam_match_in = tcam_row_match_in[0];
        wire    [36-1:0]                      tcam_row_0_col_1_data_out_inv;
        ...
        wire     [6-1:0]                  tcam_row_0_col_1_fca = 'b0;
        ...
        logic   [TCAM_SLICE_EN_FACT-1:0]                  tcam_row_0_col_1_slice_en_s1; 
             ...
             wire    [36-1:0]                     tcam_row_0_col_1_data_out ;
        ...
        //                             |_____________________________|
                         ...
                         pcie_upp_ip783tcam128x36s0c1_0_1_wrap #(        
                                 )               
        
                                 ip783tcam128x36s0c1_wrap_tcam_row_0_col_1 (
                   //-------------------------------------------------------------
                   // Power Management Interfaces
                   //-------------------------------------------------------------   
                      `ifndef INTEL_NO_PWR_PINS
                      ...
                      `endif
                        ...
                        .tcam_row_0_col_1_bisr_inst_Q_ts1(tcam_row_0_col_1_bisr_inst_Q[6:0]), 
                        .fscan_clkungate(fscan_clkungate)
                     );
    ...
    if (BCAM_N7 == 1) 
        begin : bacm_prot_calc
            always_comb begin
                ...
                for (int i = 0; i < 2; i = i + 1) begin
                    rd_data[PHYSICAL_ROW_WIDTH-((2-i-1)*2)-2] = ^rd_data_before_prot_calc[(i*(36-2))+:((36-2)/2)];
                    rd_data[PHYSICAL_ROW_WIDTH-((2-i-1)*2)-1] = ^rd_data_before_prot_calc[(i*(36-2))+((36-2)/2)+:((36-2)/2)];
                end
            end
        end
    else
...
parameter       MEM_SEG_PADDING_W = 0 ; 
    ...
    generate if (BCAM_N7==0 && TCAM_PROTECTION_TYPE==1 && BCAM_RELIEF==0) begin : protect    
       ...
       task automatic update_col_sweeper(input bit pattern, input logic[72-1:0] old_val, input logic[72-1:0] new_val);
           logic [72-1:0] temp;
    ...
    // ********** this task is valid for TCAM only *****
      task automatic write_tcam_raw_key (input logic[72-1:0] key, input int row_idx);
          reg temp1,temp2;
          logic [(2 * 36) - 1:0] local_key = '1;
          logic [(2 * 36) - 1:0] prev_key_data;
          local_key[72-1:0] = TCAM_18A ? ~key : ~key;
          ...
          `ifdef HIF_PCIE_PCIE_UPP_TCAM_BEHAVE_MEMS
             `ifndef HIF_PCIE_FPGA_TCAM_MEMS
         ...
         for (int j = 0;j < 2 ; j = j + 1) 
             case (j)
                                    0: begin
                                            //write_mask
                                             gen_tcam_row_0_col_0_tcam.ip783tcam128x36s0c1_wrap_tcam_row_0_col_0.tcam_row_0_col_0.ip783tcam128x36s0c1_gen_wrp_inst.ip783tcam128x36s0c1_inst.ip783tcam128x36s0c1_array.array_x[row_idx % 128] = local_key[0*(36 -2*BCAM_N7) +: 36-2*BCAM_N7];
                                             gen_tcam_row_0_col_0_tcam.ip783tcam128x36s0c1_wrap_tcam_row_0_col_0.tcam_row_0_col_0.ip783tcam128x36s0c1_gen_wrp_inst.ip783tcam128x36s0c1_inst.ip783tcam128x36s0c1_array.array_v[row_idx % 128] = 1'b1;
                                    end
                                    1: begin
                                            //write_mask
                                             gen_tcam_row_0_col_1_tcam.ip783tcam128x36s0c1_wrap_tcam_row_0_col_1.tcam_row_0_col_1.ip783tcam128x36s0c1_gen_wrp_inst.ip783tcam128x36s0c1_inst.ip783tcam128x36s0c1_array.array_x[row_idx % 128] = local_key[1*(36 -2*BCAM_N7) +: 36-2*BCAM_N7];
                                             gen_tcam_row_0_col_1_tcam.ip783tcam128x36s0c1_wrap_tcam_row_0_col_1.tcam_row_0_col_1.ip783tcam128x36s0c1_gen_wrp_inst.ip783tcam128x36s0c1_inst.ip783tcam128x36s0c1_array.array_v[row_idx % 128] = 1'b1;
                                    end
                                    default : begin
       ...
       // ********** backdoor DATA ***********
        task automatic write_tcam_raw_inv_key (input logic[72-1:0] inv_key, input int row_idx);
          reg temp1,temp2;
          logic [(2 * 36) - 1:0] local_inv_key = '0;
          logic [(2 * 36) - 1:0] prev_inv_key_data;
          local_inv_key[72-1:0] = TCAM_18A ? ~inv_key : ~inv_key;
         ...
         for (int j = 0;j < 2 ; j = j + 1) 
             case (j)
                                    0: begin
                                            //write_DATA
                                             gen_tcam_row_0_col_0_tcam.ip783tcam128x36s0c1_wrap_tcam_row_0_col_0.tcam_row_0_col_0.ip783tcam128x36s0c1_gen_wrp_inst.ip783tcam128x36s0c1_inst.ip783tcam128x36s0c1_array.array_y[row_idx % 128] = local_inv_key[0*(36 -2*BCAM_N7) +: 36-2*BCAM_N7];
                                             gen_tcam_row_0_col_0_tcam.ip783tcam128x36s0c1_wrap_tcam_row_0_col_0.tcam_row_0_col_0.ip783tcam128x36s0c1_gen_wrp_inst.ip783tcam128x36s0c1_inst.ip783tcam128x36s0c1_array.array_v[row_idx % 128] = 1'b1;
                                    end
                                    1: begin
                                            //write_DATA
                                             gen_tcam_row_0_col_1_tcam.ip783tcam128x36s0c1_wrap_tcam_row_0_col_1.tcam_row_0_col_1.ip783tcam128x36s0c1_gen_wrp_inst.ip783tcam128x36s0c1_inst.ip783tcam128x36s0c1_array.array_y[row_idx % 128] = local_inv_key[1*(36 -2*BCAM_N7) +: 36-2*BCAM_N7];
                                             gen_tcam_row_0_col_1_tcam.ip783tcam128x36s0c1_wrap_tcam_row_0_col_1.tcam_row_0_col_1.ip783tcam128x36s0c1_gen_wrp_inst.ip783tcam128x36s0c1_inst.ip783tcam128x36s0c1_array.array_v[row_idx % 128] = 1'b1;
                                    end
                                    default : begin
                                       $error("SOMTING IS wrong with parameters given for col number %d ",j);
       ...
       task automatic write_tcam_raw_line (input logic[72-1:0] key, input logic [72-1:0] inv_key, input int row_idx);
       ...
       // ********** backdoor MASK ***********
       //****this task is valid for TCAM only*****
       task automatic read_tcam_raw_key (output logic[72-1:0] key, input int row_idx);
           reg temp1,valid_word;
           logic [(2 * 36) - 1:0] local_key;
           // check size out-of-bounds
           if (row_idx >= TCAM_RULES_NUM) begin
         ...
         for (int j = 0;j < 2 ; j = j + 1) 
             case (j)
                                    0: begin
                                            //READ_mask
                                             local_key[0*36 +: 36] = gen_tcam_row_0_col_0_tcam.ip783tcam128x36s0c1_wrap_tcam_row_0_col_0.tcam_row_0_col_0.ip783tcam128x36s0c1_gen_wrp_inst.ip783tcam128x36s0c1_inst.ip783tcam128x36s0c1_array.array_x[row_idx % 128];
                                    end
                                    1: begin
                                            //READ_mask
                                             local_key[1*36 +: 36] = gen_tcam_row_0_col_1_tcam.ip783tcam128x36s0c1_wrap_tcam_row_0_col_1.tcam_row_0_col_1.ip783tcam128x36s0c1_gen_wrp_inst.ip783tcam128x36s0c1_inst.ip783tcam128x36s0c1_array.array_x[row_idx % 128];
                                    end
                                    default : begin
                                       $error("SOMTING IS wrong with parameters given for col number %d ",j);
                                     end
     ...
     // ********** backdoor DATA ***********
       task automatic read_tcam_raw_inv_key (output logic[72-1:0] inv_key, input int row_idx);
           reg temp1,valid_word;
           logic [(2 * 36) - 1:0] local_inv_key;
           // check size out-of-bounds
           if (row_idx >= TCAM_RULES_NUM) begin
         ...
         for (int j = 0;j < 2 ; j = j + 1) 
             case (j)
                                    0: begin
                                            //READ_data
                                             local_inv_key[0*36 +: 36] = gen_tcam_row_0_col_0_tcam.ip783tcam128x36s0c1_wrap_tcam_row_0_col_0.tcam_row_0_col_0.ip783tcam128x36s0c1_gen_wrp_inst.ip783tcam128x36s0c1_inst.ip783tcam128x36s0c1_array.array_y[row_idx % 128];
                                    end
                                    1: begin
                                            //READ_data
                                             local_inv_key[1*36 +: 36] = gen_tcam_row_0_col_1_tcam.ip783tcam128x36s0c1_wrap_tcam_row_0_col_1.tcam_row_0_col_1.ip783tcam128x36s0c1_gen_wrp_inst.ip783tcam128x36s0c1_inst.ip783tcam128x36s0c1_array.array_y[row_idx % 128];
                                    end
                                    default : begin
                                       $error("SOMTING IS wrong with parameters given for col number %d ",j);
                                     end
       ...
       task automatic read_tcam_raw_line (output logic[72-1:0] key, output logic [72-1:0] inv_key, input int row_idx);
