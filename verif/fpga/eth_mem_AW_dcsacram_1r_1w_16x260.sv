...
module eth_mem_AW_dcsacram_1r_1w_16x260 (
    input    logic  		 CLKW
   ,input    logic  		 CLKR
   ...
   ,input    logic  [260-1:0] 	 WDATA
   ,input    logic  		 WE
   ,input    logic  		 RE
   ,input    logic 		 RSTB
   ,output   logic  [260-1:0] 	 RDATA
...
assign RE_SEL = RE;
//********************************Y
logic      [260-1:0]    b_dout_0;
//********************************X


eth_ip783hd2prf16x130s0c1r1p3d0v0_mem_wrapper   i_rf_16x130_b0_0 (
...
`endif
  ...
  ,.renp0 		 (RE_SEL) 		//I:
  ,.wenp0 		 (WE_SEL) 		//I:
  ,.dinp0 		 (WDATA[(0*130+130-1):(0*130)]) 	//I:


  ,.qp0 		 (b_dout_0[(0*130+130-1):(0*130)]) //O:
...
eth_ip783hd2prf16x130s0c1r1p3d0v0_mem_wrapper   i_rf_16x130_b0_1 (
...
`endif
  ...
  ,.renp0 		 (RE_SEL) 		//I:
  ,.wenp0 		 (WE_SEL) 		//I:
  ,.dinp0 		 (WDATA[(1*130+130-1):(1*130)]) 	//I:


  ,.qp0 		 (b_dout_0[(1*130+130-1):(1*130)]) //O:
...
fpgamem_top #(
  ...
  ) i_fpgamem_top_16x260 (

     .ckwr 	 (CLKW)
    ,.ckrd 	 (CLKR)
    ,.wr 	 (WE)
    ,.wrptr 	 (WADDR[4-1:0])
    ,.datain 	 (WDATA[260-1:0])
    ,.rstb 	 (RSTB)
    ,.rd 	 (RE)
    ,.rdptr 	 (RADDR[4-1:0])
    ,.dataout 	 (b_dout[260-1:0])
  );
