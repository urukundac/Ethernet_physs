...
module eth_mem_AW_dcsacram_1r_1w_64x316 (
    input    logic  		 CLKW
   ,input    logic  		 CLKR
   ...
   ,input    logic  [316-1:0] 	 WDATA
   ,input    logic  		 WE
   ,input    logic  		 RE
   ,input    logic 		 RSTB
   ,output   logic  [316-1:0] 	 RDATA
...
assign RE_SEL = RE;
//********************************Y
logic      [316-1:0]    b_dout_0;
//********************************X


eth_ip783hd2prf64x158s0c1r1p3d0v0_mem_wrapper   i_rf_64x158_b0_0 (
...
`endif
  ...
  ,.renp0 		 (RE_SEL) 		//I:
  ,.wenp0 		 (WE_SEL) 		//I:
  ,.dinp0 		 (WDATA[(0*158+158-1):(0*158)]) 	//I:


  ,.qp0 		 (b_dout_0[(0*158+158-1):(0*158)]) //O:
...
eth_ip783hd2prf64x158s0c1r1p3d0v0_mem_wrapper   i_rf_64x158_b0_1 (
...
`endif
  ...
  ,.renp0 		 (RE_SEL) 		//I:
  ,.wenp0 		 (WE_SEL) 		//I:
  ,.dinp0 		 (WDATA[(1*158+158-1):(1*158)]) 	//I:


  ,.qp0 		 (b_dout_0[(1*158+158-1):(1*158)]) //O:
...
fpgamem_top #(
  ...
  ) i_fpgamem_top_64x316 (

     .ckwr 	 (CLKW)
    ,.ckrd 	 (CLKR)
    ,.wr 	 (WE)
    ,.wrptr 	 (WADDR[6-1:0])
    ,.datain 	 (WDATA[316-1:0])
    ,.rstb 	 (RSTB)
    ,.rd 	 (RE)
    ,.rdptr 	 (RADDR[6-1:0])
    ,.dataout 	 (b_dout[316-1:0])
  );
