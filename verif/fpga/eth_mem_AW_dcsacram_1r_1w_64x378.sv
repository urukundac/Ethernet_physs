...
module eth_mem_AW_dcsacram_1r_1w_64x378 (
    input    logic  		 CLKW
   ,input    logic  		 CLKR
   ...
   ,input    logic  [378-1:0] 	 WDATA
   ,input    logic  		 WE
   ,input    logic  		 RE
   ,input    logic 		 RSTB
   ,output   logic  [378-1:0] 	 RDATA
...
assign RE_SEL = RE;
//********************************Y
logic      [378-1:0]    b_dout_0;
//********************************X


eth_ip783hd2prf64x190s0c1r1p3d0v0_mem_wrapper   i_rf_64x190_b0_0 (
...
`endif
  ...
  ,.wradrp0 		 (WADDR[6-1:0]) 	//I:
  ,.rdadrp0 		 (RADDR[6-1:0]) 	//I:
  ,.renp0 		 (RE_SEL) 		//I:
  ,.wenp0 		 (WE_SEL) 		//I:
  ,.dinp0 		 (WDATA[(0*190+190-1):(0*190)]) 	//I:


  ,.qp0 		 (b_dout_0[(0*190+190-1):(0*190)]) //O:
  ...
  ,.global_rrow_en_in_wr 		 (global_rrow_en_in_wr) 			//I:
  ,.global_rrow_en_in_rd 		 (global_rrow_en_in_rd) 			//I:
  ,.col_repair_in 	 (col_repair_in) 			//I:
  ,.row_repair_in 	 (row_repair_in) 			//I:
  ,.fastsleep(fastsleep)
  ,.deepsleep(deepsleep)
  ,.sbc(sbc)
  ,.shutoff(shutoff)
...
logic      [2-1:0]    unused_bout_0;


eth_ip783hd2prf64x190s0c1r1p3d0v0_mem_wrapper   i_rf_64x190_b0_1 (
...
`endif
  ...
  ,.renp0 		 (RE_SEL) 		//I:
  ,.wenp0 		 (WE_SEL) 		//I:
  ,.dinp0 		 ({2'b0,WDATA[(1*190+190-1-2):(1*190)]}) 	//I:


  ,.qp0 		 ({unused_bout_0,b_dout_0[(1*190+190-1-2):(1*190)]}) //O:
...
AW_unused_bits #(.WIDTH(2)) u_unused_bout_0(.a(unused_bout_0));
...
fpgamem_top #(
  ...
  ) i_fpgamem_top_64x378 (

     .ckwr 	 (CLKW)
    ,.ckrd 	 (CLKR)
    ,.wr 	 (WE)
    ,.wrptr 	 (WADDR[6-1:0])
    ,.datain 	 (WDATA[378-1:0])
    ,.rstb 	 (RSTB)
    ,.rd 	 (RE)
    ,.rdptr 	 (RADDR[6-1:0])
    ,.dataout 	 (b_dout[378-1:0])
  );
