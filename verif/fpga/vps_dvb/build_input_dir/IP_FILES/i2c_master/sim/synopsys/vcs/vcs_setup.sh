...
# ACDS 24.1 115 linux 2025.07.14.09:56:47
...
set ELAB_OPTIONS ""
append ELAB_OPTIONS [i2c_master::get_elab_options $SIMULATOR_TOOL_BITNESS]
...
source $QSYS_SIMDIR/common/vcs_files.tcl
set memory_files [concat $memory_files [i2c_master::get_memory_files "$QSYS_SIMDIR"]]
...
source $QSYS_SIMDIR/common/vcs_files.tcl
set files [dict merge $files [i2c_master::get_common_design_files "$QSYS_SIMDIR"]]
...
set QSYS_SIMDIR [lindex $argv 1]
source $QSYS_SIMDIR/common/vcs_files.tcl
if {[catch {i2c_master::get_design_files "$QSYS_SIMDIR"} errorMsg]} {
 puts "Catch error: $errorMsg"
 exit 1 
}
set files [dict create]
set files [dict merge $files [i2c_master::get_design_files "$QSYS_SIMDIR"]]
...
source $QSYS_SIMDIR/common/vcs_files.tcl
set libraries [dict merge $libraries [i2c_master::get_dpi_libraries "$QSYS_SIMDIR"]]
