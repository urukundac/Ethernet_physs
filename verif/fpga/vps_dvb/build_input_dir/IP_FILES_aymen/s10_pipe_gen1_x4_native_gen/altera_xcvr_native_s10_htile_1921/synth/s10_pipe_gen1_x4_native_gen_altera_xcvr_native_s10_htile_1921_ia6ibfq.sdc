...
# ---      alt_xcvr_native_helper_functions_ia6ibfq.tcl        --- #
# -                                                              - # 
# ---------------------------------------------------------------- #

set script_dir [file dirname [info script]] 
set split_qsys_output_name [split s10_pipe_gen1_x4_native_gen_altera_xcvr_native_s10_htile_1921_ia6ibfq "_"]
...
source "${script_dir}/${top_inst_name}_ip_parameters_ia6ibfq.tcl"
source "${script_dir}/alt_xcvr_native_helper_functions_ia6ibfq.tcl"

# Debug switch. Change to 1 in alt_xcvr_native_helper_functions_ia6ibfq.tcl to get more run-time debug information
...
# -                                                              - #
# ---------------------------------------------------------------- #
native_initialize_db_ia6ibfq nativedb_ia6ibfq

# ---------------------------------------------------------------- #
# --- Set all the instances of this core                       --- #
# ---------------------------------------------------------------- #
set alt_xcvr_native_s10_instances [ dict keys $nativedb_ia6ibfq ]
...
foreach inst $alt_xcvr_native_s10_instances {
  ...
  dict set alt_xcvr_native_s10_pins $inst [dict get $nativedb_ia6ibfq $inst]
  ...
  for {set i 0} {$i < $profile_cnt} {incr i} {
    ...
    dict for {mode mode_clks} $profile_clocks {
      ...
      set list_of_clk_names [native_prepare_to_create_clocks_all_ch_ia6ibfq $inst $num_channels $mode $mode_clks $profile_cnt $i $alt_xcvr_native_s10_pins $freq $multiply_factor_dict $divide_factor_dict $all_profile_clocks_names]
      ...
      foreach_in_collection fanin_node $tx_coreclkin_fanin_col {
        set fanin_node_name [get_node_info -name $fanin_node]
         ...
         set_active_clocks [remove_from_collection [get_active_clocks] [get_clocks $fanin_clock_name]]
          ...
          add_ddr_report_command "native_check_special_min_pulse_ia6ibfq $fanin_clock_name"
        }
      }
    }
  }
